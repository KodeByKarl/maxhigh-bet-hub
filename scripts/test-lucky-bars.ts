 ...
 * Run: npx tsx scripts/test-lucky-bars.ts
 * RTP:  npx tsx scripts/test-lucky-bars.ts --rtp 100000
 */
import {
  DEFAULT_LUCKY_BARS_CONFIG,
  normalizeLuckyBarsConfig,
  type LuckyBarsConfig,
} from "../src/lib/lucky-bars-config";
import { applyHoldReels, normalizeHoldMask } from "../src/lib/slot-primitives/holdReels";
import { evaluatePayline, shouldTriggerBonus, shouldTriggerJackpot } from "../src/components/maxhigh/lucky-bars/paylineEngine";
import { resolveReelsWithHold, generateReels, sanitizeHoldMask } from "../src/components/maxhigh/lucky-bars/reelGenerator";
import { createRng } from "../src/components/maxhigh/lucky-bars/rng";
import { setLuckyBarsConfig } from "../src/components/maxhigh/lucky-bars/runtimeConfig";
import { resolveLuckyBarsSpin } from "../src/components/maxhigh/lucky-bars/spinResolver";
import { resolveBonusLadder } from "../src/components/maxhigh/lucky-bars/bonusLadder";
import type { RrReels } from "../src/components/maxhigh/lucky-bars/types";

setLuckyBarsConfig(DEFAULT_LUCKY_BARS_CONFIG);
const cfg = DEFAULT_LUCKY_BARS_CONFIG;
...
function runUnitTests() {
  ...
  const n = normalizeLuckyBarsConfig(null);
  ...
  const jpScript = resolveLuckyBarsSpin({
    totalBet: 5,
    seed: "jp-max",
  ...
  const noJpScript = resolveLuckyBarsSpin({
    totalBet: 3,
    seed: "jp-low",
    reels: ["double_wild", "double_wild", "grape"],
  ...
  const clearCfg: LuckyBarsConfig = {
    ...cfg,
    bonusLadder: {
      ...cfg.bonusLadder,
  ...
  const bonusSpin = resolveLuckyBarsSpin({
    totalBet: 5,
    seed: "bonus-spin",
  ...
  // Full resolve smoke
  const smoke = resolveLuckyBarsSpin({ totalBet: 1, seed: "smoke", held: [false, false, false] });
