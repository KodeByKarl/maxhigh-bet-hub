...
} from "../src/lib/starlight-ace-config";
import { evaluateWays } from "../src/components/maxhigh/starlight-ace/waysEngine";
import { resolveStarlightSpin } from "../src/components/maxhigh/starlight-ace/spinResolver";
import { applyGravity, makeCell, generateInitialBoard } from "../src/components/maxhigh/starlight-ace/tumbleEngine";
import { setStarlightAceConfig } from "../src/components/maxhigh/starlight-ace/runtimeConfig";
import type { BoardCell } from "../src/components/maxhigh/starlight-ace/types";

setStarlightAceConfig(DEFAULT_STARLIGHT_ACE_CONFIG);
...
function runUnitTests() {
  ...
  const sym10 = DEFAULT_STARLIGHT_ACE_CONFIG.symbols.find((s) => s.id === "sym_10")!;
  const wildSym = DEFAULT_STARLIGHT_ACE_CONFIG.symbols.find((s) => s.id === "wild")!;
  const scatterSym = DEFAULT_STARLIGHT_ACE_CONFIG.symbols.find((s) => s.id === "scatter")!;
...
function runGuaranteedGoldenReelTest() {
  ...
  const reel = DEFAULT_STARLIGHT_ACE_CONFIG.guaranteedGoldenReelIndex;
  let samples = 0;
  for (let i = 0; i < 40; i++) {
...
function runMaxWinCapTest() {
  ...
  for (let i = 0; i < 2000; i++) {
    const script = resolveStarlightSpin({ bet: 10, ante: false, isFreeSpins: false });
...
function runCascadeTerminationTests() {
  ...
  for (let i = 0; i < 500; i++) {
    const script = resolveStarlightSpin({ bet: 1.0, ante: false, isFreeSpins: false });
...
function runRtpSimulation() {
  ...
  for (let i = 0; i < TOTAL_SPINS; i++) {
    ...
    const baseResult = resolveStarlightSpin({ bet: betPerSpin, ante: false, isFreeSpins: false });
    ...
    if (baseResult.freeSpinsAwarded > 0) {
      ...
      while (fsRemaining > 0) {
        fsRemaining--;
        freeSpinsPlayed++;
        const fsResult = resolveStarlightSpin({ bet: betPerSpin, ante: false, isFreeSpins: true });
  ...
  console.log(`- Achieved RTP: ${achievedRtp}% (Target: ${DEFAULT_STARLIGHT_ACE_CONFIG.targetRtp}% — provisional)`);
