 ...
 * Run: npx tsx scripts/test-super-ace.ts
 */
import {
  ...
  type SaRtpProfileId,
  type SuperAceConfig,
} from "../src/lib/super-ace-config";
import { evaluateWays } from "../src/components/maxhigh/super-ace/waysEngine";
import { resolveSuperAceSpin } from "../src/components/maxhigh/super-ace/spinResolver";
import {
  applyGravity,
  makeCell,
  generateInitialBoard,
} from "../src/components/maxhigh/super-ace/tumbleEngine";
import { setSuperAceConfig } from "../src/components/maxhigh/super-ace/runtimeConfig";
import type { BoardCell } from "../src/components/maxhigh/super-ace/types";

function withConfig(cfg: SuperAceConfig) {
  setSuperAceConfig(cfg);
}

withConfig(DEFAULT_SUPER_ACE_CONFIG);
...
function runUnitTests() {
  ...
  const symJ = DEFAULT_SUPER_ACE_CONFIG.symbols.find((s) => s.id === "sym_j")!;
  const little = DEFAULT_SUPER_ACE_CONFIG.symbols.find((s) => s.id === "little_joker")!;
  const big = DEFAULT_SUPER_ACE_CONFIG.symbols.find((s) => s.id === "big_joker")!;
  const scatterSym = DEFAULT_SUPER_ACE_CONFIG.symbols.find((s) => s.id === "scatter")!;
...
function runFreeSpinsAwardTests() {
  ...
  if (calcFreeSpinsAward(3, DEFAULT_SUPER_ACE_CONFIG, { isRetrigger: true }) !== 5) {
    throw new Error("Retrigger should award +5");
  }
  if (calcFreeSpinsAward(8, DEFAULT_SUPER_ACE_CONFIG, { isRetrigger: true }) !== 5) {
    throw new Error("Retrigger remains flat +5 regardless of scatter count");
  }

  // Uncapped double retrigger accumulation
  let left = 10;
  left = left - 1 + calcFreeSpinsAward(3, DEFAULT_SUPER_ACE_CONFIG, { isRetrigger: true });
  left = left - 1 + calcFreeSpinsAward(4, DEFAULT_SUPER_ACE_CONFIG, { isRetrigger: true });
...
function runMaxWinCapTest() {
  ...
  for (let i = 0; i < 2000; i++) {
    const script = resolveSuperAceSpin({ bet: 10, ante: false, isFreeSpins: false });
  ...
  withConfig(DEFAULT_SUPER_ACE_CONFIG);
  if (DEFAULT_SUPER_ACE_CONFIG.maxWinMult !== 1500) {
    throw new Error(`Expected maxWinMult 1500, got ${DEFAULT_SUPER_ACE_CONFIG.maxWinMult}`);
  }
...
function runCascadeTerminationTests() {
  ...
  let multiCascade = 0;
  for (let i = 0; i < 500; i++) {
    const script = resolveSuperAceSpin({ bet: 1.0, ante: false, isFreeSpins: false });
...
function runRtpSimulationPerProfile() {
  ...
  for (const profileId of profiles) {
    ...
    for (let i = 0; i < TOTAL_SPINS; i++) {
      ...
      const baseResult = resolveSuperAceSpin({
        bet: betPerSpin,
        ante: false,
        isFreeSpins: false,
      });
