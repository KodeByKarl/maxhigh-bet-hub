/**
 * Lucky Nine Plus RTP / EV simulation + accounting checks.
 * Run: npx tsx scripts/test-lucky-nine-plus.ts
 */
import {
  ...
  shouldDraw,
  isNatural,
} from "../src/lib/lucky-nine-plus-config";
import {
  resolveLuckyNinePlusDeal,
  compareHands,
} from "../src/components/maxhigh/games/lucky-nine-plus/resolver";
import { createRng } from "../src/components/maxhigh/games/lucky-nine-plus/rng";
import { setLuckyNinePlusConfig } from "../src/components/maxhigh/games/lucky-nine-plus/runtimeConfig";
...
function main() {
  const cfg = normalizeLuckyNinePlusConfig(DEFAULT_LUCKY_NINE_PLUS_CONFIG);
  ...
  if (!isNatural(9, 9) || isNatural(8, 9)) {
    throw new Error("isNatural fail — Lucky Nine Plus natural is exactly 9, not 8");
  }
  ...
  for (let i = 0; i < 2000; i++) {
    const d = resolveLuckyNinePlusDeal({
      playerBet: 100,
      dealerBet: 0,
      tieBet: 0,
      cfg,
  ...
  for (let i = 0; i < ROUNDS; i++) {
    const deal = resolveLuckyNinePlusDeal({
      playerBet: PLAYER,
      dealerBet: DEALER,
      tieBet: TIE,
      cfg,
  ...
  console.log(`Lucky Nine Plus sim · ${ROUNDS.toLocaleString()} rounds`);
  console.log(
