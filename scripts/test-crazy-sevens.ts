 ...
 * Run: npx tsx scripts/test-crazy-sevens.ts
 * RTP:  npx tsx scripts/test-crazy-sevens.ts --rtp 100000
 */
import {
  DEFAULT_CRAZY_SEVENS_CONFIG,
  normalizeCrazySevensConfig,
  type CrazySevensConfig,
} from "../src/lib/crazy-sevens-config";
import { applyHoldReels, normalizeHoldMask } from "../src/lib/slot-primitives/holdReels";
import { evaluatePayline, shouldTriggerBonus, shouldTriggerJackpot } from "../src/components/maxhigh/crazy-sevens/paylineEngine";
import { resolveReelsWithHold, generateReels, sanitizeHoldMask } from "../src/components/maxhigh/crazy-sevens/reelGenerator";
import { createRng } from "../src/components/maxhigh/crazy-sevens/rng";
import { setCrazySevensConfig } from "../src/components/maxhigh/crazy-sevens/runtimeConfig";
import { resolveCrazySevensSpin } from "../src/components/maxhigh/crazy-sevens/spinResolver";
import { resolveBonusLadder } from "../src/components/maxhigh/crazy-sevens/bonusLadder";
import type { RrReels } from "../src/components/maxhigh/crazy-sevens/types";

setCrazySevensConfig(DEFAULT_CRAZY_SEVENS_CONFIG);
const cfg = DEFAULT_CRAZY_SEVENS_CONFIG;
...
function runUnitTests() {
  ...
  const n = normalizeCrazySevensConfig(null);
  ...
  const jpScript = resolveCrazySevensSpin({
    totalBet: 5,
    seed: "jp-max",
  ...
  const noJpScript = resolveCrazySevensSpin({
    totalBet: 3,
    seed: "jp-low",
    reels: ["double_wild", "double_wild", "grape"],
  ...
  const stopCfg: CrazySevensConfig = {
    ...cfg,
    bonusLadder: {
      ...cfg.bonusLadder,
      lines: [
  ...
  const clearCfg: CrazySevensConfig = {
    ...cfg,
    bonusLadder: {
      ...cfg.bonusLadder,
  ...
  const bonusSpin = resolveCrazySevensSpin({
    totalBet: 5,
    seed: "bonus-spin",
  ...
  // Full resolve smoke
  const smoke = resolveCrazySevensSpin({ totalBet: 1, seed: "smoke", held: [false, false, false] });
...
function runRtp(spins: number) {
  ...
  const runScenario = (label: string, holdStrategy: "none" | "hold_single_wild") => {
    ...
    for (let i = 0; i < spins; i++) {
      ...
      const s = resolveCrazySevensSpin({
        totalBet: bet,
