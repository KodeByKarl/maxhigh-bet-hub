/**
 * Fortune Ox — regression tests + optional RTP simulation.
 * Run: npx tsx scripts/test-fortune-ox.ts
 * RTP:  npx tsx scripts/test-fortune-ox.ts --rtp 1000000
 *
 * PENDING DESIGN CONFIRMATION (documented in fortune-ox-config.ts):
...
import {
  ...
  FO_MULTIPLIER_VALUES,
  type FgRtpProfileId,
} from "../src/lib/fortune-ox-config";
import { evaluatePaylines } from "../src/components/maxhigh/fortune-ox/paylineEngine";
import {
  applyMultiplier,
  generateMultiplierReel,
} from "../src/components/maxhigh/fortune-ox/multiplierReel";
import { resolveFortuneOxSpin } from "../src/components/maxhigh/fortune-ox/spinResolver";
import { setFortuneOxConfig } from "../src/components/maxhigh/fortune-ox/runtimeConfig";
import { generateGrid } from "../src/components/maxhigh/fortune-ox/reelGenerator";
import { createRng } from "../src/components/maxhigh/fortune-ox/rng";
import type { FoGrid } from "../src/components/maxhigh/fortune-ox/types";

setFortuneOxConfig(DEFAULT_FORTUNE_OX_CONFIG);
const cfg = DEFAULT_FORTUNE_OX_CONFIG;
...
function runUnitTests() {
  console.log("==========================================");
  console.log("Fortune Ox — unit / regression tests");
  console.log("==========================================");

  // --- Section 1: config + grid ---
  const normalized = normalizeFortuneOxConfig(null);
  ...
  for (const mult of FO_MULTIPLIER_VALUES) {
  ...
  for (const mult of FO_MULTIPLIER_VALUES) {
    const script = resolveFortuneOxSpin({
      totalBet: BET,
      baseBet: BET,
      seed: `mult-${mult}`,
      grid: rubyLine,
  ...
  const allWild: FoGrid = [
    ["wild", "wild", "wild"],
    ["wild", "wild", "wild"],
    ["wild", "wild", "wild"],
  ];
  const capScript = resolveFortuneOxSpin({
    totalBet: BET,
    baseBet: BET,
    seed: "cap-test",
    grid: allWild,
  ...
  const full = resolveFortuneOxSpin({
    totalBet: 15,
    baseBet: 10,
    exMode: true,
    seed: "fg-audit-1",
  ...
  assert(FO_MULTIPLIER_VALUES.includes(full.multiplierReel.center as (typeof FO_MULTIPLIER_VALUES)[number]), "valid mult");
...
function runRtpSim(spins: number) {
  console.log("==========================================");
  console.log(`Fortune Ox — RTP simulation (${spins.toLocaleString()} spins)`);
  ...
  for (const profileId of profiles) {
    ...
    for (const exMode of [false, true]) {
      ...
      for (const v of FO_MULTIPLIER_VALUES) multHist[v] = 0;
      ...
      console.log(
        `  mult hist: ${FO_MULTIPLIER_VALUES.map((v) => `${v}x=${((multHist[v] / spins) * 100).toFixed(1)}%`).join("  ")}`,
      );
      if (exMode) {
