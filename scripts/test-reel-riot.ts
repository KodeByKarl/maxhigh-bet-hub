/**
 * Reel Riot — regression tests + optional RTP simulation.
 * Run: npx tsx scripts/test-reel-riot.ts
 * RTP:  npx tsx scripts/test-reel-riot.ts --rtp 100000
 */
import {
  DEFAULT_REEL_RIOT_CONFIG,
  normalizeReelRiotConfig,
  type ReelRiotConfig,
} from "../src/lib/reel-riot-config";
import { applyHoldReels, normalizeHoldMask } from "../src/lib/slot-primitives/holdReels";
import { evaluatePayline, shouldTriggerBonus, shouldTriggerJackpot } from "../src/components/maxhigh/reel-riot/paylineEngine";
import { resolveReelsWithHold, generateReels, sanitizeHoldMask } from "../src/components/maxhigh/reel-riot/reelGenerator";
import { createRng } from "../src/components/maxhigh/reel-riot/rng";
import { setReelRiotConfig } from "../src/components/maxhigh/reel-riot/runtimeConfig";
import { resolveReelRiotSpin } from "../src/components/maxhigh/reel-riot/spinResolver";
import { resolveBonusLadder } from "../src/components/maxhigh/reel-riot/bonusLadder";
import type { RrReels } from "../src/components/maxhigh/reel-riot/types";

setReelRiotConfig(DEFAULT_REEL_RIOT_CONFIG);
const cfg = DEFAULT_REEL_RIOT_CONFIG;
...
function runUnitTests() {
  console.log("==========================================");
  console.log("Reel Riot — unit / regression tests");
  console.log("==========================================");

  // --- Section 1: config + generator ---
  const n = normalizeReelRiotConfig(null);
  ...
  const jpScript = resolveReelRiotSpin({
    totalBet: 5,
    seed: "jp-max",
  ...
  const noJpScript = resolveReelRiotSpin({
    totalBet: 3,
    seed: "jp-low",
    reels: ["double_wild", "double_wild", "grape"],
  ...
  const bonusSpin = resolveReelRiotSpin({
    totalBet: 5,
    seed: "bonus-spin",
  ...
  // Full resolve smoke
  const smoke = resolveReelRiotSpin({ totalBet: 1, seed: "smoke", held: [false, false, false] });
  assert(smoke.reels.length === 3 && smoke.audit.seed === smoke.seed, "smoke");
  console.log("✓ Full spin resolve smoke");

  console.log("==========================================");
  console.log("All Reel Riot unit tests passed");
  console.log("==========================================");
}
