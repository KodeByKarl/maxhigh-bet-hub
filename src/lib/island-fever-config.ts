/**
 * Island Fever math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const ISLAND_FEVER_GAME_ID = "island-fever";

export type IslandFeverSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type IslandFeverBombEntry = { mult: number; weight: number };

export type IslandFeverScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type IslandFeverSymbolConfig = {
  id: string;
  kind: IslandFeverSymKind;
  label: string;
...
export type IslandFeverConfig = {
  ...
  bombTable: IslandFeverBombEntry[];
  ...
  scatterCashTiers: IslandFeverScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: IslandFeverSymbolConfig[];
};

export const SYMBOL_LABELS: Record<IslandFeverSymKind, string> = {
  grape: "Grape",
  plum: "Plum",
  melon: "Melon",
  apple: "Apple",
...
export const DEFAULT_ISLAND_FEVER_CONFIG: IslandFeverConfig = {
  schemaVersion: 1,
...
export function normalizeIslandFeverConfig(raw: unknown): IslandFeverConfig {
  const d = DEFAULT_ISLAND_FEVER_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: IslandFeverSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<IslandFeverSymbolConfig> | undefined;
  ...
  const bombTable: IslandFeverBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<IslandFeverBombEntry>;
            return {
  ...
  const scatterCashTiers: IslandFeverScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<IslandFeverScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: IslandFeverConfig): {
  id: string;
  kind: IslandFeverSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: IslandFeverBombEntry[]): Record<number, number> {
