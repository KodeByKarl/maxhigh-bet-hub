/**
 * Cash Mania — base-game-only 5×3 / 10-payline slot math config.
...
export const CASH_MANIA_GAME_ID = "cash-mania";
...
export const DEFAULT_CASH_MANIA_CONFIG: CashManiaConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: CashManiaConfig): FsRtpProfile[] {
...
export function effectiveReelWeights(cfg: CashManiaConfig): Record<McSymKind, number[]> {
  const profile =
...
export function normalizeCashManiaConfig(raw: unknown): CashManiaConfig {
  const d = DEFAULT_CASH_MANIA_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<CashManiaConfig>;
...
export function betPerLine(totalBet: number, cfg: CashManiaConfig): number {
