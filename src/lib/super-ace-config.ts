...
export const SUPER_ACE_GAME_ID = "super-ace";

export type SuperAceSymKind =
  | "sym_j"
  | "sym_q"
  | "sym_k"
  | "sym_a"
...
export type SuperAceSymbolConfig = {
  id: string;
...
export type SaRtpProfile = {
  ...
  symbolWeights: Partial<Record<SuperAceSymKind, SaSymbolWeightOverride>>;
};
...
export type SuperAceConfig = {
  ...
  symbols: SuperAceSymbolConfig[];
};

export const SYMBOL_NAMES: Record<SuperAceSymKind, string> = {
  sym_j: "Jack",
  sym_q: "Queen",
  sym_k: "King",
  sym_a: "Ace",
...
export const ALL_SYM_KINDS: SuperAceSymKind[] = [
  "sym_j",
  "sym_q",
  "sym_k",
  "sym_a",
 ...
 */
const BASE_SYMBOLS: SuperAceSymbolConfig[] = [
  {
    id: "sym_j",
    kind: "sym_j",
    name: "Jack",
...
export const DEFAULT_SUPER_ACE_CONFIG: SuperAceConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: SuperAceConfig = DEFAULT_SUPER_ACE_CONFIG,
...
export function applyRtpProfile(cfg: SuperAceConfig): SuperAceConfig {
  const profile =
...
const VOLATILITY_VALUES: SuperAceConfig["volatility"][] = [
  "low",
  "low-medium",
  "medium",
  "medium-high",
...
function normalizeRtpProfiles(raw: unknown, d: SuperAceConfig): SaRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const weightsIn = (found?.symbolWeights ?? fb.symbolWeights) as Partial<
      Record<SuperAceSymKind, SaSymbolWeightOverride>
    >;
    const symbolWeights: Partial<Record<SuperAceSymKind, SaSymbolWeightOverride>> = {};
...
export function normalizeSuperAceConfig(raw: unknown): SuperAceConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_SUPER_ACE_CONFIG);
  const obj = raw as Partial<SuperAceConfig>;
  const d = DEFAULT_SUPER_ACE_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as SuperAceSymKind) || base.kind,
  ...
  const jwIn = (obj.jokerTransformWeights ?? {}) as Partial<SuperAceConfig["jokerTransformWeights"]>;
  ...
  const volatility: SuperAceConfig["volatility"] = VOLATILITY_VALUES.includes(
    volRaw as SuperAceConfig["volatility"],
  )
    ? (volRaw as SuperAceConfig["volatility"])
    : d.volatility;
