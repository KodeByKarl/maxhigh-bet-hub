/**
 * Dragon & Phoenix math & configuration — shared by server resolver and UI components.
 * Tunable paytable / weights / ladders live here (never hardcode in engine loops).
 */

export const DRAGON_PHOENIX_GAME_ID = "dragon-phoenix";

export type DragonPhoenixSymKind =
  | "sym_10"
  | "sym_j"
  | "sym_q"
  | "sym_k"
...
export type DragonPhoenixSymbolConfig = {
  id: string;
  kind: DragonPhoenixSymKind;
  name: string;
...
export type DragonPhoenixConfig = {
  schemaVersion: 1;
  reelsCount: number;
  ...
  maxWinMult: number;
  targetRtp: number;
  symbols: DragonPhoenixSymbolConfig[];
};

export const SYMBOL_NAMES: Record<DragonPhoenixSymKind, string> = {
  sym_10: "10 Tile",
  sym_j: "J Tile",
  sym_q: "Q Tile",
  sym_k: "K Tile",
  ...
  scatter: "Dragon Gold Scatter",
};

export const DEFAULT_DRAGON_PHOENIX_CONFIG: DragonPhoenixConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(scatterCount: number, cfg: DragonPhoenixConfig = DEFAULT_DRAGON_PHOENIX_CONFIG): number {
...
export function normalizeDragonPhoenixConfig(raw: unknown): DragonPhoenixConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_DRAGON_PHOENIX_CONFIG);
  const obj = raw as Partial<DragonPhoenixConfig>;
  const d = DEFAULT_DRAGON_PHOENIX_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as DragonPhoenixSymKind) || base.kind,
