/**
 * Neon Makati math & configuration — shared by server resolver and UI.
...
export const NEON_MAKATI_GAME_ID = "neon-makati";

export type NeonMakatiSymKind =
  | "sym_10"
  | "sym_j"
  | "sym_q"
  | "sym_k"
...
export type NeonMakatiSymbolConfig = {
  id: string;
...
export type NeonMakatiConfig = {
  ...
  maxWinMult: number;
  targetRtp: number;
  symbols: NeonMakatiSymbolConfig[];
};

export const SYMBOL_NAMES: Record<NeonMakatiSymKind, string> = {
  sym_10: "10 Gem",
  sym_j: "J Gem",
  sym_q: "Q Gem",
  sym_k: "K Gem",
 ...
 */
export const DEFAULT_NEON_MAKATI_CONFIG: NeonMakatiConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 5,
  maxReelHeight: 5,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: NeonMakatiConfig = DEFAULT_NEON_MAKATI_CONFIG,
): number {
...
export function normalizeNeonMakatiConfig(raw: unknown): NeonMakatiConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_NEON_MAKATI_CONFIG);
  const obj = raw as Partial<NeonMakatiConfig>;
  const d = DEFAULT_NEON_MAKATI_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as NeonMakatiSymKind) || base.kind,
