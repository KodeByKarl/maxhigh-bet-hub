/**
 * Carabao Charge (catalog id: carabao-charge) — math & configuration.
 * 5×4 ways (1,024), cascade multipliers, Golden Campfire free spins,
 * Treasure Chest Hold & Collect, Explorer→Legend jackpots.
 */

export const CARABAO_CHARGE_GAME_ID = "carabao-charge";
export const CARABAO_CHARGE_TITLE = "Carabao Charge";
...
export type CarabaoChargeConfig = {
  schemaVersion: 1;
  reelsCount: number;
...
export const DEFAULT_CARABAO_CHARGE_CONFIG: CarabaoChargeConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: CarabaoChargeConfig = DEFAULT_CARABAO_CHARGE_CONFIG,
): number {
...
export function normalizeCarabaoChargeConfig(raw: unknown): CarabaoChargeConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_CARABAO_CHARGE_CONFIG);
  const obj = raw as Partial<CarabaoChargeConfig>;
  const d = DEFAULT_CARABAO_CHARGE_CONFIG;
  ...
  const volatility: CarabaoChargeConfig["volatility"] =
