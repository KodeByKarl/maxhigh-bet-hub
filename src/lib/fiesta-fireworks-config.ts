/**
 * Fiesta Fireworks — fixed-payline slot math config (Play'n GO–style).
...
export const FIESTA_FIREWORKS_GAME_ID = "fiesta-fireworks";
...
export type FiestaFireworksConfig = {
  schemaVersion: 2;
  reelsCount: number;
  rowsCount: number;
...
export const DEFAULT_FIESTA_FIREWORKS_CONFIG: FiestaFireworksConfig = {
  schemaVersion: 2,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 20,
...
export function normalizeFiestaFireworksConfig(raw: unknown): FiestaFireworksConfig {
  const d = DEFAULT_FIESTA_FIREWORKS_CONFIG;
...
export function payingSymbolKinds(cfg: FiestaFireworksConfig = DEFAULT_FIESTA_FIREWORKS_CONFIG): CnySymKind[] {
  return cfg.symbols
    .filter((s) => s.tier === "low" || s.tier === "high")
    .map((s) => s.kind);
}

export function betPerLine(totalBet: number, cfg: FiestaFireworksConfig): number {
