/**
 * Fortune Ox — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Fortune Ox 2/3):
...
export const FORTUNE_OX_GAME_ID = "fortune-ox";
...
export const DEFAULT_FORTUNE_OX_CONFIG: FortuneOxConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
...
function normalizeRtpProfiles(raw: unknown, d: FortuneOxConfig): FgRtpProfile[] {
...
export function effectiveReelWeights(cfg: FortuneOxConfig): Record<FoSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: FortuneOxConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeFortuneOxConfig(raw: unknown): FortuneOxConfig {
  const d = DEFAULT_FORTUNE_OX_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<FortuneOxConfig>;
...
export function betPerLine(totalBet: number, cfg: FortuneOxConfig): number {
