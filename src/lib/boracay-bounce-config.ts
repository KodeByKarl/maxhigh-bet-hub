/**
 * Boracay Bounce math config — shared by engine (client) and Superadmin (server).
 * Defaults follow educational Boracay Bounce–style cluster / multiplier / FS tables.
 */

export const BORACAY_BOUNCE_GAME_ID = "boracay-bounce";

export type BoracayBounceSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type BoracayBounceBombEntry = { mult: number; weight: number };

export type BoracayBounceScatterCashTier = {
  count: number;
  /** × bet — 0 = FS-only scatters */
  mult: number;
};

export type BoracayBounceFreeSpinAward = {
  count: number;
  spins: number;
};

export type BoracayBounceSymbolConfig = {
  id: string;
  kind: BoracayBounceSymKind;
  label: string;
  weight: number;
...
export type BoracayBounceConfig = {
  schemaVersion: 1;
  ...
  bombTable: BoracayBounceBombEntry[];
  ...
  freeSpinsByScatterCount: BoracayBounceFreeSpinAward[];
  anteScatterWeightMult: number;
  scatterCashTiers: BoracayBounceScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: BoracayBounceSymbolConfig[];
};

export const SYMBOL_LABELS: Record<BoracayBounceSymKind, string> = {
  grape: "Red bear",
  plum: "Orange bear",
  melon: "Purple bear",
  apple: "Green star",
...
export const DEFAULT_BORACAY_BOUNCE_CONFIG: BoracayBounceConfig = {
  schemaVersion: 1,
...
export function normalizeBoracayBounceConfig(raw: unknown): BoracayBounceConfig {
  const d = DEFAULT_BORACAY_BOUNCE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: BoracayBounceSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<BoracayBounceSymbolConfig> | undefined;
  ...
  const bombTable: BoracayBounceBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<BoracayBounceBombEntry>;
            return {
  ...
  const scatterCashTiers: BoracayBounceScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<BoracayBounceScatterCashTier>;
            return {
  ...
  const freeSpinsByScatterCount: BoracayBounceFreeSpinAward[] =
    fsAwardIn && fsAwardIn.length > 0
      ? fsAwardIn
          .map((t) => {
            const row = t as Partial<BoracayBounceFreeSpinAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: BoracayBounceConfig): {
  id: string;
  kind: BoracayBounceSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: BoracayBounceBombEntry[]): Record<number, number> {
