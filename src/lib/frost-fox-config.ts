/**
 * Frost Fox math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const FROST_FOX_GAME_ID = "frost-fox";

export type FrostFoxSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type FrostFoxBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const FROST_FOX_BOMB_MULTS = [2, 3, 4, 5] as const;

export type FrostFoxScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type FrostFoxSymbolConfig = {
  id: string;
  kind: FrostFoxSymKind;
  label: string;
...
export type FrostFoxConfig = {
  ...
  scatterCashTiers: FrostFoxScatterCashTier[];
  ...
  symbols: FrostFoxSymbolConfig[];
};

export const SYMBOL_LABELS: Record<FrostFoxSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_FROST_FOX_CONFIG: FrostFoxConfig = {
  schemaVersion: 1,
  ...
  anteBetMult: 1.25,
  /** Glacier pillars 4-5-6-5-4 (24 cells) — cluster pays from 6 matching. */
  minCluster: 6,
...
export function clampBombMult(v: unknown): FrostFoxBombEntry["mult"] {
...
export function normalizeFrostFoxConfig(raw: unknown): FrostFoxConfig {
  const d = DEFAULT_FROST_FOX_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: FrostFoxSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<FrostFoxSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<FrostFoxBombEntry["mult"], number>();
  for (const mult of FROST_FOX_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<FrostFoxBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(FROST_FOX_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as FrostFoxBombEntry["mult"], weight);
    }
  }
  const bombTable: FrostFoxBombEntry[] = FROST_FOX_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: FrostFoxScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<FrostFoxScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: FrostFoxConfig): {
  id: string;
  kind: FrostFoxSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: FrostFoxBombEntry[]): Record<number, number> {
