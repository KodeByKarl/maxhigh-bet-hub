/**
 * Knockout King — base-game-only 5×3 / 10-payline slot math config.
...
export const KNOCKOUT_KING_GAME_ID = "knockout-king";

export type BkSymKind =
  | "bar"
  | "dice"
  | "diamond"
  | "chip"
...
export type FsRtpProfile = {
  ...
  reelWeights: Record<BkSymKind, number[]>;
};

export type KnockoutKingConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const SYMBOL_NAMES: Record<BkSymKind, string> = {
  bar: "Bars",
  dice: "Dice",
  diamond: "Diamonds",
  chip: "Chips",
  lucky7: "Lucky 7s",
  wild: "Fire Strike Wild",
  scatter: "Fire Strike Scatter",
};
...
function profileWeights(
  entries: Record<BkSymKind, [number, number, number, number, number]>,
): Record<BkSymKind, number[]> {
  const out = {} as Record<BkSymKind, number[]>;
...
export const DEFAULT_KNOCKOUT_KING_CONFIG: KnockoutKingConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: KnockoutKingConfig): FsRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const reelWeights = {} as Record<BkSymKind, number[]>;
    for (const k of SYM_KINDS) {
...
export function effectiveReelWeights(cfg: KnockoutKingConfig): Record<BkSymKind, number[]> {
  const profile =
...
export function normalizeKnockoutKingConfig(raw: unknown): KnockoutKingConfig {
  const d = DEFAULT_KNOCKOUT_KING_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<KnockoutKingConfig>;
...
export function betPerLine(totalBet: number, cfg: KnockoutKingConfig): number {
