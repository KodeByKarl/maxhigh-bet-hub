/**
 * Rice Field Riches (catalog id: rice-field-riches) — math & configuration.
 * 5×4 ways (1,024), cascade multipliers, Golden Campfire free spins,
 * Treasure Chest Hold & Collect, Explorer→Legend jackpots.
 */

export const RICE_FIELD_RICHES_GAME_ID = "rice-field-riches";
export const RICE_FIELD_RICHES_TITLE = "Rice Field Riches";
...
export type RiceFieldRichesConfig = {
  schemaVersion: 1;
  reelsCount: number;
...
export const DEFAULT_RICE_FIELD_RICHES_CONFIG: RiceFieldRichesConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: RiceFieldRichesConfig = DEFAULT_RICE_FIELD_RICHES_CONFIG,
): number {
...
export function normalizeRiceFieldRichesConfig(raw: unknown): RiceFieldRichesConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_RICE_FIELD_RICHES_CONFIG);
  const obj = raw as Partial<RiceFieldRichesConfig>;
  const d = DEFAULT_RICE_FIELD_RICHES_CONFIG;
  ...
  const volatility: RiceFieldRichesConfig["volatility"] =
