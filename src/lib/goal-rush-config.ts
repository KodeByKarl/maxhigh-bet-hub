/**
 * Goal Rush — base-game-only 5×3 / 10-payline slot math config.
...
export const GOAL_RUSH_GAME_ID = "goal-rush";
...
export const DEFAULT_GOAL_RUSH_CONFIG: GoalRushConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: GoalRushConfig): FsRtpProfile[] {
...
export function effectiveReelWeights(cfg: GoalRushConfig): Record<GrSymKind, number[]> {
  const profile =
...
export function normalizeGoalRushConfig(raw: unknown): GoalRushConfig {
  const d = DEFAULT_GOAL_RUSH_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<GoalRushConfig>;
...
export function betPerLine(totalBet: number, cfg: GoalRushConfig): number {
