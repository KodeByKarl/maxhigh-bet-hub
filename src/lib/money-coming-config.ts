/**
 * Money Coming — base-game-only 5×3 / 10-payline slot math config.
...
export const MONEY_COMING_GAME_ID = "money-coming";
...
export const DEFAULT_MONEY_COMING_CONFIG: MoneyComingConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: MoneyComingConfig): FsRtpProfile[] {
...
export function effectiveReelWeights(cfg: MoneyComingConfig): Record<McSymKind, number[]> {
  const profile =
...
export function normalizeMoneyComingConfig(raw: unknown): MoneyComingConfig {
  const d = DEFAULT_MONEY_COMING_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<MoneyComingConfig>;
...
export function betPerLine(totalBet: number, cfg: MoneyComingConfig): number {
