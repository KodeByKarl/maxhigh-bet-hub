/**
 * Lotus Lynx math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const LOTUS_LYNX_GAME_ID = "lotus-lynx";

export type LotusLynxSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type LotusLynxBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const LOTUS_LYNX_BOMB_MULTS = [2, 3, 4, 5] as const;

export type LotusLynxScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type LotusLynxSymbolConfig = {
  id: string;
  kind: LotusLynxSymKind;
  label: string;
...
export type LotusLynxConfig = {
  ...
  scatterCashTiers: LotusLynxScatterCashTier[];
  ...
  symbols: LotusLynxSymbolConfig[];
};

export const SYMBOL_LABELS: Record<LotusLynxSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_LOTUS_LYNX_CONFIG: LotusLynxConfig = {
  schemaVersion: 1,
...
export function clampBombMult(v: unknown): LotusLynxBombEntry["mult"] {
...
export function normalizeLotusLynxConfig(raw: unknown): LotusLynxConfig {
  const d = DEFAULT_LOTUS_LYNX_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: LotusLynxSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<LotusLynxSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<LotusLynxBombEntry["mult"], number>();
  for (const mult of LOTUS_LYNX_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<LotusLynxBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(LOTUS_LYNX_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as LotusLynxBombEntry["mult"], weight);
    }
  }
  const bombTable: LotusLynxBombEntry[] = LOTUS_LYNX_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: LotusLynxScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<LotusLynxScatterCashTier>;
            return {
  ...
  return {
    ...
    seedClusterMin: clamp(Math.round(num(o.seedClusterMin, d.seedClusterMin)), 3, 16),
    seedClusterMax: clamp(Math.round(num(o.seedClusterMax, d.seedClusterMax)), 3, 16),
    ...
    /** 4×4 board — never require more matches than cells. */
    minCluster: clamp(Math.round(num(o.minCluster, d.minCluster)), 3, 16),
...
export function configToCellSyms(cfg: LotusLynxConfig): {
  id: string;
  kind: LotusLynxSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: LotusLynxBombEntry[]): Record<number, number> {
