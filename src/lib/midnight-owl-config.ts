/**
 * Midnight Owl math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const MIDNIGHT_OWL_GAME_ID = "midnight-owl";

export type MidnightOwlSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type MidnightOwlBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const MIDNIGHT_OWL_BOMB_MULTS = [2, 3, 4, 5] as const;

export type MidnightOwlScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type MidnightOwlSymbolConfig = {
  id: string;
  kind: MidnightOwlSymKind;
  label: string;
...
export type MidnightOwlConfig = {
  ...
  bombTable: MidnightOwlBombEntry[];
  ...
  scatterCashTiers: MidnightOwlScatterCashTier[];
  ...
  symbols: MidnightOwlSymbolConfig[];
};

export const SYMBOL_LABELS: Record<MidnightOwlSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_MIDNIGHT_OWL_CONFIG: MidnightOwlConfig = {
  schemaVersion: 1,
  deadSpinChancePercent: 75,
...
export function clampBombMult(v: unknown): MidnightOwlBombEntry["mult"] {
...
export function normalizeMidnightOwlConfig(raw: unknown): MidnightOwlConfig {
  const d = DEFAULT_MIDNIGHT_OWL_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: MidnightOwlSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<MidnightOwlSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<MidnightOwlBombEntry["mult"], number>();
  for (const mult of MIDNIGHT_OWL_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<MidnightOwlBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(MIDNIGHT_OWL_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as MidnightOwlBombEntry["mult"], weight);
    }
  }
  const bombTable: MidnightOwlBombEntry[] = MIDNIGHT_OWL_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: MidnightOwlScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<MidnightOwlScatterCashTier>;
            return {
  ...
  return {
    ...
    /** 5×4 moon board has 20 cells — never require more matches than CELLS. */
    seedClusterMin: clamp(Math.round(num(o.seedClusterMin, d.seedClusterMin)), 3, 20),
    seedClusterMax: clamp(Math.round(num(o.seedClusterMax, d.seedClusterMax)), 3, 20),
    ...
    minCluster: clamp(Math.round(num(o.minCluster, d.minCluster)), 3, 20),
...
export function configToCellSyms(cfg: MidnightOwlConfig): {
  id: string;
  kind: MidnightOwlSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: MidnightOwlBombEntry[]): Record<number, number> {
