 ...
 * - Progressive jackpot → jackpot.id="crazy-sevens"; unlocks only at max bet + exactly 2 DW.
 ...
 * - Must stay under 100%. Re-run `npx tsx scripts/test-crazy-sevens.ts --rtp N` after any weight change.
 */

export const CRAZY_SEVENS_GAME_ID = "crazy-sevens";

/** Platform jackpot table row id for this game's progressive pool. */
export const CRAZY_SEVENS_JACKPOT_ID = "crazy-sevens";
 ...
 * Tune via `npx tsx scripts/test-crazy-sevens.ts --rtp N`.
 */
export const DEFAULT_CRAZY_SEVENS_CONFIG: CrazySevensConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  ...
  volatility: "unknown",
  jackpot: {
    poolId: CRAZY_SEVENS_JACKPOT_ID,
...
export function normalizeCrazySevensConfig(raw: unknown): CrazySevensConfig {
  const d = DEFAULT_CRAZY_SEVENS_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<CrazySevensConfig>;
