/**
 * Starlight Ways math & configuration — shared by server resolver and UI.
...
export const STARLIGHT_WAYS_GAME_ID = "starlight-ways";

export type StarlightWaysSymKind =
  | "sym_10"
  | "sym_j"
  | "sym_q"
  | "sym_k"
...
export type StarlightWaysSymbolConfig = {
  id: string;
...
export type StarlightWaysConfig = {
  schemaVersion: 1;
  reelsCount: number;
  ...
  maxWinMult: number;
  targetRtp: number;
  symbols: StarlightWaysSymbolConfig[];
};

export const SYMBOL_NAMES: Record<StarlightWaysSymKind, string> = {
  sym_10: "10 Gem",
  sym_j: "J Gem",
  sym_q: "Q Gem",
  sym_k: "K Gem",
  ...
  comet: "Comet",
  wild: "Winged Wild",
  scatter: "Starlight Ways Scatter",
};
 ...
 */
export const DEFAULT_STARLIGHT_WAYS_CONFIG: StarlightWaysConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 5,
  maxReelHeight: 5,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: StarlightWaysConfig = DEFAULT_STARLIGHT_WAYS_CONFIG,
): number {
...
export function normalizeStarlightWaysConfig(raw: unknown): StarlightWaysConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_STARLIGHT_WAYS_CONFIG);
  const obj = raw as Partial<StarlightWaysConfig>;
  const d = DEFAULT_STARLIGHT_WAYS_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as StarlightWaysSymKind) || base.kind,
