/**
 * MaxHigh Whale War — shared fish-hunter math + superadmin-editable tables.
...
export const WHALE_WAR_GAME_ID = "whale-war";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = WHALE_WAR_GAME_ID;
export const WHALE_WAR_TITLE = "Whale War";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/whale-war/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Whale War Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_WHALE_WAR_CONFIG: WhaleWarConfig = {
  schemaVersion: 1,
...
export function normalizeWhaleWarConfig(raw: unknown): WhaleWarConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_WHALE_WAR_CONFIG);
  }
  const o = raw as Partial<WhaleWarConfig> & { targetRtp?: number };
  const d = DEFAULT_WHALE_WAR_CONFIG;
