...
export const MAYA_GOLD_GAME_ID = "maya-gold";

export type MayaGoldSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type MayaGoldBombEntry = { mult: number; weight: number };

export type MayaGoldScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type MayaGoldSymbolConfig = {
  id: string;
  kind: MayaGoldSymKind;
  label: string;
...
export type MayaGoldConfig = {
  ...
  scatterCashTiers: MayaGoldScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: MayaGoldSymbolConfig[];
};

export const SYMBOL_LABELS: Record<MayaGoldSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_MAYA_GOLD_CONFIG: MayaGoldConfig = {
  schemaVersion: 1,
...
export function normalizeMayaGoldConfig(raw: unknown): MayaGoldConfig {
  const d = DEFAULT_MAYA_GOLD_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: MayaGoldSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<MayaGoldSymbolConfig> | undefined;
  ...
  const bombTable: MayaGoldBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<MayaGoldBombEntry>;
            return {
  ...
  const scatterCashTiers: MayaGoldScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<MayaGoldScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: MayaGoldConfig): {
  id: string;
  kind: MayaGoldSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: MayaGoldBombEntry[]): Record<number, number> {
