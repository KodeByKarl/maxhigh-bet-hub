/**
 * MaxHigh Deep Bass — shared fish-hunter math + superadmin-editable tables.
...
export const DEEP_BASS_GAME_ID = "deep-bass";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = DEEP_BASS_GAME_ID;
export const DEEP_BASS_TITLE = "Deep Bass";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/deep-bass/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export const DEFAULT_FISH_TIERS: FishTierConfig[] = [
  ...
  },
  {
    ...
    hitsMin: 16,
    hitsMax: 24,
    // Harder to land: E[mult]≈105, E[hits]≈20 → p ≈ 0.183
    baseHitChance: 0.183,
    spawnWeight: 3,
    size: 210,
  },
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Deep Bass Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_DEEP_BASS_CONFIG: DeepBassConfig = {
  schemaVersion: 1,
...
function normalizeFishTier(raw: unknown): FishTierConfig[] {
  ...
  return defaults.map((d) => {
    ...
    return {
      ...
      size: clamp(Math.round(num(o.size, d.size)), 24, 320),
    };
  });
}
...
export function normalizeDeepBassConfig(raw: unknown): DeepBassConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_DEEP_BASS_CONFIG);
  }
  const o = raw as Partial<DeepBassConfig> & { targetRtp?: number };
  const d = DEFAULT_DEEP_BASS_CONFIG;
