/**
 * Sweet Rush math config — shared by engine (client) and Superadmin (server).
 * Defaults follow educational Sweet Rush–style cluster / multiplier / FS tables.
 */

export const SWEET_RUSH_GAME_ID = "sweet-rush";

export type SweetRushSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type SweetRushBombEntry = { mult: number; weight: number };

export type SweetRushScatterCashTier = {
  count: number;
  /** × bet — 0 = FS-only scatters */
  mult: number;
};

export type SweetRushFreeSpinAward = {
  count: number;
  spins: number;
};

export type SweetRushSymbolConfig = {
  id: string;
  kind: SweetRushSymKind;
  label: string;
  weight: number;
...
export type SweetRushConfig = {
  ...
  freeSpinsByScatterCount: SweetRushFreeSpinAward[];
  anteScatterWeightMult: number;
  scatterCashTiers: SweetRushScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: SweetRushSymbolConfig[];
};

export const SYMBOL_LABELS: Record<SweetRushSymKind, string> = {
  grape: "Red bear",
  plum: "Orange bear",
  melon: "Purple bear",
  apple: "Green star",
...
export const DEFAULT_SWEET_RUSH_CONFIG: SweetRushConfig = {
  schemaVersion: 1,
...
export function normalizeSweetRushConfig(raw: unknown): SweetRushConfig {
  const d = DEFAULT_SWEET_RUSH_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: SweetRushSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<SweetRushSymbolConfig> | undefined;
  ...
  const bombTable: SweetRushBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<SweetRushBombEntry>;
            return {
  ...
  const scatterCashTiers: SweetRushScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<SweetRushScatterCashTier>;
            return {
  ...
  const freeSpinsByScatterCount: SweetRushFreeSpinAward[] =
    fsAwardIn && fsAwardIn.length > 0
      ? fsAwardIn
          .map((t) => {
            const row = t as Partial<SweetRushFreeSpinAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: SweetRushConfig): {
  id: string;
  kind: SweetRushSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: SweetRushBombEntry[]): Record<number, number> {
