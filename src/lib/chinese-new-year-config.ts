/**
 * Chinese New Year — fixed-payline slot math config (Play'n GO–style).
...
export function payingSymbolKinds(cfg: ChineseNewYearConfig = DEFAULT_CHINESE_NEW_YEAR_CONFIG): CnySymKind[] {
  return cfg.symbols
    .filter((s) => s.tier === "low" || s.tier === "high")
    .map((s) => s.kind);
}

export function betPerLine(totalBet: number, cfg: ChineseNewYearConfig): number {
