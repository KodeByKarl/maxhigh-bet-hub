/**
 * Mystic Runes math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Egyptian ways values.
 */

export const MYSTIC_RUNES_GAME_ID = "mystic-runes";

export type MysticRunesSymKind =
  | "ten"
  | "jack"
  | "queen"
  | "king"
...
export type MysticRunesScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type MysticRunesFreeSpinsAward = {
  count: number;
  spins: number;
};

export type MysticRunesSymbolConfig = {
  id: string;
  kind: MysticRunesSymKind;
  label: string;
...
export type MysticRunesConfig = {
  ...
  freeSpinsAwards: MysticRunesFreeSpinsAward[];
  scatterCashTiers: MysticRunesScatterCashTier[];
  buyFeatureMult: number;
  fsMultStart: number;
  fsMultStep: number;
  symbols: MysticRunesSymbolConfig[];
};

export const DEFAULT_MYSTIC_RUNES_CONFIG: MysticRunesConfig = {
  schemaVersion: 1,
...
export function normalizeMysticRunesConfig(raw: unknown): MysticRunesConfig {
  const d = DEFAULT_MYSTIC_RUNES_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: MysticRunesSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<MysticRunesSymbolConfig> | undefined;
  ...
  const scatterCashTiers: MysticRunesScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<MysticRunesScatterCashTier>;
            return {
  ...
  const freeSpinsAwards: MysticRunesFreeSpinsAward[] =
    awardsIn && awardsIn.length > 0
      ? awardsIn
          .map((t) => {
            const row = t as Partial<MysticRunesFreeSpinsAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: MysticRunesConfig) {
...
export function freeSpinsAwardForCount(cfg: MysticRunesConfig, count: number): number {
