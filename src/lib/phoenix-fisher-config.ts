/**
 * MaxHigh Phoenix Fisher — shared fish-hunter math + superadmin-editable tables.
...
export const PHOENIX_FISHER_GAME_ID = "phoenix-fisher";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = PHOENIX_FISHER_GAME_ID;
export const PHOENIX_FISHER_TITLE = "Phoenix Fisher";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/phoenix-fisher/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export type PhoenixFisherConfig = {
  schemaVersion: 1;
...
export const DEFAULT_FISH_TIERS: FishTierConfig[] = [
  ...
  {
    id: "boss",
    label: "The Phoenix Fisher",
    sprite: "phoenix-fisher-boss",
    payoutMin: 100,
    payoutMax: 500,
    hitsMin: 40,
    hitsMax: 60,
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Phoenix Fisher Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_PHOENIX_FISHER_CONFIG: PhoenixFisherConfig = {
  schemaVersion: 1,
...
export function normalizePhoenixFisherConfig(raw: unknown): PhoenixFisherConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_PHOENIX_FISHER_CONFIG);
  }
  const o = raw as Partial<PhoenixFisherConfig> & { targetRtp?: number };
  const d = DEFAULT_PHOENIX_FISHER_CONFIG;
