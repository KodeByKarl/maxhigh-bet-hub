/**
 * Olympus Wrath math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Egyptian ways values.
 */

export const OLYMPUS_WRATH_GAME_ID = "olympus-wrath";

export type OlympusWrathSymKind =
  | "ten"
  | "jack"
  | "queen"
  | "king"
...
export type OlympusWrathScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type OlympusWrathFreeSpinsAward = {
  count: number;
  spins: number;
};

export type OlympusWrathSymbolConfig = {
  id: string;
  kind: OlympusWrathSymKind;
  label: string;
...
export type OlympusWrathConfig = {
  schemaVersion: 1;
  ...
  freeSpinsAwards: OlympusWrathFreeSpinsAward[];
  scatterCashTiers: OlympusWrathScatterCashTier[];
  buyFeatureMult: number;
  fsMultStart: number;
  fsMultStep: number;
  symbols: OlympusWrathSymbolConfig[];
};

export const DEFAULT_OLYMPUS_WRATH_CONFIG: OlympusWrathConfig = {
  schemaVersion: 1,
...
export function normalizeOlympusWrathConfig(raw: unknown): OlympusWrathConfig {
  const d = DEFAULT_OLYMPUS_WRATH_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: OlympusWrathSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<OlympusWrathSymbolConfig> | undefined;
  ...
  const scatterCashTiers: OlympusWrathScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<OlympusWrathScatterCashTier>;
            return {
  ...
  const freeSpinsAwards: OlympusWrathFreeSpinsAward[] =
    awardsIn && awardsIn.length > 0
      ? awardsIn
          .map((t) => {
            const row = t as Partial<OlympusWrathFreeSpinsAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: OlympusWrathConfig) {
...
export function freeSpinsAwardForCount(cfg: OlympusWrathConfig, count: number): number {
