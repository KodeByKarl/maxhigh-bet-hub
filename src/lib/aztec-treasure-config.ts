...
export const AZTEC_TREASURE_GAME_ID = "aztec-treasure";

export type AztecTreasureSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type AztecTreasureBombEntry = { mult: number; weight: number };

export type AztecTreasureScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type AztecTreasureSymbolConfig = {
  id: string;
  kind: AztecTreasureSymKind;
  label: string;
...
export type AztecTreasureConfig = {
  schemaVersion: 1;
  ...
  bombTable: AztecTreasureBombEntry[];
  ...
  scatterCashTiers: AztecTreasureScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: AztecTreasureSymbolConfig[];
};

export const SYMBOL_LABELS: Record<AztecTreasureSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_AZTEC_TREASURE_CONFIG: AztecTreasureConfig = {
  schemaVersion: 1,
...
export function normalizeAztecTreasureConfig(raw: unknown): AztecTreasureConfig {
  const d = DEFAULT_AZTEC_TREASURE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: AztecTreasureSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<AztecTreasureSymbolConfig> | undefined;
  ...
  const bombTable: AztecTreasureBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<AztecTreasureBombEntry>;
            return {
  ...
  const scatterCashTiers: AztecTreasureScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<AztecTreasureScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: AztecTreasureConfig): {
  id: string;
  kind: AztecTreasureSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: AztecTreasureBombEntry[]): Record<number, number> {
