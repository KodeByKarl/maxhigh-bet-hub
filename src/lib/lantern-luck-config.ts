/**
 * Lantern Luck — fixed-payline slot math config (Play'n GO–style).
...
export const LANTERN_LUCK_GAME_ID = "lantern-luck";
...
export const DEFAULT_LANTERN_LUCK_CONFIG: LanternLuckConfig = {
  schemaVersion: 2,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 20,
...
export function normalizeLanternLuckConfig(raw: unknown): LanternLuckConfig {
  const d = DEFAULT_LANTERN_LUCK_CONFIG;
...
export function payingSymbolKinds(cfg: LanternLuckConfig = DEFAULT_LANTERN_LUCK_CONFIG): CnySymKind[] {
  return cfg.symbols
    .filter((s) => s.tier === "low" || s.tier === "high")
    .map((s) => s.kind);
}

export function betPerLine(totalBet: number, cfg: LanternLuckConfig): number {
