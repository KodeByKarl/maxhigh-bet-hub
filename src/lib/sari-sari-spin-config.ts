/**
 * Sari-Sari Spin — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Sari-Sari Spin 2/3):
...
export const SARI_SARI_SPIN_GAME_ID = "sari-sari-spin";
...
export type SariSariSpinConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const DEFAULT_SARI_SARI_SPIN_CONFIG: SariSariSpinConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
...
function normalizeRtpProfiles(raw: unknown, d: SariSariSpinConfig): FgRtpProfile[] {
...
export function effectiveReelWeights(cfg: SariSariSpinConfig): Record<FoSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: SariSariSpinConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeSariSariSpinConfig(raw: unknown): SariSariSpinConfig {
  const d = DEFAULT_SARI_SARI_SPIN_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<SariSariSpinConfig>;
...
export function betPerLine(totalBet: number, cfg: SariSariSpinConfig): number {
