/**
 * Tricycle Treasure: Hold & Win — diamond connecting-ways western slot math.
...
export const TRICYCLE_TREASURE_GAME_ID = "tricycle-treasure";
...
export type TricycleTreasureConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
...
export function totalCells(cfg: Pick<TricycleTreasureConfig, "reelHeights">): number {
...
export const DEFAULT_TRICYCLE_TREASURE_CONFIG: TricycleTreasureConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: TricycleTreasureConfig = DEFAULT_TRICYCLE_TREASURE_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: TricycleTreasureConfig = DEFAULT_TRICYCLE_TREASURE_CONFIG,
): number {
...
export function normalizeTricycleTreasureConfig(raw: unknown): TricycleTreasureConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_TRICYCLE_TREASURE_CONFIG);
  const obj = raw as Partial<TricycleTreasureConfig>;
  const d = DEFAULT_TRICYCLE_TREASURE_CONFIG;
