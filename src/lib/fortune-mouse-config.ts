/**
 * Fortune Mouse — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Fortune Mouse 2/3):
...
export const FORTUNE_MOUSE_GAME_ID = "fortune-mouse";
...
export type FortuneMouseConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const DEFAULT_FORTUNE_MOUSE_CONFIG: FortuneMouseConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
...
function normalizeRtpProfiles(raw: unknown, d: FortuneMouseConfig): FgRtpProfile[] {
...
export function effectiveReelWeights(cfg: FortuneMouseConfig): Record<FoSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: FortuneMouseConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeFortuneMouseConfig(raw: unknown): FortuneMouseConfig {
  const d = DEFAULT_FORTUNE_MOUSE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<FortuneMouseConfig>;
...
export function betPerLine(totalBet: number, cfg: FortuneMouseConfig): number {
