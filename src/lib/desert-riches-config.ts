/**
 * Desert Riches: Hold & Win — diamond connecting-ways western slot math.
...
export const DESERT_RICHES_GAME_ID = "desert-riches";
...
export type DesertRichesConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
...
export function totalCells(cfg: Pick<DesertRichesConfig, "reelHeights">): number {
...
export const DEFAULT_DESERT_RICHES_CONFIG: DesertRichesConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: DesertRichesConfig = DEFAULT_DESERT_RICHES_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: DesertRichesConfig = DEFAULT_DESERT_RICHES_CONFIG,
): number {
...
export function normalizeDesertRichesConfig(raw: unknown): DesertRichesConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_DESERT_RICHES_CONFIG);
  const obj = raw as Partial<DesertRichesConfig>;
  const d = DEFAULT_DESERT_RICHES_CONFIG;
