/**
 * Fortune Gems — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Fortune Gems 2/3):
...
export const FORTUNE_GEMS_GAME_ID = "fortune-gems";
...
export const FG_MULTIPLIER_VALUES = [1, 2, 3, 5, 10, 15] as const;
export type FgMultiplierValue = (typeof FG_MULTIPLIER_VALUES)[number];
...
export type FgRtpProfile = {
  ...
  reelWeights: Record<FgSymKind, number[]>;
};
...
function profileWeights(
  entries: Record<FgSymKind, [number, number, number]>,
): Record<FgSymKind, number[]> {
  const out = {} as Record<FgSymKind, number[]>;
...
export const DEFAULT_FORTUNE_GEMS_CONFIG: FortuneGemsConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
  ...
  rtpProfiles: DEFAULT_RTP_PROFILES.map((p) => ({
    ...
    ) as Record<FgSymKind, number[]>,
  })),
  multiplierValues: [...FG_MULTIPLIER_VALUES],
  multiplierStripBase: {
...
function normalizeRtpProfiles(raw: unknown, d: FortuneGemsConfig): FgRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const reelWeights = {} as Record<FgSymKind, number[]>;
    for (const k of SYM_KINDS) {
...
export function effectiveReelWeights(cfg: FortuneGemsConfig): Record<FgSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: FortuneGemsConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeFortuneGemsConfig(raw: unknown): FortuneGemsConfig {
  const d = DEFAULT_FORTUNE_GEMS_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<FortuneGemsConfig>;
  ...
  if (multiplierValues.length === 0) multiplierValues.push(...FG_MULTIPLIER_VALUES);
...
export function betPerLine(totalBet: number, cfg: FortuneGemsConfig): number {
