/**
 * Fire Spike — base-game-only 5×3 / 10-payline slot math config.
...
export const FIRE_SPIKE_GAME_ID = "fire-spike";

export type FsSymKind =
  | "bar"
  | "dice"
  | "diamond"
  | "chip"
...
export type FsRtpProfile = {
  ...
  reelWeights: Record<FsSymKind, number[]>;
};

export type FireSpikeConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const SYMBOL_NAMES: Record<FsSymKind, string> = {
  bar: "Bars",
  dice: "Dice",
  diamond: "Diamonds",
  chip: "Chips",
...
function profileWeights(
  entries: Record<FsSymKind, [number, number, number, number, number]>,
): Record<FsSymKind, number[]> {
  const out = {} as Record<FsSymKind, number[]>;
...
export const DEFAULT_FIRE_SPIKE_CONFIG: FireSpikeConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: FireSpikeConfig): FsRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const reelWeights = {} as Record<FsSymKind, number[]>;
    for (const k of SYM_KINDS) {
...
export function effectiveReelWeights(cfg: FireSpikeConfig): Record<FsSymKind, number[]> {
  const profile =
...
export function normalizeFireSpikeConfig(raw: unknown): FireSpikeConfig {
  const d = DEFAULT_FIRE_SPIKE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<FireSpikeConfig>;
...
export function betPerLine(totalBet: number, cfg: FireSpikeConfig): number {
