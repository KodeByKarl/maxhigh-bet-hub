/**
 * Boxing King — base-game-only 5×3 / 10-payline slot math config.
...
export const BOXING_KING_GAME_ID = "boxing-king";
...
export const DEFAULT_BOXING_KING_CONFIG: BoxingKingConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: BoxingKingConfig): FsRtpProfile[] {
...
export function effectiveReelWeights(cfg: BoxingKingConfig): Record<BkSymKind, number[]> {
  const profile =
...
export function normalizeBoxingKingConfig(raw: unknown): BoxingKingConfig {
  const d = DEFAULT_BOXING_KING_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<BoxingKingConfig>;
...
export function betPerLine(totalBet: number, cfg: BoxingKingConfig): number {
