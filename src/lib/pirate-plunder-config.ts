/**
 * Pirate Plunder math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const PIRATE_PLUNDER_GAME_ID = "pirate-plunder";

export type PiratePlunderSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type PiratePlunderBombEntry = { mult: number; weight: number };

export type PiratePlunderScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type PiratePlunderSymbolConfig = {
  id: string;
  kind: PiratePlunderSymKind;
  label: string;
...
export type PiratePlunderConfig = {
  schemaVersion: 1;
  ...
  bombTable: PiratePlunderBombEntry[];
  ...
  scatterCashTiers: PiratePlunderScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: PiratePlunderSymbolConfig[];
};

export const SYMBOL_LABELS: Record<PiratePlunderSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_PIRATE_PLUNDER_CONFIG: PiratePlunderConfig = {
  schemaVersion: 1,
...
export function normalizePiratePlunderConfig(raw: unknown): PiratePlunderConfig {
  const d = DEFAULT_PIRATE_PLUNDER_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: PiratePlunderSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<PiratePlunderSymbolConfig> | undefined;
  ...
  const bombTable: PiratePlunderBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<PiratePlunderBombEntry>;
            return {
  ...
  const scatterCashTiers: PiratePlunderScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<PiratePlunderScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: PiratePlunderConfig): {
  id: string;
  kind: PiratePlunderSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: PiratePlunderBombEntry[]): Record<number, number> {
