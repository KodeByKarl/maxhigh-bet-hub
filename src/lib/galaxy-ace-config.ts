/**
 * Galaxy Ace math & configuration — shared by server resolver and UI.
...
export const GALAXY_ACE_GAME_ID = "galaxy-ace";

export type GalaxyAceSymKind =
  | "sym_10"
  | "sym_j"
  | "sym_q"
  | "sym_k"
...
export type GalaxyAceSymbolConfig = {
  id: string;
...
export type GalaxyAceConfig = {
  ...
  maxWinMult: number;
  targetRtp: number;
  symbols: GalaxyAceSymbolConfig[];
};

export const SYMBOL_NAMES: Record<GalaxyAceSymKind, string> = {
  sym_10: "10 Gem",
  sym_j: "J Gem",
  sym_q: "Q Gem",
  sym_k: "K Gem",
 ...
 */
export const DEFAULT_GALAXY_ACE_CONFIG: GalaxyAceConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 5,
  maxReelHeight: 5,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: GalaxyAceConfig = DEFAULT_GALAXY_ACE_CONFIG,
): number {
...
export function normalizeGalaxyAceConfig(raw: unknown): GalaxyAceConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_GALAXY_ACE_CONFIG);
  const obj = raw as Partial<GalaxyAceConfig>;
  const d = DEFAULT_GALAXY_ACE_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as GalaxyAceSymKind) || base.kind,
