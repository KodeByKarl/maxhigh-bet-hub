/**
 * MaxHigh Color Game Pro (PH perya-style, simplified).
...
export const COLOR_GAME_PRO_GAME_ID = "color-game-pro";
export const GAME_ID = COLOR_GAME_PRO_GAME_ID;
export const COLOR_GAME_PRO_TITLE = "Color Game Pro";
...
export type ColorGameProConfig = {
  schemaVersion: 1;
  spots: ColorSpotDef[];
...
export const DEFAULT_COLOR_GAME_PRO_CONFIG: ColorGameProConfig = {
  schemaVersion: 1,
...
export const BET_STEPS = DEFAULT_COLOR_GAME_PRO_CONFIG.betSteps;
...
export function normalizeColorGameProConfig(raw: unknown): ColorGameProConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_COLOR_GAME_PRO_CONFIG);
  const o = raw as Partial<ColorGameProConfig> & { minSpotBet?: number };
  const d = DEFAULT_COLOR_GAME_PRO_CONFIG;
