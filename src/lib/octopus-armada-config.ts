/**
 * MaxHigh Octopus Armada — shared fish-hunter math + superadmin-editable tables.
...
export const OCTOPUS_ARMADA_GAME_ID = "octopus-armada";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = OCTOPUS_ARMADA_GAME_ID;
export const OCTOPUS_ARMADA_TITLE = "Octopus Armada";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/octopus-armada/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export type OctopusArmadaConfig = {
  schemaVersion: 1;
...
export const DEFAULT_FISH_TIERS: FishTierConfig[] = [
  ...
  {
    id: "boss",
    label: "The Octopus Armada",
    sprite: "octopus-armada-boss",
    payoutMin: 100,
    payoutMax: 500,
    hitsMin: 40,
    hitsMax: 60,
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Octopus Armada Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_OCTOPUS_ARMADA_CONFIG: OctopusArmadaConfig = {
  schemaVersion: 1,
...
export function normalizeOctopusArmadaConfig(raw: unknown): OctopusArmadaConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_OCTOPUS_ARMADA_CONFIG);
  }
  const o = raw as Partial<OctopusArmadaConfig> & { targetRtp?: number };
  const d = DEFAULT_OCTOPUS_ARMADA_CONFIG;
