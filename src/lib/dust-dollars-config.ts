/**
 * Dust & Dollars: Hold & Win — diamond connecting-ways western slot math.
...
export const DUST_DOLLARS_GAME_ID = "dust-dollars";
...
export function totalCells(cfg: Pick<DustDollarsConfig, "reelHeights">): number {
...
export const DEFAULT_DUST_DOLLARS_CONFIG: DustDollarsConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: DustDollarsConfig = DEFAULT_DUST_DOLLARS_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: DustDollarsConfig = DEFAULT_DUST_DOLLARS_CONFIG,
): number {
...
export function normalizeDustDollarsConfig(raw: unknown): DustDollarsConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_DUST_DOLLARS_CONFIG);
  const obj = raw as Partial<DustDollarsConfig>;
  const d = DEFAULT_DUST_DOLLARS_CONFIG;
