/**
 * MaxHigh Dragon Fisher — shared fish-hunter math + superadmin-editable tables.
...
export const DRAGON_FISHER_GAME_ID = "dragon-fisher";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = DRAGON_FISHER_GAME_ID;
export const DRAGON_FISHER_TITLE = "Dragon Fisher";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/dragon-fisher/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export type DragonFisherConfig = {
  schemaVersion: 1;
...
export const DEFAULT_FISH_TIERS: FishTierConfig[] = [
  ...
  {
    id: "boss",
    label: "The Dragon Fisher",
    sprite: "dragon-fisher-boss",
    payoutMin: 100,
    payoutMax: 500,
    hitsMin: 40,
    hitsMax: 60,
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Dragon Fisher Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_DRAGON_FISHER_CONFIG: DragonFisherConfig = {
  schemaVersion: 1,
...
export function normalizeDragonFisherConfig(raw: unknown): DragonFisherConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_DRAGON_FISHER_CONFIG);
  }
  const o = raw as Partial<DragonFisherConfig> & { targetRtp?: number };
  const d = DEFAULT_DRAGON_FISHER_CONFIG;
