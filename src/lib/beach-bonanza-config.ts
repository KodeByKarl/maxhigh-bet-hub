/**
 * Beach Bonanza math config — shared by engine (client) and Superadmin (server).
 * Defaults follow educational Beach Bonanza–style cluster / multiplier / FS tables.
 */

export const BEACH_BONANZA_GAME_ID = "beach-bonanza";

export type BeachBonanzaSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type BeachBonanzaBombEntry = { mult: number; weight: number };

export type BeachBonanzaScatterCashTier = {
  count: number;
  /** × bet — 0 = FS-only scatters */
  mult: number;
};

export type BeachBonanzaFreeSpinAward = {
  count: number;
  spins: number;
};

export type BeachBonanzaSymbolConfig = {
  id: string;
  kind: BeachBonanzaSymKind;
  label: string;
  weight: number;
...
export type BeachBonanzaConfig = {
  schemaVersion: 1;
  ...
  bombTable: BeachBonanzaBombEntry[];
  ...
  freeSpinsByScatterCount: BeachBonanzaFreeSpinAward[];
  anteScatterWeightMult: number;
  scatterCashTiers: BeachBonanzaScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: BeachBonanzaSymbolConfig[];
};

export const SYMBOL_LABELS: Record<BeachBonanzaSymKind, string> = {
  grape: "Red bear",
  plum: "Orange bear",
  melon: "Purple bear",
  apple: "Green star",
...
export const DEFAULT_BEACH_BONANZA_CONFIG: BeachBonanzaConfig = {
  schemaVersion: 1,
...
export function normalizeBeachBonanzaConfig(raw: unknown): BeachBonanzaConfig {
  const d = DEFAULT_BEACH_BONANZA_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: BeachBonanzaSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<BeachBonanzaSymbolConfig> | undefined;
  ...
  const bombTable: BeachBonanzaBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<BeachBonanzaBombEntry>;
            return {
  ...
  const scatterCashTiers: BeachBonanzaScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<BeachBonanzaScatterCashTier>;
            return {
  ...
  const freeSpinsByScatterCount: BeachBonanzaFreeSpinAward[] =
    fsAwardIn && fsAwardIn.length > 0
      ? fsAwardIn
          .map((t) => {
            const row = t as Partial<BeachBonanzaFreeSpinAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: BeachBonanzaConfig): {
  id: string;
  kind: BeachBonanzaSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: BeachBonanzaBombEntry[]): Record<number, number> {
