/**
 * Balut Bonus — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Balut Bonus 2/3):
...
export const BALUT_BONUS_GAME_ID = "balut-bonus";
...
export const DEFAULT_BALUT_BONUS_CONFIG: BalutBonusConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
...
function normalizeRtpProfiles(raw: unknown, d: BalutBonusConfig): FgRtpProfile[] {
...
export function effectiveReelWeights(cfg: BalutBonusConfig): Record<FoSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: BalutBonusConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeBalutBonusConfig(raw: unknown): BalutBonusConfig {
  const d = DEFAULT_BALUT_BONUS_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<BalutBonusConfig>;
...
export function betPerLine(totalBet: number, cfg: BalutBonusConfig): number {
