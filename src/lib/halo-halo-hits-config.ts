/**
 * Halo-Halo Hits — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Halo-Halo Hits 2/3):
...
export const HALO_HALO_HITS_GAME_ID = "halo-halo-hits";
...
export type HaloHaloHitsConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const DEFAULT_HALO_HALO_HITS_CONFIG: HaloHaloHitsConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
...
function normalizeRtpProfiles(raw: unknown, d: HaloHaloHitsConfig): FgRtpProfile[] {
...
export function effectiveReelWeights(cfg: HaloHaloHitsConfig): Record<FoSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: HaloHaloHitsConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeHaloHaloHitsConfig(raw: unknown): HaloHaloHitsConfig {
  const d = DEFAULT_HALO_HALO_HITS_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<HaloHaloHitsConfig>;
...
export function betPerLine(totalBet: number, cfg: HaloHaloHitsConfig): number {
