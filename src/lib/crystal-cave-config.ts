/**
 * Crystal Cave: Hold & Win — diamond connecting-ways western slot math.
...
export const CRYSTAL_CAVE_GAME_ID = "crystal-cave";
...
export function totalCells(cfg: Pick<CrystalCaveConfig, "reelHeights">): number {
...
export const DEFAULT_CRYSTAL_CAVE_CONFIG: CrystalCaveConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: CrystalCaveConfig = DEFAULT_CRYSTAL_CAVE_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: CrystalCaveConfig = DEFAULT_CRYSTAL_CAVE_CONFIG,
): number {
...
export function normalizeCrystalCaveConfig(raw: unknown): CrystalCaveConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_CRYSTAL_CAVE_CONFIG);
  const obj = raw as Partial<CrystalCaveConfig>;
  const d = DEFAULT_CRYSTAL_CAVE_CONFIG;
