/**
 * Coral Cobra math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const CORAL_COBRA_GAME_ID = "coral-cobra";

export type CoralCobraSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type CoralCobraBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const CORAL_COBRA_BOMB_MULTS = [2, 3, 4, 5] as const;

export type CoralCobraScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type CoralCobraSymbolConfig = {
  id: string;
  kind: CoralCobraSymKind;
  label: string;
...
export type CoralCobraConfig = {
  ...
  scatterCashTiers: CoralCobraScatterCashTier[];
  ...
  symbols: CoralCobraSymbolConfig[];
};

export const SYMBOL_LABELS: Record<CoralCobraSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_CORAL_COBRA_CONFIG: CoralCobraConfig = {
  schemaVersion: 1,
  ...
  anteBetMult: 1.25,
  /** 5×5 board — cluster pays from 8 matching symbols. */
  minCluster: 8,
...
export function clampBombMult(v: unknown): CoralCobraBombEntry["mult"] {
...
export function normalizeCoralCobraConfig(raw: unknown): CoralCobraConfig {
  const d = DEFAULT_CORAL_COBRA_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: CoralCobraSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<CoralCobraSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<CoralCobraBombEntry["mult"], number>();
  for (const mult of CORAL_COBRA_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<CoralCobraBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(CORAL_COBRA_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as CoralCobraBombEntry["mult"], weight);
    }
  }
  const bombTable: CoralCobraBombEntry[] = CORAL_COBRA_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: CoralCobraScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<CoralCobraScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: CoralCobraConfig): {
  id: string;
  kind: CoralCobraSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: CoralCobraBombEntry[]): Record<number, number> {
