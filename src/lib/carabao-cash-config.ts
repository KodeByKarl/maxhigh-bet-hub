/**
 * Carabao Cash (catalog id: carabao-cash) — math & configuration.
 * 5×4 ways (1,024), cascade multipliers, Golden Campfire free spins,
 * Treasure Chest Hold & Collect, Explorer→Legend jackpots.
 */

export const CARABAO_CASH_GAME_ID = "carabao-cash";
export const CARABAO_CASH_TITLE = "Carabao Cash";
...
export const DEFAULT_CARABAO_CASH_CONFIG: CarabaoCashConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: CarabaoCashConfig = DEFAULT_CARABAO_CASH_CONFIG,
): number {
...
export function normalizeCarabaoCashConfig(raw: unknown): CarabaoCashConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_CARABAO_CASH_CONFIG);
  const obj = raw as Partial<CarabaoCashConfig>;
  const d = DEFAULT_CARABAO_CASH_CONFIG;
  ...
  const volatility: CarabaoCashConfig["volatility"] =
