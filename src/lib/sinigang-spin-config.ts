/**
 * Sinigang Spin — fixed-payline slot math config (Play'n GO–style).
...
export const SINIGANG_SPIN_GAME_ID = "sinigang-spin";
...
export type SinigangSpinConfig = {
  schemaVersion: 2;
  reelsCount: number;
  rowsCount: number;
...
export const DEFAULT_SINIGANG_SPIN_CONFIG: SinigangSpinConfig = {
  schemaVersion: 2,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 20,
...
export function normalizeSinigangSpinConfig(raw: unknown): SinigangSpinConfig {
  const d = DEFAULT_SINIGANG_SPIN_CONFIG;
...
export function payingSymbolKinds(cfg: SinigangSpinConfig = DEFAULT_SINIGANG_SPIN_CONFIG): CnySymKind[] {
  return cfg.symbols
    .filter((s) => s.tier === "low" || s.tier === "high")
    .map((s) => s.kind);
}

export function betPerLine(totalBet: number, cfg: SinigangSpinConfig): number {
