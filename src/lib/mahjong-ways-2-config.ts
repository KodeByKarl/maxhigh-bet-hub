/**
 * Mahjong Ways 2 math & configuration — shared by server resolver and UI components.
 * Tunable paytable / weights / ladders live here (never hardcode in engine loops).
 */

export const MAHJONG_WAYS_2_GAME_ID = "mahjong-ways-2";
...
export type MahjongWays2Config = {
  schemaVersion: 1;
  reelsCount: number;
...
export const DEFAULT_MAHJONG_WAYS_2_CONFIG: MahjongWays2Config = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(scatterCount: number, cfg: MahjongWays2Config = DEFAULT_MAHJONG_WAYS_2_CONFIG): number {
...
export function normalizeMahjongWays2Config(raw: unknown): MahjongWays2Config {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_MAHJONG_WAYS_2_CONFIG);
  const obj = raw as Partial<MahjongWays2Config>;
  const d = DEFAULT_MAHJONG_WAYS_2_CONFIG;
