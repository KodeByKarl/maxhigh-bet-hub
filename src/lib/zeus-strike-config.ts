/**
 * Zeus Strike math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Egyptian ways values.
 */

export const ZEUS_STRIKE_GAME_ID = "zeus-strike";

export type ZeusStrikeSymKind =
  | "ten"
  | "jack"
  | "queen"
  | "king"
...
export type ZeusStrikeScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type ZeusStrikeFreeSpinsAward = {
  count: number;
  spins: number;
};

export type ZeusStrikeSymbolConfig = {
  id: string;
  kind: ZeusStrikeSymKind;
  label: string;
...
export type ZeusStrikeConfig = {
  ...
  freeSpinsAwards: ZeusStrikeFreeSpinsAward[];
  scatterCashTiers: ZeusStrikeScatterCashTier[];
  buyFeatureMult: number;
  fsMultStart: number;
  fsMultStep: number;
  symbols: ZeusStrikeSymbolConfig[];
};

export const DEFAULT_ZEUS_STRIKE_CONFIG: ZeusStrikeConfig = {
  schemaVersion: 1,
...
export function normalizeZeusStrikeConfig(raw: unknown): ZeusStrikeConfig {
  const d = DEFAULT_ZEUS_STRIKE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: ZeusStrikeSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<ZeusStrikeSymbolConfig> | undefined;
  ...
  const scatterCashTiers: ZeusStrikeScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<ZeusStrikeScatterCashTier>;
            return {
  ...
  const freeSpinsAwards: ZeusStrikeFreeSpinsAward[] =
    awardsIn && awardsIn.length > 0
      ? awardsIn
          .map((t) => {
            const row = t as Partial<ZeusStrikeFreeSpinsAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: ZeusStrikeConfig) {
...
export function freeSpinsAwardForCount(cfg: ZeusStrikeConfig, count: number): number {
