/**
 * Mega Ace math & configuration — shared by server resolver and UI.
...
export const WILD_ACE_GAME_ID = "wild-ace";

export type WildAceSymKind =
  | "sym_j"
  | "sym_q"
  | "sym_k"
  | "sym_a"
...
export type WildAceSymbolConfig = {
  id: string;
  kind: WildAceSymKind;
  name: string;
...
export type SaRtpProfile = {
  ...
  symbolWeights: Partial<Record<WildAceSymKind, SaSymbolWeightOverride>>;
};

export type WildAceConfig = {
  schemaVersion: 1;
  reelsCount: number;
  ...
  symbols: WildAceSymbolConfig[];
};

export const SYMBOL_NAMES: Record<WildAceSymKind, string> = {
  sym_j: "Jack",
  sym_q: "Queen",
  sym_k: "King",
  sym_a: "Ace",
...
export const ALL_SYM_KINDS: WildAceSymKind[] = [
  "sym_j",
  "sym_q",
  "sym_k",
  "sym_a",
 ...
 */
const BASE_SYMBOLS: WildAceSymbolConfig[] = [
  {
    id: "sym_j",
    kind: "sym_j",
    name: "Jack",
...
export const DEFAULT_WILD_ACE_CONFIG: WildAceConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: WildAceConfig = DEFAULT_WILD_ACE_CONFIG,
...
export function applyRtpProfile(cfg: WildAceConfig): WildAceConfig {
  const profile =
...
const VOLATILITY_VALUES: WildAceConfig["volatility"][] = [
  "low",
  "low-medium",
  "medium",
  "medium-high",
...
function normalizeRtpProfiles(raw: unknown, d: WildAceConfig): SaRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const weightsIn = (found?.symbolWeights ?? fb.symbolWeights) as Partial<
      Record<WildAceSymKind, SaSymbolWeightOverride>
    >;
    const symbolWeights: Partial<Record<WildAceSymKind, SaSymbolWeightOverride>> = {};
...
export function normalizeWildAceConfig(raw: unknown): WildAceConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_WILD_ACE_CONFIG);
  const obj = raw as Partial<WildAceConfig>;
  const d = DEFAULT_WILD_ACE_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as WildAceSymKind) || base.kind,
  ...
  const jwIn = (obj.jokerTransformWeights ?? {}) as Partial<WildAceConfig["jokerTransformWeights"]>;
  ...
  const volatility: WildAceConfig["volatility"] = VOLATILITY_VALUES.includes(
    volRaw as WildAceConfig["volatility"],
  )
    ? (volRaw as WildAceConfig["volatility"])
    : d.volatility;
