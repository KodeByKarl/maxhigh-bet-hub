/**
 * MaxHigh Turtle Tide — shared fish-hunter math + superadmin-editable tables.
...
export const TURTLE_TIDE_GAME_ID = "turtle-tide";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = TURTLE_TIDE_GAME_ID;
export const TURTLE_TIDE_TITLE = "Turtle Tide";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/turtle-tide/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export const DEFAULT_FISH_TIERS: FishTierConfig[] = [
  ...
  {
    id: "boss",
    label: "The Turtle Tide",
    sprite: "turtle-tide-boss",
    payoutMin: 100,
    payoutMax: 500,
    hitsMin: 40,
    hitsMax: 60,
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Turtle Tide Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_TURTLE_TIDE_CONFIG: TurtleTideConfig = {
  schemaVersion: 1,
...
export function normalizeTurtleTideConfig(raw: unknown): TurtleTideConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_TURTLE_TIDE_CONFIG);
  }
  const o = raw as Partial<TurtleTideConfig> & { targetRtp?: number };
  const d = DEFAULT_TURTLE_TIDE_CONFIG;
