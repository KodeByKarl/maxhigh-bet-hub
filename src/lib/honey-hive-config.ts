/**
 * Honey Hive math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const HONEY_HIVE_GAME_ID = "honey-hive";

export type HoneyHiveSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type HoneyHiveBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const HONEY_HIVE_BOMB_MULTS = [2, 3, 4, 5] as const;

export type HoneyHiveScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type HoneyHiveSymbolConfig = {
  id: string;
  kind: HoneyHiveSymKind;
  label: string;
...
export type HoneyHiveConfig = {
  ...
  scatterCashTiers: HoneyHiveScatterCashTier[];
  ...
  symbols: HoneyHiveSymbolConfig[];
};

export const SYMBOL_LABELS: Record<HoneyHiveSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_HONEY_HIVE_CONFIG: HoneyHiveConfig = {
  schemaVersion: 1,
...
export function clampBombMult(v: unknown): HoneyHiveBombEntry["mult"] {
...
export function normalizeHoneyHiveConfig(raw: unknown): HoneyHiveConfig {
  const d = DEFAULT_HONEY_HIVE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: HoneyHiveSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<HoneyHiveSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<HoneyHiveBombEntry["mult"], number>();
  for (const mult of HONEY_HIVE_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<HoneyHiveBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(HONEY_HIVE_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as HoneyHiveBombEntry["mult"], weight);
    }
  }
  const bombTable: HoneyHiveBombEntry[] = HONEY_HIVE_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: HoneyHiveScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<HoneyHiveScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: HoneyHiveConfig): {
  id: string;
  kind: HoneyHiveSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: HoneyHiveBombEntry[]): Record<number, number> {
