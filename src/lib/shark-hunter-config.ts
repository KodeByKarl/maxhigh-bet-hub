/**
 * MaxHigh Shark Hunter — shared fish-hunter math + superadmin-editable tables.
...
export const SHARK_HUNTER_GAME_ID = "shark-hunter";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = SHARK_HUNTER_GAME_ID;
export const SHARK_HUNTER_TITLE = "Shark Hunter";
...
export type FishTierConfig = {
  id: FishTierId;
  label: string;
  /** Sprite / asset key under public/images/symbols/shark-hunter/ */
  sprite: string;
  payoutMin: number;
  payoutMax: number;
  hitsMin: number;
...
export const DEFAULT_FISH_TIERS: FishTierConfig[] = [
  ...
  {
    id: "boss",
    label: "The Shark Hunter",
    sprite: "shark-hunter-boss",
    payoutMin: 100,
    payoutMax: 500,
    hitsMin: 40,
    hitsMax: 60,
...
export const DEFAULT_WEAPONS: WeaponTierConfig[] = [
  ...
  { id: "master", label: "Shark Hunter Master Rig", betCost: 25, hitMod: 1.1 },
];
...
export const DEFAULT_SHARK_HUNTER_CONFIG: SharkHunterConfig = {
  schemaVersion: 1,
...
export function normalizeSharkHunterConfig(raw: unknown): SharkHunterConfig {
  if (!raw || typeof raw !== "object") {
    return structuredClone(DEFAULT_SHARK_HUNTER_CONFIG);
  }
  const o = raw as Partial<SharkHunterConfig> & { targetRtp?: number };
  const d = DEFAULT_SHARK_HUNTER_CONFIG;
