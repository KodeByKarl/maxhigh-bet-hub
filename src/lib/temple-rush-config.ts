...
export const TEMPLE_RUSH_GAME_ID = "temple-rush";

export type TempleRushSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type TempleRushBombEntry = { mult: number; weight: number };

export type TempleRushScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type TempleRushSymbolConfig = {
  id: string;
  kind: TempleRushSymKind;
  label: string;
...
export type TempleRushConfig = {
  ...
  scatterCashTiers: TempleRushScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: TempleRushSymbolConfig[];
};

export const SYMBOL_LABELS: Record<TempleRushSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_TEMPLE_RUSH_CONFIG: TempleRushConfig = {
  schemaVersion: 1,
...
export function normalizeTempleRushConfig(raw: unknown): TempleRushConfig {
  const d = DEFAULT_TEMPLE_RUSH_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: TempleRushSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<TempleRushSymbolConfig> | undefined;
  ...
  const bombTable: TempleRushBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<TempleRushBombEntry>;
            return {
  ...
  const scatterCashTiers: TempleRushScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<TempleRushScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: TempleRushConfig): {
  id: string;
  kind: TempleRushSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: TempleRushBombEntry[]): Record<number, number> {
