/**
 * Fortune Tiger — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Fortune Tiger 2/3):
...
export const FORTUNE_TIGER_GAME_ID = "fortune-tiger";
...
export type FortuneTigerConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const DEFAULT_FORTUNE_TIGER_CONFIG: FortuneTigerConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
...
function normalizeRtpProfiles(raw: unknown, d: FortuneTigerConfig): FgRtpProfile[] {
...
export function effectiveReelWeights(cfg: FortuneTigerConfig): Record<FtSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: FortuneTigerConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeFortuneTigerConfig(raw: unknown): FortuneTigerConfig {
  const d = DEFAULT_FORTUNE_TIGER_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<FortuneTigerConfig>;
...
export function betPerLine(totalBet: number, cfg: FortuneTigerConfig): number {
