/**
 * Outlaw Coins: Hold & Win — diamond connecting-ways western slot math.
...
export const OUTLAW_COINS_GAME_ID = "outlaw-coins";
...
export function totalCells(cfg: Pick<OutlawCoinsConfig, "reelHeights">): number {
...
export const DEFAULT_OUTLAW_COINS_CONFIG: OutlawCoinsConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: OutlawCoinsConfig = DEFAULT_OUTLAW_COINS_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: OutlawCoinsConfig = DEFAULT_OUTLAW_COINS_CONFIG,
): number {
...
export function normalizeOutlawCoinsConfig(raw: unknown): OutlawCoinsConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_OUTLAW_COINS_CONFIG);
  const obj = raw as Partial<OutlawCoinsConfig>;
  const d = DEFAULT_OUTLAW_COINS_CONFIG;
