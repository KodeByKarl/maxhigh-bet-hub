/**
 * Starlight Ace math & configuration — shared by server resolver and UI.
...
export const STARLIGHT_ACE_GAME_ID = "starlight-ace";

export type StarlightSymKind =
  | "sym_10"
  | "sym_j"
  | "sym_q"
  | "sym_k"
...
export type StarlightSymbolConfig = {
  id: string;
...
export type StarlightAceConfig = {
  schemaVersion: 1;
  reelsCount: number;
  ...
  maxWinMult: number;
  targetRtp: number;
  symbols: StarlightSymbolConfig[];
};

export const SYMBOL_NAMES: Record<StarlightSymKind, string> = {
  sym_10: "10 Gem",
  sym_j: "J Gem",
  sym_q: "Q Gem",
  sym_k: "K Gem",
 ...
 */
export const DEFAULT_STARLIGHT_ACE_CONFIG: StarlightAceConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 5,
  maxReelHeight: 5,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: StarlightAceConfig = DEFAULT_STARLIGHT_ACE_CONFIG,
): number {
...
export function normalizeStarlightAceConfig(raw: unknown): StarlightAceConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_STARLIGHT_ACE_CONFIG);
  const obj = raw as Partial<StarlightAceConfig>;
  const d = DEFAULT_STARLIGHT_ACE_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as StarlightSymKind) || base.kind,
