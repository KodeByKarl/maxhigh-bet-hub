/**
 * Candy Blast math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const CANDY_BLAST_GAME_ID = "candy-blast";

export type CandyBlastSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type CandyBlastBombEntry = { mult: number; weight: number };

export type CandyBlastScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type CandyBlastSymbolConfig = {
  id: string;
  kind: CandyBlastSymKind;
  label: string;
...
export type CandyBlastConfig = {
  ...
  scatterCashTiers: CandyBlastScatterCashTier[];
  buyFeatureMult: number;
  superBuyFeatureMult: number;
  anteBetMult: number;
  minCluster: number;
  symbols: CandyBlastSymbolConfig[];
};

export const SYMBOL_LABELS: Record<CandyBlastSymKind, string> = {
  grape: "Grape",
  plum: "Plum",
  melon: "Melon",
  apple: "Apple",
...
export const DEFAULT_CANDY_BLAST_CONFIG: CandyBlastConfig = {
  schemaVersion: 1,
...
export function normalizeCandyBlastConfig(raw: unknown): CandyBlastConfig {
  const d = DEFAULT_CANDY_BLAST_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: CandyBlastSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<CandyBlastSymbolConfig> | undefined;
  ...
  const bombTable: CandyBlastBombEntry[] =
    bombIn && bombIn.length > 0
      ? bombIn
          .map((b) => {
            const row = b as Partial<CandyBlastBombEntry>;
            return {
  ...
  const scatterCashTiers: CandyBlastScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<CandyBlastScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: CandyBlastConfig): {
  id: string;
  kind: CandyBlastSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: CandyBlastBombEntry[]): Record<number, number> {
