/**
 * Arena Champ — base-game-only 5×3 / 10-payline slot math config.
...
export const ARENA_CHAMP_GAME_ID = "arena-champ";

export type GrSymKind =
  | "bar"
  | "dice"
  | "diamond"
  | "chip"
...
export type FsRtpProfile = {
  ...
  reelWeights: Record<GrSymKind, number[]>;
};

export type ArenaChampConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const SYMBOL_NAMES: Record<GrSymKind, string> = {
  bar: "Bars",
  dice: "Dice",
  diamond: "Diamonds",
  chip: "Chips",
...
function profileWeights(
  entries: Record<GrSymKind, [number, number, number, number, number]>,
): Record<GrSymKind, number[]> {
  const out = {} as Record<GrSymKind, number[]>;
...
export const DEFAULT_ARENA_CHAMP_CONFIG: ArenaChampConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 3,
  paylineCount: 10,
...
function normalizeRtpProfiles(raw: unknown, d: ArenaChampConfig): FsRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const reelWeights = {} as Record<GrSymKind, number[]>;
    for (const k of SYM_KINDS) {
...
export function effectiveReelWeights(cfg: ArenaChampConfig): Record<GrSymKind, number[]> {
  const profile =
...
export function normalizeArenaChampConfig(raw: unknown): ArenaChampConfig {
  const d = DEFAULT_ARENA_CHAMP_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<ArenaChampConfig>;
...
export function betPerLine(totalBet: number, cfg: ArenaChampConfig): number {
