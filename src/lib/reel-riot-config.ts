/**
 * Reel Riot — classic 3-reel / 1-payline slot math config.
 ...
 * - Progressive jackpot → jackpot.id="reel-riot"; unlocks only at max bet + exactly 2 DW.
 ...
 * - Must stay under 100%. Re-run `npx tsx scripts/test-reel-riot.ts --rtp N` after any weight change.
 */

export const REEL_RIOT_GAME_ID = "fruit-riot";

/** Platform jackpot table row id for this game's progressive pool. */
export const REEL_RIOT_JACKPOT_ID = "fruit-riot";
 ...
 * Tune via `npx tsx scripts/test-reel-riot.ts --rtp N`.
 */
export const DEFAULT_REEL_RIOT_CONFIG: ReelRiotConfig = {
  schemaVersion: 1,
  reelsCount: 3,
...
export function normalizeReelRiotConfig(raw: unknown): ReelRiotConfig {
  const d = DEFAULT_REEL_RIOT_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<ReelRiotConfig>;
