/**
 * Enchanted Grove math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Egyptian ways values.
 */

export const ENCHANTED_GROVE_GAME_ID = "enchanted-grove";

export type EnchantedGroveSymKind =
  | "ten"
  | "jack"
  | "queen"
  | "king"
...
export type EnchantedGroveScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type EnchantedGroveFreeSpinsAward = {
  count: number;
  spins: number;
};

export type EnchantedGroveSymbolConfig = {
  id: string;
  kind: EnchantedGroveSymKind;
  label: string;
...
export type EnchantedGroveConfig = {
  schemaVersion: 1;
  ...
  freeSpinsAwards: EnchantedGroveFreeSpinsAward[];
  scatterCashTiers: EnchantedGroveScatterCashTier[];
  buyFeatureMult: number;
  fsMultStart: number;
  fsMultStep: number;
  symbols: EnchantedGroveSymbolConfig[];
};

export const DEFAULT_ENCHANTED_GROVE_CONFIG: EnchantedGroveConfig = {
  schemaVersion: 1,
...
export function normalizeEnchantedGroveConfig(raw: unknown): EnchantedGroveConfig {
  const d = DEFAULT_ENCHANTED_GROVE_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: EnchantedGroveSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<EnchantedGroveSymbolConfig> | undefined;
  ...
  const scatterCashTiers: EnchantedGroveScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<EnchantedGroveScatterCashTier>;
            return {
  ...
  const freeSpinsAwards: EnchantedGroveFreeSpinsAward[] =
    awardsIn && awardsIn.length > 0
      ? awardsIn
          .map((t) => {
            const row = t as Partial<EnchantedGroveFreeSpinsAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: EnchantedGroveConfig) {
...
export function freeSpinsAwardForCount(cfg: EnchantedGroveConfig, count: number): number {
