...
export const MEGA_ACE_GAME_ID = "mega-ace";

export type MegaAceSymKind =
  | "sym_j"
  | "sym_q"
  | "sym_k"
  | "sym_a"
...
export type MegaAceSymbolConfig = {
  id: string;
  kind: MegaAceSymKind;
  name: string;
...
export type SaRtpProfile = {
  ...
  symbolWeights: Partial<Record<MegaAceSymKind, SaSymbolWeightOverride>>;
};

export type MegaAceConfig = {
  schemaVersion: 1;
  reelsCount: number;
  ...
  symbols: MegaAceSymbolConfig[];
};

export const SYMBOL_NAMES: Record<MegaAceSymKind, string> = {
  sym_j: "Jack",
  sym_q: "Queen",
  sym_k: "King",
  sym_a: "Ace",
...
export const ALL_SYM_KINDS: MegaAceSymKind[] = [
  "sym_j",
  "sym_q",
  "sym_k",
  "sym_a",
 ...
 */
const BASE_SYMBOLS: MegaAceSymbolConfig[] = [
  {
    id: "sym_j",
    kind: "sym_j",
    name: "Jack",
...
export const DEFAULT_MEGA_ACE_CONFIG: MegaAceConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: MegaAceConfig = DEFAULT_MEGA_ACE_CONFIG,
...
export function applyRtpProfile(cfg: MegaAceConfig): MegaAceConfig {
  const profile =
...
const VOLATILITY_VALUES: MegaAceConfig["volatility"][] = [
  "low",
  "low-medium",
  "medium",
  "medium-high",
...
function normalizeRtpProfiles(raw: unknown, d: MegaAceConfig): SaRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const weightsIn = (found?.symbolWeights ?? fb.symbolWeights) as Partial<
      Record<MegaAceSymKind, SaSymbolWeightOverride>
    >;
    const symbolWeights: Partial<Record<MegaAceSymKind, SaSymbolWeightOverride>> = {};
...
export function normalizeMegaAceConfig(raw: unknown): MegaAceConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_MEGA_ACE_CONFIG);
  const obj = raw as Partial<MegaAceConfig>;
  const d = DEFAULT_MEGA_ACE_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as MegaAceSymKind) || base.kind,
  ...
  const jwIn = (obj.jokerTransformWeights ?? {}) as Partial<MegaAceConfig["jokerTransformWeights"]>;
  ...
  const volatility: MegaAceConfig["volatility"] = VOLATILITY_VALUES.includes(
    volRaw as MegaAceConfig["volatility"],
  )
    ? (volRaw as MegaAceConfig["volatility"])
    : d.volatility;
