 ...
 * - Progressive jackpot → jackpot.id="neon-fruits"; unlocks only at max bet + exactly 2 DW.
 ...
 * - Must stay under 100%. Re-run `npx tsx scripts/test-neon-fruits.ts --rtp N` after any weight change.
 */

export const NEON_FRUITS_GAME_ID = "neon-fruits";

/** Platform jackpot table row id for this game's progressive pool. */
export const NEON_FRUITS_JACKPOT_ID = "neon-fruits";
 ...
 * Tune via `npx tsx scripts/test-neon-fruits.ts --rtp N`.
 */
export const DEFAULT_NEON_FRUITS_CONFIG: NeonFruitsConfig = {
  schemaVersion: 1,
  reelsCount: 3,
...
export function normalizeNeonFruitsConfig(raw: unknown): NeonFruitsConfig {
  const d = DEFAULT_NEON_FRUITS_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<NeonFruitsConfig>;
