/**
 * Diamond Dig Dig: Hold & Win — diamond connecting-ways western slot math.
...
export const DIAMOND_DIG_GAME_ID = "diamond-dig";
...
export function totalCells(cfg: Pick<DiamondDigConfig, "reelHeights">): number {
...
export const DEFAULT_DIAMOND_DIG_CONFIG: DiamondDigConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: DiamondDigConfig = DEFAULT_DIAMOND_DIG_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: DiamondDigConfig = DEFAULT_DIAMOND_DIG_CONFIG,
): number {
...
export function normalizeDiamondDigConfig(raw: unknown): DiamondDigConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_DIAMOND_DIG_CONFIG);
  const obj = raw as Partial<DiamondDigConfig>;
  const d = DEFAULT_DIAMOND_DIG_CONFIG;
