/**
 * Jeepney Jackpot: Hold & Win — diamond connecting-ways western slot math.
...
export const JEEPNEY_JACKPOT_GAME_ID = "jeepney-jackpot";
...
export type JeepneyJackpotConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
...
export function totalCells(cfg: Pick<JeepneyJackpotConfig, "reelHeights">): number {
...
export const DEFAULT_JEEPNEY_JACKPOT_CONFIG: JeepneyJackpotConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: JeepneyJackpotConfig = DEFAULT_JEEPNEY_JACKPOT_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: JeepneyJackpotConfig = DEFAULT_JEEPNEY_JACKPOT_CONFIG,
): number {
...
export function normalizeJeepneyJackpotConfig(raw: unknown): JeepneyJackpotConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_JEEPNEY_JACKPOT_CONFIG);
  const obj = raw as Partial<JeepneyJackpotConfig>;
  const d = DEFAULT_JEEPNEY_JACKPOT_CONFIG;
