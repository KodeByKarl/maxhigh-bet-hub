/**
 * Prosperity Lion — 3×3 / 5-payline + multiplier-reel slot math config.
 ...
 *
 * Locked product rules for this title (NOT Prosperity Lion 2/3):
...
export const PROSPERITY_LION_GAME_ID = "prosperity-lion";

export type FtSymKind =
  | "ruby"
  | "emerald"
  | "sapphire"
  | "amethyst"
...
export const FT_MULTIPLIER_VALUES = [1, 2, 3, 5, 10, 15] as const;
export type FgMultiplierValue = (typeof FT_MULTIPLIER_VALUES)[number];
...
export type FgRtpProfile = {
  ...
  reelWeights: Record<FtSymKind, number[]>;
};
...
export type ProsperityLionConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
  paylineCount: number;
...
export const SYMBOL_NAMES: Record<FtSymKind, string> = {
  ruby: "Golden Tiger",
...
function profileWeights(
  entries: Record<FtSymKind, [number, number, number]>,
): Record<FtSymKind, number[]> {
  const out = {} as Record<FtSymKind, number[]>;
...
export const DEFAULT_PROSPERITY_LION_CONFIG: ProsperityLionConfig = {
  schemaVersion: 1,
  reelsCount: 3,
  rowsCount: 3,
  paylineCount: 5,
  ...
  rtpProfiles: DEFAULT_RTP_PROFILES.map((p) => ({
    ...
    ) as Record<FtSymKind, number[]>,
  })),
  multiplierValues: [...FT_MULTIPLIER_VALUES],
  multiplierStripBase: {
...
function normalizeRtpProfiles(raw: unknown, d: ProsperityLionConfig): FgRtpProfile[] {
  ...
  return RTP_IDS.map((id) => {
    ...
    const reelWeights = {} as Record<FtSymKind, number[]>;
    for (const k of SYM_KINDS) {
...
export function effectiveReelWeights(cfg: ProsperityLionConfig): Record<FtSymKind, number[]> {
  const profile =
 ...
 */
export function spinStake(baseBet: number, exMode: boolean, cfg: ProsperityLionConfig): number {
  const mult = exMode ? cfg.exBetMult : 1;
  return +(baseBet * mult).toFixed(2);
}

export function normalizeProsperityLionConfig(raw: unknown): ProsperityLionConfig {
  const d = DEFAULT_PROSPERITY_LION_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Partial<ProsperityLionConfig>;
  ...
  if (multiplierValues.length === 0) multiplierValues.push(...FT_MULTIPLIER_VALUES);
...
export function betPerLine(totalBet: number, cfg: ProsperityLionConfig): number {
