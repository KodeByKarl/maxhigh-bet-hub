/**
 * Quezon Quest math & configuration — shared by server resolver and UI.
...
export const QUEZON_QUEST_GAME_ID = "quezon-quest";

export type QuezonQuestSymKind =
  | "sym_10"
  | "sym_j"
  | "sym_q"
  | "sym_k"
...
export type QuezonQuestSymbolConfig = {
  id: string;
...
export type QuezonQuestConfig = {
  ...
  maxWinMult: number;
  targetRtp: number;
  symbols: QuezonQuestSymbolConfig[];
};

export const SYMBOL_NAMES: Record<QuezonQuestSymKind, string> = {
  sym_10: "10 Gem",
  sym_j: "J Gem",
  sym_q: "Q Gem",
  sym_k: "K Gem",
 ...
 */
export const DEFAULT_QUEZON_QUEST_CONFIG: QuezonQuestConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 5,
  maxReelHeight: 5,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: QuezonQuestConfig = DEFAULT_QUEZON_QUEST_CONFIG,
): number {
...
export function normalizeQuezonQuestConfig(raw: unknown): QuezonQuestConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_QUEZON_QUEST_CONFIG);
  const obj = raw as Partial<QuezonQuestConfig>;
  const d = DEFAULT_QUEZON_QUEST_CONFIG;
    ...
    Array.isArray(obj.symbols) && obj.symbols.length > 0
      ? obj.symbols.map((s, i) => {
          ...
          return {
            ...
            kind: (s?.kind as QuezonQuestSymKind) || base.kind,
