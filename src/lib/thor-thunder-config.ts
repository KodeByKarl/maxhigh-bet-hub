/**
 * Thor Thunder math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Egyptian ways values.
 */

export const THOR_THUNDER_GAME_ID = "thor-thunder";

export type ThorThunderSymKind =
  | "ten"
  | "jack"
  | "queen"
  | "king"
...
export type ThorThunderScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type ThorThunderFreeSpinsAward = {
  count: number;
  spins: number;
};

export type ThorThunderSymbolConfig = {
  id: string;
  kind: ThorThunderSymKind;
  label: string;
...
export type ThorThunderConfig = {
  ...
  freeSpinsAwards: ThorThunderFreeSpinsAward[];
  scatterCashTiers: ThorThunderScatterCashTier[];
  buyFeatureMult: number;
  fsMultStart: number;
  fsMultStep: number;
  symbols: ThorThunderSymbolConfig[];
};

export const DEFAULT_THOR_THUNDER_CONFIG: ThorThunderConfig = {
  schemaVersion: 1,
...
export function normalizeThorThunderConfig(raw: unknown): ThorThunderConfig {
  const d = DEFAULT_THOR_THUNDER_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: ThorThunderSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<ThorThunderSymbolConfig> | undefined;
  ...
  const scatterCashTiers: ThorThunderScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<ThorThunderScatterCashTier>;
            return {
  ...
  const freeSpinsAwards: ThorThunderFreeSpinsAward[] =
    awardsIn && awardsIn.length > 0
      ? awardsIn
          .map((t) => {
            const row = t as Partial<ThorThunderFreeSpinsAward>;
            return {
              count: clamp(Math.round(num(row.count, 3)), 1, 30),
...
export function configToCellSyms(cfg: ThorThunderConfig) {
...
export function freeSpinsAwardForCount(cfg: ThorThunderConfig, count: number): number {
