/**
 * Palengke Pays: Hold & Win — diamond connecting-ways western slot math.
...
export const PALENGKE_PAYS_GAME_ID = "palengke-pays";
...
export type PalengkePaysConfig = {
  schemaVersion: 1;
  reelsCount: number;
  rowsCount: number;
...
export function totalCells(cfg: Pick<PalengkePaysConfig, "reelHeights">): number {
...
export const DEFAULT_PALENGKE_PAYS_CONFIG: PalengkePaysConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  rowsCount: 5,
...
export function betPerLine(totalBet: number, cfg: PalengkePaysConfig = DEFAULT_PALENGKE_PAYS_CONFIG): number {
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: PalengkePaysConfig = DEFAULT_PALENGKE_PAYS_CONFIG,
): number {
...
export function normalizePalengkePaysConfig(raw: unknown): PalengkePaysConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_PALENGKE_PAYS_CONFIG);
  const obj = raw as Partial<PalengkePaysConfig>;
  const d = DEFAULT_PALENGKE_PAYS_CONFIG;
