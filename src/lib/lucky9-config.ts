/**
 * MaxHigh Lucky 9 (Variant A — baccarat-family, closer-to-9).
...
export const LUCKY9_GAME_ID = "lucky9";
/** Alias for resolvers that import `GAME_ID`. */
export const GAME_ID = LUCKY9_GAME_ID;
export const LUCKY9_TITLE = "Lucky 9";
...
export type Lucky9Config = {
  ...
  /**
   * independent — each side draws on ≤ drawThreshold (Lucky 9 default).
...
export const DEFAULT_LUCKY9_CONFIG: Lucky9Config = {
  schemaVersion: 1,
  playerPayout: 1.0,
  dealerPayout: 1.0,
  tiePayout: 8,
...
export const BET_STEPS = DEFAULT_LUCKY9_CONFIG.betSteps;
 ...
 */
export function shouldDraw(total: number, threshold = DEFAULT_LUCKY9_CONFIG.drawThreshold): boolean {
...
export function isNatural(twoCardTotal: number, naturalTotal = DEFAULT_LUCKY9_CONFIG.naturalTotal): boolean {
  return twoCardTotal === naturalTotal;
}

export function normalizeLucky9Config(raw: unknown): Lucky9Config {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_LUCKY9_CONFIG);
  const o = raw as Partial<Lucky9Config> & {
    bankerPayout?: number;
    minBankerBet?: number;
    maxBankerBet?: number;
    minBet?: number;
  ...
  const dealerDrawMode: Lucky9Config["dealerDrawMode"] =
