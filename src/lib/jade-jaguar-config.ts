/**
 * Jade Jaguar math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const JADE_JAGUAR_GAME_ID = "jade-jaguar";

export type JadeJaguarSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type JadeJaguarBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const JADE_JAGUAR_BOMB_MULTS = [2, 3, 4, 5] as const;

export type JadeJaguarScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type JadeJaguarSymbolConfig = {
  id: string;
  kind: JadeJaguarSymKind;
  label: string;
...
export type JadeJaguarConfig = {
  ...
  scatterCashTiers: JadeJaguarScatterCashTier[];
  ...
  symbols: JadeJaguarSymbolConfig[];
};

export const SYMBOL_LABELS: Record<JadeJaguarSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_JADE_JAGUAR_CONFIG: JadeJaguarConfig = {
  schemaVersion: 1,
...
export function clampBombMult(v: unknown): JadeJaguarBombEntry["mult"] {
...
export function normalizeJadeJaguarConfig(raw: unknown): JadeJaguarConfig {
  const d = DEFAULT_JADE_JAGUAR_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: JadeJaguarSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<JadeJaguarSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<JadeJaguarBombEntry["mult"], number>();
  for (const mult of JADE_JAGUAR_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<JadeJaguarBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(JADE_JAGUAR_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as JadeJaguarBombEntry["mult"], weight);
    }
  }
  const bombTable: JadeJaguarBombEntry[] = JADE_JAGUAR_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: JadeJaguarScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<JadeJaguarScatterCashTier>;
            return {
  ...
  return {
    ...
    seedClusterMin: clamp(Math.round(num(o.seedClusterMin, d.seedClusterMin)), 3, 9),
    seedClusterMax: clamp(Math.round(num(o.seedClusterMax, d.seedClusterMax)), 3, 9),
    ...
    /** 3×3 board — never require more matches than cells. */
    minCluster: clamp(Math.round(num(o.minCluster, d.minCluster)), 3, 9),
...
export function configToCellSyms(cfg: JadeJaguarConfig): {
  id: string;
  kind: JadeJaguarSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: JadeJaguarBombEntry[]): Record<number, number> {
