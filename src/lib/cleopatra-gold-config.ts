/**
 * Cleopatra Gold (catalog id: cleopatra-gold) — math & configuration.
 * 5×4 ways (1,024), cascade multipliers, Golden Campfire free spins,
 * Treasure Chest Hold & Collect, Explorer→Legend jackpots.
 */

export const CLEOPATRA_GOLD_GAME_ID = "cleopatra-gold";
export const CLEOPATRA_GOLD_TITLE = "Cleopatra Gold";
...
export type CleopatraGoldConfig = {
  schemaVersion: 1;
  reelsCount: number;
...
export const DEFAULT_CLEOPATRA_GOLD_CONFIG: CleopatraGoldConfig = {
  schemaVersion: 1,
  reelsCount: 5,
  minReelHeight: 4,
  maxReelHeight: 4,
...
export function calcFreeSpinsAward(
  scatterCount: number,
  cfg: CleopatraGoldConfig = DEFAULT_CLEOPATRA_GOLD_CONFIG,
): number {
...
export function normalizeCleopatraGoldConfig(raw: unknown): CleopatraGoldConfig {
  if (!raw || typeof raw !== "object") return structuredClone(DEFAULT_CLEOPATRA_GOLD_CONFIG);
  const obj = raw as Partial<CleopatraGoldConfig>;
  const d = DEFAULT_CLEOPATRA_GOLD_CONFIG;
  ...
  const volatility: CleopatraGoldConfig["volatility"] =
