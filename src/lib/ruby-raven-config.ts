/**
 * Ruby Raven math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const RUBY_RAVEN_GAME_ID = "ruby-raven";

export type RubyRavenSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type RubyRavenBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const RUBY_RAVEN_BOMB_MULTS = [2, 3, 4, 5] as const;

export type RubyRavenScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type RubyRavenSymbolConfig = {
  id: string;
  kind: RubyRavenSymKind;
  label: string;
...
export type RubyRavenConfig = {
  ...
  scatterCashTiers: RubyRavenScatterCashTier[];
  ...
  symbols: RubyRavenSymbolConfig[];
};

export const SYMBOL_LABELS: Record<RubyRavenSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_RUBY_RAVEN_CONFIG: RubyRavenConfig = {
  schemaVersion: 1,
...
export function clampBombMult(v: unknown): RubyRavenBombEntry["mult"] {
...
export function normalizeRubyRavenConfig(raw: unknown): RubyRavenConfig {
  const d = DEFAULT_RUBY_RAVEN_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: RubyRavenSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<RubyRavenSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<RubyRavenBombEntry["mult"], number>();
  for (const mult of RUBY_RAVEN_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<RubyRavenBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(RUBY_RAVEN_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as RubyRavenBombEntry["mult"], weight);
    }
  }
  const bombTable: RubyRavenBombEntry[] = RUBY_RAVEN_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: RubyRavenScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<RubyRavenScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: RubyRavenConfig): {
  id: string;
  kind: RubyRavenSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: RubyRavenBombEntry[]): Record<number, number> {
