/**
 * Ember Tiger math config — shared by engine (client) and Superadmin (server).
 * Defaults match the original hardcoded Sweet Bonanza-style values.
 */

export const EMBER_TIGER_GAME_ID = "ember-tiger";

export type EmberTigerSymKind =
  | "grape"
  | "plum"
  | "melon"
  | "apple"
...
export type EmberTigerBombEntry = { mult: 2 | 3 | 4 | 5; weight: number };

export const EMBER_TIGER_BOMB_MULTS = [2, 3, 4, 5] as const;

export type EmberTigerScatterCashTier = {
  count: number;
  /** × bet */
  mult: number;
};

export type EmberTigerSymbolConfig = {
  id: string;
  kind: EmberTigerSymKind;
  label: string;
...
export type EmberTigerConfig = {
  ...
  scatterCashTiers: EmberTigerScatterCashTier[];
  ...
  symbols: EmberTigerSymbolConfig[];
};

export const SYMBOL_LABELS: Record<EmberTigerSymKind, string> = {
  grape: "10",
  plum: "J",
  melon: "Q",
  apple: "K",
...
export const DEFAULT_EMBER_TIGER_CONFIG: EmberTigerConfig = {
  schemaVersion: 1,
  ...
  anteBetMult: 1.25,
  /** Diamond 3-4-5-4-3 (19 cells) — cluster pays from 6 matching symbols. */
  minCluster: 6,
...
export function clampBombMult(v: unknown): EmberTigerBombEntry["mult"] {
...
export function normalizeEmberTigerConfig(raw: unknown): EmberTigerConfig {
  const d = DEFAULT_EMBER_TIGER_CONFIG;
  if (!raw || typeof raw !== "object") return structuredClone(d);
  const o = raw as Record<string, unknown>;

  const symbolsIn = Array.isArray(o.symbols) ? o.symbols : null;
  const symbols: EmberTigerSymbolConfig[] = d.symbols.map((def) => {
    const found = symbolsIn?.find(
      (s) => s && typeof s === "object" && (s as { id?: string }).id === def.id,
    ) as Partial<EmberTigerSymbolConfig> | undefined;
  ...
  const bombWeights = new Map<EmberTigerBombEntry["mult"], number>();
  for (const mult of EMBER_TIGER_BOMB_MULTS) {
    bombWeights.set(mult, d.bombTable.find((b) => b.mult === mult)?.weight ?? 1);
  }
  if (bombIn && bombIn.length > 0) {
    for (const b of bombIn) {
      const row = b as Partial<EmberTigerBombEntry>;
      const n = Math.round(num(row.mult, 0));
      if (!(EMBER_TIGER_BOMB_MULTS as readonly number[]).includes(n)) continue;
      const weight = clamp(num(row.weight, 1), 0, 10_000);
      bombWeights.set(n as EmberTigerBombEntry["mult"], weight);
    }
  }
  const bombTable: EmberTigerBombEntry[] = EMBER_TIGER_BOMB_MULTS.map((mult) => ({
    mult,
    weight: bombWeights.get(mult) ?? 0,
  }));

  const tiersIn = Array.isArray(o.scatterCashTiers) ? o.scatterCashTiers : null;
  const scatterCashTiers: EmberTigerScatterCashTier[] =
    tiersIn && tiersIn.length > 0
      ? tiersIn
          .map((t) => {
            const row = t as Partial<EmberTigerScatterCashTier>;
            return {
...
export function configToCellSyms(cfg: EmberTigerConfig): {
  id: string;
  kind: EmberTigerSymKind;
  weight: number;
  pay: [number, number, number];
...
export function bombTablePercents(table: EmberTigerBombEntry[]): Record<number, number> {
