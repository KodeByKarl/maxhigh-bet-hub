/**
 * Pirate Plunder — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { PIRATE_PLUNDER_GAME_ID, normalizePiratePlunderConfig } from "@/lib/pirate-plunder-config";
...
const GAME_NAME = "Pirate Plunder";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizePiratePlunderConfig(null);
    return normalizePiratePlunderConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizePiratePlunderConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setPiratePlunderConfig } = await import("@/components/maxhigh/pirate-plunder/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/pirate-plunder/spinResolver"
    );
    setPiratePlunderConfig(cfg);
...
export type PiratePlunderSessionState = {
...
export type PiratePlunderSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/pirate-plunder/spinResolver").resolveSpin>>;
  session: PiratePlunderSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): PiratePlunderSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, PIRATE_PLUNDER_GAME_ID),
...
export async function getPiratePlunderOpenSession(): Promise<PiratePlunderSessionState> {
...
export async function piratePlunderPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<PiratePlunderSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${PIRATE_PLUNDER_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PIRATE_PLUNDER_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function piratePlunderFreeSpin(data: {
  sessionId: string;
}): Promise<PiratePlunderSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: PiratePlunderSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${PIRATE_PLUNDER_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function piratePlunderBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: PiratePlunderSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${PIRATE_PLUNDER_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${PIRATE_PLUNDER_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
