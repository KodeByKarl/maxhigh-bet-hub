/**
 * Poker Showdown — server-authoritative two-phase settlement.
 *
...
import {
  ...
  resolveFold,
  resolvePlay,
} from "@/components/maxhigh/games/poker-showdown/resolver";
import { setPokerShowdownConfig } from "@/components/maxhigh/games/poker-showdown/runtimeConfig";
import type { PlayingCard } from "@/components/maxhigh/games/poker-showdown/deckEngine";
import type { HandRank } from "@/components/maxhigh/games/poker-showdown/handEvaluator";
import type {
  PublicDealScript,
  PublicSettleScript,
} from "@/components/maxhigh/games/poker-showdown/types";
...
const GAME_NAME = "Poker Showdown";

let cachedConfig: { cfg: PokerShowdownConfig; time: number } | null = null;

export function clearPokerShowdownEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<PokerShowdownConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizePokerShowdownConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizePokerShowdownConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getPokerShowdownEngineConfigPublic(): Promise<PokerShowdownConfig> {
  return loadEngineConfig();
}

type PokerShowdownFeatureState = {
  kind: "poker-showdown";
  phase: "decision";
  seed: string;
  ante: number;
...
function parseFeatureState(raw: string | null): PokerShowdownFeatureState | null {
  if (!raw) return null;
  try {
    const o = JSON.parse(raw) as PokerShowdownFeatureState;
    if (o?.kind !== "poker-showdown" || o.phase !== "decision") return null;
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Poker Showdown is currently disabled");
  }
}

export type PokerShowdownOpenSessionView = {
  sessionId: string;
  ante: number;
  pairPlus: number;
  pairPlusWin: number;
...
export async function getPokerShowdownOpenSession(): Promise<PokerShowdownOpenSessionView | null> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, POKER_SHOWDOWN_GAME_ID),
...
export type PokerShowdownDealResult = {
  balance: number;
  sessionId: string;
  script: PublicDealScript;
};

export async function pokerShowdownPaidDeal(input: {
  ante: number;
  pairPlus?: number;
}): Promise<PokerShowdownDealResult> {
  await assertNotInMaintenanceForBets();
  const user = await requireUser();
  await assertGameEnabled();

  const cfg = await loadEngineConfig();
  setPokerShowdownConfig(cfg);
  ...
  const feat: PokerShowdownFeatureState = {
    kind: "poker-showdown",
    phase: "decision",
    seed: resolved.seed,
    ante,
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${POKER_SHOWDOWN_GAME_ID} · ${GAME_NAME} · ante ₱${ante.toFixed(2)} pp ₱${pairPlus.toFixed(2)}`,
    });
    ...
    if (resolved.pairPlusWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${POKER_SHOWDOWN_GAME_ID} · ${GAME_NAME} · pair plus ₱${resolved.pairPlusWin.toFixed(2)}`,
      });
    }
...
export type PokerShowdownSettleResult = {
  balance: number;
  script: PublicSettleScript;
};

export async function pokerShowdownPlay(input: {
  sessionId: string;
}): Promise<PokerShowdownSettleResult> {
  await assertNotInMaintenanceForBets();
  const user = await requireUser();
  await assertGameEnabled();
  ...
  if (!current || !feat) throw new Error("No open Poker Showdown hand to play");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${POKER_SHOWDOWN_GAME_ID} · ${GAME_NAME} · play ₱${playWager.toFixed(2)}`,
    });
    ...
    if (settled.immediateCredit > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${POKER_SHOWDOWN_GAME_ID} · ${GAME_NAME} · ${settled.outcome} settle ₱${settled.immediateCredit.toFixed(2)}`,
      });
    }
...
export async function pokerShowdownFold(input: {
  sessionId: string;
}): Promise<PokerShowdownSettleResult> {
  await assertNotInMaintenanceForBets();
  const user = await requireUser();
  ...
  const current = rows[0];
  const feat = current ? parseFeatureState(current.featureState) : null;
  if (!current || !feat) throw new Error("No open Poker Showdown hand to fold");
