/**
 * Super Ace — server-authoritative spin settlement and round lifecycle.
...
import { setSuperAceConfig } from "@/components/maxhigh/super-ace/runtimeConfig";
import { resolveSuperAceSpin } from "@/components/maxhigh/super-ace/spinResolver";

const GAME_NAME = "Super Ace";

let cachedConfig: { cfg: SuperAceConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearSuperAceEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<SuperAceConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeSuperAceConfig(JSON.parse(raw) as unknown) : normalizeSuperAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeSuperAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setSuperAceConfig(cfg);
  const script = resolveSuperAceSpin({
    bet: opts.bet,
...
export type SuperAceSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/super-ace/spinResolver").resolveSuperAceSpin>
  >;
  session: SuperAceSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): SuperAceSessionState {
...
export async function getSuperAceOpenSession(): Promise<SuperAceSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, SUPER_ACE_GAME_ID),
...
export async function superAcePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<SuperAceSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SUPER_ACE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SUPER_ACE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function superAceFreeSpin(data: {
  sessionId: string;
}): Promise<SuperAceSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: SuperAceSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SUPER_ACE_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function superAceBuyFeature(data: {
  bet: number;
}): Promise<SuperAceSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SUPER_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SUPER_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
