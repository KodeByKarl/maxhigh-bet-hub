/**
 * Candy Blast — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { CANDY_BLAST_GAME_ID, normalizeCandyBlastConfig } from "@/lib/candy-blast-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeCandyBlastConfig(null);
    return normalizeCandyBlastConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeCandyBlastConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setCandyBlastConfig } = await import("@/components/maxhigh/candy-blast/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/candy-blast/spinResolver"
    );
    setCandyBlastConfig(cfg);
...
export type CandyBlastSessionState = {
...
export type CandyBlastSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/candy-blast/spinResolver").resolveSpin>>;
  session: CandyBlastSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): CandyBlastSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CANDY_BLAST_GAME_ID),
...
export async function getCandyBlastOpenSession(): Promise<CandyBlastSessionState> {
...
export async function candyBlastPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<CandyBlastSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CANDY_BLAST_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CANDY_BLAST_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function candyBlastFreeSpin(data: {
  sessionId: string;
}): Promise<CandyBlastSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: CandyBlastSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${CANDY_BLAST_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function candyBlastBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: CandyBlastSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${CANDY_BLAST_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${CANDY_BLAST_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
