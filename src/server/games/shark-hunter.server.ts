/**
 * Shark Hunter — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/shark-hunter/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/shark-hunter/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/shark-hunter/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/shark-hunter/rng";
import { setSharkHunterConfig } from "@/components/maxhigh/games/shark-hunter/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/shark-hunter/hitResolver";
...
const GAME_NAME = "Shark Hunter";

let cachedConfig: { cfg: SharkHunterConfig; time: number } | null = null;
...
export function clearSharkHunterEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: SharkHunterConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function sharkHunterSyncSpawns() {
  await assertGameEnabled();
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  setSharkHunterConfig(cfg);
  pruneFish(user.id);
...
async function loadEngineConfig(): Promise<SharkHunterConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeSharkHunterConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeSharkHunterConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getSharkHunterEngineConfigPublic(): Promise<SharkHunterConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Shark Hunter is currently disabled");
  }
}
...
export async function getSharkHunterOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, SHARK_HUNTER_GAME_ID),
...
export async function sharkHunterEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: SharkHunterConfig) {
  const now = Date.now();
...
export async function sharkHunterFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await sharkHunterEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: SHARK_HUNTER_GAME_ID,
      note: `Shark Hunter shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Shark Hunter boss share"
          : shot.crateDrop
            ? `Shark Hunter crate (${shot.crateDrop.label})`
            : `Shark Hunter catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: SHARK_HUNTER_GAME_ID,
          note: "Shark Hunter boss contributor share",
        });
      }
    }
...
export async function sharkHunterSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function sharkHunterBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  let session = await loadOpenSessionRow(user.id);
  if (!session) {
    await sharkHunterEnsureSession({});
    session = await loadOpenSessionRow(user.id);
  }
  if (!session) throw new Error("Could not open play session");
  const feat = parseFeature(session.featureState);
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: SHARK_HUNTER_GAME_ID,
      note: `Shark Hunter power-up (${power.label})`,
    });
...
export async function sharkHunterUseNetBomb(input: {
  weaponId: string;
...
export async function sharkHunterUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function sharkHunterBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
