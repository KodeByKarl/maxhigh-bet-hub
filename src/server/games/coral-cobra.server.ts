/**
 * Coral Cobra — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { CORAL_COBRA_GAME_ID, normalizeCoralCobraConfig, remainingFreeSpinsAfterSpin } from "@/lib/coral-cobra-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeCoralCobraConfig(null);
    return normalizeCoralCobraConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeCoralCobraConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setCoralCobraConfig } = await import("@/components/maxhigh/coral-cobra/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/coral-cobra/spinResolver"
    );
    setCoralCobraConfig(cfg);
...
export type CoralCobraSessionState = {
...
export type CoralCobraSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/coral-cobra/spinResolver").resolveSpin>>;
  session: CoralCobraSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): CoralCobraSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CORAL_COBRA_GAME_ID),
...
export async function getCoralCobraOpenSession(): Promise<CoralCobraSessionState> {
...
export async function coralCobraPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<CoralCobraSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CORAL_COBRA_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CORAL_COBRA_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function coralCobraFreeSpin(data: {
  sessionId: string;
}): Promise<CoralCobraSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: CoralCobraSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${CORAL_COBRA_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function coralCobraBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: CoralCobraSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${CORAL_COBRA_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${CORAL_COBRA_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
