/**
 * Lotus Lynx — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { LOTUS_LYNX_GAME_ID, normalizeLotusLynxConfig, remainingFreeSpinsAfterSpin } from "@/lib/lotus-lynx-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeLotusLynxConfig(null);
    return normalizeLotusLynxConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeLotusLynxConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setLotusLynxConfig } = await import("@/components/maxhigh/lotus-lynx/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/lotus-lynx/spinResolver"
    );
    setLotusLynxConfig(cfg);
...
export type LotusLynxSessionState = {
...
export type LotusLynxSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/lotus-lynx/spinResolver").resolveSpin>>;
  session: LotusLynxSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): LotusLynxSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, LOTUS_LYNX_GAME_ID),
...
export async function getLotusLynxOpenSession(): Promise<LotusLynxSessionState> {
...
export async function lotusLynxPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<LotusLynxSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${LOTUS_LYNX_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${LOTUS_LYNX_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function lotusLynxFreeSpin(data: {
  sessionId: string;
}): Promise<LotusLynxSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: LotusLynxSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${LOTUS_LYNX_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function lotusLynxBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: LotusLynxSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${LOTUS_LYNX_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${LOTUS_LYNX_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
