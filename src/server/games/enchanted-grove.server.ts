/**
 * Enchanted Grove — server-authoritative spin settle.
 */
...
export { ENCHANTED_GROVE_GAME_ID };
const GAME_NAME = "Enchanted Grove";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeEnchantedGroveConfig(null);
    return normalizeEnchantedGroveConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeEnchantedGroveConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setEnchantedGroveConfig } = await import(
      "@/components/maxhigh/enchanted-grove/runtimeConfig"
    );
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/enchanted-grove/spinResolver"
    );
    setEnchantedGroveConfig(cfg);
...
export type EnchantedGroveSessionState = {
...
export type EnchantedGroveSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/enchanted-grove/spinResolver").resolveSpin>>;
  session: EnchantedGroveSessionState;
  fsPayout?: {
    amount: number;
    baseEarn: number;
    multiplier: number;
...
function sessionState(row: typeof playSessions.$inferSelect | null): EnchantedGroveSessionState {
...
export async function getEnchantedGroveOpenSession(): Promise<EnchantedGroveSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, ENCHANTED_GROVE_GAME_ID),
...
export async function enchantedGrovePaidSpin(data: { bet: number }): Promise<EnchantedGroveSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ENCHANTED_GROVE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ENCHANTED_GROVE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function enchantedGroveFreeSpin(data: {
  sessionId: string;
}): Promise<EnchantedGroveSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: EnchantedGroveSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${ENCHANTED_GROVE_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function enchantedGroveBuyFeature(data: {
  bet: number;
}): Promise<{ balance: number; session: EnchantedGroveSessionState; awarded: number }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ENCHANTED_GROVE_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
