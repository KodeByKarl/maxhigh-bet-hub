/**
 * Goal Rush — server-authoritative spin settlement (base game only).
...
import { setGoalRushConfig } from "@/components/maxhigh/goal-rush/runtimeConfig";
import { resolveGoalRushSpin } from "@/components/maxhigh/goal-rush/spinResolver";
import type { SpinScript } from "@/components/maxhigh/goal-rush/types";

const GAME_NAME = "Goal Rush";

let cachedConfig: { cfg: GoalRushConfig; time: number } | null = null;

export function clearGoalRushEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<GoalRushConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeGoalRushConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeGoalRushConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type GoalRushSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function goalRushPaidSpin(data: { bet: number }): Promise<GoalRushSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setGoalRushConfig(cfg);
  ...
  const script = resolveGoalRushSpin({ totalBet: cost });
  const roundId = newId();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${GOAL_RUSH_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GOAL_RUSH_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getGoalRushEngineConfigPublic(): Promise<GoalRushConfig> {
