/**
 * Diamond Dig Dig — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import {
  ...
  type DiamondDigConfig,
} from "@/lib/diamond-dig-config";
...
import { setDiamondDigConfig } from "@/components/maxhigh/diamond-dig/runtimeConfig";
import { resolveDiamondDigSpin } from "@/components/maxhigh/diamond-dig/spinResolver";

const GAME_NAME = "Diamond Dig Dig";

let cachedConfig: { cfg: DiamondDigConfig; time: number } | null = null;

export function clearDiamondDigEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<DiamondDigConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeDiamondDigConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeDiamondDigConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: { bet: number; isFreeSpins: boolean }) {
  ...
  setDiamondDigConfig(cfg);
  const script = resolveDiamondDigSpin({
    totalBet: opts.bet,
...
export type DiamondDigSessionState = {
...
export type DiamondDigSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveDiamondDigSpin>;
  session: DiamondDigSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): DiamondDigSessionState {
...
export async function getDiamondDigOpenSession(): Promise<DiamondDigSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, DIAMOND_DIG_GAME_ID),
...
export async function diamondDigPaidSpin(data: {
  bet: number;
}): Promise<DiamondDigSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${DIAMOND_DIG_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DIAMOND_DIG_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function diamondDigFreeSpin(data: {
  sessionId: string;
}): Promise<DiamondDigSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: DiamondDigSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DIAMOND_DIG_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function diamondDigBuyFeature(data: {
  bet: number;
}): Promise<DiamondDigSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${DIAMOND_DIG_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DIAMOND_DIG_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
