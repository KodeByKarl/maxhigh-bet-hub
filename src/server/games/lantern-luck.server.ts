/**
 * Lantern Luck — server-authoritative spin settlement, gamble round, audit.
 */
...
import { resolveGamble } from "@/components/maxhigh/lantern-luck/gambleEngine";
import { setLanternLuckConfig } from "@/components/maxhigh/lantern-luck/runtimeConfig";
import { resolveLanternLuckSpin } from "@/components/maxhigh/lantern-luck/spinResolver";
import type { GambleChoice, SpinScript } from "@/components/maxhigh/lantern-luck/types";
import { newSpinSeed } from "@/components/maxhigh/lantern-luck/rng";

const GAME_NAME = "Lantern Luck";

let cachedConfig: { cfg: LanternLuckConfig; time: number } | null = null;

export function clearLanternLuckEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<LanternLuckConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeLanternLuckConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeLanternLuckConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
export type LanternLuckSessionState = {
...
export type LanternLuckSpinResult = {
  balance: number;
  script: SpinScript;
  session: LanternLuckSessionState;
  /** True when win was credited immediately (auto-collect / no win). */
  collected: boolean;
};

export type LanternLuckGambleApiResult = {
  balance: number;
...
export type LanternLuckCollectResult = {
  balance: number;
  amount: number;
  session: LanternLuckSessionState;
};

function emptySession(): LanternLuckSessionState {
  return {
    sessionId: null,
    pendingWin: 0,
    gambleRoundsUsed: 0,
...
export async function getLanternLuckOpenSession(): Promise<LanternLuckSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, LANTERN_LUCK_GAME_ID),
...
export async function lanternLuckPaidSpin(data: {
  ...
  const script = resolveLanternLuckSpin({ totalBet: cost });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${LANTERN_LUCK_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (shouldHoldGamble) {
      ...
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${LANTERN_LUCK_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function lanternLuckCollect(data: {
  sessionId: string;
}): Promise<LanternLuckCollectResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (amount > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${LANTERN_LUCK_GAME_ID} · ${GAME_NAME} · collect ₱${amount.toFixed(2)}`,
      });
...
export async function lanternLuckGamble(data: {
  sessionId: string;
  choice: GambleChoice;
}): Promise<LanternLuckGambleApiResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const cfg = await loadEngineConfig();
  setLanternLuckConfig(cfg);
  ...
  const out = await db.transaction(async (tx) => {
    ...
    if (!result.canGambleAgain) {
      ...
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${LANTERN_LUCK_GAME_ID} · ${GAME_NAME} · gamble settle ₱${result.amount.toFixed(2)}`,
      });
      await tx
        .update(playSessions)
    ...
    return {
      ...
      session: {
        ...
        totalBet: feat.totalBet,
        inGamble: true,
      } satisfies LanternLuckSessionState,
      collected: false,
      username: row.username,
    };
  });
