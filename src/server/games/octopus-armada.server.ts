/**
 * Octopus Armada — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/octopus-armada/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/octopus-armada/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/octopus-armada/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/octopus-armada/rng";
import { setOctopusArmadaConfig } from "@/components/maxhigh/games/octopus-armada/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/octopus-armada/hitResolver";
...
const GAME_NAME = "Octopus Armada";

let cachedConfig: { cfg: OctopusArmadaConfig; time: number } | null = null;
...
export function clearOctopusArmadaEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: OctopusArmadaConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function octopusArmadaSyncSpawns() {
  await assertGameEnabled();
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  setOctopusArmadaConfig(cfg);
  pruneFish(user.id);
...
async function loadEngineConfig(): Promise<OctopusArmadaConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeOctopusArmadaConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeOctopusArmadaConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getOctopusArmadaEngineConfigPublic(): Promise<OctopusArmadaConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Octopus Armada is currently disabled");
  }
}
...
export async function getOctopusArmadaOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, OCTOPUS_ARMADA_GAME_ID),
...
export async function octopusArmadaEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: OctopusArmadaConfig) {
  const now = Date.now();
...
export async function octopusArmadaFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await octopusArmadaEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: OCTOPUS_ARMADA_GAME_ID,
      note: `Octopus Armada shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Octopus Armada boss share"
          : shot.crateDrop
            ? `Octopus Armada crate (${shot.crateDrop.label})`
            : `Octopus Armada catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: OCTOPUS_ARMADA_GAME_ID,
          note: "Octopus Armada boss contributor share",
        });
      }
    }
...
export async function octopusArmadaSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function octopusArmadaBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  let session = await loadOpenSessionRow(user.id);
  if (!session) {
    await octopusArmadaEnsureSession({});
    session = await loadOpenSessionRow(user.id);
  }
  if (!session) throw new Error("Could not open play session");
  const feat = parseFeature(session.featureState);
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: OCTOPUS_ARMADA_GAME_ID,
      note: `Octopus Armada power-up (${power.label})`,
    });
...
export async function octopusArmadaUseNetBomb(input: {
  weaponId: string;
...
export async function octopusArmadaUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function octopusArmadaBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
