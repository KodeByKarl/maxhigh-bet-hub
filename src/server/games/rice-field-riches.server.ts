/**
 * Rice Field Riches � server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type RiceFieldRichesConfig,
} from "@/lib/rice-field-riches-config";
...
import { setRiceFieldRichesConfig } from "@/components/maxhigh/rice-field-riches/runtimeConfig";
import { resolveRiceFieldRichesSpin } from "@/components/maxhigh/rice-field-riches/spinResolver";

const GAME_NAME = "Rice Field Riches";

let cachedConfig: { cfg: RiceFieldRichesConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearRiceFieldRichesEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<RiceFieldRichesConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeRiceFieldRichesConfig(JSON.parse(raw) as unknown) : normalizeRiceFieldRichesConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeRiceFieldRichesConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  const script = resolveRiceFieldRichesSpin({
    bet: opts.bet,
    ante: opts.ante && !opts.isFreeSpins,
    isFreeSpins: opts.isFreeSpins,
    sessionMultiplier: opts.sessionMultiplier,
  });
  return { script, cfg };
}

export type RiceFieldRichesSessionState = {
...
export type RiceFieldRichesSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/rice-field-riches/spinResolver").resolveRiceFieldRichesSpin>
  >;
  session: RiceFieldRichesSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): RiceFieldRichesSessionState {
...
export async function getRiceFieldRichesOpenSession(): Promise<RiceFieldRichesSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, RICE_FIELD_RICHES_GAME_ID),
...
export async function riceFieldRichesPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<RiceFieldRichesSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${RICE_FIELD_RICHES_GAME_ID} � ${GAME_NAME} � wager ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${RICE_FIELD_RICHES_GAME_ID} � ${GAME_NAME} � win ?${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function riceFieldRichesFreeSpin(data: {
  sessionId: string;
}): Promise<RiceFieldRichesSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const cfg = await loadEngineConfig();
  const playedSoFar = Number(currentSession.fsSpinsPlayed);
  const sessionMultiplier =
    cfg.freeSpinsSessionMultStart + playedSoFar * cfg.freeSpinsSessionMultStep;
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: RiceFieldRichesSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${RICE_FIELD_RICHES_GAME_ID} � ${GAME_NAME} � Free spins feature final payout ?${accumulatedWin.toFixed(2)}`,
      });
...
export async function riceFieldRichesBuyFeature(data: {
  bet: number;
}): Promise<RiceFieldRichesSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${RICE_FIELD_RICHES_GAME_ID} � ${GAME_NAME} � Feature Buy ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${RICE_FIELD_RICHES_GAME_ID} � ${GAME_NAME} � Feature Buy initial spin win ?${script.totalWin.toFixed(2)}`,
      });
    }
