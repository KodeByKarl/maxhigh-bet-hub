/**
 * Galaxy Ace — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type GalaxyAceConfig,
} from "@/lib/galaxy-ace-config";
...
import { setGalaxyAceConfig } from "@/components/maxhigh/galaxy-ace/runtimeConfig";
import { resolveGalaxyAceSpin } from "@/components/maxhigh/galaxy-ace/spinResolver";

const GAME_NAME = "Galaxy Ace";

let cachedConfig: { cfg: GalaxyAceConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearGalaxyAceEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<GalaxyAceConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeGalaxyAceConfig(JSON.parse(raw) as unknown) : normalizeGalaxyAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeGalaxyAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  const script = resolveGalaxyAceSpin({
    bet: opts.bet,
...
export type GalaxyAceSessionState = {
...
export type GalaxyAceSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/galaxy-ace/spinResolver").resolveGalaxyAceSpin>
  >;
  session: GalaxyAceSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): GalaxyAceSessionState {
...
export async function getGalaxyAceOpenSession(): Promise<GalaxyAceSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, GALAXY_ACE_GAME_ID),
...
export async function galaxyAcePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<GalaxyAceSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${GALAXY_ACE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GALAXY_ACE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function galaxyAceFreeSpin(data: {
  sessionId: string;
}): Promise<GalaxyAceSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: GalaxyAceSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GALAXY_ACE_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function galaxyAceBuyFeature(data: {
  bet: number;
}): Promise<GalaxyAceSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${GALAXY_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GALAXY_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
