/**
 * Crystal Cave — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import { setCrystalCaveConfig } from "@/components/maxhigh/crystal-cave/runtimeConfig";
import { resolveCrystalCaveSpin } from "@/components/maxhigh/crystal-cave/spinResolver";

const GAME_NAME = "Crystal Cave";

let cachedConfig: { cfg: CrystalCaveConfig; time: number } | null = null;

export function clearCrystalCaveEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<CrystalCaveConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeCrystalCaveConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCrystalCaveConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: { bet: number; isFreeSpins: boolean }) {
  ...
  setCrystalCaveConfig(cfg);
  const script = resolveCrystalCaveSpin({
    totalBet: opts.bet,
...
export type CrystalCaveSessionState = {
...
export type CrystalCaveSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveCrystalCaveSpin>;
  session: CrystalCaveSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): CrystalCaveSessionState {
...
export async function getCrystalCaveOpenSession(): Promise<CrystalCaveSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CRYSTAL_CAVE_GAME_ID),
...
export async function crystalCavePaidSpin(data: {
  bet: number;
}): Promise<CrystalCaveSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CRYSTAL_CAVE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CRYSTAL_CAVE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function crystalCaveFreeSpin(data: {
  sessionId: string;
}): Promise<CrystalCaveSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: CrystalCaveSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CRYSTAL_CAVE_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function crystalCaveBuyFeature(data: {
  bet: number;
}): Promise<CrystalCaveSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CRYSTAL_CAVE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CRYSTAL_CAVE_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
