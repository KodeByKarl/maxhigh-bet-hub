/**
 * Wild Panther — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { WILD_PANTHER_GAME_ID, normalizeWildPantherConfig } from "@/lib/wild-panther-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeWildPantherConfig(null);
    return normalizeWildPantherConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeWildPantherConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setWildPantherConfig } = await import("@/components/maxhigh/wild-panther/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/wild-panther/spinResolver"
    );
    setWildPantherConfig(cfg);
...
export type WildPantherSessionState = {
...
export type WildPantherSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/wild-panther/spinResolver").resolveSpin>>;
  session: WildPantherSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): WildPantherSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, WILD_PANTHER_GAME_ID),
...
export async function getWildPantherOpenSession(): Promise<WildPantherSessionState> {
...
export async function wildPantherPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<WildPantherSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${WILD_PANTHER_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${WILD_PANTHER_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function wildPantherFreeSpin(data: {
  sessionId: string;
}): Promise<WildPantherSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: WildPantherSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${WILD_PANTHER_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function wildPantherBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: WildPantherSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${WILD_PANTHER_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${WILD_PANTHER_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
