/**
 * Island Fever — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { ISLAND_FEVER_GAME_ID, normalizeIslandFeverConfig } from "@/lib/island-fever-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeIslandFeverConfig(null);
    return normalizeIslandFeverConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeIslandFeverConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setIslandFeverConfig } = await import("@/components/maxhigh/island-fever/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/island-fever/spinResolver"
    );
    setIslandFeverConfig(cfg);
...
export type IslandFeverSessionState = {
...
export type IslandFeverSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/island-fever/spinResolver").resolveSpin>>;
  session: IslandFeverSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): IslandFeverSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, ISLAND_FEVER_GAME_ID),
...
export async function getIslandFeverOpenSession(): Promise<IslandFeverSessionState> {
...
export async function islandFeverPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<IslandFeverSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ISLAND_FEVER_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ISLAND_FEVER_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function islandFeverFreeSpin(data: {
  sessionId: string;
}): Promise<IslandFeverSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: IslandFeverSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${ISLAND_FEVER_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function islandFeverBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: IslandFeverSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${ISLAND_FEVER_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${ISLAND_FEVER_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
