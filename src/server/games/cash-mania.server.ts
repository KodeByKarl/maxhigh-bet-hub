/**
 * Cash Mania — server-authoritative spin settlement (base game only).
...
import { setCashManiaConfig } from "@/components/maxhigh/cash-mania/runtimeConfig";
import { resolveCashManiaSpin } from "@/components/maxhigh/cash-mania/spinResolver";
import type { SpinScript } from "@/components/maxhigh/cash-mania/types";

const GAME_NAME = "Cash Mania";

let cachedConfig: { cfg: CashManiaConfig; time: number } | null = null;

export function clearCashManiaEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<CashManiaConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeCashManiaConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCashManiaConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type CashManiaSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function cashManiaPaidSpin(data: { bet: number }): Promise<CashManiaSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setCashManiaConfig(cfg);
  ...
  const script = resolveCashManiaSpin({ totalBet: cost });
  const roundId = newId();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CASH_MANIA_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CASH_MANIA_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getCashManiaEngineConfigPublic(): Promise<CashManiaConfig> {
