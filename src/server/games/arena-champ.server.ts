/**
 * Arena Champ — server-authoritative spin settlement (base game only).
...
import { setArenaChampConfig } from "@/components/maxhigh/arena-champ/runtimeConfig";
import { resolveArenaChampSpin } from "@/components/maxhigh/arena-champ/spinResolver";
import type { SpinScript } from "@/components/maxhigh/arena-champ/types";

const GAME_NAME = "Arena Champ";

let cachedConfig: { cfg: ArenaChampConfig; time: number } | null = null;

export function clearArenaChampEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<ArenaChampConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeArenaChampConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeArenaChampConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type ArenaChampSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function arenaChampPaidSpin(data: { bet: number }): Promise<ArenaChampSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setArenaChampConfig(cfg);
  ...
  const script = resolveArenaChampSpin({ totalBet: cost });
  const roundId = newId();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ARENA_CHAMP_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ARENA_CHAMP_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getArenaChampEngineConfigPublic(): Promise<ArenaChampConfig> {
