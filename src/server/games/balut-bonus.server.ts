/**
 * Balut Bonus — server-authoritative spin settlement (base game only).
...
import { setBalutBonusConfig } from "@/components/maxhigh/balut-bonus/runtimeConfig";
import { resolveBalutBonus } from "@/components/maxhigh/balut-bonus/spinResolver";
import type { SpinScript } from "@/components/maxhigh/balut-bonus/types";

const GAME_NAME = "Balut Bonus";

let cachedConfig: { cfg: BalutBonusConfig; time: number } | null = null;

export function clearBalutBonusEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<BalutBonusConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeBalutBonusConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeBalutBonusConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type BalutBonusResult = {
  balance: number;
  script: SpinScript;
};

export async function balutBonusPaidSpin(data: {
  bet: number;
  exMode?: boolean;
}): Promise<BalutBonusResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setBalutBonusConfig(cfg);
  ...
  const script = resolveBalutBonus({
    totalBet: cost,
    baseBet,
    exMode,
  });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${BALUT_BONUS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}${exMode ? " · EX" : ""}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${BALUT_BONUS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getBalutBonusEngineConfigPublic(): Promise<BalutBonusConfig> {
