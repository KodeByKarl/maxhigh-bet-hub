/**
 * Neon Makati — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type NeonMakatiConfig,
} from "@/lib/neon-makati-config";
...
import { setNeonMakatiConfig } from "@/components/maxhigh/neon-makati/runtimeConfig";
import { resolveNeonMakatiSpin } from "@/components/maxhigh/neon-makati/spinResolver";

const GAME_NAME = "Neon Makati";

let cachedConfig: { cfg: NeonMakatiConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearNeonMakatiEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<NeonMakatiConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeNeonMakatiConfig(JSON.parse(raw) as unknown) : normalizeNeonMakatiConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeNeonMakatiConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setNeonMakatiConfig(cfg);
  const script = resolveNeonMakatiSpin({
    bet: opts.bet,
...
export type NeonMakatiSessionState = {
...
export type NeonMakatiSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/neon-makati/spinResolver").resolveNeonMakatiSpin>
  >;
  session: NeonMakatiSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): NeonMakatiSessionState {
...
export async function getNeonMakatiOpenSession(): Promise<NeonMakatiSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, NEON_MAKATI_GAME_ID),
...
export async function neonMakatiPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<NeonMakatiSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${NEON_MAKATI_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${NEON_MAKATI_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function neonMakatiFreeSpin(data: {
  sessionId: string;
}): Promise<NeonMakatiSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: NeonMakatiSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${NEON_MAKATI_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function neonMakatiBuyFeature(data: {
  bet: number;
}): Promise<NeonMakatiSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${NEON_MAKATI_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${NEON_MAKATI_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
