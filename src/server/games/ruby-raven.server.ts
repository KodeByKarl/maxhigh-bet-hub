/**
 * Ruby Raven — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { RUBY_RAVEN_GAME_ID, normalizeRubyRavenConfig, remainingFreeSpinsAfterSpin } from "@/lib/ruby-raven-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeRubyRavenConfig(null);
    return normalizeRubyRavenConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeRubyRavenConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setRubyRavenConfig } = await import("@/components/maxhigh/ruby-raven/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/ruby-raven/spinResolver"
    );
    setRubyRavenConfig(cfg);
...
export type RubyRavenSessionState = {
...
export type RubyRavenSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/ruby-raven/spinResolver").resolveSpin>>;
  session: RubyRavenSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): RubyRavenSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, RUBY_RAVEN_GAME_ID),
...
export async function getRubyRavenOpenSession(): Promise<RubyRavenSessionState> {
...
export async function rubyRavenPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<RubyRavenSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${RUBY_RAVEN_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${RUBY_RAVEN_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function rubyRavenFreeSpin(data: {
  sessionId: string;
}): Promise<RubyRavenSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: RubyRavenSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${RUBY_RAVEN_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function rubyRavenBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: RubyRavenSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${RUBY_RAVEN_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${RUBY_RAVEN_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
