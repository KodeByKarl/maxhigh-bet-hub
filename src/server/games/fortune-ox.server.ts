/**
 * Fortune Ox — server-authoritative spin settlement (base game only).
...
import { setFortuneOxConfig } from "@/components/maxhigh/fortune-ox/runtimeConfig";
import { resolveFortuneOxSpin } from "@/components/maxhigh/fortune-ox/spinResolver";
import type { SpinScript } from "@/components/maxhigh/fortune-ox/types";

const GAME_NAME = "Fortune Ox";

let cachedConfig: { cfg: FortuneOxConfig; time: number } | null = null;

export function clearFortuneOxEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<FortuneOxConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeFortuneOxConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeFortuneOxConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type FortuneOxSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function fortuneOxPaidSpin(data: {
  bet: number;
  exMode?: boolean;
}): Promise<FortuneOxSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setFortuneOxConfig(cfg);
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${FORTUNE_OX_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}${exMode ? " · EX" : ""}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${FORTUNE_OX_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getFortuneOxEngineConfigPublic(): Promise<FortuneOxConfig> {
