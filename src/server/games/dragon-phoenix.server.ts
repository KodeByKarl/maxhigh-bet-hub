/**
 * Dragon & Phoenix — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type DragonPhoenixConfig,
} from "@/lib/dragon-phoenix-config";
...
import { setDragonPhoenixConfig } from "@/components/maxhigh/dragon-phoenix/runtimeConfig";
import { resolveDragonPhoenixSpin } from "@/components/maxhigh/dragon-phoenix/spinResolver";

const GAME_NAME = "Dragon & Phoenix";

let cachedConfig: { cfg: DragonPhoenixConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearDragonPhoenixEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<DragonPhoenixConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeDragonPhoenixConfig(JSON.parse(raw) as unknown) : normalizeDragonPhoenixConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeDragonPhoenixConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setDragonPhoenixConfig(cfg);
  const script = resolveDragonPhoenixSpin({
    bet: opts.bet,
...
export type DragonPhoenixSessionState = {
...
export type DragonPhoenixSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/dragon-phoenix/spinResolver").resolveDragonPhoenixSpin>
  >;
  session: DragonPhoenixSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): DragonPhoenixSessionState {
...
export async function getDragonPhoenixOpenSession(): Promise<DragonPhoenixSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, DRAGON_PHOENIX_GAME_ID),
...
export async function dragonPhoenixPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<DragonPhoenixSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${DRAGON_PHOENIX_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DRAGON_PHOENIX_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function dragonPhoenixFreeSpin(data: {
  sessionId: string;
}): Promise<DragonPhoenixSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: DragonPhoenixSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DRAGON_PHOENIX_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function dragonPhoenixBuyFeature(data: {
  bet: number;
}): Promise<DragonPhoenixSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${DRAGON_PHOENIX_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DRAGON_PHOENIX_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
