/**
 * Ember Tiger — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { EMBER_TIGER_GAME_ID, normalizeEmberTigerConfig, remainingFreeSpinsAfterSpin } from "@/lib/ember-tiger-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeEmberTigerConfig(null);
    return normalizeEmberTigerConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeEmberTigerConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setEmberTigerConfig } = await import("@/components/maxhigh/ember-tiger/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/ember-tiger/spinResolver"
    );
    setEmberTigerConfig(cfg);
...
export type EmberTigerSessionState = {
...
export type EmberTigerSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/ember-tiger/spinResolver").resolveSpin>>;
  session: EmberTigerSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): EmberTigerSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, EMBER_TIGER_GAME_ID),
...
export async function getEmberTigerOpenSession(): Promise<EmberTigerSessionState> {
...
export async function emberTigerPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<EmberTigerSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${EMBER_TIGER_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${EMBER_TIGER_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function emberTigerFreeSpin(data: {
  sessionId: string;
}): Promise<EmberTigerSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: EmberTigerSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${EMBER_TIGER_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function emberTigerBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: EmberTigerSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${EMBER_TIGER_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${EMBER_TIGER_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
