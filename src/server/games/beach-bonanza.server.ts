/**
 * Beach Bonanza — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { BEACH_BONANZA_GAME_ID, normalizeBeachBonanzaConfig } from "@/lib/beach-bonanza-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeBeachBonanzaConfig(null);
    return normalizeBeachBonanzaConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeBeachBonanzaConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setBeachBonanzaConfig } = await import("@/components/maxhigh/beach-bonanza/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/beach-bonanza/spinResolver"
    );
    setBeachBonanzaConfig(cfg);
...
export type BeachBonanzaSessionState = {
...
export type BeachBonanzaSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/beach-bonanza/spinResolver").resolveSpin>>;
  session: BeachBonanzaSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): BeachBonanzaSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, BEACH_BONANZA_GAME_ID),
...
export async function getBeachBonanzaOpenSession(): Promise<BeachBonanzaSessionState> {
...
export async function beachBonanzaPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<BeachBonanzaSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${BEACH_BONANZA_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${BEACH_BONANZA_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function beachBonanzaFreeSpin(data: {
  sessionId: string;
}): Promise<BeachBonanzaSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: BeachBonanzaSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${BEACH_BONANZA_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function beachBonanzaBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: BeachBonanzaSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${BEACH_BONANZA_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${BEACH_BONANZA_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
