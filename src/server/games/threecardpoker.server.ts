/**
 * Three Card Poker — server-authoritative two-phase settlement.
 *
...
import {
  ...
  resolveFold,
  resolvePlay,
} from "@/components/maxhigh/games/threecardpoker/resolver";
import { setThreeCardPokerConfig } from "@/components/maxhigh/games/threecardpoker/runtimeConfig";
import type { PlayingCard } from "@/components/maxhigh/games/threecardpoker/deckEngine";
import type { HandRank } from "@/components/maxhigh/games/threecardpoker/handEvaluator";
import type {
  PublicDealScript,
  PublicSettleScript,
} from "@/components/maxhigh/games/threecardpoker/types";
...
const GAME_NAME = "Three Card Poker";

let cachedConfig: { cfg: ThreeCardPokerConfig; time: number } | null = null;

export function clearThreeCardPokerEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<ThreeCardPokerConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeThreeCardPokerConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeThreeCardPokerConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getThreeCardPokerEngineConfigPublic(): Promise<ThreeCardPokerConfig> {
  return loadEngineConfig();
}

type TcpFeatureState = {
  kind: "threecardpoker";
  phase: "decision";
  seed: string;
  ante: number;
...
function parseFeatureState(raw: string | null): TcpFeatureState | null {
  if (!raw) return null;
  try {
    const o = JSON.parse(raw) as TcpFeatureState;
    if (o?.kind !== "threecardpoker" || o.phase !== "decision") return null;
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Three Card Poker is currently disabled");
  }
}

export type TcpOpenSessionView = {
  sessionId: string;
  ante: number;
  pairPlus: number;
  pairPlusWin: number;
...
export async function getThreeCardPokerOpenSession(): Promise<TcpOpenSessionView | null> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, THREE_CARD_POKER_GAME_ID),
...
export type TcpDealResult = {
  balance: number;
  sessionId: string;
  script: PublicDealScript;
};

export async function threeCardPokerPaidDeal(input: {
  ante: number;
  pairPlus?: number;
}): Promise<TcpDealResult> {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${THREE_CARD_POKER_GAME_ID} · ${GAME_NAME} · ante ₱${ante.toFixed(2)} pp ₱${pairPlus.toFixed(2)}`,
    });
    ...
    if (resolved.pairPlusWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${THREE_CARD_POKER_GAME_ID} · ${GAME_NAME} · pair plus ₱${resolved.pairPlusWin.toFixed(2)}`,
      });
    }
...
export type TcpSettleResult = {
  balance: number;
  script: PublicSettleScript;
};

export async function threeCardPokerPlay(input: {
  sessionId: string;
}): Promise<TcpSettleResult> {
  await assertNotInMaintenanceForBets();
  const user = await requireUser();
  await assertGameEnabled();
  ...
  if (!current || !feat) throw new Error("No open Three Card Poker hand to play");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${THREE_CARD_POKER_GAME_ID} · ${GAME_NAME} · play ₱${playWager.toFixed(2)}`,
    });
    ...
    if (settled.immediateCredit > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${THREE_CARD_POKER_GAME_ID} · ${GAME_NAME} · ${settled.outcome} settle ₱${settled.immediateCredit.toFixed(2)}`,
      });
    }
...
export async function threeCardPokerFold(input: {
  sessionId: string;
}): Promise<TcpSettleResult> {
  await assertNotInMaintenanceForBets();
  const user = await requireUser();
  ...
  const current = rows[0];
  const feat = current ? parseFeatureState(current.featureState) : null;
  if (!current || !feat) throw new Error("No open Three Card Poker hand to fold");
