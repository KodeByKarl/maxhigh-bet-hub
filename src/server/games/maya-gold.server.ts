/**
 * Maya Gold — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { MAYA_GOLD_GAME_ID, normalizeMayaGoldConfig } from "@/lib/maya-gold-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeMayaGoldConfig(null);
    return normalizeMayaGoldConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeMayaGoldConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setMayaGoldConfig } = await import("@/components/maxhigh/maya-gold/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/maya-gold/spinResolver"
    );
    setMayaGoldConfig(cfg);
...
export type MayaGoldSessionState = {
...
export type MayaGoldSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/maya-gold/spinResolver").resolveSpin>>;
  session: MayaGoldSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): MayaGoldSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, MAYA_GOLD_GAME_ID),
...
export async function getMayaGoldOpenSession(): Promise<MayaGoldSessionState> {
...
export async function mayaGoldPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<MayaGoldSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MAYA_GOLD_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAYA_GOLD_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function mayaGoldFreeSpin(data: {
  sessionId: string;
}): Promise<MayaGoldSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: MayaGoldSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${MAYA_GOLD_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function mayaGoldBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: MayaGoldSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${MAYA_GOLD_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${MAYA_GOLD_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
