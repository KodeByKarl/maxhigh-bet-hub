/**
 * Tricycle Treasure — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import {
  ...
  type TricycleTreasureConfig,
} from "@/lib/tricycle-treasure-config";
...
import { setTricycleTreasureConfig } from "@/components/maxhigh/tricycle-treasure/runtimeConfig";
import { resolveTricycleTreasureSpin } from "@/components/maxhigh/tricycle-treasure/spinResolver";

const GAME_NAME = "Tricycle Treasure";

let cachedConfig: { cfg: TricycleTreasureConfig; time: number } | null = null;

export function clearTricycleTreasureEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<TricycleTreasureConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeTricycleTreasureConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeTricycleTreasureConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: { bet: number; isFreeSpins: boolean }) {
  ...
  setTricycleTreasureConfig(cfg);
  const script = resolveTricycleTreasureSpin({
    totalBet: opts.bet,
...
export type TricycleTreasureSessionState = {
...
export type TricycleTreasureSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveTricycleTreasureSpin>;
  session: TricycleTreasureSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): TricycleTreasureSessionState {
...
export async function getTricycleTreasureOpenSession(): Promise<TricycleTreasureSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, TRICYCLE_TREASURE_GAME_ID),
...
export async function tricycleTreasurePaidSpin(data: {
  bet: number;
}): Promise<TricycleTreasureSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${TRICYCLE_TREASURE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${TRICYCLE_TREASURE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function tricycleTreasureFreeSpin(data: {
  sessionId: string;
}): Promise<TricycleTreasureSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: TricycleTreasureSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${TRICYCLE_TREASURE_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function tricycleTreasureBuyFeature(data: {
  bet: number;
}): Promise<TricycleTreasureSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${TRICYCLE_TREASURE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${TRICYCLE_TREASURE_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
