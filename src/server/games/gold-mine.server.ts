/**
 * Gold Mine Dig — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import { setGoldMineConfig } from "@/components/maxhigh/gold-mine/runtimeConfig";
import { resolveGoldMineSpin } from "@/components/maxhigh/gold-mine/spinResolver";

const GAME_NAME = "Gold Mine Dig";

let cachedConfig: { cfg: GoldMineConfig; time: number } | null = null;

export function clearGoldMineEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<GoldMineConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeGoldMineConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeGoldMineConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: { bet: number; isFreeSpins: boolean }) {
  ...
  setGoldMineConfig(cfg);
  const script = resolveGoldMineSpin({
    totalBet: opts.bet,
...
export type GoldMineSessionState = {
...
export type GoldMineSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveGoldMineSpin>;
  session: GoldMineSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): GoldMineSessionState {
...
export async function getGoldMineOpenSession(): Promise<GoldMineSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, GOLD_MINE_GAME_ID),
...
export async function goldMinePaidSpin(data: {
  bet: number;
}): Promise<GoldMineSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${GOLD_MINE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GOLD_MINE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function goldMineFreeSpin(data: {
  sessionId: string;
}): Promise<GoldMineSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: GoldMineSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GOLD_MINE_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function goldMineBuyFeature(data: {
  bet: number;
}): Promise<GoldMineSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${GOLD_MINE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${GOLD_MINE_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
