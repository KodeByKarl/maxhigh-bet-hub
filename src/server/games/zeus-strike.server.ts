/**
 * Zeus Strike — server-authoritative spin settle.
 */
...
export { ZEUS_STRIKE_GAME_ID };
const GAME_NAME = "Zeus Strike";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeZeusStrikeConfig(null);
    return normalizeZeusStrikeConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeZeusStrikeConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setZeusStrikeConfig } = await import(
      "@/components/maxhigh/zeus-strike/runtimeConfig"
    );
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/zeus-strike/spinResolver"
    );
    setZeusStrikeConfig(cfg);
...
export type ZeusStrikeSessionState = {
...
export type ZeusStrikeSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/zeus-strike/spinResolver").resolveSpin>>;
  session: ZeusStrikeSessionState;
  fsPayout?: {
    amount: number;
    baseEarn: number;
    multiplier: number;
...
function sessionState(row: typeof playSessions.$inferSelect | null): ZeusStrikeSessionState {
...
export async function getZeusStrikeOpenSession(): Promise<ZeusStrikeSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, ZEUS_STRIKE_GAME_ID),
...
export async function zeusStrikePaidSpin(data: { bet: number }): Promise<ZeusStrikeSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ZEUS_STRIKE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ZEUS_STRIKE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function zeusStrikeFreeSpin(data: {
  sessionId: string;
}): Promise<ZeusStrikeSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: ZeusStrikeSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${ZEUS_STRIKE_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function zeusStrikeBuyFeature(data: {
  bet: number;
}): Promise<{ balance: number; session: ZeusStrikeSessionState; awarded: number }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ZEUS_STRIKE_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
