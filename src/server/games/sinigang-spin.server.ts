/**
 * Sinigang Spin — server-authoritative spin settlement, gamble round, audit.
 */
...
import { resolveGamble } from "@/components/maxhigh/sinigang-spin/gambleEngine";
import { setSinigangSpinConfig } from "@/components/maxhigh/sinigang-spin/runtimeConfig";
import { resolveSinigangSpinSpin } from "@/components/maxhigh/sinigang-spin/spinResolver";
import type { GambleChoice, SpinScript } from "@/components/maxhigh/sinigang-spin/types";
import { newSpinSeed } from "@/components/maxhigh/sinigang-spin/rng";

const GAME_NAME = "Sinigang Spin";

let cachedConfig: { cfg: SinigangSpinConfig; time: number } | null = null;

export function clearSinigangSpinEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<SinigangSpinConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeSinigangSpinConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeSinigangSpinConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
export type SinigangSpinSessionState = {
...
export type SinigangSpinSpinResult = {
  balance: number;
  script: SpinScript;
  session: SinigangSpinSessionState;
  /** True when win was credited immediately (auto-collect / no win). */
  collected: boolean;
};

export type SinigangSpinGambleApiResult = {
  balance: number;
...
export type SinigangSpinCollectResult = {
  balance: number;
  amount: number;
  session: SinigangSpinSessionState;
};

function emptySession(): SinigangSpinSessionState {
  return {
    sessionId: null,
    pendingWin: 0,
    gambleRoundsUsed: 0,
...
export async function getSinigangSpinOpenSession(): Promise<SinigangSpinSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, SINIGANG_SPIN_GAME_ID),
...
export async function sinigangSpinPaidSpin(data: {
  bet: number;
  /** Autoplay / turbo collect — skip gamble hold. */
  autoCollect?: boolean;
}): Promise<SinigangSpinSpinResult> {
  const user = await requireUser();
  ...
  const script = resolveSinigangSpinSpin({ totalBet: cost });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SINIGANG_SPIN_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (shouldHoldGamble) {
      ...
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SINIGANG_SPIN_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function sinigangSpinCollect(data: {
  sessionId: string;
}): Promise<SinigangSpinCollectResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (amount > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SINIGANG_SPIN_GAME_ID} · ${GAME_NAME} · collect ₱${amount.toFixed(2)}`,
      });
...
export async function sinigangSpinGamble(data: {
  sessionId: string;
  choice: GambleChoice;
}): Promise<SinigangSpinGambleApiResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const cfg = await loadEngineConfig();
  setSinigangSpinConfig(cfg);
  ...
  const out = await db.transaction(async (tx) => {
    ...
    if (!result.canGambleAgain) {
      ...
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SINIGANG_SPIN_GAME_ID} · ${GAME_NAME} · gamble settle ₱${result.amount.toFixed(2)}`,
      });
      await tx
        .update(playSessions)
    ...
    return {
      ...
      session: {
        ...
        totalBet: feat.totalBet,
        inGamble: true,
      } satisfies SinigangSpinSessionState,
      collected: false,
      username: row.username,
    };
  });
