/**
 * Buffalo Reign — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type BuffaloReignConfig,
} from "@/lib/buffalo-reign-config";
...
import { setBuffaloReignConfig } from "@/components/maxhigh/wild-frontier-stampede/runtimeConfig";
import { resolveBuffaloSpin } from "@/components/maxhigh/wild-frontier-stampede/spinResolver";

const GAME_NAME = "Buffalo Reign";

let cachedConfig: { cfg: BuffaloReignConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearBuffaloReignEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<BuffaloReignConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeBuffaloReignConfig(JSON.parse(raw) as unknown) : normalizeBuffaloReignConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeBuffaloReignConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setBuffaloReignConfig(cfg);
  const script = resolveBuffaloSpin({
    bet: opts.bet,
...
export type BuffaloReignSessionState = {
...
export type BuffaloReignSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/wild-frontier-stampede/spinResolver").resolveBuffaloSpin>
  >;
  session: BuffaloReignSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): BuffaloReignSessionState {
...
export async function getBuffaloReignOpenSession(): Promise<BuffaloReignSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, BUFFALO_REIGN_GAME_ID),
...
export async function buffaloReignPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<BuffaloReignSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${BUFFALO_REIGN_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${BUFFALO_REIGN_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function buffaloReignFreeSpin(data: {
  sessionId: string;
}): Promise<BuffaloReignSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: BuffaloReignSpinResult["fsPayout"];
...
export async function buffaloReignBuyFeature(data: {
  bet: number;
}): Promise<BuffaloReignSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${BUFFALO_REIGN_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${BUFFALO_REIGN_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
