/**
 * Crab Cannon — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/crab-cannon/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/crab-cannon/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/crab-cannon/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/crab-cannon/rng";
import { setCrabCannonConfig } from "@/components/maxhigh/games/crab-cannon/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/crab-cannon/hitResolver";
...
const GAME_NAME = "Crab Cannon";

let cachedConfig: { cfg: CrabCannonConfig; time: number } | null = null;
...
export function clearCrabCannonEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: CrabCannonConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function crabCannonSyncSpawns() {
  await assertGameEnabled();
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  setCrabCannonConfig(cfg);
  pruneFish(user.id);
...
async function loadEngineConfig(): Promise<CrabCannonConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeCrabCannonConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCrabCannonConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getCrabCannonEngineConfigPublic(): Promise<CrabCannonConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Crab Cannon is currently disabled");
  }
}
...
export async function getCrabCannonOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CRAB_CANNON_GAME_ID),
...
export async function crabCannonEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: CrabCannonConfig) {
  const now = Date.now();
...
export async function crabCannonFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await crabCannonEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: CRAB_CANNON_GAME_ID,
      note: `Crab Cannon shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Crab Cannon boss share"
          : shot.crateDrop
            ? `Crab Cannon crate (${shot.crateDrop.label})`
            : `Crab Cannon catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: CRAB_CANNON_GAME_ID,
          note: "Crab Cannon boss contributor share",
        });
      }
    }
...
export async function crabCannonSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function crabCannonBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: CRAB_CANNON_GAME_ID,
      note: `Crab Cannon power-up (${power.label})`,
    });
...
export async function crabCannonUseNetBomb(input: {
  weaponId: string;
...
export async function crabCannonUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function crabCannonBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
