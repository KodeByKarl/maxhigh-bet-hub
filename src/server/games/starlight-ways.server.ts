/**
 * Starlight Ways — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type StarlightWaysConfig,
} from "@/lib/starlight-ways-config";
...
import { setStarlightWaysConfig } from "@/components/maxhigh/starlight-ways/runtimeConfig";
import { resolveStarlightWaysSpin } from "@/components/maxhigh/starlight-ways/spinResolver";

const GAME_NAME = "Starlight Ways";

let cachedConfig: { cfg: StarlightWaysConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearStarlightWaysEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<StarlightWaysConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeStarlightWaysConfig(JSON.parse(raw) as unknown) : normalizeStarlightWaysConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeStarlightWaysConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setStarlightWaysConfig(cfg);
  const script = resolveStarlightWaysSpin({
    bet: opts.bet,
...
export type StarlightWaysSessionState = {
...
export type StarlightWaysSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/starlight-ways/spinResolver").resolveStarlightWaysSpin>
  >;
  session: StarlightWaysSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): StarlightWaysSessionState {
...
export async function getStarlightWaysOpenSession(): Promise<StarlightWaysSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, STARLIGHT_WAYS_GAME_ID),
...
export async function starlightWaysPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<StarlightWaysSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${STARLIGHT_WAYS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${STARLIGHT_WAYS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function starlightWaysFreeSpin(data: {
  sessionId: string;
}): Promise<StarlightWaysSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: StarlightWaysSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${STARLIGHT_WAYS_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function starlightWaysBuyFeature(data: {
  bet: number;
}): Promise<StarlightWaysSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${STARLIGHT_WAYS_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${STARLIGHT_WAYS_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
