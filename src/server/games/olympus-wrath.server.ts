/**
 * Olympus Wrath — server-authoritative spin settle.
 */
...
export { OLYMPUS_WRATH_GAME_ID };
const GAME_NAME = "Olympus Wrath";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeOlympusWrathConfig(null);
    return normalizeOlympusWrathConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeOlympusWrathConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setOlympusWrathConfig } = await import(
      "@/components/maxhigh/olympus-wrath/runtimeConfig"
    );
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/olympus-wrath/spinResolver"
    );
    setOlympusWrathConfig(cfg);
...
export type OlympusWrathSessionState = {
...
export type OlympusWrathSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/olympus-wrath/spinResolver").resolveSpin>>;
  session: OlympusWrathSessionState;
  fsPayout?: {
    amount: number;
    baseEarn: number;
    multiplier: number;
...
function sessionState(row: typeof playSessions.$inferSelect | null): OlympusWrathSessionState {
...
export async function getOlympusWrathOpenSession(): Promise<OlympusWrathSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, OLYMPUS_WRATH_GAME_ID),
...
export async function olympusWrathPaidSpin(data: { bet: number }): Promise<OlympusWrathSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${OLYMPUS_WRATH_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${OLYMPUS_WRATH_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function olympusWrathFreeSpin(data: {
  sessionId: string;
}): Promise<OlympusWrathSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: OlympusWrathSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${OLYMPUS_WRATH_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function olympusWrathBuyFeature(data: {
  bet: number;
}): Promise<{ balance: number; session: OlympusWrathSessionState; awarded: number }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${OLYMPUS_WRATH_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
