/**
 * Frost Fox — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { FROST_FOX_GAME_ID, normalizeFrostFoxConfig, remainingFreeSpinsAfterSpin } from "@/lib/frost-fox-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeFrostFoxConfig(null);
    return normalizeFrostFoxConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeFrostFoxConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setFrostFoxConfig } = await import("@/components/maxhigh/frost-fox/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/frost-fox/spinResolver"
    );
    setFrostFoxConfig(cfg);
...
export type FrostFoxSessionState = {
...
export type FrostFoxSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/frost-fox/spinResolver").resolveSpin>>;
  session: FrostFoxSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): FrostFoxSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, FROST_FOX_GAME_ID),
...
export async function getFrostFoxOpenSession(): Promise<FrostFoxSessionState> {
...
export async function frostFoxPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<FrostFoxSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${FROST_FOX_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${FROST_FOX_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function frostFoxFreeSpin(data: {
  sessionId: string;
}): Promise<FrostFoxSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: FrostFoxSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${FROST_FOX_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function frostFoxBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: FrostFoxSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${FROST_FOX_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${FROST_FOX_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
