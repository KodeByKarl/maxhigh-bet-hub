/**
 * Mahjong Ways 2 — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type MahjongWays2Config,
} from "@/lib/mahjong-ways-2-config";
...
import { setMahjongWays2Config } from "@/components/maxhigh/mahjong-ways-2/runtimeConfig";
import { resolveMahjongWays2Spin } from "@/components/maxhigh/mahjong-ways-2/spinResolver";

const GAME_NAME = "Mahjong Ways 2";

let cachedConfig: { cfg: MahjongWays2Config; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearMahjongWays2EngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<MahjongWays2Config> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeMahjongWays2Config(JSON.parse(raw) as unknown) : normalizeMahjongWays2Config(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeMahjongWays2Config(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setMahjongWays2Config(cfg);
  const script = resolveMahjongWays2Spin({
    bet: opts.bet,
...
export type MahjongWays2SessionState = {
...
export type MahjongWays2SpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/mahjong-ways-2/spinResolver").resolveMahjongWays2Spin>
  >;
  session: MahjongWays2SessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): MahjongWays2SessionState {
...
export async function getMahjongWays2OpenSession(): Promise<MahjongWays2SessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, MAHJONG_WAYS_2_GAME_ID),
...
export async function mahjongWays2PaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<MahjongWays2SpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MAHJONG_WAYS_2_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAHJONG_WAYS_2_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function mahjongWays2FreeSpin(data: {
  sessionId: string;
}): Promise<MahjongWays2SpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: MahjongWays2SpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAHJONG_WAYS_2_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function mahjongWays2BuyFeature(data: {
  bet: number;
}): Promise<MahjongWays2SpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MAHJONG_WAYS_2_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAHJONG_WAYS_2_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
