/**
 * Temple Rush — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { TEMPLE_RUSH_GAME_ID, normalizeTempleRushConfig } from "@/lib/temple-rush-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeTempleRushConfig(null);
    return normalizeTempleRushConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeTempleRushConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setTempleRushConfig } = await import("@/components/maxhigh/temple-rush/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/temple-rush/spinResolver"
    );
    setTempleRushConfig(cfg);
...
export type TempleRushSessionState = {
...
export type TempleRushSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/temple-rush/spinResolver").resolveSpin>>;
  session: TempleRushSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): TempleRushSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, TEMPLE_RUSH_GAME_ID),
...
export async function getTempleRushOpenSession(): Promise<TempleRushSessionState> {
...
export async function templeRushPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<TempleRushSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${TEMPLE_RUSH_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${TEMPLE_RUSH_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function templeRushFreeSpin(data: {
  sessionId: string;
}): Promise<TempleRushSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: TempleRushSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${TEMPLE_RUSH_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function templeRushBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: TempleRushSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${TEMPLE_RUSH_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${TEMPLE_RUSH_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
