/**
 * Chinese New Year — server-authoritative spin settlement, gamble round, audit.
 */
...
import { resolveGamble } from "@/components/maxhigh/chinese-new-year/gambleEngine";
import { setChineseNewYearConfig } from "@/components/maxhigh/chinese-new-year/runtimeConfig";
import { resolveCnySpin } from "@/components/maxhigh/chinese-new-year/spinResolver";
import type { GambleChoice, SpinScript } from "@/components/maxhigh/chinese-new-year/types";
import { newSpinSeed } from "@/components/maxhigh/chinese-new-year/rng";

const GAME_NAME = "Chinese New Year";

let cachedConfig: { cfg: ChineseNewYearConfig; time: number } | null = null;

export function clearChineseNewYearEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<ChineseNewYearConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeChineseNewYearConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeChineseNewYearConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
export type CnySpinResult = {
  balance: number;
  script: SpinScript;
  session: CnySessionState;
  /** True when win was credited immediately (auto-collect / no win). */
  collected: boolean;
};

export type CnyGambleApiResult = {
  balance: number;
...
export type CnyCollectResult = {
  balance: number;
  amount: number;
  session: CnySessionState;
};

function emptySession(): CnySessionState {
  return {
    sessionId: null,
    pendingWin: 0,
    gambleRoundsUsed: 0,
...
export async function getChineseNewYearOpenSession(): Promise<CnySessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CHINESE_NEW_YEAR_GAME_ID),
...
export async function chineseNewYearPaidSpin(data: {
  bet: number;
  /** Autoplay / turbo collect — skip gamble hold. */
  autoCollect?: boolean;
}): Promise<CnySpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CHINESE_NEW_YEAR_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (shouldHoldGamble) {
      ...
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CHINESE_NEW_YEAR_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function chineseNewYearCollect(data: {
  sessionId: string;
}): Promise<CnyCollectResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (amount > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CHINESE_NEW_YEAR_GAME_ID} · ${GAME_NAME} · collect ₱${amount.toFixed(2)}`,
      });
...
export async function chineseNewYearGamble(data: {
  sessionId: string;
  choice: GambleChoice;
}): Promise<CnyGambleApiResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const cfg = await loadEngineConfig();
  setChineseNewYearConfig(cfg);
