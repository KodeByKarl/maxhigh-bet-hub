/**
 * Fiesta Fireworks — server-authoritative spin settlement, gamble round, audit.
 */
...
import { resolveGamble } from "@/components/maxhigh/fiesta-fireworks/gambleEngine";
import { setFiestaFireworksConfig } from "@/components/maxhigh/fiesta-fireworks/runtimeConfig";
import { resolveFiestaFireworksSpin } from "@/components/maxhigh/fiesta-fireworks/spinResolver";
import type { GambleChoice, SpinScript } from "@/components/maxhigh/fiesta-fireworks/types";
import { newSpinSeed } from "@/components/maxhigh/fiesta-fireworks/rng";

const GAME_NAME = "Fiesta Fireworks";

let cachedConfig: { cfg: FiestaFireworksConfig; time: number } | null = null;

export function clearFiestaFireworksEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<FiestaFireworksConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeFiestaFireworksConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeFiestaFireworksConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
export type FiestaFireworksSessionState = {
...
export type FiestaFireworksSpinResult = {
  balance: number;
  script: SpinScript;
  session: FiestaFireworksSessionState;
  /** True when win was credited immediately (auto-collect / no win). */
  collected: boolean;
};

export type FiestaFireworksGambleApiResult = {
  balance: number;
...
export type FiestaFireworksCollectResult = {
  balance: number;
  amount: number;
  session: FiestaFireworksSessionState;
};

function emptySession(): FiestaFireworksSessionState {
  return {
    sessionId: null,
    pendingWin: 0,
    gambleRoundsUsed: 0,
...
export async function getFiestaFireworksOpenSession(): Promise<FiestaFireworksSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, FIESTA_FIREWORKS_GAME_ID),
...
export async function fiestaFireworksPaidSpin(data: {
  ...
  const script = resolveFiestaFireworksSpin({ totalBet: cost });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${FIESTA_FIREWORKS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (shouldHoldGamble) {
      ...
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${FIESTA_FIREWORKS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function fiestaFireworksCollect(data: {
  sessionId: string;
}): Promise<FiestaFireworksCollectResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (amount > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${FIESTA_FIREWORKS_GAME_ID} · ${GAME_NAME} · collect ₱${amount.toFixed(2)}`,
      });
...
export async function fiestaFireworksGamble(data: {
  sessionId: string;
  choice: GambleChoice;
}): Promise<FiestaFireworksGambleApiResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  const cfg = await loadEngineConfig();
  setFiestaFireworksConfig(cfg);
  ...
  const out = await db.transaction(async (tx) => {
    ...
    if (!result.canGambleAgain) {
      ...
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${FIESTA_FIREWORKS_GAME_ID} · ${GAME_NAME} · gamble settle ₱${result.amount.toFixed(2)}`,
      });
      await tx
        .update(playSessions)
    ...
    return {
      ...
      session: {
        ...
        totalBet: feat.totalBet,
        inGamble: true,
      } satisfies FiestaFireworksSessionState,
      collected: false,
      username: row.username,
    };
  });
