/**
 * Jade Jaguar — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { JADE_JAGUAR_GAME_ID, normalizeJadeJaguarConfig, remainingFreeSpinsAfterSpin } from "@/lib/jade-jaguar-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeJadeJaguarConfig(null);
    return normalizeJadeJaguarConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeJadeJaguarConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setJadeJaguarConfig } = await import("@/components/maxhigh/jade-jaguar/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/jade-jaguar/spinResolver"
    );
    setJadeJaguarConfig(cfg);
...
export type JadeJaguarSessionState = {
...
export type JadeJaguarSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/jade-jaguar/spinResolver").resolveSpin>>;
  session: JadeJaguarSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): JadeJaguarSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, JADE_JAGUAR_GAME_ID),
...
export async function getJadeJaguarOpenSession(): Promise<JadeJaguarSessionState> {
...
export async function jadeJaguarPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<JadeJaguarSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${JADE_JAGUAR_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${JADE_JAGUAR_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function jadeJaguarFreeSpin(data: {
  sessionId: string;
}): Promise<JadeJaguarSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: JadeJaguarSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${JADE_JAGUAR_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function jadeJaguarBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: JadeJaguarSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${JADE_JAGUAR_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${JADE_JAGUAR_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
