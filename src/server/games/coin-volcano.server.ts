/**
 * Coin Volcano — server-authoritative spin settlement (base game only).
...
import { setCoinVolcanoConfig } from "@/components/maxhigh/coin-volcano/runtimeConfig";
import { resolveCoinVolcanoSpin } from "@/components/maxhigh/coin-volcano/spinResolver";
import type { SpinScript } from "@/components/maxhigh/coin-volcano/types";

const GAME_NAME = "Coin Volcano";

let cachedConfig: { cfg: CoinVolcanoConfig; time: number } | null = null;

export function clearCoinVolcanoEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<CoinVolcanoConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeCoinVolcanoConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCoinVolcanoConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type CoinVolcanoSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function coinVolcanoPaidSpin(data: { bet: number }): Promise<CoinVolcanoSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setCoinVolcanoConfig(cfg);
  ...
  const script = resolveCoinVolcanoSpin({ totalBet: cost });
  const roundId = newId();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${COIN_VOLCANO_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${COIN_VOLCANO_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getCoinVolcanoEngineConfigPublic(): Promise<CoinVolcanoConfig> {
