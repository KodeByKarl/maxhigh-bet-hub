/**
 * Candy Peak — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { CANDY_PEAK_GAME_ID, normalizeCandyPeakConfig, remainingFreeSpinsAfterSpin } from "@/lib/candy-peak-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeCandyPeakConfig(null);
    return normalizeCandyPeakConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeCandyPeakConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setCandyPeakConfig } = await import("@/components/maxhigh/candy-peak/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/candy-peak/spinResolver"
    );
    setCandyPeakConfig(cfg);
...
export type CandyPeakSessionState = {
...
export type CandyPeakSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/candy-peak/spinResolver").resolveSpin>>;
  session: CandyPeakSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): CandyPeakSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CANDY_PEAK_GAME_ID),
...
export async function getCandyPeakOpenSession(): Promise<CandyPeakSessionState> {
...
export async function candyPeakPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<CandyPeakSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CANDY_PEAK_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CANDY_PEAK_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function candyPeakFreeSpin(data: {
  sessionId: string;
}): Promise<CandyPeakSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: CandyPeakSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${CANDY_PEAK_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function candyPeakBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: CandyPeakSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${CANDY_PEAK_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${CANDY_PEAK_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
