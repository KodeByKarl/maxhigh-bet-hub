/**
 * Wild Ace — server-authoritative spin settlement and round lifecycle.
...
import { setWildAceConfig } from "@/components/maxhigh/wild-ace/runtimeConfig";
import { resolveWildAceSpin } from "@/components/maxhigh/wild-ace/spinResolver";

const GAME_NAME = "Wild Ace";

let cachedConfig: { cfg: WildAceConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearWildAceEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<WildAceConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeWildAceConfig(JSON.parse(raw) as unknown) : normalizeWildAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeWildAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setWildAceConfig(cfg);
  const script = resolveWildAceSpin({
    bet: opts.bet,
...
export type WildAceSessionState = {
...
export type WildAceSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/wild-ace/spinResolver").resolveWildAceSpin>
  >;
  session: WildAceSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): WildAceSessionState {
...
export async function getWildAceOpenSession(): Promise<WildAceSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, WILD_ACE_GAME_ID),
...
export async function wildAcePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<WildAceSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${WILD_ACE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${WILD_ACE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function wildAceFreeSpin(data: {
  sessionId: string;
}): Promise<WildAceSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: WildAceSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${WILD_ACE_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function wildAceBuyFeature(data: {
  bet: number;
}): Promise<WildAceSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${WILD_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${WILD_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
