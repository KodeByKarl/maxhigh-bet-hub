/**
 * Neon Fruits — server-authoritative spin settlement + game-scoped progressive pool.
 * Integrates with the existing `jackpot` table using pool id `neon-fruits`.
 */

import { eq } from "drizzle-orm";
import {
  NEON_FRUITS_GAME_ID,
  NEON_FRUITS_JACKPOT_ID,
...
import { setNeonFruitsConfig } from "@/components/maxhigh/neon-fruits/runtimeConfig";
import { resolveNeonFruitsSpin } from "@/components/maxhigh/neon-fruits/spinResolver";
import type { RrReels, SpinScript } from "@/components/maxhigh/neon-fruits/types";

const GAME_NAME = "Neon Fruits";

let cachedConfig: { cfg: NeonFruitsConfig; time: number } | null = null;

export function clearNeonFruitsEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<NeonFruitsConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeNeonFruitsConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeNeonFruitsConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function ensureGameJackpotPool(
  ...
  cfg: NeonFruitsConfig,
): Promise<{ amount: number; enabled: boolean }> {
  const poolId = cfg.jackpot.poolId || NEON_FRUITS_JACKPOT_ID;
...
export async function neonFruitsPaidSpin(data: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const script = resolveNeonFruitsSpin({
      totalBet: cost,
      held: data.held,
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${NEON_FRUITS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (nonJpWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${NEON_FRUITS_GAME_ID} · ${GAME_NAME} · win ₱${nonJpWin.toFixed(2)}`,
      });
    }
    ...
    if (script.jackpot.triggered && script.jackpot.amount > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "jackpot",
        game: GAME_NAME,
        note: `${NEON_FRUITS_GAME_ID} · progressive jackpot ₱${script.jackpot.amount.toFixed(2)}`,
      });
...
export async function getNeonFruitsEngineConfigPublic(): Promise<NeonFruitsConfig> {
  return loadEngineConfig();
}

export async function getNeonFruitsJackpotPool(): Promise<{ amount: number; enabled: boolean }> {
  const cfg = await loadEngineConfig();
  const db = getDb();
