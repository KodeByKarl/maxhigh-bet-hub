/**
 * Boracay Bounce — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { BORACAY_BOUNCE_GAME_ID, normalizeBoracayBounceConfig } from "@/lib/boracay-bounce-config";
...
const GAME_NAME = "Boracay Bounce";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeBoracayBounceConfig(null);
    return normalizeBoracayBounceConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeBoracayBounceConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setBoracayBounceConfig } = await import("@/components/maxhigh/boracay-bounce/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/boracay-bounce/spinResolver"
    );
    setBoracayBounceConfig(cfg);
...
export type BoracayBounceSessionState = {
...
export type BoracayBounceSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/boracay-bounce/spinResolver").resolveSpin>>;
  session: BoracayBounceSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): BoracayBounceSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, BORACAY_BOUNCE_GAME_ID),
...
export async function getBoracayBounceOpenSession(): Promise<BoracayBounceSessionState> {
...
export async function boracayBouncePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<BoracayBounceSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${BORACAY_BOUNCE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${BORACAY_BOUNCE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function boracayBounceFreeSpin(data: {
  sessionId: string;
}): Promise<BoracayBounceSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: BoracayBounceSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${BORACAY_BOUNCE_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function boracayBounceBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: BoracayBounceSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${BORACAY_BOUNCE_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${BORACAY_BOUNCE_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
