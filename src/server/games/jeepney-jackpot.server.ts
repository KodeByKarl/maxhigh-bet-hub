/**
 * Jeepney Jackpot — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import {
  ...
  type JeepneyJackpotConfig,
} from "@/lib/jeepney-jackpot-config";
...
import { setJeepneyJackpotConfig } from "@/components/maxhigh/jeepney-jackpot/runtimeConfig";
import { resolveJeepneyJackpotSpin } from "@/components/maxhigh/jeepney-jackpot/spinResolver";

const GAME_NAME = "Jeepney Jackpot";

let cachedConfig: { cfg: JeepneyJackpotConfig; time: number } | null = null;

export function clearJeepneyJackpotEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<JeepneyJackpotConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeJeepneyJackpotConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeJeepneyJackpotConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: { bet: number; isFreeSpins: boolean }) {
  ...
  setJeepneyJackpotConfig(cfg);
  const script = resolveJeepneyJackpotSpin({
    totalBet: opts.bet,
...
export type JeepneyJackpotSessionState = {
...
export type JeepneyJackpotSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveJeepneyJackpotSpin>;
  session: JeepneyJackpotSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): JeepneyJackpotSessionState {
...
export async function getJeepneyJackpotOpenSession(): Promise<JeepneyJackpotSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, JEEPNEY_JACKPOT_GAME_ID),
...
export async function jeepneyJackpotPaidSpin(data: {
  bet: number;
}): Promise<JeepneyJackpotSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${JEEPNEY_JACKPOT_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${JEEPNEY_JACKPOT_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function jeepneyJackpotFreeSpin(data: {
  sessionId: string;
}): Promise<JeepneyJackpotSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: JeepneyJackpotSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${JEEPNEY_JACKPOT_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function jeepneyJackpotBuyFeature(data: {
  bet: number;
}): Promise<JeepneyJackpotSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${JEEPNEY_JACKPOT_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${JEEPNEY_JACKPOT_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
