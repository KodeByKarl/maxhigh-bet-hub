/**
 * Pharaoh Fire � server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type PharaohFireConfig,
} from "@/lib/pharaoh-fire-config";
...
import { setPharaohFireConfig } from "@/components/maxhigh/pharaoh-fire/runtimeConfig";
import { resolvePharaohFireSpin } from "@/components/maxhigh/pharaoh-fire/spinResolver";

const GAME_NAME = "Pharaoh Fire";

let cachedConfig: { cfg: PharaohFireConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearPharaohFireEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<PharaohFireConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizePharaohFireConfig(JSON.parse(raw) as unknown) : normalizePharaohFireConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizePharaohFireConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setPharaohFireConfig(cfg);
  const script = resolvePharaohFireSpin({
    bet: opts.bet,
...
export type PharaohFireSessionState = {
...
export type PharaohFireSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/pharaoh-fire/spinResolver").resolvePharaohFireSpin>
  >;
  session: PharaohFireSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): PharaohFireSessionState {
...
export async function getPharaohFireOpenSession(): Promise<PharaohFireSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, PHARAOH_FIRE_GAME_ID),
...
export async function pharaohFirePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<PharaohFireSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${PHARAOH_FIRE_GAME_ID} � ${GAME_NAME} � wager ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PHARAOH_FIRE_GAME_ID} � ${GAME_NAME} � win ?${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function pharaohFireFreeSpin(data: {
  sessionId: string;
}): Promise<PharaohFireSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: PharaohFireSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PHARAOH_FIRE_GAME_ID} � ${GAME_NAME} � Free spins feature final payout ?${accumulatedWin.toFixed(2)}`,
      });
...
export async function pharaohFireBuyFeature(data: {
  bet: number;
}): Promise<PharaohFireSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${PHARAOH_FIRE_GAME_ID} � ${GAME_NAME} � Feature Buy ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PHARAOH_FIRE_GAME_ID} � ${GAME_NAME} � Feature Buy initial spin win ?${script.totalWin.toFixed(2)}`,
      });
    }
