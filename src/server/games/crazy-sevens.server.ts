/**
 * Crazy Sevens — server-authoritative spin settlement + game-scoped progressive pool.
 * Integrates with the existing `jackpot` table using pool id `crazy-sevens`.
 */

import { eq } from "drizzle-orm";
import {
  CRAZY_SEVENS_GAME_ID,
  CRAZY_SEVENS_JACKPOT_ID,
...
import { setCrazySevensConfig } from "@/components/maxhigh/crazy-sevens/runtimeConfig";
import { resolveCrazySevensSpin } from "@/components/maxhigh/crazy-sevens/spinResolver";
import type { RrReels, SpinScript } from "@/components/maxhigh/crazy-sevens/types";

const GAME_NAME = "Crazy Sevens";

let cachedConfig: { cfg: CrazySevensConfig; time: number } | null = null;

export function clearCrazySevensEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<CrazySevensConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeCrazySevensConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCrazySevensConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function ensureGameJackpotPool(
  ...
  const poolId = cfg.jackpot.poolId || CRAZY_SEVENS_JACKPOT_ID;
...
export async function crazySevensPaidSpin(data: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const script = resolveCrazySevensSpin({
      totalBet: cost,
      held: data.held,
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CRAZY_SEVENS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (nonJpWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CRAZY_SEVENS_GAME_ID} · ${GAME_NAME} · win ₱${nonJpWin.toFixed(2)}`,
      });
    }
    ...
    if (script.jackpot.triggered && script.jackpot.amount > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "jackpot",
        game: GAME_NAME,
        note: `${CRAZY_SEVENS_GAME_ID} · progressive jackpot ₱${script.jackpot.amount.toFixed(2)}`,
      });
...
export async function getCrazySevensEngineConfigPublic(): Promise<CrazySevensConfig> {
  return loadEngineConfig();
}

export async function getCrazySevensJackpotPool(): Promise<{ amount: number; enabled: boolean }> {
  const cfg = await loadEngineConfig();
  const db = getDb();
