/**
 * Carabao Cash � server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type CarabaoCashConfig,
} from "@/lib/carabao-cash-config";
...
import { setCarabaoCashConfig } from "@/components/maxhigh/carabao-cash/runtimeConfig";
import { resolveCarabaoCashSpin } from "@/components/maxhigh/carabao-cash/spinResolver";

const GAME_NAME = "Carabao Cash";

let cachedConfig: { cfg: CarabaoCashConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearCarabaoCashEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<CarabaoCashConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeCarabaoCashConfig(JSON.parse(raw) as unknown) : normalizeCarabaoCashConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCarabaoCashConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setCarabaoCashConfig(cfg);
  const script = resolveCarabaoCashSpin({
    bet: opts.bet,
...
export type CarabaoCashSessionState = {
...
export type CarabaoCashSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/carabao-cash/spinResolver").resolveCarabaoCashSpin>
  >;
  session: CarabaoCashSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): CarabaoCashSessionState {
...
export async function getCarabaoCashOpenSession(): Promise<CarabaoCashSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CARABAO_CASH_GAME_ID),
...
export async function carabaoCashPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<CarabaoCashSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CARABAO_CASH_GAME_ID} � ${GAME_NAME} � wager ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CARABAO_CASH_GAME_ID} � ${GAME_NAME} � win ?${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function carabaoCashFreeSpin(data: {
  sessionId: string;
}): Promise<CarabaoCashSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: CarabaoCashSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CARABAO_CASH_GAME_ID} � ${GAME_NAME} � Free spins feature final payout ?${accumulatedWin.toFixed(2)}`,
      });
...
export async function carabaoCashBuyFeature(data: {
  bet: number;
}): Promise<CarabaoCashSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CARABAO_CASH_GAME_ID} � ${GAME_NAME} � Feature Buy ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CARABAO_CASH_GAME_ID} � ${GAME_NAME} � Feature Buy initial spin win ?${script.totalWin.toFixed(2)}`,
      });
    }
