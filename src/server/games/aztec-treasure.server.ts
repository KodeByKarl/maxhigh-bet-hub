...
import { AZTEC_TREASURE_GAME_ID, normalizeAztecTreasureConfig } from "@/lib/aztec-treasure-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeAztecTreasureConfig(null);
    return normalizeAztecTreasureConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeAztecTreasureConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setAztecTreasureConfig } = await import("@/components/maxhigh/aztec-treasure/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/aztec-treasure/spinResolver"
    );
    setAztecTreasureConfig(cfg);
...
export type AztecTreasureSessionState = {
...
export type AztecTreasureSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/aztec-treasure/spinResolver").resolveSpin>>;
  session: AztecTreasureSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): AztecTreasureSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, AZTEC_TREASURE_GAME_ID),
...
export async function getAztecTreasureOpenSession(): Promise<AztecTreasureSessionState> {
...
export async function aztecTreasurePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<AztecTreasureSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${AZTEC_TREASURE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${AZTEC_TREASURE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function aztecTreasureFreeSpin(data: {
  sessionId: string;
}): Promise<AztecTreasureSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: AztecTreasureSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${AZTEC_TREASURE_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function aztecTreasureBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: AztecTreasureSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${AZTEC_TREASURE_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${AZTEC_TREASURE_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
