/**
 * Mahjong Ways 3 — server-authoritative spin settlement and round lifecycle.
...
import { setMahjongWays3Config } from "@/components/maxhigh/mahjong-ways-3/runtimeConfig";
import { resolveMahjongWays3Spin } from "@/components/maxhigh/mahjong-ways-3/spinResolver";

const GAME_NAME = "Mahjong Ways 3";

let cachedConfig: { cfg: MahjongWays3Config; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearMahjongWays3EngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<MahjongWays3Config> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeMahjongWays3Config(JSON.parse(raw) as unknown) : normalizeMahjongWays3Config(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeMahjongWays3Config(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setMahjongWays3Config(cfg);
  const script = resolveMahjongWays3Spin({
    bet: opts.bet,
...
export type MahjongWays3SessionState = {
...
export type MahjongWays3SpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/mahjong-ways-3/spinResolver").resolveMahjongWays3Spin>
  >;
  session: MahjongWays3SessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): MahjongWays3SessionState {
...
export async function getMahjongWays3OpenSession(): Promise<MahjongWays3SessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, MAHJONG_WAYS_3_GAME_ID),
...
export async function mahjongWays3PaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<MahjongWays3SpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MAHJONG_WAYS_3_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAHJONG_WAYS_3_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function mahjongWays3FreeSpin(data: {
  sessionId: string;
}): Promise<MahjongWays3SpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: MahjongWays3SpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAHJONG_WAYS_3_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function mahjongWays3BuyFeature(data: {
  bet: number;
}): Promise<MahjongWays3SpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MAHJONG_WAYS_3_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MAHJONG_WAYS_3_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
