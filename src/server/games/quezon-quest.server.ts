/**
 * Quezon Quest — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type QuezonQuestConfig,
} from "@/lib/quezon-quest-config";
...
import { setQuezonQuestConfig } from "@/components/maxhigh/quezon-quest/runtimeConfig";
import { resolveQuezonQuestSpin } from "@/components/maxhigh/quezon-quest/spinResolver";

const GAME_NAME = "Quezon Quest";

let cachedConfig: { cfg: QuezonQuestConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearQuezonQuestEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<QuezonQuestConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeQuezonQuestConfig(JSON.parse(raw) as unknown) : normalizeQuezonQuestConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeQuezonQuestConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setQuezonQuestConfig(cfg);
  const script = resolveQuezonQuestSpin({
    bet: opts.bet,
...
export type QuezonQuestSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/quezon-quest/spinResolver").resolveQuezonQuestSpin>
  >;
  session: QuezonQuestSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): QuezonQuestSessionState {
...
export async function getQuezonQuestOpenSession(): Promise<QuezonQuestSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, QUEZON_QUEST_GAME_ID),
...
export async function quezonQuestPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<QuezonQuestSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${QUEZON_QUEST_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${QUEZON_QUEST_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function quezonQuestFreeSpin(data: {
  sessionId: string;
}): Promise<QuezonQuestSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: QuezonQuestSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${QUEZON_QUEST_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function quezonQuestBuyFeature(data: {
  bet: number;
}): Promise<QuezonQuestSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${QUEZON_QUEST_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${QUEZON_QUEST_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
