/**
 * Carabao Charge � server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type CarabaoChargeConfig,
} from "@/lib/carabao-charge-config";
...
import { setCarabaoChargeConfig } from "@/components/maxhigh/carabao-charge/runtimeConfig";
import { resolveCarabaoChargeSpin } from "@/components/maxhigh/carabao-charge/spinResolver";

const GAME_NAME = "Carabao Charge";

let cachedConfig: { cfg: CarabaoChargeConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearCarabaoChargeEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<CarabaoChargeConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeCarabaoChargeConfig(JSON.parse(raw) as unknown) : normalizeCarabaoChargeConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeCarabaoChargeConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setCarabaoChargeConfig(cfg);
  const script = resolveCarabaoChargeSpin({
    bet: opts.bet,
...
export type CarabaoChargeSessionState = {
...
export type CarabaoChargeSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/carabao-charge/spinResolver").resolveCarabaoChargeSpin>
  >;
  session: CarabaoChargeSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): CarabaoChargeSessionState {
...
export async function getCarabaoChargeOpenSession(): Promise<CarabaoChargeSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, CARABAO_CHARGE_GAME_ID),
...
export async function carabaoChargePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<CarabaoChargeSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CARABAO_CHARGE_GAME_ID} � ${GAME_NAME} � wager ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CARABAO_CHARGE_GAME_ID} � ${GAME_NAME} � win ?${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function carabaoChargeFreeSpin(data: {
  sessionId: string;
}): Promise<CarabaoChargeSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: CarabaoChargeSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CARABAO_CHARGE_GAME_ID} � ${GAME_NAME} � Free spins feature final payout ?${accumulatedWin.toFixed(2)}`,
      });
...
export async function carabaoChargeBuyFeature(data: {
  bet: number;
}): Promise<CarabaoChargeSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${CARABAO_CHARGE_GAME_ID} � ${GAME_NAME} � Feature Buy ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${CARABAO_CHARGE_GAME_ID} � ${GAME_NAME} � Feature Buy initial spin win ?${script.totalWin.toFixed(2)}`,
      });
    }
