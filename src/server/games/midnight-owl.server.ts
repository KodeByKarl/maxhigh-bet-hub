/**
 * Midnight Owl — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { MIDNIGHT_OWL_GAME_ID, normalizeMidnightOwlConfig, remainingFreeSpinsAfterSpin } from "@/lib/midnight-owl-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeMidnightOwlConfig(null);
    return normalizeMidnightOwlConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeMidnightOwlConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setMidnightOwlConfig } = await import("@/components/maxhigh/midnight-owl/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/midnight-owl/spinResolver"
    );
    setMidnightOwlConfig(cfg);
...
export type MidnightOwlSessionState = {
...
export type MidnightOwlSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/midnight-owl/spinResolver").resolveSpin>>;
  session: MidnightOwlSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): MidnightOwlSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, MIDNIGHT_OWL_GAME_ID),
...
export async function getMidnightOwlOpenSession(): Promise<MidnightOwlSessionState> {
...
export async function midnightOwlPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<MidnightOwlSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MIDNIGHT_OWL_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MIDNIGHT_OWL_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function midnightOwlFreeSpin(data: {
  sessionId: string;
}): Promise<MidnightOwlSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: MidnightOwlSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${MIDNIGHT_OWL_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function midnightOwlBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: MidnightOwlSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${MIDNIGHT_OWL_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${MIDNIGHT_OWL_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
