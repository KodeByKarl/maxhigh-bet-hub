/**
 * Turtle Tide — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/turtle-tide/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/turtle-tide/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/turtle-tide/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/turtle-tide/rng";
import { setTurtleTideConfig } from "@/components/maxhigh/games/turtle-tide/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/turtle-tide/hitResolver";
...
const GAME_NAME = "Turtle Tide";

let cachedConfig: { cfg: TurtleTideConfig; time: number } | null = null;
...
export function clearTurtleTideEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: TurtleTideConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function turtleTideSyncSpawns() {
  await assertGameEnabled();
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  setTurtleTideConfig(cfg);
  pruneFish(user.id);
...
async function loadEngineConfig(): Promise<TurtleTideConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeTurtleTideConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeTurtleTideConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getTurtleTideEngineConfigPublic(): Promise<TurtleTideConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Turtle Tide is currently disabled");
  }
}
...
export async function getTurtleTideOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, TURTLE_TIDE_GAME_ID),
...
export async function turtleTideEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: TurtleTideConfig) {
  const now = Date.now();
...
export async function turtleTideFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await turtleTideEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: TURTLE_TIDE_GAME_ID,
      note: `Turtle Tide shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Turtle Tide boss share"
          : shot.crateDrop
            ? `Turtle Tide crate (${shot.crateDrop.label})`
            : `Turtle Tide catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: TURTLE_TIDE_GAME_ID,
          note: "Turtle Tide boss contributor share",
        });
      }
    }
...
export async function turtleTideSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function turtleTideBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: TURTLE_TIDE_GAME_ID,
      note: `Turtle Tide power-up (${power.label})`,
    });
...
export async function turtleTideUseNetBomb(input: {
  weaponId: string;
...
export async function turtleTideUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function turtleTideBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
