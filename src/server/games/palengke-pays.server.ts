/**
 * Palengke Pays — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import {
  ...
  type PalengkePaysConfig,
} from "@/lib/palengke-pays-config";
...
import { setPalengkePaysConfig } from "@/components/maxhigh/palengke-pays/runtimeConfig";
import { resolvePalengkePaysSpin } from "@/components/maxhigh/palengke-pays/spinResolver";

const GAME_NAME = "Palengke Pays";

let cachedConfig: { cfg: PalengkePaysConfig; time: number } | null = null;

export function clearPalengkePaysEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<PalengkePaysConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizePalengkePaysConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizePalengkePaysConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: { bet: number; isFreeSpins: boolean }) {
  ...
  setPalengkePaysConfig(cfg);
  const script = resolvePalengkePaysSpin({
    totalBet: opts.bet,
...
export type PalengkePaysSessionState = {
...
export type PalengkePaysSpinResult = {
  balance: number;
  script: ReturnType<typeof resolvePalengkePaysSpin>;
  session: PalengkePaysSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): PalengkePaysSessionState {
...
export async function getPalengkePaysOpenSession(): Promise<PalengkePaysSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, PALENGKE_PAYS_GAME_ID),
...
export async function palengkePaysPaidSpin(data: {
  bet: number;
}): Promise<PalengkePaysSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${PALENGKE_PAYS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PALENGKE_PAYS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function palengkePaysFreeSpin(data: {
  sessionId: string;
}): Promise<PalengkePaysSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: PalengkePaysSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PALENGKE_PAYS_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function palengkePaysBuyFeature(data: {
  bet: number;
}): Promise<PalengkePaysSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${PALENGKE_PAYS_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${PALENGKE_PAYS_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
