/**
 * Safari Gold � server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type SafariGoldConfig,
} from "@/lib/safari-gold-config";
...
import { setSafariGoldConfig } from "@/components/maxhigh/safari-gold/runtimeConfig";
import { resolveSafariGoldSpin } from "@/components/maxhigh/safari-gold/spinResolver";

const GAME_NAME = "Safari Gold";

let cachedConfig: { cfg: SafariGoldConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearSafariGoldEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<SafariGoldConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeSafariGoldConfig(JSON.parse(raw) as unknown) : normalizeSafariGoldConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeSafariGoldConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setSafariGoldConfig(cfg);
  const script = resolveSafariGoldSpin({
    bet: opts.bet,
...
export type SafariGoldSessionState = {
...
export type SafariGoldSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/safari-gold/spinResolver").resolveSafariGoldSpin>
  >;
  session: SafariGoldSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): SafariGoldSessionState {
...
export async function getSafariGoldOpenSession(): Promise<SafariGoldSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, SAFARI_GOLD_GAME_ID),
...
export async function safariGoldPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<SafariGoldSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SAFARI_GOLD_GAME_ID} � ${GAME_NAME} � wager ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SAFARI_GOLD_GAME_ID} � ${GAME_NAME} � win ?${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function safariGoldFreeSpin(data: {
  sessionId: string;
}): Promise<SafariGoldSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: SafariGoldSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SAFARI_GOLD_GAME_ID} � ${GAME_NAME} � Free spins feature final payout ?${accumulatedWin.toFixed(2)}`,
      });
...
export async function safariGoldBuyFeature(data: {
  bet: number;
}): Promise<SafariGoldSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SAFARI_GOLD_GAME_ID} � ${GAME_NAME} � Feature Buy ?${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SAFARI_GOLD_GAME_ID} � ${GAME_NAME} � Feature Buy initial spin win ?${script.totalWin.toFixed(2)}`,
      });
    }
