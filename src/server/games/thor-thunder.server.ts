/**
 * Thor Thunder — server-authoritative spin settle.
 */
...
export { THOR_THUNDER_GAME_ID };
const GAME_NAME = "Thor Thunder";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeThorThunderConfig(null);
    return normalizeThorThunderConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeThorThunderConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setThorThunderConfig } = await import(
      "@/components/maxhigh/thor-thunder/runtimeConfig"
    );
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/thor-thunder/spinResolver"
    );
    setThorThunderConfig(cfg);
...
export type ThorThunderSessionState = {
...
export type ThorThunderSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/thor-thunder/spinResolver").resolveSpin>>;
  session: ThorThunderSessionState;
  fsPayout?: {
    amount: number;
    baseEarn: number;
    multiplier: number;
...
function sessionState(row: typeof playSessions.$inferSelect | null): ThorThunderSessionState {
...
export async function getThorThunderOpenSession(): Promise<ThorThunderSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, THOR_THUNDER_GAME_ID),
...
export async function thorThunderPaidSpin(data: { bet: number }): Promise<ThorThunderSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${THOR_THUNDER_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${THOR_THUNDER_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function thorThunderFreeSpin(data: {
  sessionId: string;
}): Promise<ThorThunderSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: ThorThunderSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${THOR_THUNDER_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function thorThunderBuyFeature(data: {
  bet: number;
}): Promise<{ balance: number; session: ThorThunderSessionState; awarded: number }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${THOR_THUNDER_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
