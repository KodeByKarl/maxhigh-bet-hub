/**
 * Halo-Halo Hits — server-authoritative spin settlement (base game only).
...
import { setHaloHaloHitsConfig } from "@/components/maxhigh/halo-halo-hits/runtimeConfig";
import { resolveHaloHaloHits } from "@/components/maxhigh/halo-halo-hits/spinResolver";
import type { SpinScript } from "@/components/maxhigh/halo-halo-hits/types";

const GAME_NAME = "Halo-Halo Hits";

let cachedConfig: { cfg: HaloHaloHitsConfig; time: number } | null = null;

export function clearHaloHaloHitsEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<HaloHaloHitsConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeHaloHaloHitsConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeHaloHaloHitsConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type HaloHaloHitsResult = {
  balance: number;
  script: SpinScript;
};

export async function haloHaloHitsPaidSpin(data: {
  bet: number;
  exMode?: boolean;
}): Promise<HaloHaloHitsResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setHaloHaloHitsConfig(cfg);
  ...
  const script = resolveHaloHaloHits({
    totalBet: cost,
    baseBet,
    exMode,
  });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${HALO_HALO_HITS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}${exMode ? " · EX" : ""}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${HALO_HALO_HITS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getHaloHaloHitsEngineConfigPublic(): Promise<HaloHaloHitsConfig> {
