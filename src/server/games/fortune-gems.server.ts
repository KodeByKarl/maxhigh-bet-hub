/**
 * Fortune Gems — server-authoritative spin settlement (base game only).
...
import { setFortuneGemsConfig } from "@/components/maxhigh/fortune-gems/runtimeConfig";
import { resolveFortuneGemsSpin } from "@/components/maxhigh/fortune-gems/spinResolver";
import type { SpinScript } from "@/components/maxhigh/fortune-gems/types";

const GAME_NAME = "Fortune Gems";

let cachedConfig: { cfg: FortuneGemsConfig; time: number } | null = null;

export function clearFortuneGemsEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<FortuneGemsConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeFortuneGemsConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeFortuneGemsConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type FortuneGemsSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function fortuneGemsPaidSpin(data: {
  bet: number;
  exMode?: boolean;
}): Promise<FortuneGemsSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setFortuneGemsConfig(cfg);
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${FORTUNE_GEMS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}${exMode ? " · EX" : ""}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${FORTUNE_GEMS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getFortuneGemsEngineConfigPublic(): Promise<FortuneGemsConfig> {
