/**
 * Sweet Rush — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { SWEET_RUSH_GAME_ID, normalizeSweetRushConfig } from "@/lib/sweet-rush-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeSweetRushConfig(null);
    return normalizeSweetRushConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeSweetRushConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setSweetRushConfig } = await import("@/components/maxhigh/sweet-rush/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/sweet-rush/spinResolver"
    );
    setSweetRushConfig(cfg);
...
export type SweetRushSessionState = {
...
export type SweetRushSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/sweet-rush/spinResolver").resolveSpin>>;
  session: SweetRushSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): SweetRushSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, SWEET_RUSH_GAME_ID),
...
export async function getSweetRushOpenSession(): Promise<SweetRushSessionState> {
...
export async function sweetRushPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<SweetRushSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SWEET_RUSH_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SWEET_RUSH_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function sweetRushFreeSpin(data: {
  sessionId: string;
}): Promise<SweetRushSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: SweetRushSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${SWEET_RUSH_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function sweetRushBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: SweetRushSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${SWEET_RUSH_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${SWEET_RUSH_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
