/**
 * Starlight Ace — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type StarlightAceConfig,
} from "@/lib/starlight-ace-config";
...
import { setStarlightAceConfig } from "@/components/maxhigh/starlight-ace/runtimeConfig";
import { resolveStarlightSpin } from "@/components/maxhigh/starlight-ace/spinResolver";

const GAME_NAME = "Starlight Ace";

let cachedConfig: { cfg: StarlightAceConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearStarlightAceEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<StarlightAceConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeStarlightAceConfig(JSON.parse(raw) as unknown) : normalizeStarlightAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeStarlightAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setStarlightAceConfig(cfg);
  const script = resolveStarlightSpin({
    bet: opts.bet,
...
export type StarlightAceSessionState = {
...
export type StarlightAceSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/starlight-ace/spinResolver").resolveStarlightSpin>
  >;
  session: StarlightAceSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): StarlightAceSessionState {
...
export async function getStarlightAceOpenSession(): Promise<StarlightAceSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, STARLIGHT_ACE_GAME_ID),
...
export async function starlightAcePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<StarlightAceSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${STARLIGHT_ACE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${STARLIGHT_ACE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function starlightAceFreeSpin(data: {
  sessionId: string;
}): Promise<StarlightAceSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: StarlightAceSpinResult["fsPayout"];
...
export async function starlightAceBuyFeature(data: {
  bet: number;
}): Promise<StarlightAceSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${STARLIGHT_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${STARLIGHT_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
