/**
 * Royal Ace — server-authoritative spin settlement and round lifecycle.
...
import { setRoyalAceConfig } from "@/components/maxhigh/royal-ace/runtimeConfig";
import { resolveRoyalAceSpin } from "@/components/maxhigh/royal-ace/spinResolver";

const GAME_NAME = "Royal Ace";

let cachedConfig: { cfg: RoyalAceConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearRoyalAceEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<RoyalAceConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeRoyalAceConfig(JSON.parse(raw) as unknown) : normalizeRoyalAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeRoyalAceConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  const script = resolveRoyalAceSpin({
    bet: opts.bet,
...
export type RoyalAceSessionState = {
...
export type RoyalAceSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/royal-ace/spinResolver").resolveRoyalAceSpin>
  >;
  session: RoyalAceSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): RoyalAceSessionState {
...
export async function getRoyalAceOpenSession(): Promise<RoyalAceSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, ROYAL_ACE_GAME_ID),
...
export async function royalAcePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<RoyalAceSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ROYAL_ACE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ROYAL_ACE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function royalAceFreeSpin(data: {
  sessionId: string;
}): Promise<RoyalAceSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: RoyalAceSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ROYAL_ACE_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function royalAceBuyFeature(data: {
  bet: number;
}): Promise<RoyalAceSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${ROYAL_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${ROYAL_ACE_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
