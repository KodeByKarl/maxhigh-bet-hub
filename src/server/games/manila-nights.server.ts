/**
 * Manila Nights — server-authoritative spin settlement and round lifecycle.
...
import {
  ...
  type ManilaNightsConfig,
} from "@/lib/manila-nights-config";
...
import { setManilaNightsConfig } from "@/components/maxhigh/manila-nights/runtimeConfig";
import { resolveManilaNightsSpin } from "@/components/maxhigh/manila-nights/spinResolver";

const GAME_NAME = "Manila Nights";

let cachedConfig: { cfg: ManilaNightsConfig; time: number } | null = null;

/** Invalidate after Superadmin saves engine config. */
export function clearManilaNightsEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<ManilaNightsConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = raw ? normalizeManilaNightsConfig(JSON.parse(raw) as unknown) : normalizeManilaNightsConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeManilaNightsConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
async function resolveOnServer(opts: {
  ...
  setManilaNightsConfig(cfg);
  const script = resolveManilaNightsSpin({
    bet: opts.bet,
...
export type ManilaNightsSpinResult = {
  balance: number;
  script: Awaited<
    ReturnType<typeof import("@/components/maxhigh/manila-nights/spinResolver").resolveManilaNightsSpin>
  >;
  session: ManilaNightsSessionState;
  fsPayout?: {
    amount: number;
    spinsPlayed: number;
  };
...
function sessionState(row: typeof playSessions.$inferSelect | null): ManilaNightsSessionState {
...
export async function getManilaNightsOpenSession(): Promise<ManilaNightsSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, MANILA_NIGHTS_GAME_ID),
...
export async function manilaNightsPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<ManilaNightsSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MANILA_NIGHTS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MANILA_NIGHTS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function manilaNightsFreeSpin(data: {
  sessionId: string;
}): Promise<ManilaNightsSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: ManilaNightsSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MANILA_NIGHTS_GAME_ID} · ${GAME_NAME} · Free spins feature final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function manilaNightsBuyFeature(data: {
  bet: number;
}): Promise<ManilaNightsSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MANILA_NIGHTS_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MANILA_NIGHTS_GAME_ID} · ${GAME_NAME} · Feature Buy initial spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
