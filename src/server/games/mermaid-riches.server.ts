/**
 * Mermaid Riches — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { MERMAID_RICHES_GAME_ID, normalizeMermaidRichesConfig } from "@/lib/mermaid-riches-config";
...
const GAME_NAME = "Mermaid Riches";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeMermaidRichesConfig(null);
    return normalizeMermaidRichesConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeMermaidRichesConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setMermaidRichesConfig } = await import("@/components/maxhigh/mermaid-riches/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/mermaid-riches/spinResolver"
    );
    setMermaidRichesConfig(cfg);
...
export type MermaidRichesSessionState = {
...
export type MermaidRichesSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/mermaid-riches/spinResolver").resolveSpin>>;
  session: MermaidRichesSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): MermaidRichesSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, MERMAID_RICHES_GAME_ID),
...
export async function getMermaidRichesOpenSession(): Promise<MermaidRichesSessionState> {
...
export async function mermaidRichesPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<MermaidRichesSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${MERMAID_RICHES_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${MERMAID_RICHES_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function mermaidRichesFreeSpin(data: {
  sessionId: string;
}): Promise<MermaidRichesSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: MermaidRichesSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${MERMAID_RICHES_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function mermaidRichesBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
}): Promise<{ balance: number; session: MermaidRichesSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${MERMAID_RICHES_GAME_ID} · ${GAME_NAME} · buy super feature ₱${cost.toFixed(2)}`
          : `${MERMAID_RICHES_GAME_ID} · ${GAME_NAME} · buy feature ₱${cost.toFixed(2)}`,
    });
