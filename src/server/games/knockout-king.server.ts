/**
 * Knockout King — server-authoritative spin settlement (base game only).
...
import { setKnockoutKingConfig } from "@/components/maxhigh/knockout-king/runtimeConfig";
import { resolveKnockoutKingSpin } from "@/components/maxhigh/knockout-king/spinResolver";
import type { SpinScript } from "@/components/maxhigh/knockout-king/types";

const GAME_NAME = "Knockout King";

let cachedConfig: { cfg: KnockoutKingConfig; time: number } | null = null;

export function clearKnockoutKingEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<KnockoutKingConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeKnockoutKingConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeKnockoutKingConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type KnockoutKingSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function knockoutKingPaidSpin(data: { bet: number }): Promise<KnockoutKingSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setKnockoutKingConfig(cfg);
  ...
  const script = resolveKnockoutKingSpin({ totalBet: cost });
  const roundId = newId();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${KNOCKOUT_KING_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${KNOCKOUT_KING_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getKnockoutKingEngineConfigPublic(): Promise<KnockoutKingConfig> {
