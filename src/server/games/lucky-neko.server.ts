/**
 * Lucky Neko — server-authoritative spin settlement (base game only).
...
import { setLuckyNekoConfig } from "@/components/maxhigh/lucky-neko/runtimeConfig";
import { resolveLuckyNekoSpin } from "@/components/maxhigh/lucky-neko/spinResolver";
import type { SpinScript } from "@/components/maxhigh/lucky-neko/types";

const GAME_NAME = "Lucky Neko";

let cachedConfig: { cfg: LuckyNekoConfig; time: number } | null = null;

export function clearLuckyNekoEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<LuckyNekoConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeLuckyNekoConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeLuckyNekoConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export type LuckyNekoSpinResult = {
  balance: number;
  script: SpinScript;
};

export async function luckyNekoPaidSpin(data: {
  bet: number;
  exMode?: boolean;
}): Promise<LuckyNekoSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");

  const cfg = await loadEngineConfig();
  setLuckyNekoConfig(cfg);
  ...
  const script = resolveLuckyNekoSpin({
    totalBet: cost,
    baseBet,
    exMode,
  });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${LUCKY_NEKO_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}${exMode ? " · EX" : ""}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${LUCKY_NEKO_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function getLuckyNekoEngineConfigPublic(): Promise<LuckyNekoConfig> {
