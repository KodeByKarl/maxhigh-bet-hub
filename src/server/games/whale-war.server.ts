/**
 * Whale War — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/whale-war/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/whale-war/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/whale-war/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/whale-war/rng";
import { setWhaleWarConfig } from "@/components/maxhigh/games/whale-war/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/whale-war/hitResolver";
...
const GAME_NAME = "Whale War";

let cachedConfig: { cfg: WhaleWarConfig; time: number } | null = null;
...
export function clearWhaleWarEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: WhaleWarConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function whaleWarSyncSpawns() {
  await assertGameEnabled();
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  setWhaleWarConfig(cfg);
  pruneFish(user.id);
...
async function loadEngineConfig(): Promise<WhaleWarConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeWhaleWarConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeWhaleWarConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getWhaleWarEngineConfigPublic(): Promise<WhaleWarConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Whale War is currently disabled");
  }
}
...
export async function getWhaleWarOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, WHALE_WAR_GAME_ID),
...
export async function whaleWarEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: WhaleWarConfig) {
  const now = Date.now();
...
export async function whaleWarFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await whaleWarEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: WHALE_WAR_GAME_ID,
      note: `Whale War shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Whale War boss share"
          : shot.crateDrop
            ? `Whale War crate (${shot.crateDrop.label})`
            : `Whale War catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: WHALE_WAR_GAME_ID,
          note: "Whale War boss contributor share",
        });
      }
    }
...
export async function whaleWarSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function whaleWarBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: WHALE_WAR_GAME_ID,
      note: `Whale War power-up (${power.label})`,
    });
...
export async function whaleWarUseNetBomb(input: {
  weaponId: string;
...
export async function whaleWarUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function whaleWarBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
