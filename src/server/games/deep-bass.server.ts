/**
 * Deep Bass — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/deep-bass/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/deep-bass/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/deep-bass/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/deep-bass/rng";
import { setDeepBassConfig } from "@/components/maxhigh/games/deep-bass/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/deep-bass/hitResolver";
...
const GAME_NAME = "Deep Bass";

let cachedConfig: { cfg: DeepBassConfig; time: number } | null = null;
...
export function clearDeepBassEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: DeepBassConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function deepBassSyncSpawns() {
...
async function loadEngineConfig(): Promise<DeepBassConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeDeepBassConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeDeepBassConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getDeepBassEngineConfigPublic(): Promise<DeepBassConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Deep Bass is currently disabled");
  }
}
...
export async function getDeepBassOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, DEEP_BASS_GAME_ID),
...
export async function deepBassEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: DeepBassConfig) {
  const now = Date.now();
...
export async function deepBassFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await deepBassEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: DEEP_BASS_GAME_ID,
      note: `Deep Bass shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Deep Bass boss share"
          : shot.crateDrop
            ? `Deep Bass crate (${shot.crateDrop.label})`
            : `Deep Bass catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: DEEP_BASS_GAME_ID,
          note: "Deep Bass boss contributor share",
        });
      }
    }
...
export async function deepBassSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function deepBassBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: DEEP_BASS_GAME_ID,
      note: `Deep Bass power-up (${power.label})`,
    });
...
export async function deepBassUseNetBomb(input: {
  weaponId: string;
...
export async function deepBassUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function deepBassBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
