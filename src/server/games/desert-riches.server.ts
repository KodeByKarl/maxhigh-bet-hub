/**
 * Desert Riches — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import {
  ...
  type DesertRichesConfig,
} from "@/lib/desert-riches-config";
...
import { setDesertRichesConfig } from "@/components/maxhigh/desert-riches/runtimeConfig";
import { resolveFrontierSpin } from "@/components/maxhigh/desert-riches/spinResolver";

const GAME_NAME = "Desert Riches";

let cachedConfig: { cfg: DesertRichesConfig; time: number } | null = null;

export function clearDesertRichesEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<DesertRichesConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeDesertRichesConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeDesertRichesConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
export type DesertRichesSessionState = {
...
export type DesertRichesSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveFrontierSpin>;
  session: DesertRichesSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): DesertRichesSessionState {
...
export async function getDesertRichesOpenSession(): Promise<DesertRichesSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, DESERT_RICHES_GAME_ID),
...
export async function desertRichesPaidSpin(data: {
  bet: number;
}): Promise<DesertRichesSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${DESERT_RICHES_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DESERT_RICHES_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function desertRichesFreeSpin(data: {
  sessionId: string;
}): Promise<DesertRichesSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: DesertRichesSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DESERT_RICHES_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function desertRichesBuyFeature(data: {
  bet: number;
}): Promise<DesertRichesSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${DESERT_RICHES_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${DESERT_RICHES_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
