/**
 * Phoenix Fisher — server-authoritative shot settlement + boss shared pool.
...
import {
  ...
  splitBossPayout,
  type BossPoolState,
} from "@/components/maxhigh/games/phoenix-fisher/bossEngine";
import { resolveNetBomb, resolveShot } from "@/components/maxhigh/games/phoenix-fisher/hitResolver";
import { createSpawnedFish } from "@/components/maxhigh/games/phoenix-fisher/fishSpawner";
import { createRng, newShotSeed } from "@/components/maxhigh/games/phoenix-fisher/rng";
import { setPhoenixFisherConfig } from "@/components/maxhigh/games/phoenix-fisher/runtimeConfig";
import type { ShotTargetSnapshot } from "@/components/maxhigh/games/phoenix-fisher/hitResolver";
...
const GAME_NAME = "Phoenix Fisher";

let cachedConfig: { cfg: PhoenixFisherConfig; time: number } | null = null;
...
export function clearPhoenixFisherEngineCache() {
  cachedConfig = null;
}
...
function spawnOneFish(cfg: PhoenixFisherConfig, userId: string): ServerFish {
  const now = Date.now();
...
export async function phoenixFisherSyncSpawns() {
  await assertGameEnabled();
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  setPhoenixFisherConfig(cfg);
  pruneFish(user.id);
...
async function loadEngineConfig(): Promise<PhoenixFisherConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizePhoenixFisherConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizePhoenixFisherConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getPhoenixFisherEngineConfigPublic(): Promise<PhoenixFisherConfig> {
...
async function assertGameEnabled() {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Phoenix Fisher is currently disabled");
  }
}
...
export async function getPhoenixFisherOpenSession() {
  const user = await requireUser();
  const db = getDb();
  const rows = await db
    .select()
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, PHOENIX_FISHER_GAME_ID),
...
export async function phoenixFisherEnsureSession(input?: { weaponId?: string }) {
...
function maybeSpawnBoss(cfg: PhoenixFisherConfig) {
  const now = Date.now();
...
export async function phoenixFisherFireShot(input: {
  weaponId: string;
  ...
  if (!session) {
    await phoenixFisherEnsureSession({ weaponId: input.weaponId });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: PHOENIX_FISHER_GAME_ID,
      note: `Phoenix Fisher shot (${weapon.label})`,
    });
    ...
    if (credit > 0) {
      ({ balance } = await writeLedgerDelta(tx, {
        ...
        note: shot.isBossKill
          ? "Phoenix Fisher boss share"
          : shot.crateDrop
            ? `Phoenix Fisher crate (${shot.crateDrop.label})`
            : `Phoenix Fisher catch (${target.tierId})`,
      }));
    }
    ...
    if (bossShares) {
      for (const share of bossShares) {
        ...
        await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: PHOENIX_FISHER_GAME_ID,
          note: "Phoenix Fisher boss contributor share",
        });
      }
    }
...
export async function phoenixFisherSetWeapon(input: { weaponId: string }) {
  await assertGameEnabled();
  const user = await requireUser();
  if (!isWeaponTierId(input.weaponId)) throw new Error("Invalid weapon");
  const cfg = await loadEngineConfig();
...
export async function phoenixFisherBuyPowerUp(input: { powerUpId: string }) {
  await assertNotInMaintenanceForBets();
  await assertGameEnabled();
  const user = await requireUser();
  ...
  let session = await loadOpenSessionRow(user.id);
  if (!session) {
    await phoenixFisherEnsureSession({});
    session = await loadOpenSessionRow(user.id);
  }
  if (!session) throw new Error("Could not open play session");
  const feat = parseFeature(session.featureState);
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const { balance } = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: PHOENIX_FISHER_GAME_ID,
      note: `Phoenix Fisher power-up (${power.label})`,
    });
...
export async function phoenixFisherUseNetBomb(input: {
  weaponId: string;
...
export async function phoenixFisherUseFreezeLure() {
  const user = await requireUser();
  const cfg = await loadEngineConfig();
  const session = await loadOpenSessionRow(user.id);
  if (!session) throw new Error("No open session");
...
export async function phoenixFisherBossStatus() {
  await requireUser();
  const cfg = await loadEngineConfig();
  maybeSpawnBoss(cfg);
