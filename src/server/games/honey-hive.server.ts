/**
 * Honey Hive — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { HONEY_HIVE_GAME_ID, normalizeHoneyHiveConfig, remainingFreeSpinsAfterSpin } from "@/lib/honey-hive-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeHoneyHiveConfig(null);
    return normalizeHoneyHiveConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeHoneyHiveConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setHoneyHiveConfig } = await import("@/components/maxhigh/honey-hive/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/honey-hive/spinResolver"
    );
    setHoneyHiveConfig(cfg);
...
export type HoneyHiveSessionState = {
...
export type HoneyHiveSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/honey-hive/spinResolver").resolveSpin>>;
  session: HoneyHiveSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): HoneyHiveSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, HONEY_HIVE_GAME_ID),
...
export async function getHoneyHiveOpenSession(): Promise<HoneyHiveSessionState> {
...
export async function honeyHivePaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<HoneyHiveSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${HONEY_HIVE_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${HONEY_HIVE_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function honeyHiveFreeSpin(data: {
  sessionId: string;
}): Promise<HoneyHiveSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: HoneyHiveSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${HONEY_HIVE_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function honeyHiveBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: HoneyHiveSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${HONEY_HIVE_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${HONEY_HIVE_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
