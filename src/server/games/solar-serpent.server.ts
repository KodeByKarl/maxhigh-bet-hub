/**
 * Solar Serpent — server-authoritative spin settle.
 * Client only receives scripts + balances; it cannot choose win amounts.
 */
import { and, eq } from "drizzle-orm";
import { SOLAR_SERPENT_GAME_ID, normalizeSolarSerpentConfig, remainingFreeSpinsAfterSpin } from "@/lib/solar-serpent-config";
...
async function loadEngineConfig() {
  ...
  try {
    ...
    if (!raw) return normalizeSolarSerpentConfig(null);
    return normalizeSolarSerpentConfig(JSON.parse(raw) as unknown);
  } catch {
    return normalizeSolarSerpentConfig(null);
  }
}
...
async function resolveOnServer(opts: {
  ...
  return withEngineExclusive(async () => {
    const { setSolarSerpentConfig } = await import("@/components/maxhigh/solar-serpent/runtimeConfig");
    const { resolveSpin, finalizeFreeSpinTotal } = await import(
      "@/components/maxhigh/solar-serpent/spinResolver"
    );
    setSolarSerpentConfig(cfg);
...
export type SolarSerpentSessionState = {
...
export type SolarSerpentSpinResult = {
  balance: number;
  script: Awaited<ReturnType<typeof import("@/components/maxhigh/solar-serpent/spinResolver").resolveSpin>>;
  session: SolarSerpentSessionState;
...
function sessionState(row: typeof playSessions.$inferSelect | null): SolarSerpentSessionState {
  ...
  const open = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, SOLAR_SERPENT_GAME_ID),
...
export async function getSolarSerpentOpenSession(): Promise<SolarSerpentSessionState> {
...
export async function solarSerpentPaidSpin(data: {
  bet: number;
  ante: boolean;
}): Promise<SolarSerpentSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${SOLAR_SERPENT_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${SOLAR_SERPENT_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function solarSerpentFreeSpin(data: {
  sessionId: string;
}): Promise<SolarSerpentSpinResult> {
  const user = await requireUser();
  const db = getDb();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: SolarSerpentSpinResult["fsPayout"];
    ...
    if (nextLeft === 0) {
      ...
      if (final > 0) {
        const winRes = await writeLedgerDelta(tx, {
          ...
          type: "win",
          game: GAME_NAME,
          note: `${SOLAR_SERPENT_GAME_ID} · ${GAME_NAME} · free spins win ₱${final.toFixed(2)}`,
        });
...
export async function solarSerpentBuyFeature(data: {
  bet: number;
  mode: "normal" | "super";
  /** Bonus units to purchase. Price = bet × unitMult; Total = Price × quantity. */
  quantity: number;
}): Promise<{ balance: number; session: SolarSerpentSessionState }> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const betRes = await writeLedgerDelta(tx, {
      ...
      note:
        data.mode === "super"
          ? `${SOLAR_SERPENT_GAME_ID} · ${GAME_NAME} · buy super feature ×${quantity} ₱${cost.toFixed(2)}`
          : `${SOLAR_SERPENT_GAME_ID} · ${GAME_NAME} · buy feature ×${quantity} ₱${cost.toFixed(2)}`,
    });
