/**
 * Color Game — server-authoritative one-shot deal + settlement.
 */
...
import { resolveColorGameDeal } from "@/components/maxhigh/games/color-game/resolver";
import { setColorGameConfig } from "@/components/maxhigh/games/color-game/runtimeConfig";
import type { PublicDealScript } from "@/components/maxhigh/games/color-game/types";
...
const GAME_NAME = "Color Game";

let cachedConfig: { cfg: ColorGameConfig; time: number } | null = null;

export function clearColorGameEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<ColorGameConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeColorGameConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeColorGameConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}

export async function getColorGameEngineConfigPublic(): Promise<ColorGameConfig> {
  return loadEngineConfig();
}

export type ColorGameDealResult = {
  balance: number;
  script: PublicDealScript;
};

export async function colorGamePaidDeal(input: {
  bets: Partial<Record<ColorSpotId, number>>;
}): Promise<ColorGameDealResult> {
  ...
  if (controlRows[0]?.enabled === "no") {
    throw new Error("Color Game is currently disabled");
  }

  const cfg = await loadEngineConfig();
  setColorGameConfig(cfg);
  ...
  const resolved = resolveColorGameDeal({ bets, cfg });
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${COLOR_GAME_GAME_ID} · ${GAME_NAME} · ₱${totalWager.toFixed(2)}`,
    });
    ...
    if (resolved.immediateCredit > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${COLOR_GAME_GAME_ID} · ${GAME_NAME} · ${resolved.winningColor} ₱${resolved.immediateCredit.toFixed(2)}`,
      });
    }
