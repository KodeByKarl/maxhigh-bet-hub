/**
 * Outlaw Coins — server-authoritative spin settlement (Hold & Win + Free Spins).
 */
...
import {
  ...
  type OutlawCoinsConfig,
} from "@/lib/outlaw-coins-config";
...
import { setOutlawCoinsConfig } from "@/components/maxhigh/outlaw-coins/runtimeConfig";
import { resolveFrontierSpin } from "@/components/maxhigh/outlaw-coins/spinResolver";

const GAME_NAME = "Outlaw Coins";

let cachedConfig: { cfg: OutlawCoinsConfig; time: number } | null = null;

export function clearOutlawCoinsEngineCache() {
  cachedConfig = null;
}

async function loadEngineConfig(): Promise<OutlawCoinsConfig> {
  const now = Date.now();
  ...
  try {
    ...
    const cfg = normalizeOutlawCoinsConfig(raw ? (JSON.parse(raw) as unknown) : null);
    cachedConfig = { cfg, time: now };
    return cfg;
  } catch {
    const cfg = normalizeOutlawCoinsConfig(null);
    cachedConfig = { cfg, time: now };
    return cfg;
  }
}
...
export type OutlawCoinsSessionState = {
...
export type OutlawCoinsSpinResult = {
  balance: number;
  script: ReturnType<typeof resolveFrontierSpin>;
  session: OutlawCoinsSessionState;
  fsPayout?: { amount: number; spinsPlayed: number };
};

function sessionState(row: typeof playSessions.$inferSelect | null): OutlawCoinsSessionState {
...
export async function getOutlawCoinsOpenSession(): Promise<OutlawCoinsSessionState> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(playSessions.gameId, OUTLAW_COINS_GAME_ID),
...
export async function outlawCoinsPaidSpin(data: {
  bet: number;
}): Promise<OutlawCoinsSpinResult> {
  const user = await requireUser();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${OUTLAW_COINS_GAME_ID} · ${GAME_NAME} · wager ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${OUTLAW_COINS_GAME_ID} · ${GAME_NAME} · win ₱${script.totalWin.toFixed(2)}`,
      });
    }
...
export async function outlawCoinsFreeSpin(data: {
  sessionId: string;
}): Promise<OutlawCoinsSpinResult> {
  const user = await requireUser();
  await assertNotInMaintenanceForBets();
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let fsPayout: OutlawCoinsSpinResult["fsPayout"];
    ...
    if (isFinished && accumulatedWin > 0) {
      const ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${OUTLAW_COINS_GAME_ID} · ${GAME_NAME} · Free spins final payout ₱${accumulatedWin.toFixed(2)}`,
      });
...
export async function outlawCoinsBuyFeature(data: {
  bet: number;
}): Promise<OutlawCoinsSpinResult> {
  const user = await requireUser();
  if (!Number.isFinite(data.bet) || data.bet <= 0) throw new Error("Invalid bet amount");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    let ledger = await writeLedgerDelta(tx, {
      ...
      type: "bet",
      game: GAME_NAME,
      note: `${OUTLAW_COINS_GAME_ID} · ${GAME_NAME} · Feature Buy ₱${cost.toFixed(2)}`,
    });
    ...
    if (script.totalWin > 0) {
      ledger = await writeLedgerDelta(tx, {
        ...
        type: "win",
        game: GAME_NAME,
        note: `${OUTLAW_COINS_GAME_ID} · Feature Buy spin win ₱${script.totalWin.toFixed(2)}`,
      });
    }
