...
import { frostFoxAudio } from "./frost-fox/audio";
import {
  frostFoxBuyFeatureFn,
  frostFoxFreeSpinFn,
  frostFoxSpinFn,
  getFrostFoxSessionFn,
} from "@/functions/api";
import { ANIM } from "./frost-fox/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./frost-fox/playbackHelpers";
import { BetSelectModal } from "./frost-fox/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./frost-fox/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./frost-fox/paytable";
import { setFrostFoxConfig } from "./frost-fox/runtimeConfig";
import type { BoardCell, SpinScript } from "./frost-fox/types";
import { CELLS } from "./frost-fox/types";
import { WinCelebration } from "./frost-fox/WinCelebration";
import { FreeSpinsCongrats } from "./frost-fox/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./frost-fox/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./frost-fox/BuyFeatureModal";
import { PaytableModal } from "./frost-fox/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./frost-fox/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./frost-fox/ReelGrid";
import { frostFoxTheme } from "./frost-fox/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./frost-fox/ReelCell";
import { getFrostFoxEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  frostFoxAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    frostFoxTheme.assets.backdrop,
    frostFoxTheme.assets.loadingBg,
    ...Object.values(frostFoxTheme.assets.icons),
  ];
...
export function FrostFoxSlot({
  ...
  const [muted, setMuted] = useState(() => frostFoxAudio.isMuted);
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(15,23,42,0.96), rgba(12,74,110,0.96))",
                color: "#C7D2FE",
              }}
            >
        ...
        () => (
          <div
            ...
            style={{
              ...
              background: isWin
                ? "linear-gradient(135deg, #0284C7 0%, #0C4A6E 55%, #0F172A 100%)"
                : "linear-gradient(135deg, rgba(15,23,42,0.96), rgba(12,74,110,0.96))",
              color: isWin ? "#E0F2FE" : "#C7D2FE",
            }}
          >
            <div
              className="text-[10px] font-black uppercase tracking-[0.18em]"
              style={{
                color: isWin ? "rgba(224,242,254,0.85)" : "rgba(199,210,254,0.85)",
              }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      frostFoxAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        frostFoxAudio.stopSpinLoop();
        return;
      }
      frostFoxAudio.stopSpinLoop();
      frostFoxAudio.playReelStop(5);
      frostFoxAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === FROST_FOX_GAME_ID || gameId === "frost-fox") {
      void getFrostFoxEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setFrostFoxConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getFrostFoxSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[FrostFox] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== FROST_FOX_GAME_ID && gameId !== "frost-fox") return;
    try {
      const cfg = await getFrostFoxEngineConfigFn();
      if (mountedRef.current) setFrostFoxConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    frostFoxAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    frostFoxAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        frostFoxAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getFrostFoxSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof frostFoxSpinFn>>;
        if (isFree) {
          ...
          settled = await frostFoxFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await frostFoxSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[FrostFox] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await frostFoxBuyFeatureFn({
        ...
        <div className="mx-auto flex h-full min-h-0 w-full max-w-[840px] flex-col">
          ...
          <div className="relative flex min-h-0 w-full flex-1 flex-col">
            ...
            >
              ...
              {/* Ice honeycomb — 4-5-6-5-4 hex pillars */}
