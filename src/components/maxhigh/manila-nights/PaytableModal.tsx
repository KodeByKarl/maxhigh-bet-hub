...
import { SYMBOL_NAMES, type ManilaNightsSymKind } from "@/lib/manila-nights-config";
import { getManilaNightsConfig } from "./runtimeConfig";
import { ManilaNightsIcon } from "./ManilaNightsIcon";
...
const ORDER: ManilaNightsSymKind[] = [
  "comet",
  "princess",
  "castle",
  "star",
...
export function PaytableModal({ open, onClose }: PaytableModalProps) {
  if (!open) return null;
  const cfg = getManilaNightsConfig();
  const ways =
      ...
      >
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-black text-[#F5D76E]">Manila Nights · Info</h2>
            <button
              type="button"
              onClick={onClose}
