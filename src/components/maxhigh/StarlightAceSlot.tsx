...
import { getStarlightAceEngineConfigFn } from "@/functions/superadmin";
import {
  getStarlightAceSessionFn,
  starlightAceBuyFeatureFn,
  starlightAceFreeSpinFn,
  starlightAceSpinFn,
} from "@/functions/api";
import { ANIM } from "./starlight-ace/animationConfig";
import { starlightAudio } from "./starlight-ace/audio";
import { StarlightSidePanel } from "./starlight-ace/StarlightSidePanel";
import { ReelCell, type ReelPhase } from "./starlight-ace/ReelCell";
import { TILE_IMAGE_MAP } from "./starlight-ace/StarlightIcon";
import { PaytableModal } from "./starlight-ace/PaytableModal";
import { getStarlightAceConfig, setStarlightAceConfig } from "./starlight-ace/runtimeConfig";
import type { BoardCell, SpinScript } from "./starlight-ace/types";

type StarlightAceSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_STARLIGHT_ACE_CONFIG.symbols;
...
export function StarlightAceSlot({ onBalanceUpdate }: StarlightAceSlotProps) {
  const { user } = useAuth();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getStarlightAceEngineConfigFn();
      if (!mountedRef.current) return;
      setStarlightAceConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await starlightAceFreeSpinFn({ data: { sessionId } });
        ...
        const res = await starlightAceSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await starlightAceBuyFeatureFn({ data: { bet } });
