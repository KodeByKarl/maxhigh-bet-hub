...
import { getAztecTreasureConfig, getRuntimeSymbols } from "./runtimeConfig";
...
export function payForCount(sym: CellSym, count: number): number {
  const min = getAztecTreasureConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getAztecTreasureConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getAztecTreasureConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getAztecTreasureConfig().superBuyFeatureMult;
}
export function getAnteMult() {
  return getAztecTreasureConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getAztecTreasureConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getAztecTreasureConfig().freeSpinsRetrigger;
}
