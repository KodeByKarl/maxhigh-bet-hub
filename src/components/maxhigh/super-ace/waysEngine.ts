 ...
 * Wins = 3+ matching symbols on consecutive reels from reel 1;
 * either Joker wild substitutes (not for scatter).
 */
export function evaluateWays(
  ...
  const config = getSuperAceConfig();
  ...
  for (const symConfig of config.symbols) {
    ...
    if (consecutiveReels >= minConsecutive) {
      ...
      const totalWinAmount = +(bet * basePayMultiplier * waysCount).toFixed(4);
  ...
  if (payScatter && scatterCount >= 3) {
    ...
    if (scatterConfig) {
      ...
      const scatterWin = +(bet * scatterPayMult).toFixed(4);
      if (scatterWin > 0) {
        waysWins.push({
