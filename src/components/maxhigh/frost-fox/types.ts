/** Frost Fox — ice honeycomb pillars (cluster tumble). Distinct from diamond & 5×5. */
...
/** Glacier pillars — taller mid than Pug Den diamond (3-4-5-4-3). */
export const REEL_HEIGHTS = [4, 5, 6, 5, 4] as const;
export const COLS = REEL_HEIGHTS.length; // 5
export const ROWS = Math.max(...REEL_HEIGHTS); // 6
export const TOP_COLS = 0;
export const MAIN_CELLS = REEL_HEIGHTS.reduce((a, h) => a + h, 0); // 24
