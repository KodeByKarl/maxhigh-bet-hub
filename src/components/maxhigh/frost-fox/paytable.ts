...
import { getFrostFoxConfig, getRuntimeSymbols } from "./runtimeConfig";
import { frostFoxTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...frostFoxTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getFrostFoxConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getFrostFoxConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getFrostFoxConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getFrostFoxConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getFrostFoxConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getFrostFoxConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getFrostFoxConfig().freeSpinsRetrigger;
}
