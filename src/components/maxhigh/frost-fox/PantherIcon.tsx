...
import { clampBombMult } from "@/lib/frost-fox-config";
...
/** Full glyph visible — contain only, never cover-crop. */
export const PantherIcon = memo(function PantherIcon({
  ...
  const src = ICON_SRC[kind] || "/images/symbols/frost-fox/10-cell.png?v=12";
  ...
  return (
    ...
    >
      {winLit && (
        <div
          aria-hidden
          className="pointer-events-none absolute inset-[8%] z-0 rounded-xl"
          style={{
            background:
      ...
      {isScatter && (
        <span
          className="absolute bottom-0.5 left-1/2 z-[30] -translate-x-1/2 rounded-full border px-1.5 py-0.5 text-[7px] font-black uppercase shadow-sm whitespace-nowrap sm:text-[8px]"
          style={{
