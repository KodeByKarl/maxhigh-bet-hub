/**
 * Fortune Gems slot UI — owns its own spin state machine.
...
import { fortuneGemsSpinFn, getFortuneGemsEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./fortune-gems/animationConfig";
import { MultiplierReelView } from "./fortune-gems/MultiplierReelView";
import { PaytableModal } from "./fortune-gems/PaytableModal";
import { ReelCell, type ReelPhase } from "./fortune-gems/ReelCell";
import { getFortuneGemsConfig, setFortuneGemsConfig } from "./fortune-gems/runtimeConfig";
import { planFortuneGemsPlayback, type FgPlaybackStep } from "./fortune-gems/spinPlayback";
import { cellKey, type FgGrid, type MultiplierReelResult } from "./fortune-gems/types";

function idleGrid(): FgGrid {
  const cfg = getFortuneGemsConfig();
  const kinds: FgSymKind[] = ["ruby", "emerald", "sapphire", "amethyst", "topaz", "temple"];
...
export function FortuneGemsSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<FortuneGemsConfig>(() => getFortuneGemsConfig());
  ...
  const runStep = useCallback(
    (runId: number, steps: FgPlaybackStep[], index: number) => {
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await fortuneGemsSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planFortuneGemsPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getFortuneGemsEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setFortuneGemsConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[FortuneGems] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
