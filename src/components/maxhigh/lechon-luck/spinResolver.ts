import { betPerLine, type CnySymKind } from "@/lib/lechon-luck-config";
 ...
 */
export function resolveLechonLuckSpin(opts: {
  totalBet: number;
  isFreeSpins?: boolean;
  ...
  if (!disableFeatures && !hitCap && detectMonkeyTrigger(grid)) {
    ...
    for (let i = 0; i < monkeyBonus.freeSpinsAwarded && !hitCap; i++) {
      ...
      const fsScript = resolveLechonLuckSpin({
        totalBet,
        isFreeSpins: true,
...
export const resolveSpinScript = (bet: number, _ante = false, isFreeSpins = false) =>
  resolveLechonLuckSpin({ totalBet: bet, isFreeSpins });
