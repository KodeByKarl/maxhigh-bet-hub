/**
 * Sugar Surge — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  sugarSurgeBuyFeatureFn,
  sugarSurgeFreeSpinFn,
  sugarSurgeSpinFn,
  getSugarSurgeSessionFn,
} from "@/functions/api";
import { ANIM, cascadeFallMs, columnDominoMs } from "./sugar-surge/animationConfig";
import { SugarFeatureBadge } from "./sugar-surge/SugarFeatureBadge";
import { FreeSpinsBadge } from "./sugar-surge/FreeSpinsBadge";
import { SugarSurgeSidePanel } from "./sugar-surge/SugarSurgeSidePanel";
import { initialBoard, nextKey, buildBoard } from "./sugar-surge/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./sugar-surge/paytable";
import { getRuntimeSymbols, setSugarSurgeConfig } from "./sugar-surge/runtimeConfig";
import type { BoardCell, SpinScript } from "./sugar-surge/types";
import { CELLS, COLS, ROWS } from "./sugar-surge/types";
import { WinCelebration } from "./sugar-surge/WinCelebration";
import { FreeSpinsCongrats } from "./sugar-surge/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./sugar-surge/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./sugar-surge/BuyFeatureModal";
import { PaytableModal } from "./sugar-surge/PaytableModal";
import { GameMenuModal } from "./sugar-surge/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./sugar-surge/WinLedger";
import { ReelCell, type ReelPhase } from "./sugar-surge/ReelCell";
import { getSugarSurgeEngineConfigFn } from "@/functions/superadmin";
...
export function SugarSurgeSlot({
  gameId = "sugar-surge",
}: {
  gameId?: string;
  gameName?: string;
  ...
  useEffect(() => {
    ...
    preloadAssets();
    sounds.preload();
    if (gameId === SUGAR_SURGE_GAME_ID || gameId === "sugar-surge") {
      void getSugarSurgeEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setSugarSurgeConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getSugarSurgeSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== SUGAR_SURGE_GAME_ID && gameId !== "sugar-surge") return;
    try {
      const cfg = await getSugarSurgeEngineConfigFn();
      if (mountedRef.current) setSugarSurgeConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof sugarSurgeSpinFn>>;
        if (isFree) {
          ...
          settled = await sugarSurgeFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await sugarSurgeSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[SugarSurge] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await sugarSurgeBuyFeatureFn({
