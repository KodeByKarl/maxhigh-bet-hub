import { betPerLine, type FortuneOxConfig, type FoSymKind } from "@/lib/fortune-ox-config";
import type { FoGrid, PaylineWin } from "./types";

function isWild(kind: FoSymKind, cfg: FortuneOxConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FoSymKind, cfg: FortuneOxConfig) {
