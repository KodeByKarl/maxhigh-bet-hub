import { betPerLine, type CnySymKind } from "@/lib/lantern-luck-config";
 ...
 */
export function resolveLanternLuckSpin(opts: {
  totalBet: number;
  isFreeSpins?: boolean;
  ...
  if (!disableFeatures && !hitCap && detectMonkeyTrigger(grid)) {
    ...
    for (let i = 0; i < monkeyBonus.freeSpinsAwarded && !hitCap; i++) {
      ...
      const fsScript = resolveLanternLuckSpin({
        totalBet,
        isFreeSpins: true,
...
export const resolveSpinScript = (bet: number, _ante = false, isFreeSpins = false) =>
  resolveLanternLuckSpin({ totalBet: bet, isFreeSpins });
