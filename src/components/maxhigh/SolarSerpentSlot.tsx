...
import { solarSerpentAudio } from "./solar-serpent/audio";
import {
  solarSerpentBuyFeatureFn,
  solarSerpentFreeSpinFn,
  solarSerpentSpinFn,
  getSolarSerpentSessionFn,
} from "@/functions/api";
import { ANIM } from "./solar-serpent/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./solar-serpent/playbackHelpers";
import { BetSelectModal } from "./solar-serpent/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./solar-serpent/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./solar-serpent/paytable";
import { setSolarSerpentConfig } from "./solar-serpent/runtimeConfig";
import type { BoardCell, SpinScript } from "./solar-serpent/types";
import { CELLS } from "./solar-serpent/types";
import { WinCelebration } from "./solar-serpent/WinCelebration";
import { FreeSpinsCongrats } from "./solar-serpent/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./solar-serpent/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./solar-serpent/BuyFeatureModal";
import { PaytableModal } from "./solar-serpent/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./solar-serpent/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./solar-serpent/ReelGrid";
import { solarSerpentTheme } from "./solar-serpent/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./solar-serpent/ReelCell";
import { getSolarSerpentEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  solarSerpentAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    solarSerpentTheme.assets.backdrop,
    solarSerpentTheme.assets.loadingBg,
    ...Object.values(solarSerpentTheme.assets.icons),
  ];
...
export function SolarSerpentSlot({
  gameId = "solar-serpent",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [muted, setMuted] = useState(() => solarSerpentAudio.isMuted);
  ...
  /** Overlay toast — never inserts into HUD (avoids layout jump). Gold / amber. */
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(20,12,4,0.96), rgba(120,53,15,0.96))",
                color: "#FDE68A",
              }}
            >
        ...
        () => (
          <div
            ...
            style={{
              ...
              background: isWin
                ? "linear-gradient(135deg, #D97706 0%, #92400E 55%, #1C1006 100%)"
                : "linear-gradient(135deg, rgba(20,12,4,0.96), rgba(120,53,15,0.96))",
              color: isWin ? "#FFFBEB" : "#FDE68A",
            }}
          >
            <div
              className="text-[10px] font-black uppercase tracking-[0.18em]"
              style={{
                color: isWin ? "rgba(254,243,199,0.9)" : "rgba(253,230,138,0.85)",
              }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      solarSerpentAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        solarSerpentAudio.stopSpinLoop();
        return;
      }
      solarSerpentAudio.stopSpinLoop();
      solarSerpentAudio.playReelStop(5);
      solarSerpentAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === SOLAR_SERPENT_GAME_ID || gameId === "solar-serpent") {
      void getSolarSerpentEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setSolarSerpentConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getSolarSerpentSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[SolarSerpent] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== SOLAR_SERPENT_GAME_ID && gameId !== "solar-serpent") return;
    try {
      const cfg = await getSolarSerpentEngineConfigFn();
      if (mountedRef.current) setSolarSerpentConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        solarSerpentAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    solarSerpentAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    solarSerpentAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        solarSerpentAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getSolarSerpentSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof solarSerpentSpinFn>>;
        if (isFree) {
          ...
          settled = await solarSerpentFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await solarSerpentSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[SolarSerpent] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await solarSerpentBuyFeatureFn({
    ...
    >
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col px-1.5 pt-[max(0.25rem,env(safe-area-inset-top))] pb-[max(3.25rem,env(safe-area-inset-bottom))] sm:items-center sm:justify-center sm:px-3 sm:py-2 sm:pb-2">
        <div className="mx-auto flex h-full min-h-0 w-full max-w-[840px] flex-col">
          {/* Board well — hug the grid (no tall empty temple box) */}
          <div className="relative flex min-h-0 w-full flex-1 items-center justify-center">
            <div
              className="relative flex max-h-full max-w-full flex-col overflow-hidden rounded-[1.25rem] p-1.5 sm:rounded-2xl sm:p-2"
              style={{
                width: "fit-content",
                height: "fit-content",
                background:
