import { betPerLine, type FortuneMouseConfig, type FoSymKind } from "@/lib/fortune-mouse-config";
import type { FoGrid, PaylineWin } from "./types";

function isWild(kind: FoSymKind, cfg: FortuneMouseConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FoSymKind, cfg: FortuneMouseConfig) {
