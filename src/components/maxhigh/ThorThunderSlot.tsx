/**
 * Thor Thunder — 6×5 ways cascading slot (ported from Thor Thunder).
...
import {
  getThorThunderSessionFn,
  thorThunderBuyFeatureFn,
  thorThunderFreeSpinFn,
  thorThunderSpinFn,
} from "@/functions/api";
import { ANIM, EASE, columnDominoMs } from "./thor-thunder/animationConfig";
import { thorThunderAudio, THOR_THUNDER_SOUND_FILES } from "./thor-thunder/audio";
import { AudioLoadBar } from "./thor-thunder/AudioLoadBar";
import { ControlBar } from "./thor-thunder/ControlBar";
import { FreeSpinsBadge } from "./thor-thunder/FreeSpinsBadge";
import { FreeSpinIntro, ScatterGatherOverlay } from "./thor-thunder/FreeSpinIntro";
import { ThorThunderSidePanel } from "./thor-thunder/ThorThunderSidePanel";
import { buildBoard, initialBoard, nextKey } from "./thor-thunder/gridState";
import { ASSET, getBuyFeatureMult } from "./thor-thunder/paytable";
import { getRuntimeSymbols, setThorThunderConfig } from "./thor-thunder/runtimeConfig";
import type { BoardCell, SpinScript, WayWin } from "./thor-thunder/types";
import { COLS, ROWS } from "./thor-thunder/types";
import { SymbolIcon } from "./thor-thunder/SymbolIcon";
import { WaysWinOverlay } from "./thor-thunder/WaysWinOverlay";
import { WinCelebration } from "./thor-thunder/WinCelebration";
import { PharaohMascot, type MascotMood } from "./thor-thunder/PharaohMascot";
...
export function ThorThunderSlot({
  gameId = "thor-thunder",
  gameName = "Thor Thunder",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [muted, setMuted] = useState(() => thorThunderAudio.isMuted);
  ...
  const [audioTotal, setAudioTotal] = useState(() => Object.keys(THOR_THUNDER_SOUND_FILES).length);
  ...
  useEffect(() => {
    ...
    void thorThunderAudio
      ...
      .finally(() => {
        ...
        const total = Object.keys(THOR_THUNDER_SOUND_FILES).length;
        setAudioTotal(total);
        setAudioLoaded(total);
      });
    return () => {
      alive = false;
      thorThunderAudio.stopAmbient();
    };
  }, []);
  ...
  const playColumnStops = (cols: number) => {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      window.setTimeout(() => thorThunderAudio.playReelStop(c), delay);
    }
  };
  ...
  const gatherScatters = async (board: BoardCell[]) => {
    ...
    thorThunderAudio.playScatterTrigger();
  ...
  const playScript = useCallback(async (script: SpinScript, isFree: boolean) => {
    ...
    setAnimTick((t) => t + 1);
    thorThunderAudio.startSpinLoop();
    playColumnStops(COLS);
    await wait(columnDominoMs(COLS));
    thorThunderAudio.stopSpinLoop();
    ...
    for (const step of script.steps) {
      ...
      if (step.cascadeWin > 0) {
        setMascotMood("win");
        setMascotPulse((n) => n + 1);
        thorThunderAudio.playCascadeTick();
      }
      if (isFree && running > 0) {
  ...
  const showTotalWin = (amount: number, multiplier = 1) => {
    if (amount <= 0) return;
    thorThunderAudio.playWin(amount, bet);
  ...
  const finishBase = async (
    ...
    if (session.inFree && session.freeSpinsLeft > 0) {
      thorThunderAudio.playFreespinIntro();
  ...
  const spin = async (asFree = false) => {
    ...
    try {
      let settled: Awaited<ReturnType<typeof thorThunderSpinFn>>;
      if (isFree) {
        ...
        settled = await thorThunderFreeSpinFn({ data: { sessionId } });
      } else {
        settled = await thorThunderSpinFn({ data: { bet } });
        setDropTotal(0);
        fsMultRef.current = 1;
        setFsMult(1);
      }
      ...
      if (isFree) {
        ...
        if (script.totalWin > 0) thorThunderAudio.playWin(script.totalWin, bet);
      ...
      console.error("[ThorThunder] spin failed", err);
  ...
  const buyFeature = async () => {
    ...
    busyRef.current = true;
    try {
      const bought = await thorThunderBuyFeatureFn({ data: { bet } });
        ...
        <div className="flex h-full max-h-full w-full max-w-[1600px] flex-col items-center gap-1.5 sm:gap-2">
          ...
          <div className="mt-auto w-full shrink-0 pt-1.5">
            <ControlBar
              ...
              onMuteToggle={() => {
                const next = thorThunderAudio.toggleMute();
                setMuted(next);
              }}
