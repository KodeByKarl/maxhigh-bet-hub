import { betPerLine, type ArenaChampConfig, type GrSymKind } from "@/lib/arena-champ-config";
import type { GrGrid, PaylineWin } from "./types";

function isWild(kind: GrSymKind, cfg: ArenaChampConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatter(kind: GrSymKind, cfg: ArenaChampConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

function isPaying(kind: GrSymKind, cfg: ArenaChampConfig) {
...
export function evaluatePaylines(
  ...
  for (let lineIndex = 0; lineIndex < cfg.paylineCount; lineIndex++) {
    ...
    let target: GrSymKind | null = null;
