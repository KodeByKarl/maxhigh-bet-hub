/**
 * Arena Champ — dedicated spin playback planner (not shared with other games).
...
export function planArenaChampPlayback(
  script: SpinScript,
  reelsCount: number,
  turbo: boolean,
): GrPlaybackStep[] {
  const scale = (ms: number) => (turbo ? Math.min(ms, 60) : ms);
  const steps: GrPlaybackStep[] = [];
