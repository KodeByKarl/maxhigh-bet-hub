/**
 * Palengke Pays — connecting ways on a 3-4-5-4-3 diamond board.
 ...
 */
import type { FgSymKind, PalengkePaysConfig } from "@/lib/palengke-pays-config";
import type { FgGrid, PaylineWin } from "./types";

function isWild(kind: FgSymKind, cfg: PalengkePaysConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: PalengkePaysConfig): boolean {
