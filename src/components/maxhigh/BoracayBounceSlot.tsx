/**
 * Boracay Bounce — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  boracayBounceBuyFeatureFn,
  boracayBounceFreeSpinFn,
  boracayBounceSpinFn,
  getBoracayBounceSessionFn,
} from "@/functions/api";
import { ANIM, cascadeFallMs, columnDominoMs } from "./boracay-bounce/animationConfig";
import { BoracayBounceFeatureBadge } from "./boracay-bounce/BoracayBounceFeatureBadge";
import { FreeSpinsBadge } from "./boracay-bounce/FreeSpinsBadge";
import { BoracayBounceSidePanel } from "./boracay-bounce/BoracayBounceSidePanel";
import { initialBoard, nextKey, buildBoard } from "./boracay-bounce/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./boracay-bounce/paytable";
import { getRuntimeSymbols, setBoracayBounceConfig } from "./boracay-bounce/runtimeConfig";
import type { BoardCell, SpinScript } from "./boracay-bounce/types";
import { CELLS, COLS, ROWS } from "./boracay-bounce/types";
import { WinCelebration } from "./boracay-bounce/WinCelebration";
import { FreeSpinsCongrats } from "./boracay-bounce/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./boracay-bounce/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./boracay-bounce/BuyFeatureModal";
import { PaytableModal } from "./boracay-bounce/PaytableModal";
import { GameMenuModal } from "./boracay-bounce/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./boracay-bounce/WinLedger";
import { ReelCell, type ReelPhase } from "./boracay-bounce/ReelCell";
import { getBoracayBounceEngineConfigFn } from "@/functions/superadmin";
...
const SLOT_INDICES = Array.from({ length: CELLS }, (_, i) => i);
...
export function BoracayBounceSlot({
  ...
  const [positionMults, setPositionMults] = useState<number[]>(() =>
    Array.from({ length: CELLS }, () => 0),
  );
  ...
  useEffect(() => {
    ...
    // Always start with a full 7×7 board (avoids stale shorter grids after HMR).
    setSlots(asSlots(initialBoard()));
    preloadAssets();
    sounds.preload();
    if (gameId === BORACAY_BOUNCE_GAME_ID || gameId === "boracay-bounce") {
      void getBoracayBounceEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setBoracayBounceConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getBoracayBounceSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== BORACAY_BOUNCE_GAME_ID && gameId !== "boracay-bounce") return;
    try {
      const cfg = await getBoracayBounceEngineConfigFn();
      if (mountedRef.current) setBoracayBounceConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number) => {
      ...
      setPositionMults(
        script.initialPositionMults?.length === CELLS
          ? script.initialPositionMults.slice()
          : Array.from({ length: CELLS }, () => 0),
      );
      ...
      sounds.playSpin();
      await wait(columnDominoMs(COLS, ROWS), gen);
      sounds.stopSpinLoop();
      ...
      for (const step of script.steps) {
        ...
        if (step.positionMults?.length === CELLS) {
          setPositionMults(step.positionMults.slice());
        }
        ...
        setPhase("falling");
        await wait(cascadeFallMs(COLS), gen);
      ...
      if (gen === playbackGen.current) {
        ...
        setFallenKeys(EMPTY_SET);
        setFallDistance(EMPTY_FALL);
        if (script.finalPositionMults?.length === CELLS) {
          setPositionMults(script.finalPositionMults.slice());
        } else if (!isFree) {
          setPositionMults(Array.from({ length: CELLS }, () => 0));
        }
      }
      return script;
    },
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof boracayBounceSpinFn>>;
        if (isFree) {
          ...
          settled = await boracayBounceFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await boracayBounceSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[BoracayBounce] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await boracayBounceBuyFeatureFn({
              ...
              >
                ...
                <div className="relative flex min-h-0 w-full flex-1 items-center justify-center">
                  ...
                  >
                    ...
                    {/* Candy-cane frame — clips so drops enter from the top edge */}
                    <div
                      className="relative size-full overflow-hidden rounded-[1rem] p-[7px] shadow-[0_18px_50px_rgba(91,33,182,0.45)] sm:rounded-[1.25rem] sm:p-[9px]"
                      style={{
                        background:
                    ...
                    >
                      <div
                        className={cn(
                          "relative size-full rounded-[0.65rem] sm:rounded-[0.9rem]",
                      ...
                      >
                        <div
                          className={cn(
                            "grid size-full gap-0.5 p-0.5 sm:gap-1 sm:p-1",
                            phase === "dropping" || phase === "falling"
                              ? "overflow-visible"
                              : "overflow-hidden",
                          )}
                          ...
                          {SLOT_INDICES.map((i) => {
                            ...
                            return (
                              <ReelCell
                                ...
                                cols={COLS}
                                positionMult={positionMults[i] ?? 0}
                                turbo={turbo}
                              />
                            );
                          })}
