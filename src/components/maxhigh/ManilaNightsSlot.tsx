/**
 * Manila Nights — polished layout matching Candy Peak chrome.
...
import { getManilaNightsEngineConfigFn } from "@/functions/superadmin";
import {
  getManilaNightsSessionFn,
  manilaNightsBuyFeatureFn,
  manilaNightsFreeSpinFn,
  manilaNightsSpinFn,
} from "@/functions/api";
import { ANIM } from "./manila-nights/animationConfig";
import { manilaNightsAudio } from "./manila-nights/audio";
import { ManilaNightsSidePanel } from "./manila-nights/ManilaNightsSidePanel";
import { ReelCell, type ReelPhase } from "./manila-nights/ReelCell";
import { TILE_IMAGE_MAP } from "./manila-nights/ManilaNightsIcon";
import { PaytableModal } from "./manila-nights/PaytableModal";
import { getManilaNightsConfig, setManilaNightsConfig } from "./manila-nights/runtimeConfig";
import type { BoardCell, SpinScript } from "./manila-nights/types";

type ManilaNightsSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_MANILA_NIGHTS_CONFIG.symbols;
...
const MANILA_NIGHTS_BACKDROP = "/images/symbols/manila-nights/backdrop.webp";

function preloadAssets() {
  if (typeof Image === "undefined") return;
  const urls = [
    "/games/manila-nights.webp",
    MANILA_NIGHTS_BACKDROP,
...
export function ManilaNightsSlot({ onBalanceUpdate }: ManilaNightsSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getManilaNightsConfig();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    manilaNightsAudio.preload();
    void getManilaNightsEngineConfigFn()
      .then((cfg) => {
    ...
    return () => {
      ...
      manilaNightsAudio.stopSpinLoop();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getManilaNightsEngineConfigFn();
      if (!mountedRef.current) return;
      setManilaNightsConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      manilaNightsAudio.playReelStop();
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await manilaNightsFreeSpinFn({ data: { sessionId } });
        ...
        const res = await manilaNightsSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await manilaNightsBuyFeatureFn({ data: { bet } });
