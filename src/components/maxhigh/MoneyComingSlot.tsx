/**
 * Money Coming slot UI — owns its own spin state machine.
...
import { moneyComingSpinFn, getMoneyComingEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM } from "./money-coming/animationConfig";
import { MoneyBorder } from "./money-coming/MoneyBorder";
import { ReelCell, type ReelPhase } from "./money-coming/ReelCell";
import { getMoneyComingConfig, setMoneyComingConfig } from "./money-coming/runtimeConfig";
import { planMoneyComingPlayback, type McPlaybackStep } from "./money-coming/spinPlayback";
import { cellKey, type McGrid } from "./money-coming/types";

function idleGrid(): McGrid {
  const cfg = getMoneyComingConfig();
  const kinds: McSymKind[] = ["bar", "dice", "diamond", "chip", "lucky7"];
...
export function MoneyComingSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<MoneyComingConfig>(() => getMoneyComingConfig());
  ...
  const runStep = useCallback(
    (runId: number, steps: McPlaybackStep[], index: number) => {
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await moneyComingSpinFn({ data: { bet: stake } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const cfg = getMoneyComingConfig();
        const steps = planMoneyComingPlayback(res.script, cfg.reelsCount, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getMoneyComingEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setMoneyComingConfig(cfg);
        setCfgSnap(next);
        setUi((prev) =>
          prev.locked
            ? prev
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[MoneyComing] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#120806] text-white">
      ...
      <div
        ...
        style={{ backgroundImage: "url(/images/money-coming-bg.webp)" }}
      />
      <div
      ...
      <header className="relative z-10 flex shrink-0 flex-col items-center gap-2.5 px-3 pt-3 sm:pt-4">
        <MoneyBorder rounded="trap" thickness={3} className="w-full max-w-md">
          <div className="px-5 py-2 text-center">
            <p className="text-[9px] font-bold uppercase tracking-[0.28em] text-orange-400/90">
              MaxHigh · Money Coming
            </p>
        ...
        <div className="flex w-full max-w-md items-center justify-center gap-3">
          <MoneyBorder rounded="xl" thickness={2} className="min-w-[8.5rem] flex-1">
      ...
      <footer className="relative z-10 shrink-0 px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
        <MoneyBorder rounded="2xl" thickness={3} className="mx-auto w-full max-w-lg">
