...
import { getMidnightOwlConfig, getRuntimeSymbols } from "./runtimeConfig";
import { midnightOwlTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...midnightOwlTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getMidnightOwlConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getMidnightOwlConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getMidnightOwlConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getMidnightOwlConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getMidnightOwlConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getMidnightOwlConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getMidnightOwlConfig().freeSpinsRetrigger;
}
