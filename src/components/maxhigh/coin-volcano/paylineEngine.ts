import { betPerLine, type CoinVolcanoConfig, type McSymKind } from "@/lib/coin-volcano-config";
import type { McGrid, PaylineWin } from "./types";

function isWild(kind: McSymKind, cfg: CoinVolcanoConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatter(kind: McSymKind, cfg: CoinVolcanoConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

function isPaying(kind: McSymKind, cfg: CoinVolcanoConfig) {
