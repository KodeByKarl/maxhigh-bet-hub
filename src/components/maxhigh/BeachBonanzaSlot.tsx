/**
 * Beach Bonanza — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  beachBonanzaBuyFeatureFn,
  beachBonanzaFreeSpinFn,
  beachBonanzaSpinFn,
  getBeachBonanzaSessionFn,
} from "@/functions/api";
import { ANIM, cascadeFallMs, columnDominoMs } from "./beach-bonanza/animationConfig";
import { BeachBonanzaFeatureBadge } from "./beach-bonanza/BeachBonanzaFeatureBadge";
import { FreeSpinsBadge } from "./beach-bonanza/FreeSpinsBadge";
import { BeachBonanzaSidePanel } from "./beach-bonanza/BeachBonanzaSidePanel";
import { initialBoard, nextKey, buildBoard } from "./beach-bonanza/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./beach-bonanza/paytable";
import { getRuntimeSymbols, setBeachBonanzaConfig } from "./beach-bonanza/runtimeConfig";
import type { BoardCell, SpinScript } from "./beach-bonanza/types";
import { CELLS, COLS, ROWS } from "./beach-bonanza/types";
import { WinCelebration } from "./beach-bonanza/WinCelebration";
import { FreeSpinsCongrats } from "./beach-bonanza/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./beach-bonanza/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./beach-bonanza/BuyFeatureModal";
import { PaytableModal } from "./beach-bonanza/PaytableModal";
import { GameMenuModal } from "./beach-bonanza/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./beach-bonanza/WinLedger";
import { ReelCell, type ReelPhase } from "./beach-bonanza/ReelCell";
import { getBeachBonanzaEngineConfigFn } from "@/functions/superadmin";
...
export function BeachBonanzaSlot({
  ...
  useEffect(() => {
    ...
    preloadAssets();
    sounds.preload();
    if (gameId === BEACH_BONANZA_GAME_ID || gameId === "beach-bonanza") {
      void getBeachBonanzaEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setBeachBonanzaConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getBeachBonanzaSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== BEACH_BONANZA_GAME_ID && gameId !== "beach-bonanza") return;
    try {
      const cfg = await getBeachBonanzaEngineConfigFn();
      if (mountedRef.current) setBeachBonanzaConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof beachBonanzaSpinFn>>;
        if (isFree) {
          ...
          settled = await beachBonanzaFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await beachBonanzaSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[BeachBonanza] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await beachBonanzaBuyFeatureFn({
