/**
 * Gold Mine Dig — connecting ways on a 3-4-5-4-3 diamond board.
...
function isWild(kind: FgSymKind, cfg: GoldMineConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: GoldMineConfig): boolean {
