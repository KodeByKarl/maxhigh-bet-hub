/**
 * Mermaid Riches — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  mermaidRichesBuyFeatureFn,
  mermaidRichesFreeSpinFn,
  mermaidRichesSpinFn,
  getMermaidRichesSessionFn,
} from "@/functions/api";
import { ANIM } from "./mermaid-riches/animationConfig";
import { MermaidFeatureBadge } from "./mermaid-riches/MermaidFeatureBadge";
import { FreeSpinsBadge } from "./mermaid-riches/FreeSpinsBadge";
import { MermaidRichesSidePanel } from "./mermaid-riches/MermaidRichesSidePanel";
import { initialBoard, nextKey, buildBoard } from "./mermaid-riches/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./mermaid-riches/paytable";
import { getRuntimeSymbols, setMermaidRichesConfig } from "./mermaid-riches/runtimeConfig";
import type { BoardCell, SpinScript } from "./mermaid-riches/types";
import { CELLS, COLS, ROWS } from "./mermaid-riches/types";
import { WinCelebration } from "./mermaid-riches/WinCelebration";
import { FreeSpinsCongrats } from "./mermaid-riches/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./mermaid-riches/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./mermaid-riches/BuyFeatureModal";
import { PaytableModal } from "./mermaid-riches/PaytableModal";
import { GameMenuModal } from "./mermaid-riches/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./mermaid-riches/WinLedger";
import { ReelCell, type ReelPhase } from "./mermaid-riches/ReelCell";
import { getMermaidRichesEngineConfigFn } from "@/functions/superadmin";
...
export function MermaidRichesSlot({
  ...
  useEffect(() => {
    ...
    preloadAssets();
    sounds.preload();
    if (gameId === MERMAID_RICHES_GAME_ID || gameId === "mermaid-riches") {
      void getMermaidRichesEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setMermaidRichesConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getMermaidRichesSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== MERMAID_RICHES_GAME_ID && gameId !== "mermaid-riches") return;
    try {
      const cfg = await getMermaidRichesEngineConfigFn();
      if (mountedRef.current) setMermaidRichesConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof mermaidRichesSpinFn>>;
        if (isFree) {
          ...
          settled = await mermaidRichesFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await mermaidRichesSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[MermaidRiches] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await mermaidRichesBuyFeatureFn({
