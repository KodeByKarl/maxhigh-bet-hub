import type { WildAceSymbolConfig, WildAceSymKind } from "@/lib/wild-ace-config";
...
export function pickRandomSymbol(ante: boolean, isFreeSpins: boolean): WildAceSymbolConfig {
  const config = getWildAceConfig();
  let total = 0;
