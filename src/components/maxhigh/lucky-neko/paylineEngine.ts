import { betPerLine, type LuckyNekoConfig, type FoSymKind } from "@/lib/lucky-neko-config";
import type { FoGrid, PaylineWin } from "./types";

function isWild(kind: FoSymKind, cfg: LuckyNekoConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FoSymKind, cfg: LuckyNekoConfig) {
