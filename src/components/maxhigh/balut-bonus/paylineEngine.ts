import { betPerLine, type BalutBonusConfig, type FoSymKind } from "@/lib/balut-bonus-config";
import type { FoGrid, PaylineWin } from "./types";

function isWild(kind: FoSymKind, cfg: BalutBonusConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FoSymKind, cfg: BalutBonusConfig) {
