/**
 * Super Ace � polished layout matching Candy Peak chrome.
...
import { DEFAULT_SUPER_ACE_CONFIG } from "@/lib/super-ace-config";
import { getSuperAceEngineConfigFn } from "@/functions/superadmin";
import {
  getSuperAceSessionFn,
  superAceBuyFeatureFn,
  superAceFreeSpinFn,
  superAceSpinFn,
} from "@/functions/api";
import { ANIM } from "./super-ace/animationConfig";
import { superAceAudio } from "./super-ace/audio";
import { SuperAceSidePanel } from "./super-ace/SuperAceSidePanel";
import { ReelCell, type ReelPhase } from "./super-ace/ReelCell";
import { TILE_IMAGE_MAP } from "./super-ace/SuperAceIcon";
import { PaytableModal } from "./super-ace/PaytableModal";
import { getSuperAceConfig, setSuperAceConfig } from "./super-ace/runtimeConfig";
import type { BoardCell, SpinScript } from "./super-ace/types";

type SuperAceSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_SUPER_ACE_CONFIG.symbols;
...
export function SuperAceSlot({ onBalanceUpdate }: SuperAceSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getSuperAceConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getSuperAceEngineConfigFn();
      if (!mountedRef.current) return;
      setSuperAceConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await superAceFreeSpinFn({ data: { sessionId } });
        ...
        const res = await superAceSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await superAceBuyFeatureFn({ data: { bet } });
