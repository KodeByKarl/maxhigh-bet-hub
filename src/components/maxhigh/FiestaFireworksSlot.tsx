/**
 * Fiesta Fireworks — fixed-payline slot UI.
...
import { getFiestaFireworksEngineConfigFn } from "@/functions/superadmin";
import {
  fiestaFireworksCollectFn,
  fiestaFireworksGambleFn,
  fiestaFireworksSpinFn,
  getFiestaFireworksSessionFn,
} from "@/functions/api";
import { ANIM } from "./fiesta-fireworks/animationConfig";
import { fiestaFireworksAudio } from "./fiesta-fireworks/audio";
import { ICON_SRC, BET_STEPS } from "./fiesta-fireworks/paytable";
import { BetSelectModal } from "./fiesta-fireworks/BetSelectModal";
import { getFiestaFireworksConfig, setFiestaFireworksConfig } from "./fiesta-fireworks/runtimeConfig";
import { PaytableModal } from "./fiesta-fireworks/PaytableModal";
import { ReelCell, type ReelPhase } from "./fiesta-fireworks/ReelCell";
import { WinCelebration } from "./fiesta-fireworks/WinCelebration";
import type { CnyGrid, GambleChoice, SpinScript } from "./fiesta-fireworks/types";
import { cellKey } from "./fiesta-fireworks/types";
...
export function FiestaFireworksSlot({
  ...
  useEffect(() => {
    ...
    fiestaFireworksAudio.preload();
    preloadAssets();
    void getFiestaFireworksEngineConfigFn()
      .then((cfg) => {
      ...
      .catch(() => undefined);
    void getFiestaFireworksSessionFn()
      .then((s) => {
    ...
    return () => {
      ...
      fiestaFireworksAudio.stopAmbient();
    };
  }, []);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      fiestaFireworksAudio.unlock();
      fiestaFireworksAudio.startSpinLoop();

      // Spin blur, then reveal final grid with staggered drop-stop
      await wait(ANIM.reelSpin, gen);
      fiestaFireworksAudio.playReelStop();
      setGrid(script.grid);
      setPhase("stopping");
      ...
      if (script.dragonBonus?.triggered) {
        ...
        for (const launch of script.dragonBonus.launches) {
          if (launch.success) {
            ...
            fiestaFireworksAudio.playDragonFirework();
            setDragonOverlay({
            ...
            fiestaFireworksAudio.playDragonBust();
      ...
      if (script.monkeyBonus?.triggered) {
        fiestaFireworksAudio.playMonkeyTrigger();
        setMonkeyOverlay({
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      ...
      const res = await fiestaFireworksSpinFn({
  ...
  const onCollect = async () => {
    ...
    try {
      fiestaFireworksAudio.unlock();
      fiestaFireworksAudio.playCollect();
      const res = await fiestaFireworksCollectFn({ data: { sessionId } });
      setBalanceLocal(res.balance);
      setPendingWin(0);
      setSessionId(null);
      setGambleOpen(false);
  ...
  const onGamble = async (choice: GambleChoice) => {
    ...
    try {
      fiestaFireworksAudio.unlock();
      fiestaFireworksAudio.playGambleFlip();
      const res = await fiestaFireworksGambleFn({ data: { sessionId, choice } });
      setBalanceLocal(res.balance);
      if (res.result.won) {
