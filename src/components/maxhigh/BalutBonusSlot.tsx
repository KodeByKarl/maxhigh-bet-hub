/**
 * Balut Bonus slot UI — owns its own spin state machine.
...
import { balutBonusFn, getBalutBonusEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./balut-bonus/animationConfig";
import { MultiplierReelView } from "./balut-bonus/MultiplierReelView";
import { PaytableModal } from "./balut-bonus/PaytableModal";
import { ReelCell, type ReelPhase } from "./balut-bonus/ReelCell";
import { getBalutBonusConfig, setBalutBonusConfig } from "./balut-bonus/runtimeConfig";
import { planBalutBonusPlayback, type FoPlaybackStep } from "./balut-bonus/spinPlayback";
import { cellKey, type FoGrid, type MultiplierReelResult } from "./balut-bonus/types";

function idleGrid(): FoGrid {
  const cfg = getBalutBonusConfig();
...
export function BalutBonusSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<BalutBonusConfig>(() => getBalutBonusConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await balutBonusFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planBalutBonusPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getBalutBonusEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setBalutBonusConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[BalutBonus] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
