/**
 * Outlaw Coins — connecting ways on a 3-4-5-4-3 diamond board.
...
function isWild(kind: FgSymKind, cfg: OutlawCoinsConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: OutlawCoinsConfig): boolean {
...
/** @deprecated Alias — Outlaw Coins now uses connecting ways. */
