/**
 * Crazy Sevens — server-authoritative full spin resolution.
 *
 * Order (Section 7):
...
import type { NeonFruitsConfig } from "@/lib/neon-fruits-config";
...
export type ResolveNeonFruitsSpinOpts = {
  totalBet: number;
  seed?: string;
...
export function resolveNeonFruitsSpin(opts: ResolveNeonFruitsSpinOpts): SpinScript {
  const cfg = opts.cfg ?? getNeonFruitsConfig();
