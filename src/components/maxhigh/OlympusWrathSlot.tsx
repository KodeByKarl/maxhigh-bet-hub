/**
 * Olympus Wrath — 6×5 ways cascading slot (ported from Golden Empire).
...
import {
  getOlympusWrathSessionFn,
  olympusWrathBuyFeatureFn,
  olympusWrathFreeSpinFn,
  olympusWrathSpinFn,
} from "@/functions/api";
import { ANIM, EASE, columnDominoMs } from "./olympus-wrath/animationConfig";
import { geAudio, GE_SOUND_FILES } from "./olympus-wrath/audio";
import { AudioLoadBar } from "./olympus-wrath/AudioLoadBar";
import { ControlBar } from "./olympus-wrath/ControlBar";
import { FreeSpinsBadge } from "./olympus-wrath/FreeSpinsBadge";
import { FreeSpinIntro, ScatterGatherOverlay } from "./olympus-wrath/FreeSpinIntro";
import { OlympusWrathSidePanel } from "./olympus-wrath/OlympusWrathSidePanel";
import { buildBoard, initialBoard, nextKey } from "./olympus-wrath/gridState";
import { ASSET, getBuyFeatureMult } from "./olympus-wrath/paytable";
import { getRuntimeSymbols, setOlympusWrathConfig } from "./olympus-wrath/runtimeConfig";
import type { BoardCell, SpinScript, WayWin } from "./olympus-wrath/types";
import { COLS, ROWS } from "./olympus-wrath/types";
import { SymbolIcon } from "./olympus-wrath/SymbolIcon";
import { WaysWinOverlay } from "./olympus-wrath/WaysWinOverlay";
import { WinCelebration } from "./olympus-wrath/WinCelebration";
import { PharaohMascot, type MascotMood } from "./olympus-wrath/PharaohMascot";
...
export function OlympusWrathSlot({
  gameId = "olympus-wrath",
  gameName = "Olympus Wrath",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [muted, setMuted] = useState(() => geAudio.isMuted);
  ...
  const [audioTotal, setAudioTotal] = useState(() => Object.keys(GE_SOUND_FILES).length);
  ...
  useEffect(() => {
    ...
    void geAudio
      ...
      .finally(() => {
        ...
        const total = Object.keys(GE_SOUND_FILES).length;
        setAudioTotal(total);
  ...
  const playColumnStops = (cols: number) => {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      window.setTimeout(() => geAudio.playReelStop(c), delay);
    }
  };
  ...
  const spin = async (asFree = false) => {
    ...
    try {
      let settled: Awaited<ReturnType<typeof olympusWrathSpinFn>>;
      if (isFree) {
        ...
        settled = await olympusWrathFreeSpinFn({ data: { sessionId } });
      } else {
        settled = await olympusWrathSpinFn({ data: { bet } });
        setDropTotal(0);
        fsMultRef.current = 1;
        setFsMult(1);
      }
      ...
      if (isFree) {
        ...
        if (script.totalWin > 0) geAudio.playWin(script.totalWin, bet);
      ...
      console.error("[OlympusWrath] spin failed", err);
  ...
  const buyFeature = async () => {
    ...
    busyRef.current = true;
    try {
      const bought = await olympusWrathBuyFeatureFn({ data: { bet } });
