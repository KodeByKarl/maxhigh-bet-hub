import type { CashManiaConfig, McSymKind } from "@/lib/cash-mania-config";
import type { McGrid, InstantMixResult, WildScatterCount } from "./types";

function isWildKind(kind: McSymKind, cfg: CashManiaConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatterKind(kind: McSymKind, cfg: CashManiaConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

/** Count Wild + Scatter anywhere in view (not payline-restricted). */
export function countWildScatter(grid: McGrid, cfg: CashManiaConfig): WildScatterCount {
  let wild = 0;
  let scatter = 0;
 ...
 */
export function isGrandJackpot(grid: McGrid, cfg: CashManiaConfig): boolean {
  let cells = 0;
