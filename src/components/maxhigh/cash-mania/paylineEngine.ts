import { betPerLine, type CashManiaConfig, type McSymKind } from "@/lib/cash-mania-config";
import type { McGrid, PaylineWin } from "./types";

function isWild(kind: McSymKind, cfg: CashManiaConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatter(kind: McSymKind, cfg: CashManiaConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

function isPaying(kind: McSymKind, cfg: CashManiaConfig) {
