import type { FireSpikeConfig, FsSymKind } from "@/lib/fire-spike-config";
import type { FsGrid, InstantMixResult, WildScatterCount } from "./types";

function isWildKind(kind: FsSymKind, cfg: FireSpikeConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatterKind(kind: FsSymKind, cfg: FireSpikeConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

/** Count Wild + Scatter anywhere in view (not payline-restricted). */
export function countWildScatter(grid: FsGrid, cfg: FireSpikeConfig): WildScatterCount {
  let wild = 0;
  let scatter = 0;
 ...
 */
export function isGrandJackpot(grid: FsGrid, cfg: FireSpikeConfig): boolean {
  let cells = 0;
