/**
 * Fire Spike — dedicated spin playback planner (not shared with other games).
...
export function planFireSpikePlayback(
  script: SpinScript,
  reelsCount: number,
  turbo: boolean,
): FsPlaybackStep[] {
  const scale = (ms: number) => (turbo ? Math.min(ms, 60) : ms);
  const steps: FsPlaybackStep[] = [];
