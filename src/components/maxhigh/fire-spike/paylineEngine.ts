import { betPerLine, type FireSpikeConfig, type FsSymKind } from "@/lib/fire-spike-config";
import type { FsGrid, PaylineWin } from "./types";

function isWild(kind: FsSymKind, cfg: FireSpikeConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatter(kind: FsSymKind, cfg: FireSpikeConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

function isPaying(kind: FsSymKind, cfg: FireSpikeConfig) {
...
export function evaluatePaylines(
  ...
  for (let lineIndex = 0; lineIndex < cfg.paylineCount; lineIndex++) {
    ...
    let target: FsSymKind | null = null;
