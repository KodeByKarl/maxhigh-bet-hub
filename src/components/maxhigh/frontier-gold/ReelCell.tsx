...
const SPIN_STRIP_LOOP: FgSymKind[] = [...SPIN_STRIP, ...SPIN_STRIP];
...
export const ReelCell = memo(function ReelCell({
  ...
  return (
    ...
    >
      ...
      {!spinning && (
        <motion.div
          key={stopping || removing ? `land-${spinId}` : "idle"}
          className="absolute inset-0 z-[1] will-change-transform"
          initial={stopping || phase === "win" ? { y: "-120%", opacity: 0.35, scale: 0.92 } : false}
          animate={
            removing
              ? { y: 0, opacity: 0, scale: 0.35 }
              : win
                ? {
                    y: 0,
                    opacity: 1,
