/**
 * Mahjong Ways 2 — polished layout matching Candy Peak chrome.
...
import { getMahjongWays2EngineConfigFn } from "@/functions/superadmin";
import {
  getMahjongWays2SessionFn,
  mahjongWays2BuyFeatureFn,
  mahjongWays2FreeSpinFn,
  mahjongWays2SpinFn,
} from "@/functions/api";
import { ANIM } from "./mahjong-ways-2/animationConfig";
import { mahjongAudio } from "./mahjong-ways-2/audio";
import { TILE_IMAGE_MAP } from "./mahjong-ways-2/Mahjong2Icon";
import { Mahjong2SidePanel } from "./mahjong-ways-2/Mahjong2SidePanel";
import { ReelCell, type ReelPhase } from "./mahjong-ways-2/ReelCell";
import { getMahjongWays2Config, setMahjongWays2Config } from "./mahjong-ways-2/runtimeConfig";
import type { BoardCell, SpinScript } from "./mahjong-ways-2/types";

type MahjongWays2SlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_MAHJONG_WAYS_2_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/mahjong-ways-2.webp", ...Object.values(TILE_IMAGE_MAP)];
...
export function MahjongWays2Slot({ onBalanceUpdate }: MahjongWays2SlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getMahjongWays2Config();
  ...
  useEffect(() => {
    ...
    void getMahjongWays2EngineConfigFn()
      .then((cfg) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getMahjongWays2EngineConfigFn();
      if (!mountedRef.current) return;
      setMahjongWays2Config(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await mahjongWays2FreeSpinFn({ data: { sessionId } });
        ...
        const res = await mahjongWays2SpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await mahjongWays2BuyFeatureFn({ data: { bet } });
