/**
 * Boxing King slot UI — owns its own spin state machine.
...
import { boxingKingSpinFn, getBoxingKingEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM } from "./boxing-king/animationConfig";
import { BoxingBorder } from "./boxing-king/BoxingBorder";
import { ReelCell, type ReelPhase } from "./boxing-king/ReelCell";
import { getBoxingKingConfig, setBoxingKingConfig } from "./boxing-king/runtimeConfig";
import { planBoxingKingPlayback, type BkPlaybackStep } from "./boxing-king/spinPlayback";
import { cellKey, type BkGrid } from "./boxing-king/types";
...
export function BoxingKingSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<BoxingKingConfig>(() => getBoxingKingConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await boxingKingSpinFn({ data: { bet: stake } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const cfg = getBoxingKingConfig();
        const steps = planBoxingKingPlayback(res.script, cfg.reelsCount, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getBoxingKingEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setBoxingKingConfig(cfg);
        setCfgSnap(next);
        setUi((prev) =>
          prev.locked
            ? prev
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[BoxingKing] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#120806] text-white">
      ...
      <div
        ...
        style={{ backgroundImage: "url(/images/boxing-king-bg.png)" }}
      />
      <div
      ...
      <header className="relative z-10 flex shrink-0 flex-col items-center gap-2.5 px-3 pt-3 sm:pt-4">
        <BoxingBorder rounded="trap" thickness={3} className="w-full max-w-md">
        ...
        <div className="flex w-full max-w-md items-center justify-center gap-3">
          <BoxingBorder rounded="xl" thickness={2} className="min-w-[8.5rem] flex-1">
      ...
      <footer className="relative z-10 shrink-0 px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
        <BoxingBorder rounded="2xl" thickness={3} className="mx-auto w-full max-w-lg">
