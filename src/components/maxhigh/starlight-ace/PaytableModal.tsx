...
import { SYMBOL_NAMES, type StarlightSymKind } from "@/lib/starlight-ace-config";
import { getStarlightAceConfig } from "./runtimeConfig";
import { StarlightIcon } from "./StarlightIcon";
...
export function PaytableModal({ open, onClose }: PaytableModalProps) {
  if (!open) return null;
  const cfg = getStarlightAceConfig();
  const ways =
      ...
      >
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-black text-[#F5D76E]">Starlight Ace · Info</h2>
            <button
              type="button"
              onClick={onClose}
