import { betPerLine, type CnySymKind } from "@/lib/sinigang-spin-config";
 ...
 */
export function resolveSinigangSpinSpin(opts: {
  totalBet: number;
  isFreeSpins?: boolean;
  ...
  if (!disableFeatures && !hitCap && detectMonkeyTrigger(grid)) {
    ...
    for (let i = 0; i < monkeyBonus.freeSpinsAwarded && !hitCap; i++) {
      ...
      const fsScript = resolveSinigangSpinSpin({
        totalBet,
        isFreeSpins: true,
...
export const resolveSpinScript = (bet: number, _ante = false, isFreeSpins = false) =>
  resolveSinigangSpinSpin({ totalBet: bet, isFreeSpins });
