...
import { honeyHiveAudio } from "./honey-hive/audio";
import {
  honeyHiveBuyFeatureFn,
  honeyHiveFreeSpinFn,
  honeyHiveSpinFn,
  getHoneyHiveSessionFn,
} from "@/functions/api";
import { ANIM } from "./honey-hive/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./honey-hive/playbackHelpers";
import { BetSelectModal } from "./honey-hive/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./honey-hive/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./honey-hive/paytable";
import { setHoneyHiveConfig } from "./honey-hive/runtimeConfig";
import type { BoardCell, SpinScript } from "./honey-hive/types";
import { CELLS } from "./honey-hive/types";
import { WinCelebration } from "./honey-hive/WinCelebration";
import { FreeSpinsCongrats } from "./honey-hive/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./honey-hive/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./honey-hive/BuyFeatureModal";
import { PaytableModal } from "./honey-hive/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./honey-hive/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./honey-hive/ReelGrid";
import { honeyHiveTheme } from "./honey-hive/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./honey-hive/ReelCell";
import { getHoneyHiveEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  honeyHiveAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    honeyHiveTheme.assets.backdrop,
    honeyHiveTheme.assets.loadingBg,
    ...Object.values(honeyHiveTheme.assets.icons),
  ];
...
export function HoneyHiveSlot({
  ...
  const [muted, setMuted] = useState(() => honeyHiveAudio.isMuted);
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(28,16,4,0.96), rgba(120,53,15,0.96))",
                color: "#FDE68A",
              }}
            >
        ...
        () => (
          <div
            ...
            style={{
              ...
              background: isWin
                ? "linear-gradient(135deg, #D97706 0%, #92400E 55%, #1C1004 100%)"
                : "linear-gradient(135deg, rgba(28,16,4,0.96), rgba(120,53,15,0.96))",
          ...
          >
            <div
              ...
              style={{
                color: isWin ? "rgba(254,243,199,0.85)" : "rgba(253,230,138,0.85)",
              }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      honeyHiveAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        honeyHiveAudio.stopSpinLoop();
        return;
      }
      honeyHiveAudio.stopSpinLoop();
      honeyHiveAudio.playReelStop(5);
      honeyHiveAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === HONEY_HIVE_GAME_ID || gameId === "honey-hive") {
      void getHoneyHiveEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setHoneyHiveConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getHoneyHiveSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[HoneyHive] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== HONEY_HIVE_GAME_ID && gameId !== "honey-hive") return;
    try {
      const cfg = await getHoneyHiveEngineConfigFn();
      if (mountedRef.current) setHoneyHiveConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        honeyHiveAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    honeyHiveAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    honeyHiveAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        honeyHiveAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getHoneyHiveSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof honeyHiveSpinFn>>;
        if (isFree) {
          ...
          settled = await honeyHiveFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await honeyHiveSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[HoneyHive] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await honeyHiveBuyFeatureFn({
        ...
        <div className="mx-auto flex h-full min-h-0 w-full max-w-[840px] flex-col">
          ...
          <div className="relative flex min-h-0 w-full flex-1 flex-col">
            ...
            >
              ...
              {/* Honeycomb nest — 5-6-7-6-5 hex pillars */}
