/**
 * Fortune Tiger slot UI — owns its own spin state machine.
...
import { fortuneTigerSpinFn, getFortuneTigerEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./fortune-tiger/animationConfig";
import { MultiplierReelView } from "./fortune-tiger/MultiplierReelView";
import { PaytableModal } from "./fortune-tiger/PaytableModal";
import { ReelCell, type ReelPhase } from "./fortune-tiger/ReelCell";
import { getFortuneTigerConfig, setFortuneTigerConfig } from "./fortune-tiger/runtimeConfig";
import { planFortuneTigerPlayback, type FtPlaybackStep } from "./fortune-tiger/spinPlayback";
import { cellKey, type FtGrid, type MultiplierReelResult } from "./fortune-tiger/types";

function idleGrid(): FtGrid {
  const cfg = getFortuneTigerConfig();
...
export function FortuneTigerSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<FortuneTigerConfig>(() => getFortuneTigerConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await fortuneTigerSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planFortuneTigerPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getFortuneTigerEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setFortuneTigerConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[FortuneTiger] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
