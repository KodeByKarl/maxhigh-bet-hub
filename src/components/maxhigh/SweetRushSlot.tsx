/**
 * Sweet Rush — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  sweetRushBuyFeatureFn,
  sweetRushFreeSpinFn,
  sweetRushSpinFn,
  getSweetRushSessionFn,
} from "@/functions/api";
import { ANIM, cascadeFallMs, columnDominoMs } from "./sweet-rush/animationConfig";
import { SweetRushFeatureBadge } from "./sweet-rush/SweetRushFeatureBadge";
import { FreeSpinsBadge } from "./sweet-rush/FreeSpinsBadge";
import { SweetRushSidePanel } from "./sweet-rush/SweetRushSidePanel";
import { initialBoard, nextKey, buildBoard } from "./sweet-rush/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./sweet-rush/paytable";
import { getRuntimeSymbols, setSweetRushConfig } from "./sweet-rush/runtimeConfig";
import type { BoardCell, SpinScript } from "./sweet-rush/types";
import { CELLS, COLS, ROWS } from "./sweet-rush/types";
import { WinCelebration } from "./sweet-rush/WinCelebration";
import { FreeSpinsCongrats } from "./sweet-rush/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./sweet-rush/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./sweet-rush/BuyFeatureModal";
import { PaytableModal } from "./sweet-rush/PaytableModal";
import { GameMenuModal } from "./sweet-rush/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./sweet-rush/WinLedger";
import { ReelCell, type ReelPhase } from "./sweet-rush/ReelCell";
import { getSweetRushEngineConfigFn } from "@/functions/superadmin";
...
export function SweetRushSlot({
  ...
  useEffect(() => {
    ...
    preloadAssets();
    sounds.preload();
    if (gameId === SWEET_RUSH_GAME_ID || gameId === "sweet-rush") {
      void getSweetRushEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setSweetRushConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getSweetRushSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== SWEET_RUSH_GAME_ID && gameId !== "sweet-rush") return;
    try {
      const cfg = await getSweetRushEngineConfigFn();
      if (mountedRef.current) setSweetRushConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof sweetRushSpinFn>>;
        if (isFree) {
          ...
          settled = await sweetRushFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await sweetRushSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[SweetRush] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await sweetRushBuyFeatureFn({
