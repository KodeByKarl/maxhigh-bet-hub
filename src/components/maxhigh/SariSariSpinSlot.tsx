/**
 * Sari-Sari Spin slot UI — owns its own spin state machine.
...
import { sariSariSpinFn, getSariSariSpinEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./sari-sari-spin/animationConfig";
import { MultiplierReelView } from "./sari-sari-spin/MultiplierReelView";
import { PaytableModal } from "./sari-sari-spin/PaytableModal";
import { ReelCell, type ReelPhase } from "./sari-sari-spin/ReelCell";
import { getSariSariSpinConfig, setSariSariSpinConfig } from "./sari-sari-spin/runtimeConfig";
import { planSariSariSpinPlayback, type FoPlaybackStep } from "./sari-sari-spin/spinPlayback";
import { cellKey, type FoGrid, type MultiplierReelResult } from "./sari-sari-spin/types";

function idleGrid(): FoGrid {
  const cfg = getSariSariSpinConfig();
...
export function SariSariSpinSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<SariSariSpinConfig>(() => getSariSariSpinConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await sariSariSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planSariSariSpinPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getSariSariSpinEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setSariSariSpinConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[SariSariSpin] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
