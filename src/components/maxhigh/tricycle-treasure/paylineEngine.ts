/**
 * Tricycle Treasure — connecting ways on a 3-4-5-4-3 diamond board.
 ...
 */
import type { FgSymKind, TricycleTreasureConfig } from "@/lib/tricycle-treasure-config";
import type { FgGrid, PaylineWin } from "./types";

function isWild(kind: FgSymKind, cfg: TricycleTreasureConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: TricycleTreasureConfig): boolean {
