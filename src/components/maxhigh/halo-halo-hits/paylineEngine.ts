import { betPerLine, type HaloHaloHitsConfig, type FoSymKind } from "@/lib/halo-halo-hits-config";
import type { FoGrid, PaylineWin } from "./types";

function isWild(kind: FoSymKind, cfg: HaloHaloHitsConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FoSymKind, cfg: HaloHaloHitsConfig) {
