import { betPerLine, type BoxingKingConfig, type BkSymKind } from "@/lib/boxing-king-config";
import type { BkGrid, PaylineWin } from "./types";

function isWild(kind: BkSymKind, cfg: BoxingKingConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatter(kind: BkSymKind, cfg: BoxingKingConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

function isPaying(kind: BkSymKind, cfg: BoxingKingConfig) {
