import type { BoxingKingConfig, BkSymKind } from "@/lib/boxing-king-config";
import type { BkGrid, InstantMixResult, WildScatterCount } from "./types";

function isWildKind(kind: BkSymKind, cfg: BoxingKingConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatterKind(kind: BkSymKind, cfg: BoxingKingConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

/** Count Wild + Scatter anywhere in view (not payline-restricted). */
export function countWildScatter(grid: BkGrid, cfg: BoxingKingConfig): WildScatterCount {
  let wild = 0;
  let scatter = 0;
 ...
 */
export function isGrandJackpot(grid: BkGrid, cfg: BoxingKingConfig): boolean {
  let cells = 0;
