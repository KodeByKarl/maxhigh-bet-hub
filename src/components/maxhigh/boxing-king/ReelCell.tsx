...
import { BoxingKingIcon } from "./BoxingKingIcon";

export type ReelPhase = "idle" | "spinning" | "stopping" | "win";

const SPIN_STRIP: BkSymKind[] = ["bar", "dice", "diamond", "chip", "lucky7", "wild", "scatter"];
...
/**
 * Boxing King cell — CSS win ring with smooth fade-out (no hard cut).
 */
      ...
      {spinning ? (
        ...
        >
          {strip.map((sym, i) => (
            ...
            >
              <BoxingKingIcon kind={sym} framed />
            </div>
          ))}
        </div>
      ) : (
