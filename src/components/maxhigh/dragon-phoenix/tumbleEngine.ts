import type { DragonPhoenixSymbolConfig } from "@/lib/dragon-phoenix-config";
import { getDragonPhoenixConfig } from "./runtimeConfig";
...
export function pickRandomSymbol(ante: boolean, isFreeSpins: boolean): DragonPhoenixSymbolConfig {
  const config = getDragonPhoenixConfig();
  let total = 0;
...
function rollGold(reelIndex: number, sym: DragonPhoenixSymbolConfig, chance: number): boolean {
