/**
 * Lechon Luck — fixed-payline slot UI.
...
import { getLechonLuckEngineConfigFn } from "@/functions/superadmin";
import {
  lechonLuckCollectFn,
  lechonLuckGambleFn,
  lechonLuckSpinFn,
  getLechonLuckSessionFn,
} from "@/functions/api";
import { ANIM } from "./lechon-luck/animationConfig";
import { lechonLuckAudio } from "./lechon-luck/audio";
import { ICON_SRC, BET_STEPS } from "./lechon-luck/paytable";
import { BetSelectModal } from "./lechon-luck/BetSelectModal";
import { getLechonLuckConfig, setLechonLuckConfig } from "./lechon-luck/runtimeConfig";
import { PaytableModal } from "./lechon-luck/PaytableModal";
import { ReelCell, type ReelPhase } from "./lechon-luck/ReelCell";
import { WinCelebration } from "./lechon-luck/WinCelebration";
import type { CnyGrid, GambleChoice, SpinScript } from "./lechon-luck/types";
import { cellKey } from "./lechon-luck/types";

const EMPTY_SET = new Set<string>();

function idleGrid(): CnyGrid {
  const cfg = getLechonLuckConfig();
...
export function LechonLuckSlot({
  ...
  useEffect(() => {
    ...
    lechonLuckAudio.preload();
    preloadAssets();
    void getLechonLuckEngineConfigFn()
      .then((cfg) => {
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      if (script.dragonBonus?.triggered) {
        ...
        for (const launch of script.dragonBonus.launches) {
          if (launch.success) {
            ...
            lechonLuckAudio.playDragonFirework();
            setDragonOverlay({
            ...
            lechonLuckAudio.playDragonBust();
      ...
      if (script.monkeyBonus?.triggered) {
        lechonLuckAudio.playMonkeyTrigger();
        setMonkeyOverlay({
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      ...
      const res = await lechonLuckSpinFn({
  ...
  const onCollect = async () => {
    ...
    try {
      lechonLuckAudio.unlock();
      lechonLuckAudio.playCollect();
      const res = await lechonLuckCollectFn({ data: { sessionId } });
      setBalanceLocal(res.balance);
      setPendingWin(0);
      setSessionId(null);
      setGambleOpen(false);
  ...
  const onGamble = async (choice: GambleChoice) => {
    ...
    try {
      lechonLuckAudio.unlock();
      lechonLuckAudio.playGambleFlip();
      const res = await lechonLuckGambleFn({ data: { sessionId, choice } });
      setBalanceLocal(res.balance);
      if (res.result.won) {
