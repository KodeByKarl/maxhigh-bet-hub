/**
 * Lucky Neko slot UI — owns its own spin state machine.
...
import { luckyNekoSpinFn, getLuckyNekoEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./lucky-neko/animationConfig";
import { MultiplierReelView } from "./lucky-neko/MultiplierReelView";
import { PaytableModal } from "./lucky-neko/PaytableModal";
import { ReelCell, type ReelPhase } from "./lucky-neko/ReelCell";
import { getLuckyNekoConfig, setLuckyNekoConfig } from "./lucky-neko/runtimeConfig";
import { planLuckyNekoPlayback, type FoPlaybackStep } from "./lucky-neko/spinPlayback";
import { cellKey, type FoGrid, type MultiplierReelResult } from "./lucky-neko/types";

function idleGrid(): FoGrid {
  const cfg = getLuckyNekoConfig();
...
export function LuckyNekoSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<LuckyNekoConfig>(() => getLuckyNekoConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await luckyNekoSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planLuckyNekoPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getLuckyNekoEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setLuckyNekoConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[LuckyNeko] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
