/**
 * Neon Makati — polished layout matching Candy Peak chrome.
...
import { getNeonMakatiEngineConfigFn } from "@/functions/superadmin";
import {
  getNeonMakatiSessionFn,
  neonMakatiBuyFeatureFn,
  neonMakatiFreeSpinFn,
  neonMakatiSpinFn,
} from "@/functions/api";
import { ANIM } from "./neon-makati/animationConfig";
import { neonMakatiAudio } from "./neon-makati/audio";
import { NeonMakatiSidePanel } from "./neon-makati/NeonMakatiSidePanel";
import { ReelCell, type ReelPhase } from "./neon-makati/ReelCell";
import { TILE_IMAGE_MAP } from "./neon-makati/NeonMakatiIcon";
import { PaytableModal } from "./neon-makati/PaytableModal";
import { getNeonMakatiConfig, setNeonMakatiConfig } from "./neon-makati/runtimeConfig";
import type { BoardCell, SpinScript } from "./neon-makati/types";

type NeonMakatiSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_NEON_MAKATI_CONFIG.symbols;
...
const NEON_MAKATI_BACKDROP = "/images/symbols/neon-makati/backdrop.webp";

function preloadAssets() {
  if (typeof Image === "undefined") return;
  const urls = [
    "/games/neon-makati.webp",
    NEON_MAKATI_BACKDROP,
...
export function NeonMakatiSlot({ onBalanceUpdate }: NeonMakatiSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getNeonMakatiConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getNeonMakatiEngineConfigFn();
      if (!mountedRef.current) return;
      setNeonMakatiConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await neonMakatiFreeSpinFn({ data: { sessionId } });
        ...
        const res = await neonMakatiSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await neonMakatiBuyFeatureFn({ data: { bet } });
