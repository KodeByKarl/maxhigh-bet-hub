/**
 * Royal Ace � polished layout matching Candy Peak chrome.
...
import { DEFAULT_ROYAL_ACE_CONFIG } from "@/lib/royal-ace-config";
import { getRoyalAceEngineConfigFn } from "@/functions/superadmin";
import {
  getRoyalAceSessionFn,
  royalAceBuyFeatureFn,
  royalAceFreeSpinFn,
  royalAceSpinFn,
} from "@/functions/api";
import { ANIM } from "./royal-ace/animationConfig";
import { royalAceAudio } from "./royal-ace/audio";
import { RoyalAceSidePanel } from "./royal-ace/RoyalAceSidePanel";
import { ReelCell, type ReelPhase } from "./royal-ace/ReelCell";
import { TILE_IMAGE_MAP } from "./royal-ace/RoyalAceIcon";
import { PaytableModal } from "./royal-ace/PaytableModal";
import { getRoyalAceConfig, setRoyalAceConfig } from "./royal-ace/runtimeConfig";
import type { BoardCell, SpinScript } from "./royal-ace/types";

type RoyalAceSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_ROYAL_ACE_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/royal-ace.webp", ...Object.values(TILE_IMAGE_MAP)];
...
export function RoyalAceSlot({ onBalanceUpdate }: RoyalAceSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getRoyalAceConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getRoyalAceEngineConfigFn();
      if (!mountedRef.current) return;
      setRoyalAceConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await royalAceFreeSpinFn({ data: { sessionId } });
        ...
        } else if (res.script.freeSpinsAwarded > 0) {
          setBanner(
            `+${res.script.freeSpinsAwarded} Retrigger � ${res.session.freeSpinsLeft} left`,
          );
          toast.success(
            `Retrigger +${res.script.freeSpinsAwarded} � ${res.session.freeSpinsLeft} Free Spins remaining`,
          );
        }
      } else {
        const res = await royalAceSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await royalAceBuyFeatureFn({ data: { bet } });
            ...
            >
              ...
              >
                ...
                <p className="mb-3 text-xs text-amber-100/80">
                  ...
                  {engineCfg.freeSpinsRetriggerCount} on retrigger) with progressive cascade
                  multipliers (
