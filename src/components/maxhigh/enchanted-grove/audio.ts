/**
 * Enchanted Grove — Web Audio sound design.
 ...
 */
const BASE = "/sounds/enchanted-grove";

export const ENCHANTED_GROVE_SOUND_FILES = {
...
type SoundId = keyof typeof ENCHANTED_GROVE_SOUND_FILES;

const MUTE_KEY = "enchanted-grove-muted";
const VOL_KEY = "enchanted-grove-volume";

type ProgressCb = (loaded: number, total: number) => void;

class EnchantedGroveAudio {
  ...
  get isReady() {
    return this.buffers.size === Object.keys(ENCHANTED_GROVE_SOUND_FILES).length;
  }
  ...
  private async loadAll(onProgress?: ProgressCb) {
    const entries = Object.entries(ENCHANTED_GROVE_SOUND_FILES) as [SoundId, string][];
...
export const enchantedGroveAudio = new EnchantedGroveAudio();
