...
import { getHoneyHiveConfig } from "./runtimeConfig";
import type { BoardCell, ClusterWin } from "./types";
import { CELLS, COLS, cellIndex, colHeight, colStart } from "./types";
...
export function evaluateBoard(board: BoardCell[], bet: number): EvalResult {
  const minCluster = getHoneyHiveConfig().minCluster;
...
/** Gravity on honeycomb nest pillars (5-6-7-6-5). */
