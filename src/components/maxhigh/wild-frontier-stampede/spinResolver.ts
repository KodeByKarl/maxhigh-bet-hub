import { calcFreeSpinsAward } from "@/lib/buffalo-reign-config";
import { createRng, newSpinSeed } from "./rng";
import { getBuffaloReignConfig } from "./runtimeConfig";
...
/**
 * Server-authoritative Wild Frontier Stampede spin:
 * ways cascades → Golden Campfire FS → Treasure Chest Hold & Collect → max-win cap.
 */
export function resolveBuffaloSpin(opts: {
  bet: number;
  ante?: boolean;
  isFreeSpins?: boolean;
