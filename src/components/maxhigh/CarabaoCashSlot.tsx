/**
 * Carabao Cash � polished layout matching Candy Peak chrome.
...
import { getCarabaoCashEngineConfigFn } from "@/functions/superadmin";
import {
  getCarabaoCashSessionFn,
  carabaoCashBuyFeatureFn,
  carabaoCashFreeSpinFn,
  carabaoCashSpinFn,
} from "@/functions/api";
import { ANIM } from "./carabao-cash/animationConfig";
import { carabaoCashAudio } from "./carabao-cash/audio";
import { CARABAO_TILE_IMAGE_MAP } from "./carabao-cash/CarabaoCashIcon";
import { CarabaoCashSidePanel } from "./carabao-cash/CarabaoCashSidePanel";
import { ReelCell, type ReelPhase } from "./carabao-cash/ReelCell";
import { getCarabaoCashConfig, setCarabaoCashConfig } from "./carabao-cash/runtimeConfig";
import type { BoardCell, SpinScript } from "./carabao-cash/types";

type CarabaoCashSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_CARABAO_CASH_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/carabao-cash.webp", ...Object.values(CARABAO_TILE_IMAGE_MAP)];
...
export function CarabaoCashSlot({ onBalanceUpdate }: CarabaoCashSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getCarabaoCashConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getCarabaoCashEngineConfigFn();
      if (!mountedRef.current) return;
      setCarabaoCashConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      setPhase("dropping");
      carabaoCashAudio.unlock();
      carabaoCashAudio.playSpin();
      setBoard([]);
      await wait(32, gen);
      ...
      if (gen === playbackGen.current) {
        if (script.treasureChest && script.treasureChest.totalWin > 0) {
          if (script.treasureChest.legendAwarded) carabaoCashAudio.playJackpot();
          else carabaoCashAudio.playTreasureChest();
          setBanner(
        ...
        if (script.freeSpinsAwarded > 0) {
          carabaoCashAudio.playFreeSpins();
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await carabaoCashFreeSpinFn({ data: { sessionId } });
        ...
        const res = await carabaoCashSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await carabaoCashBuyFeatureFn({ data: { bet } });
