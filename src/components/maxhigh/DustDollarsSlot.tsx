/**
 * Dust & Dollars — western Hold & Win slot UI.
...
import { getDustDollarsEngineConfigFn } from "@/functions/superadmin";
import {
  dustDollarsBuyFeatureFn,
  dustDollarsFreeSpinFn,
  dustDollarsSpinFn,
  getDustDollarsSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./dust-dollars/ReelCell";
import { WinCelebration } from "./dust-dollars/WinCelebration";
import { ANIM } from "./dust-dollars/animationConfig";
import { dustDollarsAudio } from "./dust-dollars/audio";
import { getDustDollarsConfig, setDustDollarsConfig } from "./dust-dollars/runtimeConfig";
import type { FgGrid, SpinScript } from "./dust-dollars/types";
import { cellKey } from "./dust-dollars/types";
import type { FgSymKind } from "@/lib/dust-dollars-config";
import { TILE_IMAGE_MAP } from "./dust-dollars/DustDollarsIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_DUST_DOLLARS_CONFIG.reelHeights;
  const syms = DEFAULT_DUST_DOLLARS_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function DustDollarsSlot() {
  ...
  const cfg = getDustDollarsConfig();
  void cfgTick;
  ...
  useEffect(() => {
    ...
    dustDollarsAudio.preload();
    preloadAssets();
    void getDustDollarsEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setDustDollarsConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getDustDollarsConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      dustDollarsAudio.unlock();
      dustDollarsAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      dustDollarsAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) dustDollarsAudio.playCoinLand();
      ...
      if ((script.freeSpinsAwarded ?? 0) > 0) {
        setPhase("win");
        dustDollarsAudio.playFreeSpins();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await dustDollarsFreeSpinFn({ data: { sessionId } });
        ...
        const res = await dustDollarsSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  async function handleBuy() {
    ...
    try {
      dustDollarsAudio.unlock();
      dustDollarsAudio.playUiClick();
      const res = await dustDollarsBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    dustDollarsAudio.unlock();
    setMuted(dustDollarsAudio.toggleMute());
  }
    ...
    <div className="relative h-full w-full overflow-hidden select-none">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Dust & Dollars · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
