/**
 * Cash Mania slot UI — owns its own spin state machine.
...
import { cashManiaSpinFn, getCashManiaEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM } from "./cash-mania/animationConfig";
import { MoneyBorder } from "./cash-mania/MoneyBorder";
import { ReelCell, type ReelPhase } from "./cash-mania/ReelCell";
import { getCashManiaConfig, setCashManiaConfig } from "./cash-mania/runtimeConfig";
import { planCashManiaPlayback, type McPlaybackStep } from "./cash-mania/spinPlayback";
import { cellKey, type McGrid } from "./cash-mania/types";

function idleGrid(): McGrid {
  const cfg = getCashManiaConfig();
...
export function CashManiaSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<CashManiaConfig>(() => getCashManiaConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await cashManiaSpinFn({ data: { bet: stake } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const cfg = getCashManiaConfig();
        const steps = planCashManiaPlayback(res.script, cfg.reelsCount, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getCashManiaEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setCashManiaConfig(cfg);
        setCfgSnap(next);
        setUi((prev) =>
          prev.locked
            ? prev
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[CashMania] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#120806] text-white">
      ...
      <div
        ...
        style={{ backgroundImage: "url(/images/cash-mania-bg.webp)" }}
      />
      <div
