...
import type { CrazySevensConfig } from "@/lib/crazy-sevens-config";
...
export type ResolveCrazySevensSpinOpts = {
  totalBet: number;
  seed?: string;
...
export function resolveCrazySevensSpin(opts: ResolveCrazySevensSpinOpts): SpinScript {
  const cfg = opts.cfg ?? getCrazySevensConfig();
