import { betPerLine, type CnySymKind } from "@/lib/fiesta-fireworks-config";
 ...
 */
export function resolveFiestaFireworksSpin(opts: {
  totalBet: number;
  isFreeSpins?: boolean;
  ...
  if (!disableFeatures && !hitCap && detectMonkeyTrigger(grid)) {
    ...
    for (let i = 0; i < monkeyBonus.freeSpinsAwarded && !hitCap; i++) {
      ...
      const fsScript = resolveFiestaFireworksSpin({
        totalBet,
        isFreeSpins: true,
...
export const resolveSpinScript = (bet: number, _ante = false, isFreeSpins = false) =>
  resolveFiestaFireworksSpin({ totalBet: bet, isFreeSpins });
