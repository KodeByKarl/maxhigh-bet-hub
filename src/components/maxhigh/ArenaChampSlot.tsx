/**
 * Arena Champ slot UI — owns its own spin state machine.
...
import { arenaChampSpinFn, getArenaChampEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM } from "./arena-champ/animationConfig";
import { ArenaChampBorder } from "./arena-champ/ArenaChampBorder";
import { ReelCell, type ReelPhase } from "./arena-champ/ReelCell";
import { getArenaChampConfig, setArenaChampConfig } from "./arena-champ/runtimeConfig";
import { planArenaChampPlayback, type GrPlaybackStep } from "./arena-champ/spinPlayback";
import { cellKey, type GrGrid } from "./arena-champ/types";

function idleGrid(): GrGrid {
  const cfg = getArenaChampConfig();
  const kinds: GrSymKind[] = ["bar", "dice", "diamond", "chip", "lucky7"];
...
export function ArenaChampSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<ArenaChampConfig>(() => getArenaChampConfig());
  ...
  const runStep = useCallback(
    (runId: number, steps: GrPlaybackStep[], index: number) => {
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await arenaChampSpinFn({ data: { bet: stake } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const cfg = getArenaChampConfig();
        const steps = planArenaChampPlayback(res.script, cfg.reelsCount, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getArenaChampEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setArenaChampConfig(cfg);
        setCfgSnap(next);
        setUi((prev) =>
          prev.locked
            ? prev
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[ArenaChamp] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#120806] text-white">
      ...
      <div
        ...
        style={{ backgroundImage: "url(/images/arena-champ-bg.png)" }}
      />
      <div
      ...
      <header className="relative z-10 flex shrink-0 flex-col items-center gap-2.5 px-3 pt-3 sm:pt-4">
        <ArenaChampBorder rounded="trap" thickness={3} className="w-full max-w-md">
        ...
        <div className="flex w-full max-w-md items-center justify-center gap-3">
          <ArenaChampBorder rounded="xl" thickness={2} className="min-w-[8.5rem] flex-1">
      ...
      <footer className="relative z-10 shrink-0 px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
        <ArenaChampBorder rounded="2xl" thickness={3} className="mx-auto w-full max-w-lg">
