import { calcFreeSpinsAward } from "@/lib/carabao-cash-config";
import { createRng, newSpinSeed } from "./rng";
import { getCarabaoCashConfig } from "./runtimeConfig";
...
/**
 * Server-authoritative Carabao Cash spin:
 * ways cascades → Golden Campfire FS → Treasure Chest Hold & Collect → max-win cap.
 */
export function resolveCarabaoCashSpin(opts: {
  bet: number;
  ante?: boolean;
  isFreeSpins?: boolean;
  ...
  const config = getCarabaoCashConfig();
  const bet = opts.bet;
