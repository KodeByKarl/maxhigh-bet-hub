/**
 * Cleopatra Gold — polished layout matching Candy Peak chrome.
...
import { getCleopatraGoldEngineConfigFn } from "@/functions/superadmin";
import {
  getCleopatraGoldSessionFn,
  cleopatraGoldBuyFeatureFn,
  cleopatraGoldFreeSpinFn,
  cleopatraGoldSpinFn,
} from "@/functions/api";
import { ANIM } from "./cleopatra-gold/animationConfig";
import { cleopatraAudio } from "./cleopatra-gold/audio";
import { CLEO_TILE_IMAGE_MAP } from "./cleopatra-gold/CleopatraIcon";
import { CleopatraSidePanel } from "./cleopatra-gold/CleopatraSidePanel";
import { ReelCell, type ReelPhase } from "./cleopatra-gold/ReelCell";
import { getCleopatraGoldConfig, setCleopatraGoldConfig } from "./cleopatra-gold/runtimeConfig";
import type { BoardCell, SpinScript } from "./cleopatra-gold/types";

type CleopatraGoldSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_CLEOPATRA_GOLD_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/cleopatra-gold.webp", ...Object.values(CLEO_TILE_IMAGE_MAP)];
...
export function CleopatraGoldSlot({ onBalanceUpdate }: CleopatraGoldSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getCleopatraGoldConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getCleopatraGoldEngineConfigFn();
      if (!mountedRef.current) return;
      setCleopatraGoldConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      setPhase("dropping");
      cleopatraAudio.unlock();
      cleopatraAudio.playSpin();
      setBoard([]);
      await wait(32, gen);
      ...
      if (gen === playbackGen.current) {
        if (script.treasureChest && script.treasureChest.totalWin > 0) {
          if (script.treasureChest.legendAwarded) cleopatraAudio.playJackpot();
          else cleopatraAudio.playTreasureChest();
          setBanner(
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await cleopatraGoldFreeSpinFn({ data: { sessionId } });
        ...
        const res = await cleopatraGoldSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await cleopatraGoldBuyFeatureFn({ data: { bet } });
