import { betPerLine, type ProsperityLionConfig, type FtSymKind } from "@/lib/prosperity-lion-config";
import type { FtGrid, PaylineWin } from "./types";

function isWild(kind: FtSymKind, cfg: ProsperityLionConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FtSymKind, cfg: ProsperityLionConfig) {
...
export function evaluatePaylines(
  ...
  for (let lineIndex = 0; lineIndex < cfg.paylineCount; lineIndex++) {
    ...
    let target: FtSymKind | null = null;
