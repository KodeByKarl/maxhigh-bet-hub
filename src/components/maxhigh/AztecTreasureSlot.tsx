...
import { pantherAudio } from "./aztec-treasure/audio";
import {
  aztecTreasureBuyFeatureFn,
  aztecTreasureFreeSpinFn,
  aztecTreasureSpinFn,
  getAztecTreasureSessionFn,
} from "@/functions/api";
import { ANIM } from "./aztec-treasure/animationConfig";
import { AztecFeatureBadge } from "./aztec-treasure/AztecFeatureBadge";
import { FreeSpinsBadge } from "./aztec-treasure/FreeSpinsBadge";
import { BetSelectModal } from "./aztec-treasure/BetSelectModal";
import { initialBoard, nextKey, buildBoard } from "./aztec-treasure/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./aztec-treasure/paytable";
import { getRuntimeSymbols, setAztecTreasureConfig } from "./aztec-treasure/runtimeConfig";
import type { BoardCell, SpinScript } from "./aztec-treasure/types";
import { CELLS, COLS, MAIN_CELLS, ROWS, TOP_COLS } from "./aztec-treasure/types";
import { WinCelebration } from "./aztec-treasure/WinCelebration";
import { FreeSpinsCongrats } from "./aztec-treasure/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./aztec-treasure/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./aztec-treasure/BuyFeatureModal";
import { PaytableModal } from "./aztec-treasure/PaytableModal";
import { GameMenuModal } from "./aztec-treasure/GameMenuModal";
import { AutoSpinModal, type AutoSpinOptions } from "./aztec-treasure/AutoSpinModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./aztec-treasure/WinLedger";
import { ReelCell, type ReelPhase } from "./aztec-treasure/ReelCell";
import { getAztecTreasureEngineConfigFn } from "@/functions/superadmin";
...
export function AztecTreasureSlot({
  ...
  useEffect(() => {
    ...
    preloadAssets();
    pantherAudio.preload();
    if (gameId === AZTEC_TREASURE_GAME_ID || gameId === "aztec-treasure") {
      void getAztecTreasureEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setAztecTreasureConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getAztecTreasureSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== AZTEC_TREASURE_GAME_ID && gameId !== "aztec-treasure") return;
    try {
      const cfg = await getAztecTreasureEngineConfigFn();
      if (mountedRef.current) setAztecTreasureConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getAztecTreasureSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof aztecTreasureSpinFn>>;
        if (isFree) {
          ...
          settled = await aztecTreasureFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await aztecTreasureSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[AztecTreasure] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await aztecTreasureBuyFeatureFn({
