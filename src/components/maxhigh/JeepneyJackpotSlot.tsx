/**
 * Jeepney Jackpot — western Hold & Win slot UI.
...
import { getJeepneyJackpotEngineConfigFn } from "@/functions/superadmin";
import {
  jeepneyJackpotBuyFeatureFn,
  jeepneyJackpotFreeSpinFn,
  jeepneyJackpotSpinFn,
  getJeepneyJackpotSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./jeepney-jackpot/ReelCell";
import { WinCelebration } from "./jeepney-jackpot/WinCelebration";
import { ANIM } from "./jeepney-jackpot/animationConfig";
import { jeepneyJackpotAudio } from "./jeepney-jackpot/audio";
import { getJeepneyJackpotConfig, setJeepneyJackpotConfig } from "./jeepney-jackpot/runtimeConfig";
import type { FgGrid, SpinScript } from "./jeepney-jackpot/types";
import { cellKey } from "./jeepney-jackpot/types";
import type { FgSymKind } from "@/lib/jeepney-jackpot-config";
import { TILE_IMAGE_MAP } from "./jeepney-jackpot/JeepneyJackpotIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_JEEPNEY_JACKPOT_CONFIG.reelHeights;
  const syms = DEFAULT_JEEPNEY_JACKPOT_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function JeepneyJackpotSlot() {
  ...
  const cfg = getJeepneyJackpotConfig();
  void cfgTick;
  ...
  useEffect(() => {
    ...
    jeepneyJackpotAudio.preload();
    preloadAssets();
    void getJeepneyJackpotEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setJeepneyJackpotConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
    ...
    return () => {
      ...
      jeepneyJackpotAudio.stopAmbient();
    };
  }, []);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getJeepneyJackpotConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      jeepneyJackpotAudio.unlock();
      jeepneyJackpotAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      jeepneyJackpotAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        setWinPopup(null);
        setBanner("HOLD & WIN!");
        jeepneyJackpotAudio.playHoldWin();
        setHoldOverlay({
          ...holdWin,
          steps: [
            {
        ...
        jeepneyJackpotAudio.playCoinLock();
        await wait(500, gen);
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) jeepneyJackpotAudio.playCoinLand();
        ...
        if (holdWin.grandAwarded) {
          jeepneyJackpotAudio.playJackpot();
      ...
      if ((script.freeSpinsAwarded ?? 0) > 0) {
        setPhase("win");
        jeepneyJackpotAudio.playFreeSpins();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await jeepneyJackpotFreeSpinFn({ data: { sessionId } });
        ...
        const res = await jeepneyJackpotSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
      ...
      jeepneyJackpotAudio.stopSpinLoop();
  ...
  async function handleBuy() {
    ...
    try {
      jeepneyJackpotAudio.unlock();
      jeepneyJackpotAudio.playUiClick();
      const res = await jeepneyJackpotBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    jeepneyJackpotAudio.unlock();
    setMuted(jeepneyJackpotAudio.toggleMute());
  }
  ...
  return (
    <div className="relative h-full w-full overflow-hidden select-none">
      <img
        src="/games/jeepney-jackpot.webp"
        alt=""
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Jeepney Jackpot · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
