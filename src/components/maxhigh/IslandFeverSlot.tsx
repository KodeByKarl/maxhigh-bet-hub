/**
 * Island Fever — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  islandFeverBuyFeatureFn,
  islandFeverFreeSpinFn,
  islandFeverSpinFn,
  getIslandFeverSessionFn,
} from "@/functions/api";
import { ANIM } from "./island-fever/animationConfig";
import { IslandFeverFeatureBadge } from "./island-fever/IslandFeverFeatureBadge";
import { FreeSpinsBadge } from "./island-fever/FreeSpinsBadge";
import { IslandFeverSidePanel } from "./island-fever/IslandFeverSidePanel";
import { initialBoard, nextKey, buildBoard } from "./island-fever/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./island-fever/paytable";
import { getRuntimeSymbols, setIslandFeverConfig } from "./island-fever/runtimeConfig";
import type { BoardCell, SpinScript } from "./island-fever/types";
import { CELLS, COLS, ROWS } from "./island-fever/types";
import { WinCelebration } from "./island-fever/WinCelebration";
import { FreeSpinsCongrats } from "./island-fever/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./island-fever/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./island-fever/BuyFeatureModal";
import { PaytableModal } from "./island-fever/PaytableModal";
import { GameMenuModal } from "./island-fever/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./island-fever/WinLedger";
import { ReelCell, type ReelPhase } from "./island-fever/ReelCell";
import { getIslandFeverEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  ...
  const urls = [
    "/images/symbols/island-fever/backdrop.webp",
    "/images/symbols/island-fever/lollipop.png",
...
export function IslandFeverSlot({
  gameId = "island-fever",
}: {
  gameId?: string;
  gameName?: string;
  ...
  useEffect(() => {
    ...
    preloadAssets();
    sounds.preload();
    if (gameId === ISLAND_FEVER_GAME_ID || gameId === "island-fever") {
      void getIslandFeverEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setIslandFeverConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getIslandFeverSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== ISLAND_FEVER_GAME_ID && gameId !== "island-fever") return;
    try {
      const cfg = await getIslandFeverEngineConfigFn();
      if (mountedRef.current) setIslandFeverConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof islandFeverSpinFn>>;
        if (isFree) {
          ...
          settled = await islandFeverFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await islandFeverSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[IslandFever] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await islandFeverBuyFeatureFn({
  ...
  return (
    <div className="relative flex h-dvh w-full flex-col overflow-hidden select-none">
      <img
        src="/images/symbols/island-fever/backdrop.webp"
        alt=""
            ...
            <div className="flex h-full min-h-0 min-w-0 w-full flex-col items-center">
              ...
              >
                ...
                >
                  ...
                  <img
                    src="/images/symbols/island-fever/lollipop.png"
                    alt=""
