/**
 * Starlight Ways — polished layout matching Candy Peak chrome.
...
import { getStarlightWaysEngineConfigFn } from "@/functions/superadmin";
import {
  getStarlightWaysSessionFn,
  starlightWaysBuyFeatureFn,
  starlightWaysFreeSpinFn,
  starlightWaysSpinFn,
} from "@/functions/api";
import { ANIM } from "./starlight-ways/animationConfig";
import { starlightWaysAudio } from "./starlight-ways/audio";
import { StarlightWaysSidePanel } from "./starlight-ways/StarlightWaysSidePanel";
import { ReelCell, type ReelPhase } from "./starlight-ways/ReelCell";
import { TILE_IMAGE_MAP } from "./starlight-ways/StarlightWaysIcon";
import { PaytableModal } from "./starlight-ways/PaytableModal";
import { getStarlightWaysConfig, setStarlightWaysConfig } from "./starlight-ways/runtimeConfig";
import type { BoardCell, SpinScript } from "./starlight-ways/types";

type StarlightWaysSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_STARLIGHT_WAYS_CONFIG.symbols;
...
const STARLIGHT_WAYS_BACKDROP = "/images/symbols/starlight-ways/backdrop.webp";

function preloadAssets() {
  if (typeof Image === "undefined") return;
  const urls = [
    "/games/starlight-ways.webp",
    STARLIGHT_WAYS_BACKDROP,
...
export function StarlightWaysSlot({ onBalanceUpdate }: StarlightWaysSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getStarlightWaysConfig();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    starlightWaysAudio.preload();
    void getStarlightWaysEngineConfigFn()
      .then((cfg) => {
    ...
    return () => {
      ...
      starlightWaysAudio.stopSpinLoop();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getStarlightWaysEngineConfigFn();
      if (!mountedRef.current) return;
      setStarlightWaysConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      starlightWaysAudio.playReelStop();
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await starlightWaysFreeSpinFn({ data: { sessionId } });
        ...
        const res = await starlightWaysSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await starlightWaysBuyFeatureFn({ data: { bet } });
