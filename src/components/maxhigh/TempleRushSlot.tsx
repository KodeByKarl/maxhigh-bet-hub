/**
 * Temple Rush — Sweet Bonanza layout (cluster-pay tumble slot).
...
import { templeRushAudio } from "./temple-rush/audio";
import {
  templeRushBuyFeatureFn,
  templeRushFreeSpinFn,
  templeRushSpinFn,
  getTempleRushSessionFn,
} from "@/functions/api";
import { ANIM } from "./temple-rush/animationConfig";
import { TempleRushFeatureBadge } from "./temple-rush/TempleRushFeatureBadge";
import { FreeSpinsBadge } from "./temple-rush/FreeSpinsBadge";
import { BetSelectModal } from "./temple-rush/BetSelectModal";
import { initialBoard, nextKey, buildBoard } from "./temple-rush/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./temple-rush/paytable";
import { getRuntimeSymbols, setTempleRushConfig } from "./temple-rush/runtimeConfig";
import type { BoardCell, SpinScript } from "./temple-rush/types";
import { CELLS, COLS, MAIN_CELLS, ROWS, TOP_COLS } from "./temple-rush/types";
import { WinCelebration } from "./temple-rush/WinCelebration";
import { FreeSpinsCongrats } from "./temple-rush/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./temple-rush/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./temple-rush/BuyFeatureModal";
import { PaytableModal } from "./temple-rush/PaytableModal";
import { GameMenuModal } from "./temple-rush/GameMenuModal";
import { AutoSpinModal, type AutoSpinOptions } from "./temple-rush/AutoSpinModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./temple-rush/WinLedger";
import { ReelCell, type ReelPhase } from "./temple-rush/ReelCell";
import { getTempleRushEngineConfigFn } from "@/functions/superadmin";
...
export function TempleRushSlot({
  gameId = "temple-rush",
}: {
  gameId?: string;
  gameName?: string;
  ...
  const [muted, setMuted] = useState(() => templeRushAudio.isMuted);
  ...
  useEffect(() => {
    ...
    preloadAssets();
    templeRushAudio.preload();
    if (gameId === TEMPLE_RUSH_GAME_ID || gameId === "temple-rush") {
      void getTempleRushEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setTempleRushConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getTempleRushSessionFn()
        .then((session) => {
    ...
    return () => {
      ...
      clearTrackedTimers();
      templeRushAudio.stopSpinLoop();
      templeRushAudio.stopAmbient();
    };
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== TEMPLE_RUSH_GAME_ID && gameId !== "temple-rush") return;
    try {
      const cfg = await getTempleRushEngineConfigFn();
      if (mountedRef.current) setTempleRushConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        templeRushAudio.playCascadeTick();
        await wait(
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        templeRushAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }

      if (session.inFree && session.freeSpinsLeft > 0) {
        templeRushAudio.playScatterTrigger();
        applySession({
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getTempleRushSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof templeRushSpinFn>>;
        if (isFree) {
          ...
          settled = await templeRushFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await templeRushSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[TempleRush] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await templeRushBuyFeatureFn({
