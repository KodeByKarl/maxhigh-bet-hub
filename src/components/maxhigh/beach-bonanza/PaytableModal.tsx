...
import { BeachBonanzaIcon } from "./BeachBonanzaIcon";
import { getRuntimeSymbols } from "./runtimeConfig";
import { getBeachBonanzaConfig } from "./runtimeConfig";
...
export function PaytableModal({ bet, onClose }: Props) {
  const cfg = getBeachBonanzaConfig();
        ...
        >
          <div className="flex shrink-0 items-center justify-between border-b border-white/15 px-4 py-3">
            <div>
              ...
              <div className="text-[11px] font-bold text-white/75">
                Cluster pays · {minCluster}+ adjacent (H/V)
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-3">
            <div className="rounded-xl bg-black/25 px-3 py-2 text-[11px] font-semibold leading-relaxed text-white/90">
              Cluster pays: {minCluster}+ matching symbols connected horizontally or
              vertically (no diagonals). Winners tumble away; position multipliers
              appear (×2 → ×128) and sum at the end of the cascade.{" "}
              {cfg.freeSpinsTriggerCount}+ rockets award Free Spins; multipliers
              persist during Free Spins.
            </div>
            ...
            <div className="grid gap-2">
              {pays.map((sym) => (
                ...
                >
                  <BeachBonanzaIcon kind={sym.kind} className="size-10 shrink-0" />
            ...
            <div className="flex items-center gap-3 rounded-xl border border-white/15 bg-white/10 px-2.5 py-2">
              <BeachBonanzaIcon kind="lollipop" className="size-10 shrink-0" />
              <div className="text-[11px] font-bold leading-snug text-white">
                <span className="text-[#F5D76E]">Scatter</span> — {cfg.freeSpinsTriggerCount}+ rockets award Free
                Spins. Pays cash by rocket count.
              </div>
            </div>

            <div className="flex items-center gap-3 rounded-xl border border-white/15 bg-white/10 px-2.5 py-2">
              <BeachBonanzaIcon kind="bomb" mult={2} className="size-10 shrink-0" />
              <div className="text-[11px] font-bold leading-snug text-white">
                <span className="text-[#F5D76E]">Position multipliers</span> — winning
                cells leave ×2…×128 markers. All markers on the board are added
                together and multiply the cascade win. Persist in Free Spins.
              </div>
            </div>
          </div>
        </div>
