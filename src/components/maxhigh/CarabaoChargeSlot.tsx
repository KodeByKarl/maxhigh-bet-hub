/**
 * Carabao Charge � polished layout matching Candy Peak chrome.
...
import { getCarabaoChargeEngineConfigFn } from "@/functions/superadmin";
import {
  getCarabaoChargeSessionFn,
  carabaoChargeBuyFeatureFn,
  carabaoChargeFreeSpinFn,
  carabaoChargeSpinFn,
} from "@/functions/api";
import { ANIM } from "./carabao-charge/animationConfig";
import { carabaoChargeAudio } from "./carabao-charge/audio";
import { CARABAO_TILE_IMAGE_MAP } from "./carabao-charge/CarabaoChargeIcon";
import { CarabaoChargeSidePanel } from "./carabao-charge/CarabaoChargeSidePanel";
import { ReelCell, type ReelPhase } from "./carabao-charge/ReelCell";
import { getCarabaoChargeConfig, setCarabaoChargeConfig } from "./carabao-charge/runtimeConfig";
import type { BoardCell, SpinScript } from "./carabao-charge/types";

type CarabaoChargeSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_CARABAO_CHARGE_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/carabao-charge.webp", ...Object.values(CARABAO_TILE_IMAGE_MAP)];
...
export function CarabaoChargeSlot({ onBalanceUpdate }: CarabaoChargeSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getCarabaoChargeConfig();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      carabaoChargeAudio.stopSpinLoop();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getCarabaoChargeEngineConfigFn();
      if (!mountedRef.current) return;
      setCarabaoChargeConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      setPhase("dropping");
      carabaoChargeAudio.unlock();
      carabaoChargeAudio.playSpin();
      setBoard([]);
      await wait(32, gen);
      ...
      carabaoChargeAudio.playReelStop();
      ...
      if (gen === playbackGen.current) {
        if (script.treasureChest && script.treasureChest.totalWin > 0) {
          if (script.treasureChest.legendAwarded) carabaoChargeAudio.playJackpot();
          else carabaoChargeAudio.playTreasureChest();
          setBanner(
        ...
        if (script.freeSpinsAwarded > 0) {
          carabaoChargeAudio.playFreeSpins();
  ...
  const handleSpin = useCallback(async () => {
    ...
    carabaoChargeAudio.unlock();
    carabaoChargeAudio.playSpin();
    carabaoChargeAudio.playUiClick();
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await carabaoChargeFreeSpinFn({ data: { sessionId } });
        ...
        const res = await carabaoChargeSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await carabaoChargeBuyFeatureFn({ data: { bet } });
