/**
 * Olympus Wrath — Web Audio sound design.
 ...
 */
const BASE = "/sounds/olympus-wrath";

export const GE_SOUND_FILES = {
...
type SoundId = keyof typeof GE_SOUND_FILES;

const MUTE_KEY = "olympus-wrath-muted";
const VOL_KEY = "olympus-wrath-volume";

type ProgressCb = (loaded: number, total: number) => void;

class GoldenEmpireAudio {
  ...
  get isReady() {
    return this.buffers.size === Object.keys(GE_SOUND_FILES).length;
  }
  ...
  private async loadAll(onProgress?: ProgressCb) {
    const entries = Object.entries(GE_SOUND_FILES) as [SoundId, string][];
...
export const geAudio = new GoldenEmpireAudio();
