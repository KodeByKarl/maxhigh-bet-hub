...
import { emberTigerAudio } from "./ember-tiger/audio";
import {
  emberTigerBuyFeatureFn,
  emberTigerFreeSpinFn,
  emberTigerSpinFn,
  getEmberTigerSessionFn,
} from "@/functions/api";
import { ANIM } from "./ember-tiger/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./ember-tiger/playbackHelpers";
import { BetSelectModal } from "./ember-tiger/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./ember-tiger/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
...
import { BuyFeatureModal } from "./ember-tiger/BuyFeatureModal";
import { PaytableModal } from "./ember-tiger/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./ember-tiger/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./ember-tiger/ReelGrid";
import { emberTigerTheme } from "./ember-tiger/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./ember-tiger/ReelCell";
import { getEmberTigerEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  emberTigerAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    emberTigerTheme.assets.backdrop,
    emberTigerTheme.assets.loadingBg,
    ...Object.values(emberTigerTheme.assets.icons),
  ];
...
export function EmberTigerSlot({
  gameId = "ember-tiger",
}: {
  gameId?: string;
  gameName?: string;
  ...
  const [muted, setMuted] = useState(() => emberTigerAudio.isMuted);
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      emberTigerAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        emberTigerAudio.stopSpinLoop();
        return;
      }
      emberTigerAudio.stopSpinLoop();
      emberTigerAudio.playReelStop(5);
      emberTigerAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === EMBER_TIGER_GAME_ID || gameId === "ember-tiger") {
      void getEmberTigerEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setEmberTigerConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getEmberTigerSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[EmberTiger] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== EMBER_TIGER_GAME_ID && gameId !== "ember-tiger") return;
    try {
      const cfg = await getEmberTigerEngineConfigFn();
      if (mountedRef.current) setEmberTigerConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        emberTigerAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    emberTigerAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    emberTigerAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        emberTigerAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getEmberTigerSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof emberTigerSpinFn>>;
        if (isFree) {
          ...
          settled = await emberTigerFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await emberTigerSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[EmberTiger] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await emberTigerBuyFeatureFn({
