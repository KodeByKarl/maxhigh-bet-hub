import { betPerLine, type FortuneTigerConfig, type FtSymKind } from "@/lib/fortune-tiger-config";
import type { FtGrid, PaylineWin } from "./types";

function isWild(kind: FtSymKind, cfg: FortuneTigerConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FtSymKind, cfg: FortuneTigerConfig) {
