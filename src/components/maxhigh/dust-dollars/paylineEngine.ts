/**
 * Dust & Dollars — connecting ways on a 3-4-5-4-3 diamond board.
 ...
 */
import type { FgSymKind, DustDollarsConfig } from "@/lib/dust-dollars-config";
import type { FgGrid, PaylineWin } from "./types";

function isWild(kind: FgSymKind, cfg: DustDollarsConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: DustDollarsConfig): boolean {
...
/** @deprecated Alias — Dust & Dollars now uses connecting ways. */
