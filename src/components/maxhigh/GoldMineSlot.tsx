/**
 * Gold Mine Dig — western Hold & Win slot UI.
...
import { getGoldMineEngineConfigFn } from "@/functions/superadmin";
import {
  goldMineBuyFeatureFn,
  goldMineFreeSpinFn,
  goldMineSpinFn,
  getGoldMineSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./gold-mine/ReelCell";
import { WinCelebration } from "./gold-mine/WinCelebration";
import { ANIM } from "./gold-mine/animationConfig";
import { goldMineAudio } from "./gold-mine/audio";
import { getGoldMineConfig, setGoldMineConfig } from "./gold-mine/runtimeConfig";
import type { FgGrid, SpinScript } from "./gold-mine/types";
import { cellKey } from "./gold-mine/types";
import type { FgSymKind } from "@/lib/gold-mine-config";
import { TILE_IMAGE_MAP } from "./gold-mine/GoldMineIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_GOLD_MINE_CONFIG.reelHeights;
  const syms = DEFAULT_GOLD_MINE_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function GoldMineSlot() {
  ...
  useEffect(() => {
    ...
    goldMineAudio.preload();
    preloadAssets();
    void getGoldMineEngineConfigFn()
      .then((c) => {
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getGoldMineConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      goldMineAudio.unlock();
      goldMineAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      goldMineAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) goldMineAudio.playCoinLand();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await goldMineFreeSpinFn({ data: { sessionId } });
        ...
        const res = await goldMineSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  async function handleBuy() {
    ...
    try {
      goldMineAudio.unlock();
      goldMineAudio.playUiClick();
      const res = await goldMineBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    goldMineAudio.unlock();
    setMuted(goldMineAudio.toggleMute());
  }
    ...
    <div className="relative h-full w-full overflow-hidden select-none">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Gold Mine Dig · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
