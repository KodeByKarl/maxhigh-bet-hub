...
import { createRng, type PhoenixFisherRng } from "./rng";
import { getPhoenixFisherConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: PhoenixFisherConfig, rng: PhoenixFisherRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getPhoenixFisherConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getPhoenixFisherConfig();
