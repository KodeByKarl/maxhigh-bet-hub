/**
 * Phoenix Fisher arena — continuous-fire fish hunter shell.
...
import {
  phoenixFisherBossStatusFn,
  phoenixFisherSyncSpawnsFn,
  phoenixFisherBuyPowerUpFn,
  phoenixFisherEnsureSessionFn,
  phoenixFisherFireShotFn,
  phoenixFisherSetWeaponFn,
  phoenixFisherUseFreezeLureFn,
  phoenixFisherUseNetBombFn,
  getPhoenixFisherEngineConfigFn,
} from "@/functions/phoenix-fisher";
import { cn } from "@/lib/utils";
import { SWIM } from "./animationConfig";
import { playPhoenixFisherSound, setPhoenixFisherMuted, unlockPhoenixFisherAudio } from "./audio";
import { FishSprite } from "./FishSprite";
import { getPhoenixFisherConfig, setPhoenixFisherConfig } from "./runtimeConfig";
...
export function PhoenixFisherGame({
  gameId = PHOENIX_FISHER_GAME_ID,
  gameName = "Phoenix Fisher",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<PhoenixFisherConfig>(() => getPhoenixFisherConfig());
  ...
  useEffect(() => {
    unlockPhoenixFisherAudio();
    void (async () => {
      try {
        const remote = await getPhoenixFisherEngineConfigFn();
        const next = remote ?? DEFAULT_PHOENIX_FISHER_CONFIG;
        setPhoenixFisherConfig(next);
        setCfg(next);
      } catch {
        setCfg(getPhoenixFisherConfig());
      }
      try {
        const session = await phoenixFisherEnsureSessionFn({ data: { weaponId: "bamboo" } });
        ...
        if (session.config) {
          setPhoenixFisherConfig(session.config);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        const res = await phoenixFisherSyncSpawnsFn();
        if (cancelled) return;
        setFish((prev) => {
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      void phoenixFisherBossStatusFn()
        .then((res) => {
          if (res.boss?.active) {
            setBossHud({
            ...
            setFish((prev) => {
              ...
              playPhoenixFisherSound("bossRoar");
              return [
  ...
  const fireAt = useCallback(
    async (target: ArenaFish) => {
      ...
      setFiring(true);
      unlockPhoenixFisherAudio();
      playPhoenixFisherSound("shotFire");
      try {
        const res = await phoenixFisherFireShotFn({
          data: {
            weaponId,
            target: {
        ...
        playPhoenixFisherSound(res.shot.hit ? "hit" : "splash");
        ...
        if (res.fishUpdate.killed) {
          if (target.tierId === "boss") {
            playPhoenixFisherSound("bossDeath");
          } else if (target.tierId === "elite" || target.tierId === "rare") {
            playPhoenixFisherSound("fishDeathLarge");
          } else if (target.tierId === "crate") {
            playPhoenixFisherSound("crateDrop");
          } else {
            playPhoenixFisherSound("fishDeathSmall");
          }
          if (res.killBanner && res.killBanner.credit > 0) {
            playPhoenixFisherSound("coinPayout");
  ...
  async function changeWeapon(id: WeaponTierId) {
    setWeaponId(id);
    try {
      const session = await phoenixFisherSetWeaponFn({ data: { weaponId: id } });
      syncBalance(session.balance);
    } catch (e) {
  ...
  async function buyPower(id: PowerUpId) {
    try {
      const res = await phoenixFisherBuyPowerUpFn({ data: { powerUpId: id } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
  ...
  async function useNetBomb() {
    ...
    try {
      ...
      const res = await phoenixFisherUseNetBombFn({ data: { weaponId, targets } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
      playPhoenixFisherSound("splash");
  ...
  async function useFreeze() {
    ...
    try {
      const res = await phoenixFisherUseFreezeLureFn();
      setPowerUps(res.powerUps);
...
export const PhoenixFisherArena = PhoenixFisherGame;
