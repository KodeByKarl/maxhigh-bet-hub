/**
 * Whale War arena — continuous-fire fish hunter shell.
...
import {
  whaleWarBossStatusFn,
  whaleWarSyncSpawnsFn,
  whaleWarBuyPowerUpFn,
  whaleWarEnsureSessionFn,
  whaleWarFireShotFn,
  whaleWarSetWeaponFn,
  whaleWarUseFreezeLureFn,
  whaleWarUseNetBombFn,
  getWhaleWarEngineConfigFn,
} from "@/functions/whale-war";
import { cn } from "@/lib/utils";
import { SWIM } from "./animationConfig";
import { playWhaleWarSound, setWhaleWarMuted, unlockWhaleWarAudio } from "./audio";
import { FishSprite } from "./FishSprite";
import { getWhaleWarConfig, setWhaleWarConfig } from "./runtimeConfig";
...
export function WhaleWarGame({
  gameId = WHALE_WAR_GAME_ID,
  gameName = "Whale War",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<WhaleWarConfig>(() => getWhaleWarConfig());
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const remote = await getWhaleWarEngineConfigFn();
        const next = remote ?? DEFAULT_WHALE_WAR_CONFIG;
        setWhaleWarConfig(next);
        setCfg(next);
      } catch {
        setCfg(getWhaleWarConfig());
      }
      try {
        const session = await whaleWarEnsureSessionFn({ data: { weaponId: "bamboo" } });
        ...
        if (session.config) {
          setWhaleWarConfig(session.config);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        const res = await whaleWarSyncSpawnsFn();
        if (cancelled) return;
        setFish((prev) => {
  ...
  const fireAt = useCallback(
    async (target: ArenaFish) => {
      ...
      setFiring(true);
      unlockWhaleWarAudio();
      playWhaleWarSound("shotFire");
      try {
        const res = await whaleWarFireShotFn({
          data: {
            weaponId,
            target: {
        ...
        playWhaleWarSound(res.shot.hit ? "hit" : "splash");
        ...
        if (res.fishUpdate.killed) {
          if (target.tierId === "boss") {
            playWhaleWarSound("bossDeath");
          } else if (target.tierId === "elite" || target.tierId === "rare") {
            playWhaleWarSound("fishDeathLarge");
          } else if (target.tierId === "crate") {
            playWhaleWarSound("crateDrop");
          } else {
            playWhaleWarSound("fishDeathSmall");
          }
          if (res.killBanner && res.killBanner.credit > 0) {
            playWhaleWarSound("coinPayout");
  ...
  async function changeWeapon(id: WeaponTierId) {
    setWeaponId(id);
    try {
      const session = await whaleWarSetWeaponFn({ data: { weaponId: id } });
      syncBalance(session.balance);
    } catch (e) {
  ...
  async function buyPower(id: PowerUpId) {
    try {
      const res = await whaleWarBuyPowerUpFn({ data: { powerUpId: id } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
  ...
  async function useNetBomb() {
    ...
    try {
      ...
      const res = await whaleWarUseNetBombFn({ data: { weaponId, targets } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
      playWhaleWarSound("splash");
  ...
  async function useFreeze() {
    ...
    try {
      const res = await whaleWarUseFreezeLureFn();
      setPowerUps(res.powerUps);
...
export const WhaleWarArena = WhaleWarGame;
