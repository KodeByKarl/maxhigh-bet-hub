...
import { createRng, type WhaleWarRng } from "./rng";
import { getWhaleWarConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: WhaleWarConfig, rng: WhaleWarRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getWhaleWarConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getWhaleWarConfig();
