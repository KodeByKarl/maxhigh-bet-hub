...
import { getPokerShowdownConfig, setPokerShowdownConfig } from "./runtimeConfig";
...
/**
 * Poker Showdown — Ante/Play + Pair Plus with a mid-round Play/Fold decision.
 */
export function PokerShowdownTable({
  ...
  const [cfg, setCfg] = useState<PokerShowdownConfig>(() => getPokerShowdownConfig());
  const [ante, setAnte] = useState(DEFAULT_POKER_SHOWDOWN_CONFIG.minAnteBet);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const remote = await getPokerShowdownEngineConfigFn();
        if (cancelled) return;
        setPokerShowdownConfig(remote);
        setCfg(remote);
        setAnte(remote.minAnteBet);
        setSelectedChip(remote.betSteps.find((v) => v >= 5) ?? remote.betSteps[0] ?? 5);
      } catch {
        setCfg(DEFAULT_POKER_SHOWDOWN_CONFIG);
      }

      try {
        const open = await getPokerShowdownSessionFn();
  ...
  async function onDeal() {
    ...
    setBusy(true);
    try {
      const res = await pokerShowdownDealFn({
        data: { ante, pairPlus },
      });
  ...
  async function onPlay() {
    ...
    setBusy(true);
    try {
      const res = await pokerShowdownPlayFn({ data: { sessionId } });
      syncBalance(res.balance);
  ...
  async function onFold() {
    ...
    setBusy(true);
    try {
      const res = await pokerShowdownFoldFn({ data: { sessionId } });
      syncBalance(res.balance);
      await playSettleReveal(res.script);
    } catch (e) {
