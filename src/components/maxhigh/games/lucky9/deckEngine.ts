...
export function freshShuffledShoe(cfg: Lucky9Config, rng: L9Rng): ShoeState {
...
export function needsReshuffle(shoe: ShoeState, cfg: Lucky9Config): boolean {
  const total = shoe.cards.length;
  if (total <= 0) return true;
  return remaining(shoe) / total <= cfg.reshuffleBelowFraction;
}

export function ensureShoe(shoe: ShoeState | null, cfg: Lucky9Config, rng: L9Rng): ShoeState {
