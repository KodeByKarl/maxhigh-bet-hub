...
import { lucky9DealFn, getLucky9EngineConfigFn } from "@/functions/lucky9";
import {
  LUCKY9_GAME_ID,
...
import { playLucky9Sound, unlockLucky9Audio } from "./audio";
import {
  getLucky9Anim,
  preloadLucky9CriticalAssets,
  type L9AnimProfile,
  type L9BetSpot,
} from "./animationConfig";
import { getLucky9Config, setLucky9Config } from "./runtimeConfig";
...
/**
 * Lucky 9 Arena — oval felt race-to-9.
 * Intentionally unlike Baccarat (no side-by-side panels, no RESULT box, no side rail).
 */
export function Lucky9Table({
  gameId = LUCKY9_GAME_ID,
  gameName = "Lucky 9",
  onBalance,
}: Props) {
  void gameId;
  const animRef = useRef<L9AnimProfile>(getLucky9Anim());
  const [cfg, setCfg] = useState<Lucky9Config>(() => getLucky9Config());
  const [playerBet, setPlayerBet] = useState(0);
  const [dealerBet, setDealerBet] = useState(DEFAULT_LUCKY9_CONFIG.minDealerBet);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const remote = await getLucky9EngineConfigFn();
        if (cancelled) return;
        setLucky9Config(remote);
        setCfg(remote);
        setDealerBet(remote.minDealerBet);
      } catch {
        setCfg(DEFAULT_LUCKY9_CONFIG);
      }
    })();
    return () => {
      cancelled = true;
  ...
  async function playDealReveal(script: PublicDealScript) {
    ...
    if (script.outcome === "tie") {
      ...
      playLucky9Sound(credit >= (playerBet + dealerBet + tieBet) * 5 ? "bigWin" : "win");
    } else {
