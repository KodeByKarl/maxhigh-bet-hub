...
import { createRng, type TurtleTideRng } from "./rng";
import { getTurtleTideConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: TurtleTideConfig, rng: TurtleTideRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getTurtleTideConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getTurtleTideConfig();
