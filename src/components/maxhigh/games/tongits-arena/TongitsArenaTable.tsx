...
import { getTongitsArenaConfig, setTongitsArenaConfig } from "./runtimeConfig";
...
/**
 * Tongits Arena — Ante/Play + Pair Plus with a mid-round Play/Fold decision.
 */
export function TongitsArenaTable({
  gameId = TONGITS_ARENA_GAME_ID,
  gameName = "Tongits Arena",
  onBalance,
}: Props) {
  void gameId;
  const animRef = useRef<TcpAnimProfile>(getThreeCardPokerAnim());
  const [cfg, setCfg] = useState<TongitsArenaConfig>(() => getTongitsArenaConfig());
  const [ante, setAnte] = useState(DEFAULT_TONGITS_ARENA_CONFIG.minAnteBet);
  ...
  const [banner, setBanner] = useState("Buy into the fight · optional side pot");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const remote = await getTongitsArenaEngineConfigFn();
        if (cancelled) return;
        setTongitsArenaConfig(remote);
        setCfg(remote);
        setAnte(remote.minAnteBet);
        setSelectedChip(remote.betSteps.find((v) => v >= 5) ?? remote.betSteps[0] ?? 5);
      } catch {
        setCfg(DEFAULT_TONGITS_ARENA_CONFIG);
      }

      try {
        const open = await getTongitsArenaSessionFn();
        ...
        setPhase("decision");
        setBanner("Fight (match pot) or Fold");
      } catch {
        /* no open session */
      }
    })();
  ...
  function resetToBetting() {
    ...
    setLockedAnte(0);
    setLockedPairPlus(0);
    setBanner("Buy into the fight · optional side pot");
  ...
  async function onDeal() {
    ...
    setBusy(true);
    try {
      const res = await tongitsArenaDealFn({
        data: { ante, pairPlus },
      });
  ...
  async function onPlay() {
    ...
    setBusy(true);
    try {
      const res = await tongitsArenaPlayFn({ data: { sessionId } });
      syncBalance(res.balance);
  ...
  async function onFold() {
    ...
    setBusy(true);
    try {
      const res = await tongitsArenaFoldFn({ data: { sessionId } });
      syncBalance(res.balance);
      await playSettleReveal(res.script);
    } catch (e) {
