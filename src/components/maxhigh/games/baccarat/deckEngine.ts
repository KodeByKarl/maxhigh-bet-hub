...
export function shuffleInPlace(cards: PlayingCard[], rng: BcRng): void {
...
export function freshShuffledShoe(cfg: BaccaratConfig, rng: BcRng): ShoeState {
...
export function needsReshuffle(shoe: ShoeState, cfg: BaccaratConfig): boolean {
  const total = shoe.cards.length;
  if (total <= 0) return true;
  return remaining(shoe) / total <= cfg.reshuffleBelowFraction;
}

export function ensureShoe(shoe: ShoeState | null, cfg: BaccaratConfig, rng: BcRng): ShoeState {
