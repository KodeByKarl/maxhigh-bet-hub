/**
 * Crab Cannon arena — continuous-fire fish hunter shell.
...
import {
  crabCannonBossStatusFn,
  crabCannonSyncSpawnsFn,
  crabCannonBuyPowerUpFn,
  crabCannonEnsureSessionFn,
  crabCannonFireShotFn,
  crabCannonSetWeaponFn,
  crabCannonUseFreezeLureFn,
  crabCannonUseNetBombFn,
  getCrabCannonEngineConfigFn,
} from "@/functions/crab-cannon";
import { cn } from "@/lib/utils";
import { SWIM } from "./animationConfig";
import { playCrabCannonSound, setCrabCannonMuted, unlockCrabCannonAudio } from "./audio";
import { FishSprite } from "./FishSprite";
import { getCrabCannonConfig, setCrabCannonConfig } from "./runtimeConfig";
...
export function CrabCannonGame({
  gameId = CRAB_CANNON_GAME_ID,
  gameName = "Crab Cannon",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<CrabCannonConfig>(() => getCrabCannonConfig());
  ...
  useEffect(() => {
    unlockCrabCannonAudio();
    void (async () => {
      try {
        const remote = await getCrabCannonEngineConfigFn();
        const next = remote ?? DEFAULT_CRAB_CANNON_CONFIG;
        setCrabCannonConfig(next);
        setCfg(next);
      } catch {
        setCfg(getCrabCannonConfig());
      }
      try {
        const session = await crabCannonEnsureSessionFn({ data: { weaponId: "bamboo" } });
        ...
        if (session.config) {
          setCrabCannonConfig(session.config);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        const res = await crabCannonSyncSpawnsFn();
        if (cancelled) return;
        setFish((prev) => {
  ...
  const fireAt = useCallback(
    async (target: ArenaFish) => {
      ...
      setFiring(true);
      unlockCrabCannonAudio();
      playCrabCannonSound("shotFire");
      try {
        const res = await crabCannonFireShotFn({
          data: {
            weaponId,
            target: {
        ...
        playCrabCannonSound(res.shot.hit ? "hit" : "splash");
        ...
        if (res.fishUpdate.killed) {
          if (target.tierId === "boss") {
            playCrabCannonSound("bossDeath");
          } else if (target.tierId === "elite" || target.tierId === "rare") {
            playCrabCannonSound("fishDeathLarge");
          } else if (target.tierId === "crate") {
            playCrabCannonSound("crateDrop");
          } else {
            playCrabCannonSound("fishDeathSmall");
          }
          if (res.killBanner && res.killBanner.credit > 0) {
            playCrabCannonSound("coinPayout");
  ...
  async function changeWeapon(id: WeaponTierId) {
    setWeaponId(id);
    try {
      const session = await crabCannonSetWeaponFn({ data: { weaponId: id } });
      syncBalance(session.balance);
    } catch (e) {
  ...
  async function buyPower(id: PowerUpId) {
    try {
      const res = await crabCannonBuyPowerUpFn({ data: { powerUpId: id } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
  ...
  async function useNetBomb() {
    ...
    try {
      ...
      const res = await crabCannonUseNetBombFn({ data: { weaponId, targets } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
      playCrabCannonSound("splash");
  ...
  async function useFreeze() {
    ...
    try {
      const res = await crabCannonUseFreezeLureFn();
      setPowerUps(res.powerUps);
...
export const CrabCannonArena = CrabCannonGame;
