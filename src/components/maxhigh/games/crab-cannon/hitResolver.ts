...
import { createRng, type CrabCannonRng } from "./rng";
import { getCrabCannonConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: CrabCannonConfig, rng: CrabCannonRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getCrabCannonConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getCrabCannonConfig();
