...
type Props = {
  ...
  disabled?: boolean;
  activeSpot: DropDeluxeSpot;
  onActiveSpot: (s: DropDeluxeSpot) => void;
  onBet: (spot: DropDeluxeSpot, n: number) => void;
...
export function BettingPanel({
  ...
  const pickCount = DROP_DELUXE_SPOTS.filter((n) => (bets[n] ?? 0) > 0).length;
  ...
  const totalWager = +DROP_DELUXE_SPOTS.reduce((s, n) => s + (bets[n] ?? 0), 0).toFixed(2);
