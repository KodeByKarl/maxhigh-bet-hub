...
import { dropDeluxeDealFn, getDropDeluxeEngineConfigFn } from "@/functions/drop-deluxe";
import {
...
import { playDropDeluxeSound, unlockDropDeluxeAudio } from "./audio";
import { BettingPanel, type ChipPlaceEvent } from "./BettingPanel";
import { getDropDeluxeConfig, setDropDeluxeConfig } from "./runtimeConfig";
...
function emptyBets(): Record<DropDeluxeSpot, number> {
  return Object.fromEntries(DROP_DELUXE_SPOTS.map((n) => [n, 0])) as Record<
    DropDeluxeSpot,
    number
  >;
}
...
/**
 * Drop Deluxe — vertical numbered lanes + ball drop reveal.
 */
export function DropDeluxeTable({
  gameId = DROP_DELUXE_GAME_ID,
  gameName = "Drop Deluxe",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<DropDeluxeConfig>(() => getDropDeluxeConfig());
  const [bets, setBets] = useState<Record<DropDeluxeSpot, number>>(emptyBets);
  const [activeSpot, setActiveSpot] = useState<DropDeluxeSpot>(1);
  const [busy, setBusy] = useState(false);
  const [ballY, setBallY] = useState(-8);
  const [winning, setWinning] = useState<DropDeluxeSpot | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const remote = await getDropDeluxeEngineConfigFn();
        if (cancelled) return;
        setDropDeluxeConfig(remote);
        setCfg(remote);
      } catch {
        setCfg(DEFAULT_DROP_DELUXE_CONFIG);
      }
    })();
    return () => {
      cancelled = true;
  ...
  async function playReveal(script: PublicDealScript) {
    ...
    if (script.immediateCredit > 0) {
      playDropDeluxeSound("win");
      setBanner(`HIT ${script.winningNumber} · +₱${script.immediateCredit.toFixed(2)}`);
    } else {
      playDropDeluxeSound("lose", 0.4);
      setBanner(`Dropped ${script.winningNumber} · miss`);
    }
  }

  async function onDeal() {
    const total = +DROP_DELUXE_SPOTS.reduce((s, n) => s + (bets[n] ?? 0), 0).toFixed(2);
    if (total < cfg.minBet) {
    ...
    setBanner("Dropping…");
    try {
      const res = await dropDeluxeDealFn({ data: { bets } });
