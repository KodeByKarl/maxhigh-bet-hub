/**
 * Shark Hunter arena — continuous-fire fish hunter shell.
...
import {
  sharkHunterBossStatusFn,
  sharkHunterSyncSpawnsFn,
  sharkHunterBuyPowerUpFn,
  sharkHunterEnsureSessionFn,
  sharkHunterFireShotFn,
  sharkHunterSetWeaponFn,
  sharkHunterUseFreezeLureFn,
  sharkHunterUseNetBombFn,
  getSharkHunterEngineConfigFn,
} from "@/functions/shark-hunter";
import { cn } from "@/lib/utils";
import { SWIM } from "./animationConfig";
import { playSharkHunterSound, setSharkHunterMuted, unlockSharkHunterAudio } from "./audio";
import { FishSprite } from "./FishSprite";
import { getSharkHunterConfig, setSharkHunterConfig } from "./runtimeConfig";
...
export function SharkHunterGame({
  gameId = SHARK_HUNTER_GAME_ID,
  gameName = "Shark Hunter",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<SharkHunterConfig>(() => getSharkHunterConfig());
  ...
  useEffect(() => {
    unlockSharkHunterAudio();
    void (async () => {
      try {
        const remote = await getSharkHunterEngineConfigFn();
        const next = remote ?? DEFAULT_SHARK_HUNTER_CONFIG;
        setSharkHunterConfig(next);
        setCfg(next);
      } catch {
        setCfg(getSharkHunterConfig());
      }
      try {
        const session = await sharkHunterEnsureSessionFn({ data: { weaponId: "bamboo" } });
        ...
        if (session.config) {
          setSharkHunterConfig(session.config);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        const res = await sharkHunterSyncSpawnsFn();
        if (cancelled) return;
        setFish((prev) => {
  ...
  const fireAt = useCallback(
    async (target: ArenaFish) => {
      ...
      setFiring(true);
      unlockSharkHunterAudio();
      playSharkHunterSound("shotFire");
      try {
        const res = await sharkHunterFireShotFn({
          data: {
            weaponId,
            target: {
        ...
        playSharkHunterSound(res.shot.hit ? "hit" : "splash");
        ...
        if (res.fishUpdate.killed) {
          if (target.tierId === "boss") {
            playSharkHunterSound("bossDeath");
          } else if (target.tierId === "elite" || target.tierId === "rare") {
            playSharkHunterSound("fishDeathLarge");
          } else if (target.tierId === "crate") {
            playSharkHunterSound("crateDrop");
          } else {
            playSharkHunterSound("fishDeathSmall");
          }
          if (res.killBanner && res.killBanner.credit > 0) {
            playSharkHunterSound("coinPayout");
  ...
  async function changeWeapon(id: WeaponTierId) {
    setWeaponId(id);
    try {
      const session = await sharkHunterSetWeaponFn({ data: { weaponId: id } });
      syncBalance(session.balance);
    } catch (e) {
  ...
  async function buyPower(id: PowerUpId) {
    try {
      const res = await sharkHunterBuyPowerUpFn({ data: { powerUpId: id } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
  ...
  async function useNetBomb() {
    ...
    try {
      ...
      const res = await sharkHunterUseNetBombFn({ data: { weaponId, targets } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
      playSharkHunterSound("splash");
  ...
  async function useFreeze() {
    ...
    try {
      const res = await sharkHunterUseFreezeLureFn();
      setPowerUps(res.powerUps);
...
export const SharkHunterArena = SharkHunterGame;
