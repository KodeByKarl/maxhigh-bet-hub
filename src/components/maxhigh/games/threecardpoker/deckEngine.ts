...
function buildDeck(deckIndex: number): PlayingCard[] {
  ...
  for (const suit of TCP_SUITS) {
    for (const rank of TCP_RANKS) {
...
export function shuffleInPlace(cards: PlayingCard[], rng: TcpRng): void {
...
export function freshShuffledShoe(cfg: ThreeCardPokerConfig, rng: TcpRng): ShoeState {
...
export function needsReshuffle(shoe: ShoeState, cfg: ThreeCardPokerConfig): boolean {
  const total = shoe.cards.length;
  if (total <= 0) return true;
  return remaining(shoe) / total <= cfg.reshuffleBelowFraction;
}

export function ensureShoe(
  shoe: ShoeState | null,
  cfg: ThreeCardPokerConfig,
  rng: TcpRng,
): ShoeState {
