...
import { createRng, type DragonFisherRng } from "./rng";
import { getDragonFisherConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: DragonFisherConfig, rng: DragonFisherRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getDragonFisherConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getDragonFisherConfig();
