/**
 * Dragon Fisher arena — continuous-fire fish hunter shell.
...
import {
  dragonFisherBossStatusFn,
  dragonFisherSyncSpawnsFn,
  dragonFisherBuyPowerUpFn,
  dragonFisherEnsureSessionFn,
  dragonFisherFireShotFn,
  dragonFisherSetWeaponFn,
  dragonFisherUseFreezeLureFn,
  dragonFisherUseNetBombFn,
  getDragonFisherEngineConfigFn,
} from "@/functions/dragon-fisher";
import { cn } from "@/lib/utils";
import { SWIM } from "./animationConfig";
import { playDragonFisherSound, setDragonFisherMuted, unlockDragonFisherAudio } from "./audio";
import { FishSprite } from "./FishSprite";
import { getDragonFisherConfig, setDragonFisherConfig } from "./runtimeConfig";
...
export function DragonFisherGame({
  gameId = DRAGON_FISHER_GAME_ID,
  gameName = "Dragon Fisher",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<DragonFisherConfig>(() => getDragonFisherConfig());
  ...
  useEffect(() => {
    unlockDragonFisherAudio();
    void (async () => {
      try {
        const remote = await getDragonFisherEngineConfigFn();
        const next = remote ?? DEFAULT_DRAGON_FISHER_CONFIG;
        setDragonFisherConfig(next);
        setCfg(next);
      } catch {
        setCfg(getDragonFisherConfig());
      }
      try {
        const session = await dragonFisherEnsureSessionFn({ data: { weaponId: "bamboo" } });
        ...
        if (session.config) {
          setDragonFisherConfig(session.config);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        const res = await dragonFisherSyncSpawnsFn();
        if (cancelled) return;
        setFish((prev) => {
        ...
        .then((res) => {
          if (res.boss?.active) {
            ...
            setFish((prev) => {
              ...
              playDragonFisherSound("bossRoar");
              return [
  ...
  const fireAt = useCallback(
    async (target: ArenaFish) => {
      ...
      setFiring(true);
      unlockDragonFisherAudio();
      playDragonFisherSound("shotFire");
      try {
        const res = await dragonFisherFireShotFn({
          data: {
            weaponId,
            target: {
        ...
        playDragonFisherSound(res.shot.hit ? "hit" : "splash");
        ...
        if (res.fishUpdate.killed) {
          if (target.tierId === "boss") {
            playDragonFisherSound("bossDeath");
          } else if (target.tierId === "elite" || target.tierId === "rare") {
            playDragonFisherSound("fishDeathLarge");
          } else if (target.tierId === "crate") {
            playDragonFisherSound("crateDrop");
          } else {
            playDragonFisherSound("fishDeathSmall");
          }
          if (res.killBanner && res.killBanner.credit > 0) {
            playDragonFisherSound("coinPayout");
  ...
  async function changeWeapon(id: WeaponTierId) {
    setWeaponId(id);
    try {
      const session = await dragonFisherSetWeaponFn({ data: { weaponId: id } });
      syncBalance(session.balance);
    } catch (e) {
  ...
  async function buyPower(id: PowerUpId) {
    try {
      const res = await dragonFisherBuyPowerUpFn({ data: { powerUpId: id } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
  ...
  async function useNetBomb() {
    ...
    try {
      ...
      const res = await dragonFisherUseNetBombFn({ data: { weaponId, targets } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
      playDragonFisherSound("splash");
  ...
  async function useFreeze() {
    ...
    try {
      const res = await dragonFisherUseFreezeLureFn();
      setPowerUps(res.powerUps);
...
export const DragonFisherArena = DragonFisherGame;
