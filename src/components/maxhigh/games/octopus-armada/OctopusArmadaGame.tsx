/**
 * Octopus Armada arena — continuous-fire fish hunter shell.
...
import {
  octopusArmadaBossStatusFn,
  octopusArmadaSyncSpawnsFn,
  octopusArmadaBuyPowerUpFn,
  octopusArmadaEnsureSessionFn,
  octopusArmadaFireShotFn,
  octopusArmadaSetWeaponFn,
  octopusArmadaUseFreezeLureFn,
  octopusArmadaUseNetBombFn,
  getOctopusArmadaEngineConfigFn,
} from "@/functions/octopus-armada";
import { cn } from "@/lib/utils";
import { SWIM } from "./animationConfig";
import { playOctopusArmadaSound, setOctopusArmadaMuted, unlockOctopusArmadaAudio } from "./audio";
import { FishSprite } from "./FishSprite";
import { getOctopusArmadaConfig, setOctopusArmadaConfig } from "./runtimeConfig";
...
export function OctopusArmadaGame({
  gameId = OCTOPUS_ARMADA_GAME_ID,
  gameName = "Octopus Armada",
  onBalance,
}: Props) {
  void gameId;
  const [cfg, setCfg] = useState<OctopusArmadaConfig>(() => getOctopusArmadaConfig());
  ...
  useEffect(() => {
    unlockOctopusArmadaAudio();
    void (async () => {
      try {
        const remote = await getOctopusArmadaEngineConfigFn();
        const next = remote ?? DEFAULT_OCTOPUS_ARMADA_CONFIG;
        setOctopusArmadaConfig(next);
        setCfg(next);
      } catch {
        setCfg(getOctopusArmadaConfig());
      }
      try {
        const session = await octopusArmadaEnsureSessionFn({ data: { weaponId: "bamboo" } });
        ...
        if (session.config) {
          setOctopusArmadaConfig(session.config);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        const res = await octopusArmadaSyncSpawnsFn();
        if (cancelled) return;
        setFish((prev) => {
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      void octopusArmadaBossStatusFn()
        .then((res) => {
          if (res.boss?.active) {
            setBossHud({
            ...
            setFish((prev) => {
              ...
              playOctopusArmadaSound("bossRoar");
              return [
  ...
  const fireAt = useCallback(
    async (target: ArenaFish) => {
      ...
      setFiring(true);
      unlockOctopusArmadaAudio();
      playOctopusArmadaSound("shotFire");
      try {
        const res = await octopusArmadaFireShotFn({
          data: {
            weaponId,
            target: {
        ...
        playOctopusArmadaSound(res.shot.hit ? "hit" : "splash");
        ...
        if (res.fishUpdate.killed) {
          if (target.tierId === "boss") {
            playOctopusArmadaSound("bossDeath");
          } else if (target.tierId === "elite" || target.tierId === "rare") {
            playOctopusArmadaSound("fishDeathLarge");
          } else if (target.tierId === "crate") {
            playOctopusArmadaSound("crateDrop");
          } else {
            playOctopusArmadaSound("fishDeathSmall");
          }
          if (res.killBanner && res.killBanner.credit > 0) {
            playOctopusArmadaSound("coinPayout");
  ...
  async function changeWeapon(id: WeaponTierId) {
    setWeaponId(id);
    try {
      const session = await octopusArmadaSetWeaponFn({ data: { weaponId: id } });
      syncBalance(session.balance);
    } catch (e) {
  ...
  async function buyPower(id: PowerUpId) {
    try {
      const res = await octopusArmadaBuyPowerUpFn({ data: { powerUpId: id } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
  ...
  async function useNetBomb() {
    ...
    try {
      ...
      const res = await octopusArmadaUseNetBombFn({ data: { weaponId, targets } });
      syncBalance(res.balance);
      setGoldenHookMult(res.goldenHookMult);
      setPowerUps(res.powerUps);
      playOctopusArmadaSound("splash");
  ...
  async function useFreeze() {
    ...
    try {
      const res = await octopusArmadaUseFreezeLureFn();
      setPowerUps(res.powerUps);
...
export const OctopusArmadaArena = OctopusArmadaGame;
