...
import { createRng, type OctopusArmadaRng } from "./rng";
import { getOctopusArmadaConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: OctopusArmadaConfig, rng: OctopusArmadaRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getOctopusArmadaConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getOctopusArmadaConfig();
