/**
 * Color Game Pro deal resolver — pure math, no wallet I/O.
...
import { getColorGameProConfig } from "./runtimeConfig";
...
function pickWeighted(
  spots: ColorGameProConfig["spots"],
  rng: () => number,
): ColorSpotId {
...
export function resolveColorGameProDeal(opts: {
  bets: ColorBets;
  cfg?: ColorGameProConfig;
  seed?: string;
}): DealResolveResult {
  const cfg = opts.cfg ?? getColorGameProConfig();
