 ...
 */
import type { DeepBassConfig, FishTierId } from "@/lib/deep-bass-config";
import { getFishTier } from "@/lib/deep-bass-config";
import { PATH_KINDS, SWIM, type SwimPathKind } from "./animationConfig";
import type { DeepBassRng } from "./rng";
...
export function pickSpawnTier(cfg: DeepBassConfig, rng: DeepBassRng): FishTierId {
...
export function createSpawnedFish(opts: {
  ...
  return {
    ...
    // Spread across most of the arena height (CSS top %, clamped for HUD/controls).
    y: 10 + rng.next() * 78,
