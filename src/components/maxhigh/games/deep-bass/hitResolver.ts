...
import { createRng, type DeepBassRng } from "./rng";
import { getDeepBassConfig } from "./runtimeConfig";
...
function rollCrateDrop(cfg: DeepBassConfig, rng: DeepBassRng, betCost: number): CrateDropResult {
...
export function resolveShot(input: ShotResolveInput): ShotResolveResult {
  const cfg = input.cfg ?? getDeepBassConfig();
...
export function resolveNetBomb(opts: {
  ...
  const cfg = opts.cfg ?? getDeepBassConfig();
