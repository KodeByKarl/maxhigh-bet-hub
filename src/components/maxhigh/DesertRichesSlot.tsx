/**
 * Desert Riches — western Hold & Win slot UI.
...
import { getDesertRichesEngineConfigFn } from "@/functions/superadmin";
import {
  desertRichesBuyFeatureFn,
  desertRichesFreeSpinFn,
  desertRichesSpinFn,
  getDesertRichesSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./desert-riches/ReelCell";
import { WinCelebration } from "./desert-riches/WinCelebration";
import { ANIM } from "./desert-riches/animationConfig";
import { desertRichesAudio } from "./desert-riches/audio";
import { getDesertRichesConfig, setDesertRichesConfig } from "./desert-riches/runtimeConfig";
import type { FgGrid, SpinScript } from "./desert-riches/types";
import { cellKey } from "./desert-riches/types";
import type { FgSymKind } from "@/lib/desert-riches-config";
import { TILE_IMAGE_MAP } from "./desert-riches/DesertRichesIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_DESERT_RICHES_CONFIG.reelHeights;
  const syms = DEFAULT_DESERT_RICHES_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function DesertRichesSlot() {
  ...
  const cfg = getDesertRichesConfig();
  void cfgTick;
  ...
  useEffect(() => {
    ...
    desertRichesAudio.preload();
    preloadAssets();
    void getDesertRichesEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setDesertRichesConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getDesertRichesConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      desertRichesAudio.unlock();
      desertRichesAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      desertRichesAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        desertRichesAudio.playCoinLock();
        await wait(500, gen);
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) desertRichesAudio.playCoinLand();
      ...
      if ((script.freeSpinsAwarded ?? 0) > 0) {
        setPhase("win");
        desertRichesAudio.playFreeSpins();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await desertRichesFreeSpinFn({ data: { sessionId } });
        ...
        const res = await desertRichesSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
      ...
      desertRichesAudio.stopSpinLoop();
  ...
  async function handleBuy() {
    ...
    try {
      desertRichesAudio.unlock();
      desertRichesAudio.playUiClick();
      const res = await desertRichesBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    desertRichesAudio.unlock();
    setMuted(desertRichesAudio.toggleMute());
  }
    ...
    <div className="relative h-full w-full overflow-hidden select-none">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Desert Riches · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
