/**
 * Pharaoh Fire � polished layout matching Candy Peak chrome.
...
import { getPharaohFireEngineConfigFn } from "@/functions/superadmin";
import {
  getPharaohFireSessionFn,
  pharaohFireBuyFeatureFn,
  pharaohFireFreeSpinFn,
  pharaohFireSpinFn,
} from "@/functions/api";
import { ANIM } from "./pharaoh-fire/animationConfig";
import { pharaohFireAudio } from "./pharaoh-fire/audio";
import { PHARAOH_FIRE_TILE_IMAGE_MAP } from "./pharaoh-fire/PharaohFireIcon";
import { PharaohFireSidePanel } from "./pharaoh-fire/PharaohFireSidePanel";
import { ReelCell, type ReelPhase } from "./pharaoh-fire/ReelCell";
import { getPharaohFireConfig, setPharaohFireConfig } from "./pharaoh-fire/runtimeConfig";
import type { BoardCell, SpinScript } from "./pharaoh-fire/types";

type PharaohFireSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_PHARAOH_FIRE_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/pharaoh-fire.webp", ...Object.values(PHARAOH_FIRE_TILE_IMAGE_MAP)];
...
export function PharaohFireSlot({ onBalanceUpdate }: PharaohFireSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getPharaohFireConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getPharaohFireEngineConfigFn();
      if (!mountedRef.current) return;
      setPharaohFireConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      setPhase("dropping");
      pharaohFireAudio.unlock();
      pharaohFireAudio.playSpin();
      setBoard([]);
      await wait(32, gen);
      ...
      if (gen === playbackGen.current) {
        if (script.treasureChest && script.treasureChest.totalWin > 0) {
          if (script.treasureChest.legendAwarded) pharaohFireAudio.playJackpot();
          else pharaohFireAudio.playTreasureChest();
          setBanner(
        ...
        if (script.freeSpinsAwarded > 0) {
          pharaohFireAudio.playFreeSpins();
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await pharaohFireFreeSpinFn({ data: { sessionId } });
        ...
        const res = await pharaohFireSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await pharaohFireBuyFeatureFn({ data: { bet } });
