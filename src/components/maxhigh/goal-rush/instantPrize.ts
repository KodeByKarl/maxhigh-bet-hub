import type { GoalRushConfig, GrSymKind } from "@/lib/goal-rush-config";
import type { GrGrid, InstantMixResult, WildScatterCount } from "./types";

function isWildKind(kind: GrSymKind, cfg: GoalRushConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatterKind(kind: GrSymKind, cfg: GoalRushConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

/** Count Wild + Scatter anywhere in view (not payline-restricted). */
export function countWildScatter(grid: GrGrid, cfg: GoalRushConfig): WildScatterCount {
  let wild = 0;
  let scatter = 0;
 ...
 */
export function isGrandJackpot(grid: GrGrid, cfg: GoalRushConfig): boolean {
  let cells = 0;
