import { betPerLine, type GoalRushConfig, type GrSymKind } from "@/lib/goal-rush-config";
import type { GrGrid, PaylineWin } from "./types";

function isWild(kind: GrSymKind, cfg: GoalRushConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatter(kind: GrSymKind, cfg: GoalRushConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

function isPaying(kind: GrSymKind, cfg: GoalRushConfig) {
