/**
 * Crystal Cave — connecting ways on a 3-4-5-4-3 diamond board.
 ...
 */
import type { FgSymKind, CrystalCaveConfig } from "@/lib/crystal-cave-config";
import type { FgGrid, PaylineWin } from "./types";

function isWild(kind: FgSymKind, cfg: CrystalCaveConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: CrystalCaveConfig): boolean {
