/**
 * Wild Ace � polished layout matching Candy Peak chrome.
...
import { DEFAULT_WILD_ACE_CONFIG } from "@/lib/wild-ace-config";
import { getWildAceEngineConfigFn } from "@/functions/superadmin";
import {
  getWildAceSessionFn,
  wildAceBuyFeatureFn,
  wildAceFreeSpinFn,
...
import { ANIM } from "./wild-ace/animationConfig";
import { wildAceAudio } from "./wild-ace/audio";
import { WildAceSidePanel } from "./wild-ace/WildAceSidePanel";
import { ReelCell, type ReelPhase } from "./wild-ace/ReelCell";
import { TILE_IMAGE_MAP } from "./wild-ace/WildAceIcon";
import { PaytableModal } from "./wild-ace/PaytableModal";
import { getWildAceConfig, setWildAceConfig } from "./wild-ace/runtimeConfig";
import type { BoardCell, SpinScript } from "./wild-ace/types";

type WildAceSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_WILD_ACE_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/wild-ace.webp", ...Object.values(TILE_IMAGE_MAP)];
...
export function WildAceSlot({ onBalanceUpdate }: WildAceSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getWildAceConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getWildAceEngineConfigFn();
      if (!mountedRef.current) return;
      setWildAceConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await wildAceFreeSpinFn({ data: { sessionId } });
        ...
        const res = await wildAceSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await wildAceBuyFeatureFn({ data: { bet } });
