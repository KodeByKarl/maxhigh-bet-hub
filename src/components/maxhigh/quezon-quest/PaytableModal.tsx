...
import { SYMBOL_NAMES, type QuezonQuestSymKind } from "@/lib/quezon-quest-config";
import { getQuezonQuestConfig } from "./runtimeConfig";
import { QuezonQuestIcon } from "./QuezonQuestIcon";
...
const ORDER: QuezonQuestSymKind[] = [
  "comet",
  "princess",
  "castle",
  "star",
...
export function PaytableModal({ open, onClose }: PaytableModalProps) {
  if (!open) return null;
  const cfg = getQuezonQuestConfig();
  const ways =
      ...
      >
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-black text-[#F5D76E]">Quezon Quest · Info</h2>
            <button
              type="button"
              onClick={onClose}
