/**
 * Diamond Dig Dig — western Hold & Win slot UI.
...
import { getDiamondDigEngineConfigFn } from "@/functions/superadmin";
import {
  diamondDigBuyFeatureFn,
  diamondDigFreeSpinFn,
  diamondDigSpinFn,
  getDiamondDigSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./diamond-dig/ReelCell";
import { WinCelebration } from "./diamond-dig/WinCelebration";
import { ANIM } from "./diamond-dig/animationConfig";
import { diamondDigAudio } from "./diamond-dig/audio";
import { getDiamondDigConfig, setDiamondDigConfig } from "./diamond-dig/runtimeConfig";
import type { FgGrid, SpinScript } from "./diamond-dig/types";
import { cellKey } from "./diamond-dig/types";
import type { FgSymKind } from "@/lib/diamond-dig-config";
import { TILE_IMAGE_MAP } from "./diamond-dig/DiamondDigIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_DIAMOND_DIG_CONFIG.reelHeights;
  const syms = DEFAULT_DIAMOND_DIG_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function DiamondDigSlot() {
  ...
  useEffect(() => {
    ...
    diamondDigAudio.preload();
    preloadAssets();
    void getDiamondDigEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setDiamondDigConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getDiamondDigConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      diamondDigAudio.unlock();
      diamondDigAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      diamondDigAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) diamondDigAudio.playCoinLand();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await diamondDigFreeSpinFn({ data: { sessionId } });
        ...
        const res = await diamondDigSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  async function handleBuy() {
    ...
    try {
      diamondDigAudio.unlock();
      diamondDigAudio.playUiClick();
      const res = await diamondDigBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    diamondDigAudio.unlock();
    setMuted(diamondDigAudio.toggleMute());
  }
    ...
    <div className="relative h-full w-full overflow-hidden select-none">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Diamond Dig Dig · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
