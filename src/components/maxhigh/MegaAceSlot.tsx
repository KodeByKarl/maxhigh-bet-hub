/**
 * Mega Ace � polished layout matching Candy Peak chrome.
...
import { DEFAULT_MEGA_ACE_CONFIG } from "@/lib/mega-ace-config";
import { getMegaAceEngineConfigFn } from "@/functions/superadmin";
import {
  getMegaAceSessionFn,
  megaAceBuyFeatureFn,
  megaAceFreeSpinFn,
...
import { ANIM } from "./mega-ace/animationConfig";
import { megaAceAudio } from "./mega-ace/audio";
import { MegaAceSidePanel } from "./mega-ace/MegaAceSidePanel";
import { ReelCell, type ReelPhase } from "./mega-ace/ReelCell";
import { TILE_IMAGE_MAP } from "./mega-ace/MegaAceIcon";
import { PaytableModal } from "./mega-ace/PaytableModal";
import { getMegaAceConfig, setMegaAceConfig } from "./mega-ace/runtimeConfig";
import type { BoardCell, SpinScript } from "./mega-ace/types";

type MegaAceSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_MEGA_ACE_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/mega-ace.webp", ...Object.values(TILE_IMAGE_MAP)];
...
export function MegaAceSlot({ onBalanceUpdate }: MegaAceSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getMegaAceConfig();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getMegaAceEngineConfigFn();
      if (!mountedRef.current) return;
      setMegaAceConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await megaAceFreeSpinFn({ data: { sessionId } });
        ...
        const res = await megaAceSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await megaAceBuyFeatureFn({ data: { bet } });
