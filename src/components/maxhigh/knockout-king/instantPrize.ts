import type { KnockoutKingConfig, BkSymKind } from "@/lib/knockout-king-config";
import type { BkGrid, InstantMixResult, WildScatterCount } from "./types";

function isWildKind(kind: BkSymKind, cfg: KnockoutKingConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isScatterKind(kind: BkSymKind, cfg: KnockoutKingConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.scatter;
}

/** Count Wild + Scatter anywhere in view (not payline-restricted). */
export function countWildScatter(grid: BkGrid, cfg: KnockoutKingConfig): WildScatterCount {
  let wild = 0;
  let scatter = 0;
 ...
 */
export function isGrandJackpot(grid: BkGrid, cfg: KnockoutKingConfig): boolean {
  let cells = 0;
