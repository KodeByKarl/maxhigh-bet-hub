/**
 * Knockout King — dedicated spin playback planner (not shared with other games).
...
export function planKnockoutKingPlayback(
  script: SpinScript,
  reelsCount: number,
  turbo: boolean,
): BkPlaybackStep[] {
  const scale = (ms: number) => (turbo ? Math.min(ms, 60) : ms);
  const steps: BkPlaybackStep[] = [];
