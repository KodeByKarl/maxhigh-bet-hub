/**
 * Fortune Ox slot UI — owns its own spin state machine.
...
import { fortuneOxSpinFn, getFortuneOxEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./fortune-ox/animationConfig";
import { MultiplierReelView } from "./fortune-ox/MultiplierReelView";
import { PaytableModal } from "./fortune-ox/PaytableModal";
import { ReelCell, type ReelPhase } from "./fortune-ox/ReelCell";
import { getFortuneOxConfig, setFortuneOxConfig } from "./fortune-ox/runtimeConfig";
import { planFortuneOxPlayback, type FoPlaybackStep } from "./fortune-ox/spinPlayback";
import { cellKey, type FoGrid, type MultiplierReelResult } from "./fortune-ox/types";

function idleGrid(): FoGrid {
  const cfg = getFortuneOxConfig();
...
export function FortuneOxSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<FortuneOxConfig>(() => getFortuneOxConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await fortuneOxSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planFortuneOxPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getFortuneOxEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setFortuneOxConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[FortuneOx] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
