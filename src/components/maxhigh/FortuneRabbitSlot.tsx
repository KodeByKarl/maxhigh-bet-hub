/**
 * Fortune Rabbit slot UI — owns its own spin state machine.
...
import { fortuneRabbitSpinFn, getFortuneRabbitEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./fortune-rabbit/animationConfig";
import { MultiplierReelView } from "./fortune-rabbit/MultiplierReelView";
import { PaytableModal } from "./fortune-rabbit/PaytableModal";
import { ReelCell, type ReelPhase } from "./fortune-rabbit/ReelCell";
import { getFortuneRabbitConfig, setFortuneRabbitConfig } from "./fortune-rabbit/runtimeConfig";
import { planFortuneRabbitPlayback, type FrPlaybackStep } from "./fortune-rabbit/spinPlayback";
import { cellKey, type FrGrid, type MultiplierReelResult } from "./fortune-rabbit/types";

function idleGrid(): FrGrid {
  const cfg = getFortuneRabbitConfig();
  const kinds: FrSymKind[] = ["ruby", "emerald", "sapphire", "amethyst", "topaz", "temple"];
...
export function FortuneRabbitSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<FortuneRabbitConfig>(() => getFortuneRabbitConfig());
  ...
  const runStep = useCallback(
    (runId: number, steps: FrPlaybackStep[], index: number) => {
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await fortuneRabbitSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planFortuneRabbitPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getFortuneRabbitEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setFortuneRabbitConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[FortuneRabbit] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
