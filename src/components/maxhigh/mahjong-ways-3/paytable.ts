import type { MahjongSymbolConfig } from "@/lib/mahjong-ways-3-config";

/**
 * Returns the payout multiplier for a given symbol configuration and matching reel count (3, 4, 5).
 */
export function getSymbolPayoutMultiplier(sym: MahjongSymbolConfig, consecutiveReels: number): number {
  if (sym.scatter) {
