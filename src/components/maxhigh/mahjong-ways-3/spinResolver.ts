import { calcFreeSpinsAward } from "@/lib/mahjong-ways-3-config";
import { getMahjongWays3Config } from "./runtimeConfig";
 ...
 */
export function resolveMahjongWays3Spin(opts: {
  bet: number;
  ante: boolean;
  isFreeSpins: boolean;
}): SpinScript {
  const config = getMahjongWays3Config();
  const bet = opts.bet;
