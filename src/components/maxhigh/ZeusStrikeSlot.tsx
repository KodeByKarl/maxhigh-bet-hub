/**
 * Zeus Strike — 6×5 ways cascading slot (ported from Zeus Strike).
...
import {
  getZeusStrikeSessionFn,
  zeusStrikeBuyFeatureFn,
  zeusStrikeFreeSpinFn,
  zeusStrikeSpinFn,
} from "@/functions/api";
import { ANIM, EASE, columnDominoMs } from "./zeus-strike/animationConfig";
import { zeusStrikeAudio, ZEUS_STRIKE_SOUND_FILES } from "./zeus-strike/audio";
import { AudioLoadBar } from "./zeus-strike/AudioLoadBar";
import { ControlBar } from "./zeus-strike/ControlBar";
import { FreeSpinsBadge } from "./zeus-strike/FreeSpinsBadge";
import { FreeSpinIntro, ScatterGatherOverlay } from "./zeus-strike/FreeSpinIntro";
import { ZeusStrikeSidePanel } from "./zeus-strike/ZeusStrikeSidePanel";
import { buildBoard, initialBoard, nextKey } from "./zeus-strike/gridState";
import { ASSET, getBuyFeatureMult } from "./zeus-strike/paytable";
import { getRuntimeSymbols, setZeusStrikeConfig } from "./zeus-strike/runtimeConfig";
import type { BoardCell, SpinScript, WayWin } from "./zeus-strike/types";
import { COLS, ROWS } from "./zeus-strike/types";
import { SymbolIcon } from "./zeus-strike/SymbolIcon";
import { WaysWinOverlay } from "./zeus-strike/WaysWinOverlay";
import { WinCelebration } from "./zeus-strike/WinCelebration";
import { PharaohMascot, type MascotMood } from "./zeus-strike/PharaohMascot";
...
export function ZeusStrikeSlot({
  gameId = "zeus-strike",
  gameName = "Zeus Strike",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [muted, setMuted] = useState(() => zeusStrikeAudio.isMuted);
  ...
  const [audioTotal, setAudioTotal] = useState(() => Object.keys(ZEUS_STRIKE_SOUND_FILES).length);
  ...
  useEffect(() => {
    ...
    void zeusStrikeAudio
      ...
      .finally(() => {
        ...
        const total = Object.keys(ZEUS_STRIKE_SOUND_FILES).length;
        setAudioTotal(total);
        setAudioLoaded(total);
      });
    return () => {
      alive = false;
      zeusStrikeAudio.stopAmbient();
    };
  }, []);
  ...
  const playColumnStops = (cols: number) => {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      window.setTimeout(() => zeusStrikeAudio.playReelStop(c), delay);
    }
  };
  ...
  const gatherScatters = async (board: BoardCell[]) => {
    ...
    zeusStrikeAudio.playScatterTrigger();
  ...
  const playScript = useCallback(async (script: SpinScript, isFree: boolean) => {
    ...
    setAnimTick((t) => t + 1);
    zeusStrikeAudio.startSpinLoop();
    playColumnStops(COLS);
    await wait(columnDominoMs(COLS));
    zeusStrikeAudio.stopSpinLoop();
    ...
    for (const step of script.steps) {
      ...
      if (step.cascadeWin > 0) {
        setMascotMood("win");
        setMascotPulse((n) => n + 1);
        zeusStrikeAudio.playCascadeTick();
      }
      if (isFree && running > 0) {
  ...
  const showTotalWin = (amount: number, multiplier = 1) => {
    if (amount <= 0) return;
    zeusStrikeAudio.playWin(amount, bet);
  ...
  const finishBase = async (
    ...
    if (session.inFree && session.freeSpinsLeft > 0) {
      zeusStrikeAudio.playFreespinIntro();
  ...
  const spin = async (asFree = false) => {
    ...
    try {
      let settled: Awaited<ReturnType<typeof zeusStrikeSpinFn>>;
      if (isFree) {
        ...
        settled = await zeusStrikeFreeSpinFn({ data: { sessionId } });
      } else {
        settled = await zeusStrikeSpinFn({ data: { bet } });
        setDropTotal(0);
        fsMultRef.current = 1;
        setFsMult(1);
      }
      ...
      if (isFree) {
        ...
        if (script.totalWin > 0) zeusStrikeAudio.playWin(script.totalWin, bet);
      ...
      console.error("[ZeusStrike] spin failed", err);
  ...
  const buyFeature = async () => {
    ...
    busyRef.current = true;
    try {
      const bought = await zeusStrikeBuyFeatureFn({ data: { bet } });
        ...
        <div className="flex h-full max-h-full w-full max-w-[1600px] flex-col items-center gap-1.5 sm:gap-2">
          ...
          <div className="mt-auto w-full shrink-0 pt-1.5">
            <ControlBar
              ...
              onMuteToggle={() => {
                const next = zeusStrikeAudio.toggleMute();
                setMuted(next);
              }}
