...
import { rubyRavenAudio } from "./ruby-raven/audio";
import {
  rubyRavenBuyFeatureFn,
  rubyRavenFreeSpinFn,
  rubyRavenSpinFn,
  getRubyRavenSessionFn,
} from "@/functions/api";
import { ANIM } from "./ruby-raven/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./ruby-raven/playbackHelpers";
import { BetSelectModal } from "./ruby-raven/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./ruby-raven/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./ruby-raven/paytable";
import { setRubyRavenConfig } from "./ruby-raven/runtimeConfig";
import type { BoardCell, SpinScript } from "./ruby-raven/types";
import { CELLS } from "./ruby-raven/types";
import { WinCelebration } from "./ruby-raven/WinCelebration";
import { FreeSpinsCongrats } from "./ruby-raven/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./ruby-raven/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./ruby-raven/BuyFeatureModal";
import { PaytableModal } from "./ruby-raven/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./ruby-raven/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./ruby-raven/ReelGrid";
import { rubyRavenTheme } from "./ruby-raven/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./ruby-raven/ReelCell";
import { getRubyRavenEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  rubyRavenAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    rubyRavenTheme.assets.backdrop,
    rubyRavenTheme.assets.loadingBg,
    ...Object.values(rubyRavenTheme.assets.icons),
  ];
...
export function RubyRavenSlot({
  ...
  const [muted, setMuted] = useState(() => rubyRavenAudio.isMuted);
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(20,4,10,0.96), rgba(136,19,55,0.96))",
                color: "#FECDD3",
              }}
            >
          ...
          <div
            ...
            style={{
              borderColor: isWin
                ...
                : "rgba(244,63,94,0.55)",
              background: isWin
                ? "linear-gradient(135deg, #E11D48 0%, #9F1239 55%, #14040A 100%)"
                : "linear-gradient(135deg, rgba(20,4,10,0.96), rgba(136,19,55,0.96))",
              color: isWin ? "#FFF1F2" : "#FECDD3",
            }}
          >
            <div
              className="text-[10px] font-black uppercase tracking-[0.18em]"
              style={{
                color: isWin ? "rgba(255,228,230,0.85)" : "rgba(254,205,211,0.85)",
              }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      rubyRavenAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        rubyRavenAudio.stopSpinLoop();
        return;
      }
      rubyRavenAudio.stopSpinLoop();
      rubyRavenAudio.playReelStop(5);
      rubyRavenAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === RUBY_RAVEN_GAME_ID || gameId === "ruby-raven") {
      void getRubyRavenEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setRubyRavenConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getRubyRavenSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[RubyRaven] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== RUBY_RAVEN_GAME_ID && gameId !== "ruby-raven") return;
    try {
      const cfg = await getRubyRavenEngineConfigFn();
      if (mountedRef.current) setRubyRavenConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        rubyRavenAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    rubyRavenAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    rubyRavenAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        rubyRavenAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getRubyRavenSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof rubyRavenSpinFn>>;
        if (isFree) {
          ...
          settled = await rubyRavenFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await rubyRavenSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[RubyRaven] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await rubyRavenBuyFeatureFn({
