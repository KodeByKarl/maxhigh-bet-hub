/**
 * Outlaw Coins — western Hold & Win slot UI.
...
import { getOutlawCoinsEngineConfigFn } from "@/functions/superadmin";
import {
  outlawCoinsBuyFeatureFn,
  outlawCoinsFreeSpinFn,
  outlawCoinsSpinFn,
  getOutlawCoinsSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./outlaw-coins/ReelCell";
import { WinCelebration } from "./outlaw-coins/WinCelebration";
import { ANIM } from "./outlaw-coins/animationConfig";
import { outlawCoinsAudio } from "./outlaw-coins/audio";
import { getOutlawCoinsConfig, setOutlawCoinsConfig } from "./outlaw-coins/runtimeConfig";
import type { FgGrid, SpinScript } from "./outlaw-coins/types";
import { cellKey } from "./outlaw-coins/types";
import type { FgSymKind } from "@/lib/outlaw-coins-config";
import { TILE_IMAGE_MAP } from "./outlaw-coins/OutlawCoinsIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_OUTLAW_COINS_CONFIG.reelHeights;
  const syms = DEFAULT_OUTLAW_COINS_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function OutlawCoinsSlot() {
  ...
  const cfg = getOutlawCoinsConfig();
  void cfgTick;
  ...
  useEffect(() => {
    ...
    outlawCoinsAudio.preload();
    preloadAssets();
    void getOutlawCoinsEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setOutlawCoinsConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getOutlawCoinsConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      outlawCoinsAudio.unlock();
      outlawCoinsAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      outlawCoinsAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) outlawCoinsAudio.playCoinLand();
      ...
      if ((script.freeSpinsAwarded ?? 0) > 0) {
        setPhase("win");
        outlawCoinsAudio.playFreeSpins();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await outlawCoinsFreeSpinFn({ data: { sessionId } });
        ...
        const res = await outlawCoinsSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  async function handleBuy() {
    ...
    try {
      outlawCoinsAudio.unlock();
      outlawCoinsAudio.playUiClick();
      const res = await outlawCoinsBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    outlawCoinsAudio.unlock();
    setMuted(outlawCoinsAudio.toggleMute());
  }
    ...
    <div className="relative h-full w-full overflow-hidden select-none">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Outlaw Coins · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
