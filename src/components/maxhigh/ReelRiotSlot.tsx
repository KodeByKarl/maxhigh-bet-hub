/**
 * Reel Riot slot UI — classic 3-reel cabinet with hold buttons.
 * Non-authoritative: outcomes from reelRiotSpinFn only.
...
import {
  getReelRiotEngineConfigFn,
  getReelRiotJackpotFn,
  reelRiotSpinFn,
} from "@/functions/api";
import { useAuth } from "@/lib/auth";
import { REEL_RIOT_GAME_ID, type ReelRiotConfig } from "@/lib/reel-riot-config";
import { cn } from "@/lib/utils";
import { ANIM, BLUR_SYMBOLS, ICON_SRC, SYM_NAME } from "./reel-riot/animationConfig";
import { FruitRiotIcon } from "./reel-riot/FruitRiotIcon";
import { PaytableModal } from "./reel-riot/PaytableModal";
import { getReelRiotConfig, setReelRiotConfig } from "./reel-riot/runtimeConfig";
import type { RrReels, SpinScript } from "./reel-riot/types";
...
export function ReelRiotSlot({
  gameId: _gameId = REEL_RIOT_GAME_ID,
  gameName = "Fruit Riot",
}: {
  gameId?: string;
  gameName?: string;
  ...
  const [cfgSnap, setCfgSnap] = useState<ReelRiotConfig>(() => getReelRiotConfig());
  ...
  useEffect(() => {
    ...
    void Promise.all([getReelRiotEngineConfigFn(), getReelRiotJackpotFn()])
      .then(([cfg, jp]) => {
        if (!mountedRef.current) return;
        setReelRiotConfig(cfg);
        setCfgSnap(getReelRiotConfig());
        setJpPool(jp.amount);
      })
      .catch(() => undefined);
    return () => {
  ...
  const startSpin = async () => {
    ...
    try {
      const result = await reelRiotSpinFn({
        data: {
          bet,
