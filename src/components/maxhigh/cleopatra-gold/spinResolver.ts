import { calcFreeSpinsAward } from "@/lib/cleopatra-gold-config";
import { createRng, newSpinSeed } from "./rng";
import { getCleopatraGoldConfig } from "./runtimeConfig";
...
/**
 * Server-authoritative Cleopatra Gold spin:
 * ways cascades → Golden Campfire FS → Treasure Chest Hold & Collect → max-win cap.
 */
export function resolveCleopatraGoldSpin(opts: {
  bet: number;
  ante?: boolean;
  isFreeSpins?: boolean;
