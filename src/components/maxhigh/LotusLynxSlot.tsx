...
import { lotusLynxAudio } from "./lotus-lynx/audio";
import {
  lotusLynxBuyFeatureFn,
  lotusLynxFreeSpinFn,
  lotusLynxSpinFn,
  getLotusLynxSessionFn,
} from "@/functions/api";
import { ANIM } from "./lotus-lynx/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./lotus-lynx/playbackHelpers";
import { BetSelectModal } from "./lotus-lynx/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./lotus-lynx/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./lotus-lynx/paytable";
import { setLotusLynxConfig } from "./lotus-lynx/runtimeConfig";
import type { BoardCell, SpinScript } from "./lotus-lynx/types";
import { CELLS } from "./lotus-lynx/types";
import { WinCelebration } from "./lotus-lynx/WinCelebration";
import { FreeSpinsCongrats } from "./lotus-lynx/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./lotus-lynx/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./lotus-lynx/BuyFeatureModal";
import { PaytableModal } from "./lotus-lynx/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./lotus-lynx/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./lotus-lynx/ReelGrid";
import { lotusLynxTheme } from "./lotus-lynx/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./lotus-lynx/ReelCell";
import { getLotusLynxEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  lotusLynxAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    lotusLynxTheme.assets.backdrop,
    lotusLynxTheme.assets.loadingBg,
    ...Object.values(lotusLynxTheme.assets.icons),
  ];
...
export function LotusLynxSlot({
  ...
  const [muted, setMuted] = useState(() => lotusLynxAudio.isMuted);
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(18,6,20,0.96), rgba(112,26,117,0.96))",
                color: "#F5D0FE",
              }}
            >
              <div className="text-[10px] font-black uppercase tracking-[0.18em] text-fuchsia-200/80">
                Bet
              </div>
      ...
      toast.custom(
        () => (
          <div
            ...
            style={{
              borderColor: isWin
                ? "rgba(240,171,252,0.65)"
                : "rgba(232,121,249,0.55)",
              background: isWin
                ? "linear-gradient(135deg, #C026D3 0%, #701A75 55%, #120614 100%)"
                : "linear-gradient(135deg, rgba(18,6,20,0.96), rgba(112,26,117,0.96))",
              color: isWin ? "#FAE8FF" : "#F5D0FE",
            }}
          >
            <div
              className="text-[10px] font-black uppercase tracking-[0.18em]"
              style={{
                color: isWin ? "rgba(250,232,255,0.85)" : "rgba(245,208,254,0.85)",
              }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      lotusLynxAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        lotusLynxAudio.stopSpinLoop();
        return;
      }
      lotusLynxAudio.stopSpinLoop();
      lotusLynxAudio.playReelStop(5);
      lotusLynxAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === LOTUS_LYNX_GAME_ID || gameId === "lotus-lynx") {
      void getLotusLynxEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setLotusLynxConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getLotusLynxSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[LotusLynx] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== LOTUS_LYNX_GAME_ID && gameId !== "lotus-lynx") return;
    try {
      const cfg = await getLotusLynxEngineConfigFn();
      if (mountedRef.current) setLotusLynxConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        lotusLynxAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    lotusLynxAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    lotusLynxAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        lotusLynxAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getLotusLynxSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof lotusLynxSpinFn>>;
        if (isFree) {
          ...
          settled = await lotusLynxFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await lotusLynxSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[LotusLynx] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await lotusLynxBuyFeatureFn({
