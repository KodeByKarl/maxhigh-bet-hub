/**
 * Tricycle Treasure — western Hold & Win slot UI.
...
import { getTricycleTreasureEngineConfigFn } from "@/functions/superadmin";
import {
  tricycleTreasureBuyFeatureFn,
  tricycleTreasureFreeSpinFn,
  tricycleTreasureSpinFn,
  getTricycleTreasureSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./tricycle-treasure/ReelCell";
import { WinCelebration } from "./tricycle-treasure/WinCelebration";
import { ANIM } from "./tricycle-treasure/animationConfig";
import { tricycleTreasureAudio } from "./tricycle-treasure/audio";
import { getTricycleTreasureConfig, setTricycleTreasureConfig } from "./tricycle-treasure/runtimeConfig";
import type { FgGrid, SpinScript } from "./tricycle-treasure/types";
import { cellKey } from "./tricycle-treasure/types";
import type { FgSymKind } from "@/lib/tricycle-treasure-config";
import { TILE_IMAGE_MAP } from "./tricycle-treasure/TricycleTreasureIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_TRICYCLE_TREASURE_CONFIG.reelHeights;
  const syms = DEFAULT_TRICYCLE_TREASURE_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function TricycleTreasureSlot() {
  ...
  const cfg = getTricycleTreasureConfig();
  void cfgTick;
  ...
  useEffect(() => {
    ...
    tricycleTreasureAudio.preload();
    preloadAssets();
    void getTricycleTreasureEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setTricycleTreasureConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
    ...
    return () => {
      ...
      tricycleTreasureAudio.stopAmbient();
    };
  }, []);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getTricycleTreasureConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      tricycleTreasureAudio.unlock();
      tricycleTreasureAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      tricycleTreasureAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        setWinPopup(null);
        setBanner("HOLD & WIN!");
        tricycleTreasureAudio.playHoldWin();
        setHoldOverlay({
          ...holdWin,
          steps: [
            {
        ...
        tricycleTreasureAudio.playCoinLock();
        await wait(500, gen);
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) tricycleTreasureAudio.playCoinLand();
        ...
        if (holdWin.grandAwarded) {
          tricycleTreasureAudio.playJackpot();
      ...
      if ((script.freeSpinsAwarded ?? 0) > 0) {
        setPhase("win");
        tricycleTreasureAudio.playFreeSpins();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await tricycleTreasureFreeSpinFn({ data: { sessionId } });
        ...
        const res = await tricycleTreasureSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
      ...
      tricycleTreasureAudio.stopSpinLoop();
  ...
  async function handleBuy() {
    ...
    try {
      tricycleTreasureAudio.unlock();
      tricycleTreasureAudio.playUiClick();
      const res = await tricycleTreasureBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    tricycleTreasureAudio.unlock();
    setMuted(tricycleTreasureAudio.toggleMute());
  }
  ...
  return (
    <div className="relative h-full w-full overflow-hidden select-none">
      <img
        src="/games/tricycle-treasure.webp"
        alt=""
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Tricycle Treasure · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
