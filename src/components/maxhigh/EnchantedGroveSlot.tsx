/**
 * Enchanted Grove — 6×5 ways cascading slot (Godly Gates-style cascade).
...
import {
  getEnchantedGroveSessionFn,
  enchantedGroveBuyFeatureFn,
  enchantedGroveFreeSpinFn,
  enchantedGroveSpinFn,
} from "@/functions/api";
import { ANIM, EASE, columnDominoMs } from "./enchanted-grove/animationConfig";
import { enchantedGroveAudio, ENCHANTED_GROVE_SOUND_FILES } from "./enchanted-grove/audio";
import { AudioLoadBar } from "./enchanted-grove/AudioLoadBar";
import { ControlBar } from "./enchanted-grove/ControlBar";
import { FreeSpinsBadge } from "./enchanted-grove/FreeSpinsBadge";
import { FreeSpinIntro, ScatterGatherOverlay } from "./enchanted-grove/FreeSpinIntro";
import { EnchantedGroveSidePanel } from "./enchanted-grove/EnchantedGroveSidePanel";
import { buildBoard, initialBoard, nextKey } from "./enchanted-grove/gridState";
import { ASSET, getBuyFeatureMult } from "./enchanted-grove/paytable";
import { getRuntimeSymbols, setEnchantedGroveConfig } from "./enchanted-grove/runtimeConfig";
import type { BoardCell, SpinScript, WayWin } from "./enchanted-grove/types";
import { COLS, ROWS } from "./enchanted-grove/types";
import { SymbolIcon } from "./enchanted-grove/SymbolIcon";
import { WaysWinOverlay } from "./enchanted-grove/WaysWinOverlay";
import { WinCelebration } from "./enchanted-grove/WinCelebration";
import { PharaohMascot, type MascotMood } from "./enchanted-grove/PharaohMascot";
...
export function EnchantedGroveSlot({
  gameId = "enchanted-grove",
  gameName = "Enchanted Grove",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [muted, setMuted] = useState(() => enchantedGroveAudio.isMuted);
  ...
  const [audioTotal, setAudioTotal] = useState(() => Object.keys(ENCHANTED_GROVE_SOUND_FILES).length);
  ...
  useEffect(() => {
    ...
    void enchantedGroveAudio
      ...
      .finally(() => {
        ...
        const total = Object.keys(ENCHANTED_GROVE_SOUND_FILES).length;
        setAudioTotal(total);
        setAudioLoaded(total);
      });
    return () => {
      alive = false;
      enchantedGroveAudio.stopAmbient();
    };
  }, []);
  ...
  const playColumnStops = (cols: number) => {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      window.setTimeout(() => enchantedGroveAudio.playReelStop(c), delay);
    }
  };
  ...
  const gatherScatters = async (board: BoardCell[]) => {
    ...
    enchantedGroveAudio.playScatterTrigger();
  ...
  const playScript = useCallback(async (script: SpinScript, isFree: boolean) => {
    ...
    setAnimTick((t) => t + 1);
    enchantedGroveAudio.startSpinLoop();
    playColumnStops(COLS);
    await wait(columnDominoMs(COLS));
    enchantedGroveAudio.stopSpinLoop();
    ...
    for (const step of script.steps) {
      ...
      if (step.cascadeWin > 0) {
        setMascotMood("win");
        setMascotPulse((n) => n + 1);
        enchantedGroveAudio.playCascadeTick();
      }
      if (isFree && running > 0) {
  ...
  const showTotalWin = (amount: number, multiplier = 1) => {
    if (amount <= 0) return;
    enchantedGroveAudio.playWin(amount, bet);
  ...
  const finishBase = async (
    ...
    if (session.inFree && session.freeSpinsLeft > 0) {
      enchantedGroveAudio.playFreespinIntro();
  ...
  const spin = async (asFree = false) => {
    ...
    try {
      let settled: Awaited<ReturnType<typeof enchantedGroveSpinFn>>;
      if (isFree) {
        ...
        settled = await enchantedGroveFreeSpinFn({ data: { sessionId } });
      } else {
        settled = await enchantedGroveSpinFn({ data: { bet } });
        setDropTotal(0);
        fsMultRef.current = 1;
        setFsMult(1);
      }
      ...
      if (isFree) {
        ...
        if (script.totalWin > 0) enchantedGroveAudio.playWin(script.totalWin, bet);
      ...
      console.error("[EnchantedGrove] spin failed", err);
  ...
  const buyFeature = async () => {
    ...
    busyRef.current = true;
    try {
      const bought = await enchantedGroveBuyFeatureFn({ data: { bet } });
  ...
  useEffect(() => {
    void getEnchantedGroveSessionFn()
      .then((session) => {
        ...
        <div className="flex h-full max-h-full w-full max-w-[1600px] flex-col items-center gap-1.5 sm:gap-2">
          ...
          <div className="mt-auto w-full shrink-0 pt-1.5">
            <ControlBar
              ...
              onMuteToggle={() => {
                const next = enchantedGroveAudio.toggleMute();
                setMuted(next);
              }}
