/**
 * Mystic Runes — 6×5 ways cascading slot (Godly Gates-style cascade).
...
import {
  getMysticRunesSessionFn,
  mysticRunesBuyFeatureFn,
  mysticRunesFreeSpinFn,
  mysticRunesSpinFn,
} from "@/functions/api";
import { ANIM, EASE, columnDominoMs } from "./mystic-runes/animationConfig";
import { mysticRunesAudio, MYSTIC_RUNES_SOUND_FILES } from "./mystic-runes/audio";
import { AudioLoadBar } from "./mystic-runes/AudioLoadBar";
import { ControlBar } from "./mystic-runes/ControlBar";
import { FreeSpinsBadge } from "./mystic-runes/FreeSpinsBadge";
import { FreeSpinIntro, ScatterGatherOverlay } from "./mystic-runes/FreeSpinIntro";
import { MysticRunesSidePanel } from "./mystic-runes/MysticRunesSidePanel";
import { buildBoard, initialBoard, nextKey } from "./mystic-runes/gridState";
import { ASSET, getBuyFeatureMult } from "./mystic-runes/paytable";
import { getRuntimeSymbols, setMysticRunesConfig } from "./mystic-runes/runtimeConfig";
import type { BoardCell, SpinScript, WayWin } from "./mystic-runes/types";
import { COLS, ROWS } from "./mystic-runes/types";
import { SymbolIcon } from "./mystic-runes/SymbolIcon";
import { WaysWinOverlay } from "./mystic-runes/WaysWinOverlay";
import { WinCelebration } from "./mystic-runes/WinCelebration";
import { PharaohMascot, type MascotMood } from "./mystic-runes/PharaohMascot";
...
export function MysticRunesSlot({
  gameId = "mystic-runes",
  gameName = "Mystic Runes",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [muted, setMuted] = useState(() => mysticRunesAudio.isMuted);
  ...
  const [audioTotal, setAudioTotal] = useState(() => Object.keys(MYSTIC_RUNES_SOUND_FILES).length);
  ...
  useEffect(() => {
    ...
    void mysticRunesAudio
      ...
      .finally(() => {
        ...
        const total = Object.keys(MYSTIC_RUNES_SOUND_FILES).length;
        setAudioTotal(total);
        setAudioLoaded(total);
      });
    return () => {
      alive = false;
      mysticRunesAudio.stopAmbient();
    };
  }, []);
  ...
  const playColumnStops = (cols: number) => {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      window.setTimeout(() => mysticRunesAudio.playReelStop(c), delay);
    }
  };
  ...
  const gatherScatters = async (board: BoardCell[]) => {
    ...
    mysticRunesAudio.playScatterTrigger();
  ...
  const playScript = useCallback(async (script: SpinScript, isFree: boolean) => {
    ...
    setAnimTick((t) => t + 1);
    mysticRunesAudio.startSpinLoop();
    playColumnStops(COLS);
    await wait(columnDominoMs(COLS));
    mysticRunesAudio.stopSpinLoop();
    ...
    for (const step of script.steps) {
      ...
      if (step.cascadeWin > 0) {
        setMascotMood("win");
        setMascotPulse((n) => n + 1);
        mysticRunesAudio.playCascadeTick();
      }
      if (isFree && running > 0) {
  ...
  const showTotalWin = (amount: number, multiplier = 1) => {
    if (amount <= 0) return;
    mysticRunesAudio.playWin(amount, bet);
  ...
  const finishBase = async (
    ...
    if (session.inFree && session.freeSpinsLeft > 0) {
      mysticRunesAudio.playFreespinIntro();
  ...
  const spin = async (asFree = false) => {
    ...
    try {
      let settled: Awaited<ReturnType<typeof mysticRunesSpinFn>>;
      if (isFree) {
        ...
        settled = await mysticRunesFreeSpinFn({ data: { sessionId } });
      } else {
        settled = await mysticRunesSpinFn({ data: { bet } });
        setDropTotal(0);
        fsMultRef.current = 1;
        setFsMult(1);
      }
      ...
      if (isFree) {
        ...
        if (script.totalWin > 0) mysticRunesAudio.playWin(script.totalWin, bet);
      ...
      console.error("[MysticRunes] spin failed", err);
  ...
  const buyFeature = async () => {
    ...
    busyRef.current = true;
    try {
      const bought = await mysticRunesBuyFeatureFn({ data: { bet } });
        ...
        <div className="flex h-full max-h-full w-full max-w-[1600px] flex-col items-center gap-1.5 sm:gap-2">
          ...
          <div className="mt-auto w-full shrink-0 pt-1.5">
            <ControlBar
              ...
              onMuteToggle={() => {
                const next = mysticRunesAudio.toggleMute();
                setMuted(next);
              }}
