...
import { getSolarSerpentConfig, getRuntimeSymbols } from "./runtimeConfig";
import { solarSerpentTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...solarSerpentTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getSolarSerpentConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getSolarSerpentConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getSolarSerpentConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getSolarSerpentConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getSolarSerpentConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getSolarSerpentConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getSolarSerpentConfig().freeSpinsRetrigger;
}
