/**
 * Neon Fruits slot UI — classic 3-reel cabinet with hold buttons.
 * Non-authoritative: outcomes from neonFruitsSpinFn only.
...
import {
  ...
  getNeonFruitsJackpotFn,
  neonFruitsSpinFn,
} from "@/functions/api";
import { useAuth } from "@/lib/auth";
import { NEON_FRUITS_GAME_ID, type NeonFruitsConfig } from "@/lib/neon-fruits-config";
import { cn } from "@/lib/utils";
import { ANIM, BLUR_SYMBOLS, ICON_SRC, SYM_NAME } from "./neon-fruits/animationConfig";
import { NeonFruitsIcon } from "./neon-fruits/NeonFruitsIcon";
import { PaytableModal } from "./neon-fruits/PaytableModal";
import { getNeonFruitsConfig, setNeonFruitsConfig } from "./neon-fruits/runtimeConfig";
import type { RrReels, SpinScript } from "./neon-fruits/types";
...
export function NeonFruitsSlot({
  gameId: _gameId = NEON_FRUITS_GAME_ID,
  gameName = "Neon Fruits",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [cfgSnap, setCfgSnap] = useState<NeonFruitsConfig>(() => getNeonFruitsConfig());
  ...
  useEffect(() => {
    ...
    void Promise.all([getNeonFruitsEngineConfigFn(), getNeonFruitsJackpotFn()])
      .then(([cfg, jp]) => {
        if (!mountedRef.current) return;
        setNeonFruitsConfig(cfg);
        setCfgSnap(getNeonFruitsConfig());
        setJpPool(jp.amount);
      })
      .catch(() => undefined);
    return () => {
  ...
  const startSpin = async () => {
    ...
    try {
      const result = await neonFruitsSpinFn({
        data: {
          bet,
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden text-white">
      <style>{NEON_FRUITS_CSS}</style>

      <img
        src="/games/neon-fruits.webp"
        alt=""
