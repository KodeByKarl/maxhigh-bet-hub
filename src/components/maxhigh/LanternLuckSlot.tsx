/**
 * Lantern Luck — fixed-payline slot UI.
...
import { getLanternLuckEngineConfigFn } from "@/functions/superadmin";
import {
  lanternLuckCollectFn,
  lanternLuckGambleFn,
  lanternLuckSpinFn,
  getLanternLuckSessionFn,
} from "@/functions/api";
import { ANIM } from "./lantern-luck/animationConfig";
import { lanternLuckAudio } from "./lantern-luck/audio";
import { ICON_SRC, BET_STEPS } from "./lantern-luck/paytable";
import { BetSelectModal } from "./lantern-luck/BetSelectModal";
import { getLanternLuckConfig, setLanternLuckConfig } from "./lantern-luck/runtimeConfig";
import { PaytableModal } from "./lantern-luck/PaytableModal";
import { ReelCell, type ReelPhase } from "./lantern-luck/ReelCell";
import { WinCelebration } from "./lantern-luck/WinCelebration";
import type { CnyGrid, GambleChoice, SpinScript } from "./lantern-luck/types";
import { cellKey } from "./lantern-luck/types";

const EMPTY_SET = new Set<string>();

function idleGrid(): CnyGrid {
  const cfg = getLanternLuckConfig();
...
export function LanternLuckSlot({
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      if (script.dragonBonus?.triggered) {
        ...
        for (const launch of script.dragonBonus.launches) {
          if (launch.success) {
            ...
            lanternLuckAudio.playDragonFirework();
            setDragonOverlay({
            ...
            lanternLuckAudio.playDragonBust();
      ...
      if (script.monkeyBonus?.triggered) {
        lanternLuckAudio.playMonkeyTrigger();
        setMonkeyOverlay({
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      ...
      const res = await lanternLuckSpinFn({
  ...
  const onCollect = async () => {
    ...
    try {
      lanternLuckAudio.unlock();
      lanternLuckAudio.playCollect();
      const res = await lanternLuckCollectFn({ data: { sessionId } });
      setBalanceLocal(res.balance);
      setPendingWin(0);
      setSessionId(null);
      setGambleOpen(false);
  ...
  const onGamble = async (choice: GambleChoice) => {
    ...
    try {
      lanternLuckAudio.unlock();
      lanternLuckAudio.playGambleFlip();
      const res = await lanternLuckGambleFn({ data: { sessionId, choice } });
      setBalanceLocal(res.balance);
      if (res.result.won) {
