/**
 * Palengke Pays — western Hold & Win slot UI.
...
import { getPalengkePaysEngineConfigFn } from "@/functions/superadmin";
import {
  palengkePaysBuyFeatureFn,
  palengkePaysFreeSpinFn,
  palengkePaysSpinFn,
  getPalengkePaysSessionFn,
} from "@/functions/api";
import { ReelCell, type ReelPhase } from "./palengke-pays/ReelCell";
import { WinCelebration } from "./palengke-pays/WinCelebration";
import { ANIM } from "./palengke-pays/animationConfig";
import { palengkePaysAudio } from "./palengke-pays/audio";
import { getPalengkePaysConfig, setPalengkePaysConfig } from "./palengke-pays/runtimeConfig";
import type { FgGrid, SpinScript } from "./palengke-pays/types";
import { cellKey } from "./palengke-pays/types";
import type { FgSymKind } from "@/lib/palengke-pays-config";
import { TILE_IMAGE_MAP } from "./palengke-pays/PalengkePaysIcon";

const BET_STEPS = [0.25, 0.5, 1, 2, 5, 10, 20, 35];
const EMPTY_SET = new Set<string>();

function idleGrid(): FgGrid {
  const heights = DEFAULT_PALENGKE_PAYS_CONFIG.reelHeights;
  const syms = DEFAULT_PALENGKE_PAYS_CONFIG.symbols.filter((s) => s.tier === "low" || s.tier === "high");
...
export function PalengkePaysSlot() {
  ...
  useEffect(() => {
    ...
    palengkePaysAudio.preload();
    preloadAssets();
    void getPalengkePaysEngineConfigFn()
      .then((c) => {
        if (!mountedRef.current) return;
        setPalengkePaysConfig(c);
        setCfgTick((n) => n + 1);
        setGrid(idleGrid());
      })
      .catch(() => undefined);
  ...
  const playScript = useCallback(
    async (script: SpinScript, gen: number) => {
      const liveCfg = getPalengkePaysConfig();
      ...
      setSpinId((n) => n + 1);
      setPhase("spinning");
      palengkePaysAudio.unlock();
      palengkePaysAudio.startSpinLoop();

      await wait(ANIM.reelSpin, gen);
      palengkePaysAudio.playReelStop();
      ...
      if (holdWin) {
        ...
        palengkePaysAudio.playCoinLock();
        await wait(500, gen);
        ...
        for (const step of playback) {
          if ((step.newCoins?.length ?? 0) > 0) palengkePaysAudio.playCoinLand();
      ...
      if ((script.freeSpinsAwarded ?? 0) > 0) {
        setPhase("win");
        palengkePaysAudio.playFreeSpins();
  ...
  const doSpin = useCallback(async () => {
    ...
    try {
      if (inFree && sessionId) {
        const res = await palengkePaysFreeSpinFn({ data: { sessionId } });
        ...
        const res = await palengkePaysSpinFn({ data: { bet } });
        if (!mountedRef.current || gen !== playbackGen.current) return;
      ...
      palengkePaysAudio.stopSpinLoop();
  ...
  async function handleBuy() {
    ...
    try {
      palengkePaysAudio.unlock();
      palengkePaysAudio.playUiClick();
      const res = await palengkePaysBuyFeatureFn({ data: { bet } });
      if (!mountedRef.current || gen !== playbackGen.current) return;
  ...
  function toggleMute() {
    palengkePaysAudio.unlock();
    setMuted(palengkePaysAudio.toggleMute());
  }
    ...
    <div className="relative h-full w-full overflow-hidden select-none">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-1 z-30 flex flex-col items-center gap-1 px-2 sm:top-2">
        <div className="rounded-full border border-amber-700/90 bg-gradient-to-b from-amber-200 to-amber-600 px-3 py-0.5 text-[10px] font-black text-amber-950 shadow sm:border-2 sm:px-5 sm:py-1 sm:text-sm">
          Palengke Pays · Hold & Win · {cfg.paylineCount} Ways
        </div>
        {inFree && (
