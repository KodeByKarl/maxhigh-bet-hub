/**
 * Dragon & Phoenix — polished layout matching Candy Peak chrome.
...
import { getDragonPhoenixEngineConfigFn } from "@/functions/superadmin";
import {
  getDragonPhoenixSessionFn,
  dragonPhoenixBuyFeatureFn,
  dragonPhoenixFreeSpinFn,
  dragonPhoenixSpinFn,
} from "@/functions/api";
import { ANIM } from "./dragon-phoenix/animationConfig";
import { dragonPhoenixAudio } from "./dragon-phoenix/audio";
import { TILE_IMAGE_MAP } from "./dragon-phoenix/DragonPhoenixIcon";
import { DragonPhoenixSidePanel } from "./dragon-phoenix/DragonPhoenixSidePanel";
import { ReelCell, type ReelPhase } from "./dragon-phoenix/ReelCell";
import { getDragonPhoenixConfig, setDragonPhoenixConfig } from "./dragon-phoenix/runtimeConfig";
import type { BoardCell, SpinScript } from "./dragon-phoenix/types";

type DragonPhoenixSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_DRAGON_PHOENIX_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/dragon-phoenix.webp", ...Object.values(TILE_IMAGE_MAP)];
...
export function DragonPhoenixSlot({ onBalanceUpdate }: DragonPhoenixSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getDragonPhoenixConfig();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    dragonPhoenixAudio.preload();
    void getDragonPhoenixEngineConfigFn()
      .then((cfg) => {
    ...
    return () => {
      ...
      dragonPhoenixAudio.stopSpinLoop();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getDragonPhoenixEngineConfigFn();
      if (!mountedRef.current) return;
      setDragonPhoenixConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      dragonPhoenixAudio.playReelStop();
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await dragonPhoenixFreeSpinFn({ data: { sessionId } });
        ...
        const res = await dragonPhoenixSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await dragonPhoenixBuyFeatureFn({ data: { bet } });
