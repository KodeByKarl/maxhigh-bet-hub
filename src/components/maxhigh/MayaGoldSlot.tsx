/**
 * Maya Gold — Sweet Bonanza layout (cluster-pay tumble slot).
...
import { mayaGoldAudio } from "./maya-gold/audio";
import {
  mayaGoldBuyFeatureFn,
  mayaGoldFreeSpinFn,
  mayaGoldSpinFn,
  getMayaGoldSessionFn,
} from "@/functions/api";
import { ANIM } from "./maya-gold/animationConfig";
import { MayaGoldFeatureBadge } from "./maya-gold/MayaGoldFeatureBadge";
import { FreeSpinsBadge } from "./maya-gold/FreeSpinsBadge";
import { BetSelectModal } from "./maya-gold/BetSelectModal";
import { initialBoard, nextKey, buildBoard } from "./maya-gold/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./maya-gold/paytable";
import { getRuntimeSymbols, setMayaGoldConfig } from "./maya-gold/runtimeConfig";
import type { BoardCell, SpinScript } from "./maya-gold/types";
import { CELLS, COLS, MAIN_CELLS, ROWS, TOP_COLS } from "./maya-gold/types";
import { WinCelebration } from "./maya-gold/WinCelebration";
import { FreeSpinsCongrats } from "./maya-gold/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./maya-gold/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./maya-gold/BuyFeatureModal";
import { PaytableModal } from "./maya-gold/PaytableModal";
import { GameMenuModal } from "./maya-gold/GameMenuModal";
import { AutoSpinModal, type AutoSpinOptions } from "./maya-gold/AutoSpinModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./maya-gold/WinLedger";
import { ReelCell, type ReelPhase } from "./maya-gold/ReelCell";
import { getMayaGoldEngineConfigFn } from "@/functions/superadmin";
...
export function MayaGoldSlot({
  ...
  const [muted, setMuted] = useState(() => mayaGoldAudio.isMuted);
  ...
  useEffect(() => {
    ...
    preloadAssets();
    mayaGoldAudio.preload();
    if (gameId === MAYA_GOLD_GAME_ID || gameId === "maya-gold") {
      void getMayaGoldEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setMayaGoldConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getMayaGoldSessionFn()
        .then((session) => {
    ...
    return () => {
      ...
      clearTrackedTimers();
      mayaGoldAudio.stopSpinLoop();
      mayaGoldAudio.stopAmbient();
    };
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== MAYA_GOLD_GAME_ID && gameId !== "maya-gold") return;
    try {
      const cfg = await getMayaGoldEngineConfigFn();
      if (mountedRef.current) setMayaGoldConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        mayaGoldAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }

      if (session.inFree && session.freeSpinsLeft > 0) {
        mayaGoldAudio.playScatterTrigger();
        applySession({
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getMayaGoldSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof mayaGoldSpinFn>>;
        if (isFree) {
          ...
          settled = await mayaGoldFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await mayaGoldSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[MayaGold] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await mayaGoldBuyFeatureFn({
