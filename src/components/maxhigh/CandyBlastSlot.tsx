/**
 * Candy Blast — Sweet Bonanza layout (cluster-pay tumble slot).
...
import {
  candyBlastBuyFeatureFn,
  candyBlastFreeSpinFn,
  candyBlastSpinFn,
  getCandyBlastSessionFn,
} from "@/functions/api";
import { ANIM } from "./candy-blast/animationConfig";
import { CandyBlastFeatureBadge } from "./candy-blast/CandyBlastFeatureBadge";
import { FreeSpinsBadge } from "./candy-blast/FreeSpinsBadge";
import { CandyBlastSidePanel } from "./candy-blast/CandyBlastSidePanel";
import { initialBoard, nextKey, buildBoard } from "./candy-blast/gridState";
import {
  BET_STEPS,
  ICON_SRC,
  getAnteMult,
...
} from "./candy-blast/paytable";
import { getRuntimeSymbols, setCandyBlastConfig } from "./candy-blast/runtimeConfig";
import type { BoardCell, SpinScript } from "./candy-blast/types";
import { CELLS, COLS, ROWS } from "./candy-blast/types";
import { WinCelebration } from "./candy-blast/WinCelebration";
import { FreeSpinsCongrats } from "./candy-blast/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./candy-blast/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./candy-blast/BuyFeatureModal";
import { PaytableModal } from "./candy-blast/PaytableModal";
import { GameMenuModal } from "./candy-blast/GameMenuModal";
import { WinLedger, mergeLedgerRows, type LedgerRow } from "./candy-blast/WinLedger";
import { ReelCell, type ReelPhase } from "./candy-blast/ReelCell";
import { getCandyBlastEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  ...
  const urls = [
    "/images/symbols/candy-blast/backdrop.webp",
    "/images/symbols/candy-blast/lollipop.png",
...
export function CandyBlastSlot({
  gameId = "candy-blast",
}: {
  gameId?: string;
  gameName?: string;
  ...
  useEffect(() => {
    ...
    preloadAssets();
    sounds.preload();
    if (gameId === CANDY_BLAST_GAME_ID || gameId === "candy-blast") {
      void getCandyBlastEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setCandyBlastConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getCandyBlastSessionFn()
        .then((session) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== CANDY_BLAST_GAME_ID && gameId !== "candy-blast") return;
    try {
      const cfg = await getCandyBlastEngineConfigFn();
      if (mountedRef.current) setCandyBlastConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof candyBlastSpinFn>>;
        if (isFree) {
          ...
          settled = await candyBlastFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await candyBlastSpinFn({ data: { bet, ante } });
          setLedger([]);
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("[CandyBlast] spin failed", err);
  ...
  const buyFeature = useCallback(async () => {
    ...
    setBuyOpen(false);
    try {
      const bought = await candyBlastBuyFeatureFn({
  ...
  return (
    <div className="relative flex h-dvh w-full flex-col overflow-hidden select-none">
      <img
        src="/images/symbols/candy-blast/backdrop.webp"
        alt=""
            ...
            <div className="flex h-full min-h-0 min-w-0 w-full flex-col items-center">
              ...
              >
                ...
                >
                  ...
                  <img
                    src="/images/symbols/candy-blast/lollipop.png"
                    alt=""
