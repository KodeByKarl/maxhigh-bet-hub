...
import { getPiratePlunderConfig, getRuntimeSymbols } from "./runtimeConfig";
...
export function payForCount(sym: CellSym, count: number): number {
  const min = getPiratePlunderConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getPiratePlunderConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getPiratePlunderConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getPiratePlunderConfig().superBuyFeatureMult;
}
export function getAnteMult() {
  return getPiratePlunderConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getPiratePlunderConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getPiratePlunderConfig().freeSpinsRetrigger;
}
