...
import { getJadeJaguarConfig, getRuntimeSymbols } from "./runtimeConfig";
import { jadeJaguarTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...jadeJaguarTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getJadeJaguarConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getJadeJaguarConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getJadeJaguarConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getJadeJaguarConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getJadeJaguarConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getJadeJaguarConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getJadeJaguarConfig().freeSpinsRetrigger;
}
