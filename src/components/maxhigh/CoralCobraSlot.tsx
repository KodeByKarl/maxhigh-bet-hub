...
import { coralCobraAudio } from "./coral-cobra/audio";
import {
  coralCobraBuyFeatureFn,
  coralCobraFreeSpinFn,
  coralCobraSpinFn,
  getCoralCobraSessionFn,
} from "@/functions/api";
import { ANIM } from "./coral-cobra/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./coral-cobra/playbackHelpers";
import { BetSelectModal } from "./coral-cobra/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./coral-cobra/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./coral-cobra/paytable";
import { setCoralCobraConfig } from "./coral-cobra/runtimeConfig";
import type { BoardCell, SpinScript } from "./coral-cobra/types";
import { CELLS, COLS, MAIN_CELLS, ROWS } from "./coral-cobra/types";
import { WinCelebration } from "./coral-cobra/WinCelebration";
import { FreeSpinsCongrats } from "./coral-cobra/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./coral-cobra/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./coral-cobra/BuyFeatureModal";
import { PaytableModal } from "./coral-cobra/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./coral-cobra/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./coral-cobra/ReelGrid";
import { coralCobraTheme } from "./coral-cobra/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./coral-cobra/ReelCell";
import { getCoralCobraEngineConfigFn } from "@/functions/superadmin";
...
const MAIN_INDICES = Object.freeze(Array.from({ length: MAIN_CELLS }, (_, i) => i));
...
function preloadAssets() {
  coralCobraAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    coralCobraTheme.assets.backdrop,
    coralCobraTheme.assets.loadingBg,
    ...Object.values(coralCobraTheme.assets.icons),
  ];
...
export function CoralCobraSlot({
  gameId = "coral-cobra",
}: {
  gameId?: string;
  gameName?: string;
  ...
  const [muted, setMuted] = useState(() => coralCobraAudio.isMuted);
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(76,5,25,0.96), rgba(4,47,46,0.96))",
                color: "#FDA4AF",
              }}
            >
        ...
        () => (
          <div
            ...
            style={{
              ...
              background: isWin
                ? "linear-gradient(135deg, #0D9488 0%, #134E4A 55%, #4C0519 100%)"
                : "linear-gradient(135deg, rgba(76,5,25,0.96), rgba(4,47,46,0.96))",
              color: isWin ? "#99F6E4" : "#FDA4AF",
            }}
          >
            <div
              className="text-[10px] font-black uppercase tracking-[0.18em]"
              style={{ color: isWin ? "rgba(153,246,228,0.85)" : "rgba(254,205,211,0.85)" }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      coralCobraAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        coralCobraAudio.stopSpinLoop();
        return;
      }
      coralCobraAudio.stopSpinLoop();
      coralCobraAudio.playReelStop(5);
      coralCobraAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === CORAL_COBRA_GAME_ID || gameId === "coral-cobra") {
      void getCoralCobraEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setCoralCobraConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getCoralCobraSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[CoralCobra] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== CORAL_COBRA_GAME_ID && gameId !== "coral-cobra") return;
    try {
      const cfg = await getCoralCobraEngineConfigFn();
      if (mountedRef.current) setCoralCobraConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        coralCobraAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    coralCobraAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    coralCobraAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        coralCobraAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getCoralCobraSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof coralCobraSpinFn>>;
        if (isFree) {
          ...
          settled = await coralCobraFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await coralCobraSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[CoralCobra] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await coralCobraBuyFeatureFn({
        ...
        <div className="mx-auto flex h-full min-h-0 w-full max-w-[840px] flex-col">
          ...
          <div className="relative flex min-h-0 w-full flex-1 flex-col">
            ...
            >
              ...
              {/* MAIN 5×5 — square card tiles (not Golden Panther open reels) */}
              <div className="relative z-10 flex min-h-0 w-full flex-1 items-center justify-center p-1.5 sm:p-2.5">
                {showTumbleBadge && (
                ...
                <div
                  className="relative z-[2] grid size-full max-h-full max-w-full gap-1 sm:gap-1.5"
                  style={{
                    aspectRatio: "1 / 1",
                    width: "min(100%, calc((100dvh - 14rem) * 1))",
