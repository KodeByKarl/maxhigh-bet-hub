...
import { getLotusLynxConfig, getRuntimeSymbols } from "./runtimeConfig";
import { lotusLynxTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...lotusLynxTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getLotusLynxConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getLotusLynxConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getLotusLynxConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getLotusLynxConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getLotusLynxConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getLotusLynxConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getLotusLynxConfig().freeSpinsRetrigger;
}
