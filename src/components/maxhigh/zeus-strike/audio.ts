/**
 * Zeus Strike — Web Audio sound design.
 ...
 */
const BASE = "/sounds/zeus-strike";

export const ZEUS_STRIKE_SOUND_FILES = {
...
type SoundId = keyof typeof ZEUS_STRIKE_SOUND_FILES;

const MUTE_KEY = "zeus-strike-muted";
const VOL_KEY = "zeus-strike-volume";

type ProgressCb = (loaded: number, total: number) => void;

class ZeusStrikeAudio {
  ...
  get isReady() {
    return this.buffers.size === Object.keys(ZEUS_STRIKE_SOUND_FILES).length;
  }
  ...
  private async loadAll(onProgress?: ProgressCb) {
    const entries = Object.entries(ZEUS_STRIKE_SOUND_FILES) as [SoundId, string][];
...
export const zeusStrikeAudio = new ZeusStrikeAudio();
