/**
 * Halo-Halo Hits slot UI — owns its own spin state machine.
...
import { haloHaloHitsFn, getHaloHaloHitsEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./halo-halo-hits/animationConfig";
import { MultiplierReelView } from "./halo-halo-hits/MultiplierReelView";
import { PaytableModal } from "./halo-halo-hits/PaytableModal";
import { ReelCell, type ReelPhase } from "./halo-halo-hits/ReelCell";
import { getHaloHaloHitsConfig, setHaloHaloHitsConfig } from "./halo-halo-hits/runtimeConfig";
import { planHaloHaloHitsPlayback, type FoPlaybackStep } from "./halo-halo-hits/spinPlayback";
import { cellKey, type FoGrid, type MultiplierReelResult } from "./halo-halo-hits/types";

function idleGrid(): FoGrid {
  const cfg = getHaloHaloHitsConfig();
  const kinds: FoSymKind[] = ["ruby", "emerald", "sapphire", "amethyst", "topaz", "temple"];
...
export function HaloHaloHitsSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<HaloHaloHitsConfig>(() => getHaloHaloHitsConfig());
  ...
  const runStep = useCallback(
    (runId: number, steps: FoPlaybackStep[], index: number) => {
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await haloHaloHitsFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planHaloHaloHitsPlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getHaloHaloHitsEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setHaloHaloHitsConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[HaloHaloHits] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
