/**
 * Desert Riches — connecting ways on a 3-4-5-4-3 diamond board.
 ...
 */
import type { FgSymKind, DesertRichesConfig } from "@/lib/desert-riches-config";
import type { FgGrid, PaylineWin } from "./types";

function isWild(kind: FgSymKind, cfg: DesertRichesConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: DesertRichesConfig): boolean {
...
/** @deprecated Alias — Desert Riches now uses connecting ways. */
