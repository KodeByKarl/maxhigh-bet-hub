/**
 * Crazy Sevens slot UI — classic 3-reel cabinet with hold buttons.
 * Non-authoritative: outcomes from crazySevensSpinFn only.
...
import {
  getCrazySevensEngineConfigFn,
  getCrazySevensJackpotFn,
  crazySevensSpinFn,
} from "@/functions/api";
import { useAuth } from "@/lib/auth";
import { CRAZY_SEVENS_GAME_ID, type CrazySevensConfig } from "@/lib/crazy-sevens-config";
import { cn } from "@/lib/utils";
import { ANIM, BLUR_SYMBOLS, ICON_SRC, SYM_NAME } from "./crazy-sevens/animationConfig";
import { CrazySevensIcon } from "./crazy-sevens/CrazySevensIcon";
import { PaytableModal } from "./crazy-sevens/PaytableModal";
import { getCrazySevensConfig, setCrazySevensConfig } from "./crazy-sevens/runtimeConfig";
import type { RrReels, SpinScript } from "./crazy-sevens/types";
...
export function CrazySevensSlot({
  gameId: _gameId = CRAZY_SEVENS_GAME_ID,
  gameName = "Crazy Sevens",
}: {
  gameId?: string;
  gameName?: string;
} = {}) {
  ...
  const [cfgSnap, setCfgSnap] = useState<CrazySevensConfig>(() => getCrazySevensConfig());
  ...
  useEffect(() => {
    ...
    void Promise.all([getCrazySevensEngineConfigFn(), getCrazySevensJackpotFn()])
      .then(([cfg, jp]) => {
        if (!mountedRef.current) return;
        setCrazySevensConfig(cfg);
        setCfgSnap(getCrazySevensConfig());
        setJpPool(jp.amount);
      })
      .catch(() => undefined);
    return () => {
  ...
  const startSpin = async () => {
    ...
    try {
      const result = await crazySevensSpinFn({
        data: {
          bet,
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden text-white">
      <style>{CRAZY_SEVENS_CSS}</style>

      <img
        src="/games/crazy-sevens.webp"
        alt=""
