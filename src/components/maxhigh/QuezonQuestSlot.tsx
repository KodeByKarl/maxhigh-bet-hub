/**
 * Quezon Quest — polished layout matching Candy Peak chrome.
...
import { getQuezonQuestEngineConfigFn } from "@/functions/superadmin";
import {
  getQuezonQuestSessionFn,
  quezonQuestBuyFeatureFn,
  quezonQuestFreeSpinFn,
  quezonQuestSpinFn,
} from "@/functions/api";
import { ANIM } from "./quezon-quest/animationConfig";
import { quezonQuestAudio } from "./quezon-quest/audio";
import { QuezonQuestSidePanel } from "./quezon-quest/QuezonQuestSidePanel";
import { ReelCell, type ReelPhase } from "./quezon-quest/ReelCell";
import { TILE_IMAGE_MAP } from "./quezon-quest/QuezonQuestIcon";
import { PaytableModal } from "./quezon-quest/PaytableModal";
import { getQuezonQuestConfig, setQuezonQuestConfig } from "./quezon-quest/runtimeConfig";
import type { BoardCell, SpinScript } from "./quezon-quest/types";

type QuezonQuestSlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_QUEZON_QUEST_CONFIG.symbols;
...
const QUEZON_QUEST_BACKDROP = "/images/symbols/quezon-quest/backdrop.webp";

function preloadAssets() {
  if (typeof Image === "undefined") return;
  const urls = [
    "/games/quezon-quest.webp",
    QUEZON_QUEST_BACKDROP,
...
export function QuezonQuestSlot({ onBalanceUpdate }: QuezonQuestSlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getQuezonQuestConfig();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    quezonQuestAudio.preload();
    void getQuezonQuestEngineConfigFn()
      .then((cfg) => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getQuezonQuestEngineConfigFn();
      if (!mountedRef.current) return;
      setQuezonQuestConfig(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await quezonQuestFreeSpinFn({ data: { sessionId } });
        ...
        const res = await quezonQuestSpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await quezonQuestBuyFeatureFn({ data: { bet } });
