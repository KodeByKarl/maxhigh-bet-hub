/**
 * Thor Thunder — Web Audio sound design.
 ...
 */
const BASE = "/sounds/thor-thunder";

export const THOR_THUNDER_SOUND_FILES = {
...
type SoundId = keyof typeof THOR_THUNDER_SOUND_FILES;

const MUTE_KEY = "thor-thunder-muted";
const VOL_KEY = "thor-thunder-volume";

type ProgressCb = (loaded: number, total: number) => void;

class ThorThunderAudio {
  ...
  get isReady() {
    return this.buffers.size === Object.keys(THOR_THUNDER_SOUND_FILES).length;
  }
  ...
  private async loadAll(onProgress?: ProgressCb) {
    const entries = Object.entries(THOR_THUNDER_SOUND_FILES) as [SoundId, string][];
...
export const thorThunderAudio = new ThorThunderAudio();
