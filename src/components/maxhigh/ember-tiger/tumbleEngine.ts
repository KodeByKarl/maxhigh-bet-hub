...
import { CELLS, COLS, REEL_HEIGHTS, cellIndex, colHeight, colStart } from "./types";
...
export function evaluateBoard(board: BoardCell[], bet: number): EvalResult {
  const minCluster = getEmberTigerConfig().minCluster;
...
/**
 * Gravity on diamond columns (3-4-5-4-3): survivors fall down, refill at top.
 */
export function applyGravity(
  ...
  for (let col = 0; col < COLS; col++) {
    ...
    const start = colStart(col);
    const oldRowByKey = new Map<string, number>();

    const kept: BoardCell[] = [];
    for (let row = height - 1; row >= 0; row--) {
      const cell = board[start + row];
      if (cell && !remove.has(cell.key)) {
        kept.push(cell);
        oldRowByKey.set(cell.key, row);
      }
    }
    ...
    for (let i = 0; i < height; i++) {
      const cell = kept[i];
      const newRow = height - 1 - i;
      next[cellIndex(col, newRow)] = cell;
