...
import { clampBombMult } from "@/lib/ember-tiger-config";
...
/**
 * Symbol glyph — Pug Den style: contain + padded so one full icon per circle.
 * Never cover-crop (that mashed multi-asset sheets into one cell).
 */
export const PantherIcon = memo(function PantherIcon({
  ...
  const src = ICON_SRC[kind] || "/images/symbols/ember-tiger/10-cell.png?v=7";

  return (
    <div
      className={cn(
        "relative grid size-full place-items-center overflow-hidden rounded-full bg-transparent select-none",
        className,
      )}
    >
      {winLit && (
      ...
      <img
        ...
        className={cn(
          ...
          fit === "cover" ? "size-full object-cover" : "size-[82%] object-contain",
      ...
      {isScatter && (
        <span
          className="absolute bottom-1 left-1/2 z-[30] -translate-x-1/2 rounded-full border px-1.5 py-0.5 text-[7px] font-black uppercase shadow-sm whitespace-nowrap sm:text-[8px]"
          style={{
