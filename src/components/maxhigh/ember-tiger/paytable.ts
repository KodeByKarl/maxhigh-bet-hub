...
import { getEmberTigerConfig, getRuntimeSymbols } from "./runtimeConfig";
import { emberTigerTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...emberTigerTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getEmberTigerConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getEmberTigerConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getEmberTigerConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getEmberTigerConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getEmberTigerConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getEmberTigerConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getEmberTigerConfig().freeSpinsRetrigger;
}
