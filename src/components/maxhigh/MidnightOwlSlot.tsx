...
import { midnightOwlAudio } from "./midnight-owl/audio";
import {
  midnightOwlBuyFeatureFn,
  midnightOwlFreeSpinFn,
  midnightOwlSpinFn,
  getMidnightOwlSessionFn,
} from "@/functions/api";
import { ANIM } from "./midnight-owl/animationConfig";
import {
  getAutoSpinGapMs,
  getDropWaitMs,
  getPopWaitMs,
  getRefillWaitMs,
  scaleWaitMs,
} from "./midnight-owl/playbackHelpers";
import { BetSelectModal } from "./midnight-owl/BetSelectModal";
import { buildBuyScatterIntroBoard, initialBoard } from "./midnight-owl/gridState";
import {
  BET_STEPS,
  BUY_FS_START_BET,
  getAnteMult,
  getBuyUnitPrice,
} from "./midnight-owl/paytable";
import { setMidnightOwlConfig } from "./midnight-owl/runtimeConfig";
import type { BoardCell, SpinScript } from "./midnight-owl/types";
import { CELLS } from "./midnight-owl/types";
import { WinCelebration } from "./midnight-owl/WinCelebration";
import { FreeSpinsCongrats } from "./midnight-owl/FreeSpinsCongrats";
import { FreeSpinsTriggerModal } from "./midnight-owl/FreeSpinsTriggerModal";
import { BuyFeatureModal } from "./midnight-owl/BuyFeatureModal";
import { PaytableModal } from "./midnight-owl/PaytableModal";
import { AutoSpinModal, type AutoSpinOptions } from "./midnight-owl/AutoSpinModal";
import { ReelGrid, type ReelVisuals } from "./midnight-owl/ReelGrid";
import { midnightOwlTheme } from "./midnight-owl/theme";
import { themeToCssVars } from "./panther-shared/theme";
import { type ReelPhase } from "./midnight-owl/ReelCell";
import { getMidnightOwlEngineConfigFn } from "@/functions/superadmin";
...
function preloadAssets() {
  midnightOwlAudio.preload();
  if (typeof Image === "undefined") return;
  const urls = [
    midnightOwlTheme.assets.backdrop,
    midnightOwlTheme.assets.loadingBg,
    ...Object.values(midnightOwlTheme.assets.icons),
  ];
...
export function MidnightOwlSlot({
  gameId = "midnight-owl",
}: {
  gameId?: string;
  gameName?: string;
  ...
  const [muted, setMuted] = useState(() => midnightOwlAudio.isMuted);
          ...
          () => (
            <div
              ...
              style={{
                ...
                background:
                  "linear-gradient(135deg, rgba(8,8,20,0.96), rgba(49,46,129,0.96))",
                color: "#C7D2FE",
              }}
            >
              <div className="text-[10px] font-black uppercase tracking-[0.18em] text-indigo-200/80">
                Bet
              </div>
        ...
        () => (
          <div
            ...
            style={{
              ...
              background: isWin
                ? "linear-gradient(135deg, #6366F1 0%, #4C1D95 55%, #080814 100%)"
                : "linear-gradient(135deg, rgba(8,8,20,0.96), rgba(49,46,129,0.96))",
              color: isWin ? "#EEF2FF" : "#C7D2FE",
            }}
          >
            <div
              className="text-[10px] font-black uppercase tracking-[0.18em]"
              style={{
                color: isWin ? "rgba(224,231,255,0.85)" : "rgba(199,210,254,0.85)",
              }}
            >
              {title}
            </div>
  ...
  const playBuyScatterIntro = useCallback(
    async (gen: number, fsCount: number, useAnte: boolean) => {
      ...
      midnightOwlAudio.startSpinLoop();
      try {
        await wait(getDropWaitMs(), gen);
      } catch {
        midnightOwlAudio.stopSpinLoop();
        return;
      }
      midnightOwlAudio.stopSpinLoop();
      midnightOwlAudio.playReelStop(5);
      midnightOwlAudio.playScatterTrigger();
  ...
  useEffect(() => {
    ...
    preloadAssets();
    if (gameId === MIDNIGHT_OWL_GAME_ID || gameId === "midnight-owl") {
      void getMidnightOwlEngineConfigFn()
        .then((cfg) => {
          if (mountedRef.current) setMidnightOwlConfig(cfg);
        })
        .catch(() => {
          /* keep defaults */
        });
      void getMidnightOwlSessionFn()
        .then((session) => {
        ...
        .catch((err) => {
          console.error("[MidnightOwl] session load failed", err);
        });
    }
    return () => {
  ...
  const refreshEngineConfig = useCallback(async () => {
    if (gameId !== MIDNIGHT_OWL_GAME_ID && gameId !== "midnight-owl") return;
    try {
      const cfg = await getMidnightOwlEngineConfigFn();
      if (mountedRef.current) setMidnightOwlConfig(cfg);
    } catch {
      /* keep last known */
    }
  ...
  const playScript = useCallback(
    async (script: SpinScript, isFree: boolean, gen: number, startBalance: number) => {
      ...
      for (const step of script.steps) {
        ...
        setPhase("popping");
        midnightOwlAudio.playCascadeTick();
  ...
  const dismissFsSummary = useCallback(() => {
    ...
    midnightOwlAudio.endFreespins();
  }, []);

  const setMutedState = useCallback((on: boolean) => {
    midnightOwlAudio.setMuted(on);
    setMuted(on);
  }, []);
  ...
  const finishBase = useCallback(
    (script: SpinScript, session: { sessionId: string | null; freeSpinsLeft: number; inFree: boolean }) => {
      if (script.totalWin > 0) {
        ...
        midnightOwlAudio.playWin(script.totalWin, totalBet);
      } else {
        setLastWin(0);
      }
  ...
  const spin = useCallback(
    async (asFree = false) => {
      ...
      if (!preferFree || (preferFree && !playSessionIdRef.current)) {
        try {
          const session = await getMidnightOwlSessionFn();
          if (session.sessionId && session.freeSpinsLeft > 0) {
            applySession({
              sessionId: session.sessionId,
              freeSpinsLeft: session.freeSpinsLeft,
      ...
      try {
        ...
        let settled: Awaited<ReturnType<typeof midnightOwlSpinFn>>;
        if (isFree) {
          ...
          settled = await midnightOwlFreeSpinFn({ data: { sessionId } });
        } else {
          settled = await midnightOwlSpinFn({ data: { bet, ante } });
          setDropTotal(0);
        }
        ...
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          ...
          console.error("[MidnightOwl] spin failed", err);
          toast.error(msg);
          setAutoSpin(false);
        }
      } finally {
  ...
  const buyFeature = useCallback(
    async (buyBet: number, quantity: number) => {
      ...
      setBet(buyBet);
      try {
        const bought = await midnightOwlBuyFeatureFn({
