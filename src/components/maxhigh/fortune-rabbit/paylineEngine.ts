import { betPerLine, type FortuneRabbitConfig, type FrSymKind } from "@/lib/fortune-rabbit-config";
import type { FrGrid, PaylineWin } from "./types";

function isWild(kind: FrSymKind, cfg: FortuneRabbitConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPaying(kind: FrSymKind, cfg: FortuneRabbitConfig) {
...
export function evaluatePaylines(
  ...
  for (let lineIndex = 0; lineIndex < cfg.paylineCount; lineIndex++) {
    ...
    let target: FrSymKind | null = null;
