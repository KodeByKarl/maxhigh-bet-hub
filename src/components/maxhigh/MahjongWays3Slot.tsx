/**
 * Mahjong Ways 3 — polished layout matching Candy Peak chrome.
...
import { getMahjongWays3EngineConfigFn } from "@/functions/superadmin";
import {
  getMahjongWays3SessionFn,
  mahjongWays3BuyFeatureFn,
  mahjongWays3FreeSpinFn,
  mahjongWays3SpinFn,
} from "@/functions/api";
import { ANIM } from "./mahjong-ways-3/animationConfig";
import { mahjongWays3Audio } from "./mahjong-ways-3/audio";
import { TILE_IMAGE_MAP } from "./mahjong-ways-3/Mahjong3Icon";
import { Mahjong3SidePanel } from "./mahjong-ways-3/Mahjong3SidePanel";
import { ReelCell, type ReelPhase } from "./mahjong-ways-3/ReelCell";
import { getMahjongWays3Config, setMahjongWays3Config } from "./mahjong-ways-3/runtimeConfig";
import type { BoardCell, SpinScript } from "./mahjong-ways-3/types";

type MahjongWays3SlotProps = {
...
function createInitialBoard(): BoardCell[] {
  const symbols = DEFAULT_MAHJONG_WAYS_3_CONFIG.symbols;
...
function preloadAssets() {
  ...
  const urls = ["/games/mahjong-ways-3.webp", ...Object.values(TILE_IMAGE_MAP)];
...
export function MahjongWays3Slot({ onBalanceUpdate }: MahjongWays3SlotProps) {
  const { user } = useAuth();
  ...
  const engineCfg = getMahjongWays3Config();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      mahjongWays3Audio.stopSpinLoop();
  ...
  const refreshEngineConfig = useCallback(async () => {
    try {
      const cfg = await getMahjongWays3EngineConfigFn();
      if (!mountedRef.current) return;
      setMahjongWays3Config(cfg);
      setCfgTick((n) => n + 1);
    } catch {
  ...
  const animateScript = useCallback(
    async (script: SpinScript, gen: number) => {
      ...
      mahjongWays3Audio.playReelStop();
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (inFree && sessionId) {
        const res = await mahjongWays3FreeSpinFn({ data: { sessionId } });
        ...
        const res = await mahjongWays3SpinFn({ data: { bet, ante } });
        if (abortIfStale()) return;
  ...
  const handleBuyFeature = async () => {
    ...
    try {
      const res = await mahjongWays3BuyFeatureFn({ data: { bet } });
