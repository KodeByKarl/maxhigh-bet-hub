...
import { getCoralCobraConfig, getRuntimeSymbols } from "./runtimeConfig";
import { coralCobraTheme } from "./theme";
...
export const ICON_SRC: Record<SymKind, string> = { ...coralCobraTheme.assets.icons };

export function payForCount(sym: CellSym, count: number): number {
  const min = getCoralCobraConfig().minCluster;
...
export function scatterCashPay(count: number, bet: number): number {
  const tiers = [...getCoralCobraConfig().scatterCashTiers].sort((a, b) => b.count - a.count);
...
export function getBuyFeatureMult() {
  return getCoralCobraConfig().buyFeatureMult;
}
export function getSuperBuyFeatureMult() {
  return getCoralCobraConfig().superBuyFeatureMult;
}
...
export function getAnteMult() {
  return getCoralCobraConfig().anteBetMult;
}
export function getFreeSpinsBase() {
  return getCoralCobraConfig().freeSpinsBase;
}
export function getFreeSpinsRetrigger() {
  return getCoralCobraConfig().freeSpinsRetrigger;
}
