/**
 * Diamond Dig Dig — connecting ways on a 3-4-5-4-3 diamond board.
...
function isWild(kind: FgSymKind, cfg: DiamondDigConfig) {
  return !!cfg.symbols.find((s) => s.kind === kind)?.wild;
}

function isPayingTarget(kind: FgSymKind, cfg: DiamondDigConfig): boolean {
