/**
 * Fortune Mouse slot UI — owns its own spin state machine.
...
import { fortuneMouseSpinFn, getFortuneMouseEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM, BG_SRC } from "./fortune-mouse/animationConfig";
import { MultiplierReelView } from "./fortune-mouse/MultiplierReelView";
import { PaytableModal } from "./fortune-mouse/PaytableModal";
import { ReelCell, type ReelPhase } from "./fortune-mouse/ReelCell";
import { getFortuneMouseConfig, setFortuneMouseConfig } from "./fortune-mouse/runtimeConfig";
import { planFortuneMousePlayback, type FoPlaybackStep } from "./fortune-mouse/spinPlayback";
import { cellKey, type FoGrid, type MultiplierReelResult } from "./fortune-mouse/types";
...
export function FortuneMouseSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<FortuneMouseConfig>(() => getFortuneMouseConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await fortuneMouseSpinFn({ data: { bet: stake, exMode: ex } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const steps = planFortuneMousePlayback(res.script, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getFortuneMouseEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setFortuneMouseConfig(cfg);
        setCfgSnap(next);
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[FortuneMouse] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
