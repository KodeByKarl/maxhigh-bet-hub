/**
 * Goal Rush slot UI — owns its own spin state machine.
...
import { goalRushSpinFn, getGoalRushEngineConfigFn } from "@/functions/api";
...
import { BET_STEPS, ANIM } from "./goal-rush/animationConfig";
import { GoalRushBorder } from "./goal-rush/GoalRushBorder";
import { ReelCell, type ReelPhase } from "./goal-rush/ReelCell";
import { getGoalRushConfig, setGoalRushConfig } from "./goal-rush/runtimeConfig";
import { planGoalRushPlayback, type GrPlaybackStep } from "./goal-rush/spinPlayback";
import { cellKey, type GrGrid } from "./goal-rush/types";

function idleGrid(): GrGrid {
  const cfg = getGoalRushConfig();
...
export function GoalRushSlot({
  ...
  const [cfgSnap, setCfgSnap] = useState<GoalRushConfig>(() => getGoalRushConfig());
  ...
  const startSpin = useCallback(() => {
    ...
    void (async () => {
      try {
        const res = await goalRushSpinFn({ data: { bet: stake } });
        if (!mountedRef.current || runId !== runIdRef.current) return;
        setBalanceLocal(res.balance);
        const cfg = getGoalRushConfig();
        const steps = planGoalRushPlayback(res.script, cfg.reelsCount, turboRef.current);
  ...
  useEffect(() => {
    ...
    void getGoalRushEngineConfigFn()
      .then((cfg) => {
        if (!mountedRef.current) return;
        const next = setGoalRushConfig(cfg);
        setCfgSnap(next);
        setUi((prev) =>
          prev.locked
            ? prev
  ...
  useEffect(() => {
    if (!ui.locked) return;
    const id = setTimeout(() => {
      console.warn("[GoalRush] watchdog unlock — spin exceeded 12s");
      runIdRef.current += 1;
      clearTimer();
      lockedRef.current = false;
      applyIdle();
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#120806] text-white">
      ...
      <div
        ...
        style={{ backgroundImage: "url(/images/goal-rush-bg.png)" }}
      />
      <div
      ...
      <header className="relative z-10 flex shrink-0 flex-col items-center gap-2.5 px-3 pt-3 sm:pt-4">
        <GoalRushBorder rounded="trap" thickness={3} className="w-full max-w-md">
        ...
        <div className="flex w-full max-w-md items-center justify-center gap-3">
          <GoalRushBorder rounded="xl" thickness={2} className="min-w-[8.5rem] flex-1">
      ...
      <footer className="relative z-10 shrink-0 px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
        <GoalRushBorder rounded="2xl" thickness={3} className="mx-auto w-full max-w-lg">
