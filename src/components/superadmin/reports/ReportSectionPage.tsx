...
const META: Record<ReportSection, { title: string; blurb: string }> = {
  ...
  transactions: {
    ...
    blurb: "Chip transfers and ledger adjustments across the platform.",
  },
  "chip-distribution": {
    title: "Chip Distribution Log",
    blurb: "Audit log of staff balance adjustments and related platform transfers.",
  },
};
...
function FundHistoryPanel({ enabled }: { enabled: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const txs = await listAdminTransactionsFn({
        data: { q: q || undefined, type: "adjust", limit: 300 },
      });
      setRows(txs);
    } catch (e) {
...
function ChipDistributionPanel({ enabled }: { enabled: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { listChipDistributionLogsFn } = await import("@/functions/admin");
  ...
  return (
    <>
      ...
      {!loading && logs.length === 0 && (
        <Empty>No chip transfers recorded yet. Use Add Chips on Admins / Player List.</Empty>
      )}
