/**
 * Superadmin Color Game engine editor — spot multipliers + min/max bets.
 */
...
import {
  getColorGameEngineConfigFn,
  saveColorGameEngineConfigFn,
...
export function ColorGameConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ColorGameConfig>(() =>
    structuredClone(DEFAULT_COLOR_GAME_CONFIG),
  );
  ...
  const [betStepsText, setBetStepsText] = useState(
    DEFAULT_COLOR_GAME_CONFIG.betSteps.join(", "),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getColorGameEngineConfigFn()
      .then((remote) => {
        setCfg(remote);
        setBetStepsText(remote.betSteps.join(", "));
      })
      .catch(() => setCfg(structuredClone(DEFAULT_COLOR_GAME_CONFIG)))
  ...
  async function saveEngine() {
    ...
    try {
      ...
      const withSteps = normalizeColorGameConfig({ ...cfg, betSteps: steps });
      const next = await saveColorGameEngineConfigFn({ data: { config: withSteps } });
      setCfg(next);
      setBetStepsText(next.betSteps.join(", "));
      toast.success("Color Game engine saved");
    } catch (e) {
