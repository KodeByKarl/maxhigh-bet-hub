...
import {
  ...
  weightPercents,
  type SaRtpProfileId,
  type SuperAceSymbolConfig,
  type SuperAceConfig,
} from "@/lib/super-ace-config";
import {
  getSuperAceEngineConfigFn,
  saveSuperAceEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SuperAceIcon } from "@/components/maxhigh/super-ace/SuperAceIcon";
...
export function SuperAceConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SuperAceConfig>(() =>
    structuredClone(DEFAULT_SUPER_ACE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getSuperAceEngineConfigFn()
      .then((c) => setCfg(normalizeSuperAceConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_SUPER_ACE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSuperAceEngineConfigFn({
        data: { config: normalizeSuperAceConfig(cfg) },
      });
      setCfg(normalizeSuperAceConfig(next));
  ...
  function patch(partial: Partial<SuperAceConfig>) {
    setCfg((c) => normalizeSuperAceConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<SuperAceSymbolConfig>) {
    setCfg((c) =>
      normalizeSuperAceConfig({
        ...c,
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<SuperAceSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <SuperAceIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: SuperAceConfig;
  onPatch: (p: Partial<SuperAceConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-5">
            ...
            return (
              ...
              >
                <div className="size-10 overflow-hidden rounded-md bg-black/40">
                  <SuperAceIcon kind={kind} className="size-10" />
                </div>
      ...
      <label className="block max-w-xs space-y-1 text-xs font-semibold text-muted-foreground">
        ...
        <select
          ...
          onChange={(e) =>
            onPatch({
              volatility: e.target.value as SuperAceConfig["volatility"],
            })
          }
