...
import {
  getDiamondDigEngineConfigFn,
  saveDiamondDigEngineConfigFn,
...
export function DiamondDigConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<DiamondDigConfig>(() =>
    structuredClone(DEFAULT_DIAMOND_DIG_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getDiamondDigEngineConfigFn()
      .then((c) => setCfg(normalizeDiamondDigConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_DIAMOND_DIG_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<DiamondDigConfig>) {
    setCfg((c) => normalizeDiamondDigConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveDiamondDigEngineConfigFn({
        data: { config: normalizeDiamondDigConfig(cfg) },
      });
      setCfg(normalizeDiamondDigConfig(next));
      toast.success("Diamond Dig Dig engine saved");
    } catch (e) {
