...
import {
  getRiceFieldRichesEngineConfigFn,
  saveRiceFieldRichesEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { RiceFieldRichesIcon } from "@/components/maxhigh/rice-field-riches/RiceFieldRichesIcon";
...
export function RiceFieldRichesConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<RiceFieldRichesConfig>(() =>
    structuredClone(DEFAULT_RICE_FIELD_RICHES_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getRiceFieldRichesEngineConfigFn()
      .then((c) => setCfg(normalizeRiceFieldRichesConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_RICE_FIELD_RICHES_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveRiceFieldRichesEngineConfigFn({
        data: { config: normalizeRiceFieldRichesConfig(cfg) },
      });
      setCfg(normalizeRiceFieldRichesConfig(next));
      toast.success("Rice Field Riches engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<RiceFieldRichesConfig>) {
    setCfg((c) => normalizeRiceFieldRichesConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <RiceFieldRichesIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: RiceFieldRichesConfig;
  onPatch: (p: Partial<RiceFieldRichesConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
