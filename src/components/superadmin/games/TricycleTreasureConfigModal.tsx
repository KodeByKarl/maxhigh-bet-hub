...
import {
  getTricycleTreasureEngineConfigFn,
  saveTricycleTreasureEngineConfigFn,
...
export function TricycleTreasureConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<TricycleTreasureConfig>(() =>
    structuredClone(DEFAULT_TRICYCLE_TREASURE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getTricycleTreasureEngineConfigFn()
      .then((c) => setCfg(normalizeTricycleTreasureConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_TRICYCLE_TREASURE_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<TricycleTreasureConfig>) {
    setCfg((c) => normalizeTricycleTreasureConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveTricycleTreasureEngineConfigFn({
        data: { config: normalizeTricycleTreasureConfig(cfg) },
      });
      setCfg(normalizeTricycleTreasureConfig(next));
      toast.success("Tricycle Treasure engine saved");
    } catch (e) {
