...
import {
  ...
  weightPercents,
  type SaRtpProfileId,
  type MegaAceSymbolConfig,
  type MegaAceConfig,
} from "@/lib/mega-ace-config";
import {
  getMegaAceEngineConfigFn,
  saveMegaAceEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { MegaAceIcon } from "@/components/maxhigh/mega-ace/MegaAceIcon";
...
export function MegaAceConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<MegaAceConfig>(() =>
    structuredClone(DEFAULT_MEGA_ACE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getMegaAceEngineConfigFn()
      .then((c) => setCfg(normalizeMegaAceConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_MEGA_ACE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveMegaAceEngineConfigFn({
        data: { config: normalizeMegaAceConfig(cfg) },
      });
      setCfg(normalizeMegaAceConfig(next));
  ...
  function patch(partial: Partial<MegaAceConfig>) {
    setCfg((c) => normalizeMegaAceConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<MegaAceSymbolConfig>) {
    setCfg((c) =>
      normalizeMegaAceConfig({
        ...c,
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<MegaAceSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <MegaAceIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: MegaAceConfig;
  onPatch: (p: Partial<MegaAceConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-5">
            ...
            return (
              ...
              >
                <div className="size-10 overflow-hidden rounded-md bg-black/40">
                  <MegaAceIcon kind={kind} className="size-10" />
                </div>
      ...
      <label className="block max-w-xs space-y-1 text-xs font-semibold text-muted-foreground">
        ...
        <select
          ...
          onChange={(e) =>
            onPatch({
              volatility: e.target.value as MegaAceConfig["volatility"],
            })
          }
