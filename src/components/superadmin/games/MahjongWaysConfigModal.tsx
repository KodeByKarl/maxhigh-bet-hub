...
import {
  getMahjongWaysEngineConfigFn,
  saveMahjongWaysEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { MahjongIcon } from "@/components/maxhigh/mahjong-ways/MahjongIcon";
...
export function MahjongWaysConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<MahjongWaysConfig>(() =>
    structuredClone(DEFAULT_MAHJONG_WAYS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getMahjongWaysEngineConfigFn()
      .then((c) => setCfg(normalizeMahjongWaysConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_MAHJONG_WAYS_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveMahjongWaysEngineConfigFn({
        data: { config: normalizeMahjongWaysConfig(cfg) },
      });
      setCfg(normalizeMahjongWaysConfig(next));
      toast.success("Mahjong Ways engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<MahjongWaysConfig>) {
    setCfg((c) => normalizeMahjongWaysConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <MahjongIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: MahjongWaysConfig;
  onPatch: (p: Partial<MahjongWaysConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
