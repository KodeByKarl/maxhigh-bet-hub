...
import {
  ...
  weightPercents,
  type GateOfRaConfig,
  type GateOfRaSymbolConfig,
} from "@/lib/gate-of-ra-config";
import {
  getGateOfRaEngineConfigFn,
  saveGateOfRaEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SymbolIcon } from "@/components/maxhigh/gate-of-ra/SymbolIcon";
import type { SymKind } from "@/components/maxhigh/gate-of-ra/types";
...
export function GateOfRaConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<GateOfRaConfig>(() =>
    structuredClone(DEFAULT_GATE_OF_RA_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getGateOfRaEngineConfigFn()
      .then((c) => setCfg(normalizeGateOfRaConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_GATE_OF_RA_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveGateOfRaEngineConfigFn({
        data: { config: normalizeGateOfRaConfig(cfg) },
      });
      setCfg(next);
      toast.success("Gate of Ra engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<GateOfRaConfig>) {
    setCfg((c) => normalizeGateOfRaConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<GateOfRaSymbolConfig>) {
    setCfg((c) =>
      normalizeGateOfRaConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Gate of Ra appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: GateOfRaConfig;
  onPatch: (p: Partial<GateOfRaConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<GateOfRaSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
