...
import {
  ...
  weightPercents,
  type QuezonQuestSymbolConfig,
  type QuezonQuestConfig,
} from "@/lib/quezon-quest-config";
import {
  getQuezonQuestEngineConfigFn,
  saveQuezonQuestEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { QuezonQuestIcon } from "@/components/maxhigh/quezon-quest/QuezonQuestIcon";
...
export function QuezonQuestConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<QuezonQuestConfig>(() =>
    structuredClone(DEFAULT_QUEZON_QUEST_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getQuezonQuestEngineConfigFn()
      .then((c) => setCfg(normalizeQuezonQuestConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_QUEZON_QUEST_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveQuezonQuestEngineConfigFn({
        data: { config: normalizeQuezonQuestConfig(cfg) },
      });
      setCfg(normalizeQuezonQuestConfig(next));
      toast.success("Quezon Quest engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<QuezonQuestConfig>) {
    setCfg((c) => normalizeQuezonQuestConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<QuezonQuestSymbolConfig>) {
    setCfg((c) =>
      normalizeQuezonQuestConfig({
        ...c,
...
function SymbolsSection({
  ...
  cfg: QuezonQuestConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<QuezonQuestSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <QuezonQuestIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: QuezonQuestConfig;
  onPatch: (p: Partial<QuezonQuestConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
