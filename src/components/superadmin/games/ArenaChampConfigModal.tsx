...
import {
  getArenaChampEngineConfigFn,
  saveArenaChampEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/arena-champ/animationConfig";
...
function SymThumb({ kind, name }: { kind: GrSymKind; name: string }) {
...
export function ArenaChampConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ArenaChampConfig>(() =>
    structuredClone(DEFAULT_ARENA_CHAMP_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getArenaChampEngineConfigFn()
      .then((c) => setCfg(normalizeArenaChampConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_ARENA_CHAMP_CONFIG));
      })
  ...
  function patch(p: Partial<ArenaChampConfig>) {
    setCfg((c) => {
      const next = normalizeArenaChampConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: GrSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeArenaChampConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveArenaChampEngineConfigFn({
        data: { config: normalizeArenaChampConfig(cfg) },
      });
      setCfg(normalizeArenaChampConfig(next));
      toast.success("Arena Champ engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/arena-champ/</code>
              </p>
