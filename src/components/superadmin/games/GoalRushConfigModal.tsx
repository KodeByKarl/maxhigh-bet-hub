...
import {
  getGoalRushEngineConfigFn,
  saveGoalRushEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/goal-rush/animationConfig";
...
export function GoalRushConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<GoalRushConfig>(() =>
    structuredClone(DEFAULT_GOAL_RUSH_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getGoalRushEngineConfigFn()
      .then((c) => setCfg(normalizeGoalRushConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_GOAL_RUSH_CONFIG));
      })
  ...
  function patch(p: Partial<GoalRushConfig>) {
    setCfg((c) => {
      const next = normalizeGoalRushConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: GrSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeGoalRushConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveGoalRushEngineConfigFn({
        data: { config: normalizeGoalRushConfig(cfg) },
      });
      setCfg(normalizeGoalRushConfig(next));
      toast.success("Goal Rush engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/goal-rush/</code>
              </p>
