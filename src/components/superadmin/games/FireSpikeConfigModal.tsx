...
import {
  getFireSpikeEngineConfigFn,
  saveFireSpikeEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/fire-spike/animationConfig";
...
function SymThumb({ kind, name }: { kind: FsSymKind; name: string }) {
...
export function FireSpikeConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<FireSpikeConfig>(() =>
    structuredClone(DEFAULT_FIRE_SPIKE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFireSpikeEngineConfigFn()
      .then((c) => setCfg(normalizeFireSpikeConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_FIRE_SPIKE_CONFIG));
      })
  ...
  function patch(p: Partial<FireSpikeConfig>) {
    setCfg((c) => {
      const next = normalizeFireSpikeConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: FsSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeFireSpikeConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveFireSpikeEngineConfigFn({
        data: { config: normalizeFireSpikeConfig(cfg) },
      });
      setCfg(normalizeFireSpikeConfig(next));
      toast.success("Fire Spike engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/fire-spike/</code>
              </p>
