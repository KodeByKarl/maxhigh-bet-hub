...
import {
  getChineseNewYearEngineConfigFn,
  saveChineseNewYearEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { ChineseIcon } from "@/components/maxhigh/chinese-new-year/ChineseIcon";
...
export function ChineseNewYearConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ChineseNewYearConfig>(() =>
    structuredClone(DEFAULT_CHINESE_NEW_YEAR_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getChineseNewYearEngineConfigFn()
      .then((c) => setCfg(normalizeChineseNewYearConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_CHINESE_NEW_YEAR_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveChineseNewYearEngineConfigFn({
        data: { config: normalizeChineseNewYearConfig(cfg) },
      });
      setCfg(normalizeChineseNewYearConfig(next));
      toast.success("Chinese New Year engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<ChineseNewYearConfig>) {
    setCfg((c) => normalizeChineseNewYearConfig({ ...c, ...partial }));
  }
...
function SymbolsSection({
  ...
  onPatch: (p: Partial<ChineseNewYearConfig>) => void;
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <div className="size-10 shrink-0 overflow-hidden rounded-lg bg-black/40">
                <ChineseIcon kind={sym.kind} className="!scale-100" />
              </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-muted-foreground">
        Run <code className="text-amber-300">npx tsx scripts/test-chinese-new-year.ts --rtp 100000</code>{" "}
      ...
      <button
        type="button"
        onClick={() => {
          onPatch(structuredClone(DEFAULT_CHINESE_NEW_YEAR_CONFIG));
