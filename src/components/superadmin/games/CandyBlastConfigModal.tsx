...
import {
  ...
  type CandyBlastSymbolConfig,
} from "@/lib/candy-blast-config";
import {
  getCandyBlastEngineConfigFn,
  saveCandyBlastEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { CandyBlastIcon } from "@/components/maxhigh/candy-blast/CandyBlastIcon";
import type { SymKind } from "@/components/maxhigh/candy-blast/types";
...
export function CandyBlastConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CandyBlastConfig>(() =>
    structuredClone(DEFAULT_CANDY_BLAST_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getCandyBlastEngineConfigFn()
      .then((c) => setCfg(normalizeCandyBlastConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_CANDY_BLAST_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCandyBlastEngineConfigFn({
        data: { config: normalizeCandyBlastConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<CandyBlastConfig>) {
    setCfg((c) => normalizeCandyBlastConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<CandyBlastSymbolConfig>) {
    setCfg((c) =>
      normalizeCandyBlastConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Candy Blast appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: CandyBlastConfig;
  onPatch: (p: Partial<CandyBlastConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_CANDY_BLAST_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_CANDY_BLAST_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_CANDY_BLAST_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_CANDY_BLAST_CONFIG.seedClusterMax,
              minCluster: DEFAULT_CANDY_BLAST_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<CandyBlastSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <CandyBlastIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <CandyBlastIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
