...
import {
  ...
  type EnchantedGroveSymbolConfig,
} from "@/lib/enchanted-grove-config";
import {
  getEnchantedGroveEngineConfigFn,
  saveEnchantedGroveEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SymbolIcon } from "@/components/maxhigh/enchanted-grove/SymbolIcon";
import type { SymKind } from "@/components/maxhigh/enchanted-grove/types";
...
export function EnchantedGroveConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<EnchantedGroveConfig>(() =>
    structuredClone(DEFAULT_ENCHANTED_GROVE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getEnchantedGroveEngineConfigFn()
      .then((c) => setCfg(normalizeEnchantedGroveConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_ENCHANTED_GROVE_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveEnchantedGroveEngineConfigFn({
        data: { config: normalizeEnchantedGroveConfig(cfg) },
      });
      setCfg(next);
      toast.success("Enchanted Grove engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<EnchantedGroveConfig>) {
    setCfg((c) => normalizeEnchantedGroveConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<EnchantedGroveSymbolConfig>) {
    setCfg((c) =>
      normalizeEnchantedGroveConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Enchanted Grove appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: EnchantedGroveConfig;
  onPatch: (p: Partial<EnchantedGroveConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<EnchantedGroveSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
