...
import {
  getOutlawCoinsEngineConfigFn,
  saveOutlawCoinsEngineConfigFn,
...
export function OutlawCoinsConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<OutlawCoinsConfig>(() =>
    structuredClone(DEFAULT_OUTLAW_COINS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getOutlawCoinsEngineConfigFn()
      .then((c) => setCfg(normalizeOutlawCoinsConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_OUTLAW_COINS_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<OutlawCoinsConfig>) {
    setCfg((c) => normalizeOutlawCoinsConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveOutlawCoinsEngineConfigFn({
        data: { config: normalizeOutlawCoinsConfig(cfg) },
      });
      setCfg(normalizeOutlawCoinsConfig(next));
      toast.success("Outlaw Coins engine saved");
    } catch (e) {
