...
import {
  ...
  type WildPantherSymbolConfig,
} from "@/lib/wild-panther-config";
import {
  getWildPantherEngineConfigFn,
  saveWildPantherEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { WildPantherIcon } from "@/components/maxhigh/wild-panther/WildPantherIcon";
import type { WildPantherSymKind } from "@/lib/wild-panther-config";
...
export function WildPantherConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<WildPantherConfig>(() =>
    structuredClone(DEFAULT_WILD_PANTHER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getWildPantherEngineConfigFn()
      .then((res) => {
        setCfg(normalizeWildPantherConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_WILD_PANTHER_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveWildPantherEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeWildPantherConfig(normalized));
      toast.success("Wild Panther engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<WildPantherSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Wild Panther Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <WildPantherIcon kind={sym.kind as WildPantherSymKind} className="size-8 object-contain" />
                            <div>
