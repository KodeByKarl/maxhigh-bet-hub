...
import {
  ...
  LOTUS_LYNX_BOMB_MULTS,
  normalizeLotusLynxConfig,
  weightPercents,
  type LotusLynxConfig,
  type LotusLynxSymbolConfig,
} from "@/lib/lotus-lynx-config";
import {
  getLotusLynxEngineConfigFn,
  saveLotusLynxEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/lotus-lynx/PantherIcon";
import type { LotusLynxSymKind } from "@/lib/lotus-lynx-config";
...
export function LotusLynxConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<LotusLynxConfig>(() =>
    structuredClone(DEFAULT_LOTUS_LYNX_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getLotusLynxEngineConfigFn()
      .then((res) => {
        setCfg(normalizeLotusLynxConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_LOTUS_LYNX_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveLotusLynxEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeLotusLynxConfig(normalized));
      toast.success("Lotus Lynx engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<LotusLynxSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Lotus Lynx Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as LotusLynxSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {LOTUS_LYNX_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: LOTUS_LYNX_BOMB_MULTS.map((rowMult, rowIdx) => {
