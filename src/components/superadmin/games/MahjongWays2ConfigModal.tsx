...
import {
  getMahjongWays2EngineConfigFn,
  saveMahjongWays2EngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { Mahjong2Icon } from "@/components/maxhigh/mahjong-ways-2/Mahjong2Icon";
...
export function MahjongWays2ConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<MahjongWays2Config>(() =>
    structuredClone(DEFAULT_MAHJONG_WAYS_2_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getMahjongWays2EngineConfigFn()
      .then((c) => setCfg(normalizeMahjongWays2Config(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_MAHJONG_WAYS_2_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveMahjongWays2EngineConfigFn({
        data: { config: normalizeMahjongWays2Config(cfg) },
      });
      setCfg(normalizeMahjongWays2Config(next));
  ...
  function patch(partial: Partial<MahjongWays2Config>) {
    setCfg((c) => normalizeMahjongWays2Config({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <Mahjong2Icon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: MahjongWays2Config;
  onPatch: (p: Partial<MahjongWays2Config>) => void;
  onSave: () => void;
  saving: boolean;
}) {
