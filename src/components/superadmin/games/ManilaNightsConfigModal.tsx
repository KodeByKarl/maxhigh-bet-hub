...
import {
  ...
  weightPercents,
  type ManilaNightsSymbolConfig,
  type ManilaNightsConfig,
} from "@/lib/manila-nights-config";
import {
  getManilaNightsEngineConfigFn,
  saveManilaNightsEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { ManilaNightsIcon } from "@/components/maxhigh/manila-nights/ManilaNightsIcon";
...
export function ManilaNightsConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ManilaNightsConfig>(() =>
    structuredClone(DEFAULT_MANILA_NIGHTS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getManilaNightsEngineConfigFn()
      .then((c) => setCfg(normalizeManilaNightsConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_MANILA_NIGHTS_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveManilaNightsEngineConfigFn({
        data: { config: normalizeManilaNightsConfig(cfg) },
      });
      setCfg(normalizeManilaNightsConfig(next));
      toast.success("Manila Nights engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<ManilaNightsConfig>) {
    setCfg((c) => normalizeManilaNightsConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<ManilaNightsSymbolConfig>) {
    setCfg((c) =>
      normalizeManilaNightsConfig({
        ...c,
...
function SymbolsSection({
  ...
  cfg: ManilaNightsConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<ManilaNightsSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <ManilaNightsIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: ManilaNightsConfig;
  onPatch: (p: Partial<ManilaNightsConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
