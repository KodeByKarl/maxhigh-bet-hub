...
import {
  ...
  weightPercents,
  type DragonPhoenixSymbolConfig,
  type DragonPhoenixConfig,
} from "@/lib/dragon-phoenix-config";
import {
  getDragonPhoenixEngineConfigFn,
  saveDragonPhoenixEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { DragonPhoenixIcon } from "@/components/maxhigh/dragon-phoenix/DragonPhoenixIcon";
...
export function DragonPhoenixConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<DragonPhoenixConfig>(() =>
    structuredClone(DEFAULT_DRAGON_PHOENIX_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getDragonPhoenixEngineConfigFn()
      .then((c) => setCfg(normalizeDragonPhoenixConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_DRAGON_PHOENIX_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveDragonPhoenixEngineConfigFn({
        data: { config: normalizeDragonPhoenixConfig(cfg) },
      });
      setCfg(normalizeDragonPhoenixConfig(next));
      toast.success("Dragon & Phoenix engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<DragonPhoenixConfig>) {
    setCfg((c) => normalizeDragonPhoenixConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<DragonPhoenixSymbolConfig>) {
    setCfg((c) =>
      normalizeDragonPhoenixConfig({
        ...c,
...
function SymbolsSection({
  ...
  cfg: DragonPhoenixConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<DragonPhoenixSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <DragonPhoenixIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: DragonPhoenixConfig;
  onPatch: (p: Partial<DragonPhoenixConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
