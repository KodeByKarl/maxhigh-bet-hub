/**
 * Superadmin Halo-Halo Hits engine editor — lobby + math config.
...
import {
  getHaloHaloHitsEngineConfigFn,
  saveHaloHaloHitsEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/halo-halo-hits/animationConfig";
...
function SymThumb({ kind, name }: { kind: FoSymKind; name: string }) {
  return (
    <img
...
export function HaloHaloHitsConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<HaloHaloHitsConfig>(() =>
    structuredClone(DEFAULT_HALO_HALO_HITS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getHaloHaloHitsEngineConfigFn()
      .then((c) => setCfg(normalizeHaloHaloHitsConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_HALO_HALO_HITS_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<HaloHaloHitsConfig>) {
    setCfg((c) => {
      const next = normalizeHaloHaloHitsConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveHaloHaloHitsEngineConfigFn({
        data: { config: normalizeHaloHaloHitsConfig(cfg) },
      });
      setCfg(normalizeHaloHaloHitsConfig(next));
      ...
      toast.success("Halo-Halo Hits engine saved");
    } catch (e) {
