...
import {
  getPalengkePaysEngineConfigFn,
  savePalengkePaysEngineConfigFn,
...
export function PalengkePaysConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<PalengkePaysConfig>(() =>
    structuredClone(DEFAULT_PALENGKE_PAYS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getPalengkePaysEngineConfigFn()
      .then((c) => setCfg(normalizePalengkePaysConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_PALENGKE_PAYS_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<PalengkePaysConfig>) {
    setCfg((c) => normalizePalengkePaysConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await savePalengkePaysEngineConfigFn({
        data: { config: normalizePalengkePaysConfig(cfg) },
      });
      setCfg(normalizePalengkePaysConfig(next));
      toast.success("Palengke Pays engine saved");
    } catch (e) {
