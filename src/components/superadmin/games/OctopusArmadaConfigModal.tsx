/**
 * Superadmin Octopus Armada engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getOctopusArmadaAdminConfigFn,
  saveOctopusArmadaAdminConfigFn,
...
export function OctopusArmadaConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<OctopusArmadaConfig>(() =>
    structuredClone(DEFAULT_OCTOPUS_ARMADA_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getOctopusArmadaAdminConfigFn()
      .then((c) => setCfg(normalizeOctopusArmadaConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_OCTOPUS_ARMADA_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<OctopusArmadaConfig>) {
    setCfg((prev) => normalizeOctopusArmadaConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveOctopusArmadaAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizeOctopusArmadaConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Octopus Armada engine saved — live for all players");
    } catch (e) {
