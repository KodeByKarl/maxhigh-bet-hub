...
import {
  ...
  type MermaidRichesSymbolConfig,
} from "@/lib/mermaid-riches-config";
import {
  getMermaidRichesEngineConfigFn,
  saveMermaidRichesEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { MermaidRichesIcon } from "@/components/maxhigh/mermaid-riches/MermaidRichesIcon";
import type { SymKind } from "@/components/maxhigh/mermaid-riches/types";
...
const NAV: { id: Section; label: string; icon: typeof Store }[] = [
  ...
  { id: "freespins", label: "Candy cane", icon: Candy },
];
...
export function MermaidRichesConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<MermaidRichesConfig>(() =>
    structuredClone(DEFAULT_MERMAID_RICHES_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getMermaidRichesEngineConfigFn()
      .then((c) => setCfg(normalizeMermaidRichesConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_MERMAID_RICHES_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveMermaidRichesEngineConfigFn({
        data: { config: normalizeMermaidRichesConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<MermaidRichesConfig>) {
    setCfg((c) => normalizeMermaidRichesConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<MermaidRichesSymbolConfig>) {
    setCfg((c) =>
      normalizeMermaidRichesConfig({
        ...c,
...
const GUIDE = {
  ...
  caneSpawn: {
    about: "Gaano kadalas lumabas ang candy cane (scatter) sa bawat cell. Dapat rare.",
    levels: [
  ...
  anteBoost: {
    about: "Kapag naka-Ante ang player, multiply ang candy cane chance.",
    levels: [
  ...
  fsTrigger: {
    about: "Ilang candy cane ang kailangan para magsimula ang Free Spins.",
    levels: [
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Mermaid Riches appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: MermaidRichesConfig;
  onPatch: (p: Partial<MermaidRichesConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_MERMAID_RICHES_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_MERMAID_RICHES_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_MERMAID_RICHES_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_MERMAID_RICHES_CONFIG.seedClusterMax,
              minCluster: DEFAULT_MERMAID_RICHES_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<MermaidRichesSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <MermaidRichesIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <MermaidRichesIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
          <div className="text-sm font-semibold text-rose-200">Candy cane (scatter)</div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field
          ...
          hint="Makes candy canes show more often when Ante is on."
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field
          ...
          hint="Candy canes needed to start free spins."
      ...
      <SectionHead title="Scatter cash" subtitle="× bet by candy cane count on the spin." />
