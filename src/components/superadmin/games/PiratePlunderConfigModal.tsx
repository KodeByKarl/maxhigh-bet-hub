...
import {
  ...
  type PiratePlunderSymbolConfig,
} from "@/lib/pirate-plunder-config";
import {
  getPiratePlunderEngineConfigFn,
  savePiratePlunderEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PiratePlunderIcon } from "@/components/maxhigh/pirate-plunder/PiratePlunderIcon";
import type { PiratePlunderSymKind } from "@/lib/pirate-plunder-config";
...
export function PiratePlunderConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<PiratePlunderConfig>(() =>
    structuredClone(DEFAULT_PIRATE_PLUNDER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getPiratePlunderEngineConfigFn()
      .then((res) => {
        setCfg(normalizePiratePlunderConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_PIRATE_PLUNDER_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await savePiratePlunderEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizePiratePlunderConfig(normalized));
      toast.success("Pirate Plunder engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<PiratePlunderSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Pirate Plunder Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PiratePlunderIcon kind={sym.kind as PiratePlunderSymKind} className="size-8 object-contain" />
                            <div>
