...
import {
  getCoinVolcanoEngineConfigFn,
  saveCoinVolcanoEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/coin-volcano/animationConfig";
...
function SymThumb({ kind, name }: { kind: McSymKind; name: string }) {
...
export function CoinVolcanoConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CoinVolcanoConfig>(() =>
    structuredClone(DEFAULT_COIN_VOLCANO_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getCoinVolcanoEngineConfigFn()
      .then((c) => setCfg(normalizeCoinVolcanoConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_COIN_VOLCANO_CONFIG));
      })
  ...
  function patch(p: Partial<CoinVolcanoConfig>) {
    setCfg((c) => {
      const next = normalizeCoinVolcanoConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: McSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeCoinVolcanoConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCoinVolcanoEngineConfigFn({
        data: { config: normalizeCoinVolcanoConfig(cfg) },
      });
      setCfg(normalizeCoinVolcanoConfig(next));
      toast.success("Coin Volcano engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/coin-volcano/</code>
              </p>
