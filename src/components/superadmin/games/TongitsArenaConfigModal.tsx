/**
 * Superadmin Tongits Arena engine editor — lobby + payouts + qualify + limits.
 * Wired from Super Admin → Games → Tongits Arena (full config).
 */
...
import {
  ...
  saveTongitsArenaEngineConfigFn,
...
export function TongitsArenaConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<TongitsArenaConfig>(() =>
    structuredClone(DEFAULT_TONGITS_ARENA_CONFIG),
  );
  ...
  const [betStepsText, setBetStepsText] = useState(
    DEFAULT_TONGITS_ARENA_CONFIG.betSteps.join(", "),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getTongitsArenaEngineConfigFn()
      .then((c) => {
        const next = normalizeTongitsArenaConfig(c);
        setCfg(next);
        setBetStepsText(next.betSteps.join(", "));
      })
      .catch(() => {
        setCfg(structuredClone(DEFAULT_TONGITS_ARENA_CONFIG));
        setBetStepsText(DEFAULT_TONGITS_ARENA_CONFIG.betSteps.join(", "));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<TongitsArenaConfig>) {
    setCfg((prev) => normalizeTongitsArenaConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const withSteps = normalizeTongitsArenaConfig({
        ...cfg,
        betSteps: parseBetSteps(betStepsText),
      });
      const next = await saveTongitsArenaEngineConfigFn({ data: { config: withSteps } });
      const normalized = normalizeTongitsArenaConfig(next);
      setCfg(normalized);
      ...
      toast.success("Tongits Arena engine saved — live for all players");
    } catch (e) {
