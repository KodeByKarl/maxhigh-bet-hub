...
import {
  getDustDollarsEngineConfigFn,
  saveDustDollarsEngineConfigFn,
...
export function DustDollarsConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<DustDollarsConfig>(() =>
    structuredClone(DEFAULT_DUST_DOLLARS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getDustDollarsEngineConfigFn()
      .then((c) => setCfg(normalizeDustDollarsConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_DUST_DOLLARS_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<DustDollarsConfig>) {
    setCfg((c) => normalizeDustDollarsConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveDustDollarsEngineConfigFn({
        data: { config: normalizeDustDollarsConfig(cfg) },
      });
      setCfg(normalizeDustDollarsConfig(next));
      toast.success("Dust & Dollars engine saved");
    } catch (e) {
