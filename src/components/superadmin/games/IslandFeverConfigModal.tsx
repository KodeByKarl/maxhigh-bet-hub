...
import {
  ...
  type IslandFeverSymbolConfig,
} from "@/lib/island-fever-config";
import {
  getIslandFeverEngineConfigFn,
  saveIslandFeverEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { IslandFeverIcon } from "@/components/maxhigh/island-fever/IslandFeverIcon";
import type { SymKind } from "@/components/maxhigh/island-fever/types";
...
export function IslandFeverConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<IslandFeverConfig>(() =>
    structuredClone(DEFAULT_ISLAND_FEVER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getIslandFeverEngineConfigFn()
      .then((c) => setCfg(normalizeIslandFeverConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_ISLAND_FEVER_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveIslandFeverEngineConfigFn({
        data: { config: normalizeIslandFeverConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<IslandFeverConfig>) {
    setCfg((c) => normalizeIslandFeverConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<IslandFeverSymbolConfig>) {
    setCfg((c) =>
      normalizeIslandFeverConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Island Fever appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: IslandFeverConfig;
  onPatch: (p: Partial<IslandFeverConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_ISLAND_FEVER_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_ISLAND_FEVER_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_ISLAND_FEVER_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_ISLAND_FEVER_CONFIG.seedClusterMax,
              minCluster: DEFAULT_ISLAND_FEVER_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<IslandFeverSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <IslandFeverIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <IslandFeverIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
