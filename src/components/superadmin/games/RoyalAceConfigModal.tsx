...
import {
  ...
  weightPercents,
  type SaRtpProfileId,
  type RoyalAceSymbolConfig,
  type RoyalAceConfig,
} from "@/lib/royal-ace-config";
import {
  getRoyalAceEngineConfigFn,
  saveRoyalAceEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { RoyalAceIcon } from "@/components/maxhigh/royal-ace/RoyalAceIcon";
import { cn } from "@/lib/utils";

type Section = "lobby" | "symbols" | "cascades" | "bonus" | "rtp";
...
const NAV: { id: Section; label: string; icon: typeof Store }[] = [
  ...
  { id: "rtp", label: "RTP Profiles", icon: Gauge },
];

const RTP_OPTIONS: { id: SaRtpProfileId; label: string }[] = [
  { id: "rtp_97_0", label: "97.0% Flagship" },
  { id: "rtp_96_5", label: "96.5% Licensed" },
  { id: "rtp_94_38", label: "94.38% Licensed" },
];
...
export function RoyalAceConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<RoyalAceConfig>(() =>
    structuredClone(DEFAULT_ROYAL_ACE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getRoyalAceEngineConfigFn()
      .then((c) => setCfg(normalizeRoyalAceConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_ROYAL_ACE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveRoyalAceEngineConfigFn({
        data: { config: normalizeRoyalAceConfig(cfg) },
      });
      setCfg(normalizeRoyalAceConfig(next));
      toast.success("Super Ace engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<RoyalAceConfig>) {
    setCfg((c) => normalizeRoyalAceConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<RoyalAceSymbolConfig>) {
    setCfg((c) =>
      normalizeRoyalAceConfig({
        ...c,
        ...
        <div className="grid min-h-0 flex-1 md:grid-cols-[12.5rem_1fr]">
          ...
          <div className="min-h-0 overflow-y-auto p-4 sm:p-5">
            {loading && section !== "lobby" ? (
              ...
              <RtpSection cfg={cfg} onPatch={patch} onSave={saveEngine} saving={saving} />
            ) : (
              <BonusSection
                cfg={cfg}
                allPct={allPct}
...
function LobbySection({
  ...
  const [rtp, setRtp] = useState(Number(game.rtp) || 97);
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<RoyalAceSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <RoyalAceIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: RoyalAceConfig;
  onPatch: (p: Partial<RoyalAceConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground sm:col-span-2">
          <span className="inline-flex items-center gap-1">
            Guaranteed Golden Reel (FS)
            <FieldGuide
              title="FS Golden Reel"
              body="0-based reel index that always spawns gold on eligible symbols during Free Spins (default middle reel = 2)."
            />
          </span>
          <NumInput
            value={cfg.guaranteedGoldenReelIndex}
            onChange={(n) => onPatch({ guaranteedGoldenReelIndex: Math.round(n) })}
            step={1}
            min={0}
            max={4}
          />
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Reels count
          <NumInput
            value={cfg.reelsCount}
            onChange={(n) => onPatch({ reelsCount: Math.round(n) })}
            min={3}
            max={6}
          />
        </label>
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Rows (reel height)
          <NumInput
            value={cfg.maxReelHeight}
            onChange={(n) =>
              onPatch({
                minReelHeight: Math.round(n),
                maxReelHeight: Math.round(n),
              })
            }
            min={2}
            max={6}
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Little Joker transform weight
          <NumInput
            value={cfg.jokerTransformWeights.little_joker}
            onChange={(n) =>
              onPatch({
                jokerTransformWeights: {
                  ...cfg.jokerTransformWeights,
                  little_joker: n,
                },
              })
            }
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Big Joker transform weight
          <NumInput
            value={cfg.jokerTransformWeights.big_joker}
            onChange={(n) =>
              onPatch({
                jokerTransformWeights: {
                  ...cfg.jokerTransformWeights,
...
function RtpSection({
  ...
  function setActiveProfile(id: SaRtpProfileId) {
    const profile = cfg.rtpProfiles.find((p) => p.id === id);
    onPatch({
      activeRtpProfile: id,
      targetRtp: profile?.targetRtp ?? cfg.targetRtp,
    });
  }

  function patchActiveWeight(
    kind: (typeof ALL_SYM_KINDS)[number],
    field: "weight" | "weightFreeSpins",
    value: number,
  ) {
    const profiles = cfg.rtpProfiles.map((p) => {
      if (p.id !== cfg.activeRtpProfile) return p;
      const prev = p.symbolWeights[kind] ?? {
        weight: cfg.symbols.find((s) => s.kind === kind)?.weight ?? 0,
        weightFreeSpins: cfg.symbols.find((s) => s.kind === kind)?.weightFreeSpins ?? 0,
      };
      return {
        ...p,
        symbolWeights: {
          ...p.symbolWeights,
          [kind]: { ...prev, [field]: value },
        },
      };
    });
    onPatch({ rtpProfiles: profiles });
  }

  function patchProfileTarget(n: number) {
    const profiles = cfg.rtpProfiles.map((p) =>
      p.id === cfg.activeRtpProfile ? { ...p, targetRtp: n } : p,
    );
    onPatch({ rtpProfiles: profiles, targetRtp: n });
  }

  return (
    <div className="space-y-5">
      <SectionHead
        title="Multi-tier RTP profiles"
        subtitle="Select the active operator/license profile. Weight overrides apply on top of the Symbols table when spins resolve."
      />
      <div className="flex flex-wrap gap-2">
        {RTP_OPTIONS.map((opt) => (
          <button
      ...
      <label className="block max-w-xs space-y-1 text-xs font-semibold text-muted-foreground">
        Profile target RTP %
        <NumInput
          value={active?.targetRtp ?? cfg.targetRtp}
          onChange={patchProfileTarget}
          min={80}
          max={99.5}
          step={0.01}
          suffix="%"
      ...
      <div className="rounded-2xl border border-amber-500/15 bg-white/[0.03] p-4">
        <div className="mb-3 text-sm font-black text-amber-300">
          Weight overrides � {cfg.activeRtpProfile}
        </div>
        <p className="mb-3 text-[11px] text-muted-foreground">
          Empty overrides fall back to the Symbols tab weights. Saving writes the full engine JSON
          to game_controls and invalidates the live spin cache.
        </p>
        <div className="mb-2 grid grid-cols-[2.5rem_1fr_5rem_5rem] gap-2 px-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
          <span />
          <span>Symbol</span>
          <span>Weight</span>
          <span>FS wt</span>
        ...
        <div className="space-y-2">
          {ALL_SYM_KINDS.map((kind) => {
            const base = cfg.symbols.find((s) => s.kind === kind);
            const ov = active?.symbolWeights[kind];
            const w = ov?.weight ?? base?.weight ?? 0;
            const wfs = ov?.weightFreeSpins ?? base?.weightFreeSpins ?? 0;
            return (
              <div
                key={kind}
                className="grid grid-cols-[2.5rem_1fr_5rem_5rem] items-center gap-2 rounded-xl border border-white/5 bg-black/20 px-2 py-1.5"
              >
                <div className="size-10 overflow-hidden rounded-md bg-black/40">
                  <RoyalAceIcon kind={kind} className="size-10" />
                </div>
                <div className="min-w-0 truncate text-xs font-bold text-foreground">
                  {SYMBOL_NAMES[kind]}
                  {!ov ? (
                    <span className="ml-1 font-normal text-muted-foreground">(base)</span>
                  ) : null}
                </div>
                <NumInput
                  value={w}
                  onChange={(n) => patchActiveWeight(kind, "weight", n)}
                  min={0}
                />
                <NumInput
                  value={wfs}
                  onChange={(n) => patchActiveWeight(kind, "weightFreeSpins", n)}
                  min={0}
                />
              </div>
            );
      ...
      <label className="block max-w-xs space-y-1 text-xs font-semibold text-muted-foreground">
        Engine volatility label
        <select
          value={cfg.volatility}
          onChange={(e) =>
            onPatch({
              volatility: e.target.value as RoyalAceConfig["volatility"],
            })
          }
          className="h-10 w-full rounded-xl border-transparent bg-white/[0.06] px-3 text-sm text-foreground"
        >
