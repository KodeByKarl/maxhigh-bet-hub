...
import {
  getLechonLuckEngineConfigFn,
  saveLechonLuckEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { LechonLuckIcon } from "@/components/maxhigh/lechon-luck/LechonLuckIcon";
...
export function LechonLuckConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<LechonLuckConfig>(() =>
    structuredClone(DEFAULT_LECHON_LUCK_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getLechonLuckEngineConfigFn()
      .then((c) => setCfg(normalizeLechonLuckConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_LECHON_LUCK_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveLechonLuckEngineConfigFn({
        data: { config: normalizeLechonLuckConfig(cfg) },
      });
      setCfg(normalizeLechonLuckConfig(next));
      toast.success("Lechon Luck engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<LechonLuckConfig>) {
    setCfg((c) => normalizeLechonLuckConfig({ ...c, ...partial }));
  }
...
function SymbolsSection({
  ...
  onPatch: (p: Partial<LechonLuckConfig>) => void;
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <div className="size-10 shrink-0 overflow-hidden rounded-lg bg-black/40">
                <LechonLuckIcon kind={sym.kind} className="!scale-100" />
              </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-muted-foreground">
        Run <code className="text-amber-300">npx tsx scripts/test-lechon-luck.ts --rtp 100000</code>{" "}
      ...
      <button
        type="button"
        onClick={() => {
          onPatch(structuredClone(DEFAULT_LECHON_LUCK_CONFIG));
