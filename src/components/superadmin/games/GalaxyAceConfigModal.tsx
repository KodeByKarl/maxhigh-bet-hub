...
import {
  ...
  weightPercents,
  type GalaxyAceSymbolConfig,
  type GalaxyAceConfig,
} from "@/lib/galaxy-ace-config";
import {
  getGalaxyAceEngineConfigFn,
  saveGalaxyAceEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { GalaxyAceIcon } from "@/components/maxhigh/galaxy-ace/GalaxyAceIcon";
...
export function GalaxyAceConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<GalaxyAceConfig>(() =>
    structuredClone(DEFAULT_GALAXY_ACE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getGalaxyAceEngineConfigFn()
      .then((c) => setCfg(normalizeGalaxyAceConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_GALAXY_ACE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveGalaxyAceEngineConfigFn({
        data: { config: normalizeGalaxyAceConfig(cfg) },
      });
      setCfg(normalizeGalaxyAceConfig(next));
      toast.success("Galaxy Ace engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<GalaxyAceConfig>) {
    setCfg((c) => normalizeGalaxyAceConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<GalaxyAceSymbolConfig>) {
    setCfg((c) =>
      normalizeGalaxyAceConfig({
        ...c,
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<GalaxyAceSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <GalaxyAceIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: GalaxyAceConfig;
  onPatch: (p: Partial<GalaxyAceConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
