...
import {
  ...
  saveLuckyBarsEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/lucky-bars/animationConfig";
...
function weightPercents(cfg: LuckyBarsConfig, reelIndex = 0): Record<string, number> {
...
export function LuckyBarsConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<LuckyBarsConfig>(() =>
    structuredClone(DEFAULT_LUCKY_BARS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getLuckyBarsEngineConfigFn()
      .then((c) => setCfg(normalizeLuckyBarsConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_LUCKY_BARS_CONFIG));
      })
  ...
  function patch(p: Partial<LuckyBarsConfig>) {
    setCfg((c) => normalizeLuckyBarsConfig({ ...c, ...p }));
  }

  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveLuckyBarsEngineConfigFn({
        data: { config: normalizeLuckyBarsConfig(cfg) },
      });
      setCfg(normalizeLuckyBarsConfig(next));
