...
import {
  ...
  weightPercents,
  type BuffaloSymbolConfig,
  type CarabaoChargeConfig,
} from "@/lib/carabao-charge-config";
import {
  getCarabaoChargeEngineConfigFn,
  saveCarabaoChargeEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { CarabaoChargeIcon } from "@/components/maxhigh/carabao-charge/CarabaoChargeIcon";
...
export function CarabaoChargeConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CarabaoChargeConfig>(() =>
    structuredClone(DEFAULT_CARABAO_CHARGE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getCarabaoChargeEngineConfigFn()
      .then((c) => setCfg(normalizeCarabaoChargeConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_CARABAO_CHARGE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCarabaoChargeEngineConfigFn({
        data: { config: normalizeCarabaoChargeConfig(cfg) },
      });
      setCfg(normalizeCarabaoChargeConfig(next));
      toast.success("Carabao Charge engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<CarabaoChargeConfig>) {
    setCfg((c) => normalizeCarabaoChargeConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<BuffaloSymbolConfig>) {
    setCfg((c) =>
      normalizeCarabaoChargeConfig({
        ...c,
...
function LobbySection({
  ...
  const [rtp, setRtp] = useState(Number(game.rtp) || 96.5);
...
function SymbolsSection({
  ...
  cfg: CarabaoChargeConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<BuffaloSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <CarabaoChargeIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: CarabaoChargeConfig;
  onPatch: (p: Partial<CarabaoChargeConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          FS session mult start
          <NumInput
            value={cfg.freeSpinsSessionMultStart}
            onChange={(n) => onPatch({ freeSpinsSessionMultStart: n })}
            min={1}
            step={1}
            suffix="�"
          />
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          FS session mult +per spin
          <NumInput
            value={cfg.freeSpinsSessionMultStep}
            onChange={(n) => onPatch({ freeSpinsSessionMultStep: n })}
            min={0}
            step={1}
            suffix="�"
          />
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Treasure chests to trigger
          <NumInput
            value={cfg.chestTriggerCount}
            onChange={(n) => onPatch({ chestTriggerCount: Math.round(n) })}
            min={3}
            max={20}
            step={1}
          />
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Chest respins
          <NumInput
            value={cfg.chestRespins}
            onChange={(n) => onPatch({ chestRespins: Math.round(n) })}
            min={1}
            max={10}
            step={1}
          />
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Chest spawn chance
          <NumInput
            value={+(cfg.chestSpawnChance * 100).toFixed(2)}
            onChange={(n) => onPatch({ chestSpawnChance: n / 100 })}
            step={0.5}
            min={0}
            max={100}
            suffix="%"
        ...
        <label className="space-y-1 text-xs font-semibold text-muted-foreground">
          Jackpot chest chance
          <NumInput
            value={+(cfg.chestJackpotChance * 100).toFixed(2)}
            onChange={(n) => onPatch({ chestJackpotChance: n / 100 })}
            step={0.1}
            min={0}
            max={100}
            suffix="%"
