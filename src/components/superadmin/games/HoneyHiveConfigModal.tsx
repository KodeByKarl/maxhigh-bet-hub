...
import {
  ...
  HONEY_HIVE_BOMB_MULTS,
  normalizeHoneyHiveConfig,
  weightPercents,
  type HoneyHiveConfig,
  type HoneyHiveSymbolConfig,
} from "@/lib/honey-hive-config";
import {
  getHoneyHiveEngineConfigFn,
  saveHoneyHiveEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/honey-hive/PantherIcon";
import type { HoneyHiveSymKind } from "@/lib/honey-hive-config";
...
export function HoneyHiveConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<HoneyHiveConfig>(() =>
    structuredClone(DEFAULT_HONEY_HIVE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getHoneyHiveEngineConfigFn()
      .then((res) => {
        setCfg(normalizeHoneyHiveConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_HONEY_HIVE_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveHoneyHiveEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeHoneyHiveConfig(normalized));
      toast.success("Honey Hive engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<HoneyHiveSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Honey Hive Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as HoneyHiveSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {HONEY_HIVE_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: HONEY_HIVE_BOMB_MULTS.map((rowMult, rowIdx) => {
