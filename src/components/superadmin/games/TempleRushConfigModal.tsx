...
import {
  ...
  type TempleRushSymbolConfig,
} from "@/lib/temple-rush-config";
import {
  getTempleRushEngineConfigFn,
  saveTempleRushEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { TempleRushIcon } from "@/components/maxhigh/temple-rush/TempleRushIcon";
import type { TempleRushSymKind } from "@/lib/temple-rush-config";
...
export function TempleRushConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<TempleRushConfig>(() =>
    structuredClone(DEFAULT_TEMPLE_RUSH_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getTempleRushEngineConfigFn()
      .then((res) => {
        setCfg(normalizeTempleRushConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_TEMPLE_RUSH_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveTempleRushEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeTempleRushConfig(normalized));
  ...
  function updateSymbol(id: string, patch: Partial<TempleRushSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <TempleRushIcon kind={sym.kind as TempleRushSymKind} className="size-8 object-contain" />
                            <div>
