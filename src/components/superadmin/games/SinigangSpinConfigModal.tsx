...
import {
  getSinigangSpinEngineConfigFn,
  saveSinigangSpinEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SinigangSpinIcon } from "@/components/maxhigh/sinigang-spin/SinigangSpinIcon";
...
export function SinigangSpinConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SinigangSpinConfig>(() =>
    structuredClone(DEFAULT_SINIGANG_SPIN_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getSinigangSpinEngineConfigFn()
      .then((c) => setCfg(normalizeSinigangSpinConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_SINIGANG_SPIN_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSinigangSpinEngineConfigFn({
        data: { config: normalizeSinigangSpinConfig(cfg) },
      });
      setCfg(normalizeSinigangSpinConfig(next));
      toast.success("Sinigang Spin engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<SinigangSpinConfig>) {
    setCfg((c) => normalizeSinigangSpinConfig({ ...c, ...partial }));
  }
...
function SymbolsSection({
  ...
  onPatch: (p: Partial<SinigangSpinConfig>) => void;
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <div className="size-10 shrink-0 overflow-hidden rounded-lg bg-black/40">
                <SinigangSpinIcon kind={sym.kind} className="!scale-100" />
              </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-muted-foreground">
        Run <code className="text-amber-300">npx tsx scripts/test-sinigang-spin.ts --rtp 100000</code>{" "}
      ...
      <button
        type="button"
        onClick={() => {
          onPatch(structuredClone(DEFAULT_SINIGANG_SPIN_CONFIG));
