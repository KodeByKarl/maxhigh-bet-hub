...
import {
  getCleopatraGoldEngineConfigFn,
  saveCleopatraGoldEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { CleopatraIcon } from "@/components/maxhigh/cleopatra-gold/CleopatraIcon";
...
export function CleopatraGoldConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CleopatraGoldConfig>(() =>
    structuredClone(DEFAULT_CLEOPATRA_GOLD_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getCleopatraGoldEngineConfigFn()
      .then((c) => setCfg(normalizeCleopatraGoldConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_CLEOPATRA_GOLD_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCleopatraGoldEngineConfigFn({
        data: { config: normalizeCleopatraGoldConfig(cfg) },
      });
      setCfg(normalizeCleopatraGoldConfig(next));
      toast.success("Cleopatra Gold engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<CleopatraGoldConfig>) {
    setCfg((c) => normalizeCleopatraGoldConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <CleopatraIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: CleopatraGoldConfig;
  onPatch: (p: Partial<CleopatraGoldConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
