...
import {
  ...
  type MysticRunesSymbolConfig,
} from "@/lib/mystic-runes-config";
import {
  getMysticRunesEngineConfigFn,
  saveMysticRunesEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SymbolIcon } from "@/components/maxhigh/mystic-runes/SymbolIcon";
import type { SymKind } from "@/components/maxhigh/mystic-runes/types";
...
export function MysticRunesConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<MysticRunesConfig>(() =>
    structuredClone(DEFAULT_MYSTIC_RUNES_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getMysticRunesEngineConfigFn()
      .then((c) => setCfg(normalizeMysticRunesConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_MYSTIC_RUNES_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveMysticRunesEngineConfigFn({
        data: { config: normalizeMysticRunesConfig(cfg) },
      });
      setCfg(next);
      toast.success("Mystic Runes engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<MysticRunesConfig>) {
    setCfg((c) => normalizeMysticRunesConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<MysticRunesSymbolConfig>) {
    setCfg((c) =>
      normalizeMysticRunesConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Mystic Runes appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: MysticRunesConfig;
  onPatch: (p: Partial<MysticRunesConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<MysticRunesSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
