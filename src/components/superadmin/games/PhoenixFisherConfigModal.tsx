/**
 * Superadmin Phoenix Fisher engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getPhoenixFisherAdminConfigFn,
  savePhoenixFisherAdminConfigFn,
...
export function PhoenixFisherConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<PhoenixFisherConfig>(() =>
    structuredClone(DEFAULT_PHOENIX_FISHER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getPhoenixFisherAdminConfigFn()
      .then((c) => setCfg(normalizePhoenixFisherConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_PHOENIX_FISHER_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<PhoenixFisherConfig>) {
    setCfg((prev) => normalizePhoenixFisherConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await savePhoenixFisherAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizePhoenixFisherConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Phoenix Fisher engine saved — live for all players");
    } catch (e) {
