...
import {
  getPharaohFireEngineConfigFn,
  savePharaohFireEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PharaohFireIcon } from "@/components/maxhigh/pharaoh-fire/PharaohFireIcon";
...
export function PharaohFireConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<PharaohFireConfig>(() =>
    structuredClone(DEFAULT_PHARAOH_FIRE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getPharaohFireEngineConfigFn()
      .then((c) => setCfg(normalizePharaohFireConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_PHARAOH_FIRE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await savePharaohFireEngineConfigFn({
        data: { config: normalizePharaohFireConfig(cfg) },
      });
      setCfg(normalizePharaohFireConfig(next));
      toast.success("Pharaoh Fire engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<PharaohFireConfig>) {
    setCfg((c) => normalizePharaohFireConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <PharaohFireIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: PharaohFireConfig;
  onPatch: (p: Partial<PharaohFireConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
