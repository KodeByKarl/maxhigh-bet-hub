...
import {
  ...
  saveCrazySevensEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/crazy-sevens/animationConfig";
...
function weightPercents(cfg: CrazySevensConfig, reelIndex = 0): Record<string, number> {
...
export function CrazySevensConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CrazySevensConfig>(() =>
    structuredClone(DEFAULT_CRAZY_SEVENS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getCrazySevensEngineConfigFn()
      .then((c) => setCfg(normalizeCrazySevensConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_CRAZY_SEVENS_CONFIG));
      })
  ...
  function patch(p: Partial<CrazySevensConfig>) {
    setCfg((c) => normalizeCrazySevensConfig({ ...c, ...p }));
  }

  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCrazySevensEngineConfigFn({
        data: { config: normalizeCrazySevensConfig(cfg) },
      });
      setCfg(normalizeCrazySevensConfig(next));
