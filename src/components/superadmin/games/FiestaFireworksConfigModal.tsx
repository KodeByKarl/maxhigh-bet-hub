...
import {
  getFiestaFireworksEngineConfigFn,
  saveFiestaFireworksEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { FiestaFireworksIcon } from "@/components/maxhigh/fiesta-fireworks/FiestaFireworksIcon";
...
export function FiestaFireworksConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<FiestaFireworksConfig>(() =>
    structuredClone(DEFAULT_FIESTA_FIREWORKS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFiestaFireworksEngineConfigFn()
      .then((c) => setCfg(normalizeFiestaFireworksConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_FIESTA_FIREWORKS_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveFiestaFireworksEngineConfigFn({
        data: { config: normalizeFiestaFireworksConfig(cfg) },
      });
      setCfg(normalizeFiestaFireworksConfig(next));
      toast.success("Fiesta Fireworks engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<FiestaFireworksConfig>) {
    setCfg((c) => normalizeFiestaFireworksConfig({ ...c, ...partial }));
  }
...
function SymbolsSection({
  ...
  cfg: FiestaFireworksConfig;
  reel0Pct: Record<string, number>;
  onPatch: (p: Partial<FiestaFireworksConfig>) => void;
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <div className="size-10 shrink-0 overflow-hidden rounded-lg bg-black/40">
                <FiestaFireworksIcon kind={sym.kind} className="!scale-100" />
              </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-muted-foreground">
        Run <code className="text-amber-300">npx tsx scripts/test-fiesta-fireworks.ts --rtp 100000</code>{" "}
      ...
      <button
        type="button"
        onClick={() => {
          onPatch(structuredClone(DEFAULT_FIESTA_FIREWORKS_CONFIG));
