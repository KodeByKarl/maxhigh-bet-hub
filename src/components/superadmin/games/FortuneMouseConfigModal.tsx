/**
 * Superadmin Fortune Mouse engine editor — lobby + math config.
...
import {
  getFortuneMouseEngineConfigFn,
  saveFortuneMouseEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/fortune-mouse/animationConfig";
...
export function FortuneMouseConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<FortuneMouseConfig>(() =>
    structuredClone(DEFAULT_FORTUNE_MOUSE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFortuneMouseEngineConfigFn()
      .then((c) => setCfg(normalizeFortuneMouseConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_FORTUNE_MOUSE_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<FortuneMouseConfig>) {
    setCfg((c) => {
      const next = normalizeFortuneMouseConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveFortuneMouseEngineConfigFn({
        data: { config: normalizeFortuneMouseConfig(cfg) },
      });
      setCfg(normalizeFortuneMouseConfig(next));
      ...
      toast.success("Fortune Mouse engine saved");
    } catch (e) {
