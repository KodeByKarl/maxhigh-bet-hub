...
import {
  ...
  CORAL_COBRA_BOMB_MULTS,
  normalizeCoralCobraConfig,
  weightPercents,
  type CoralCobraConfig,
  type CoralCobraSymbolConfig,
} from "@/lib/coral-cobra-config";
import {
  getCoralCobraEngineConfigFn,
  saveCoralCobraEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/coral-cobra/PantherIcon";
import type { CoralCobraSymKind } from "@/lib/coral-cobra-config";
...
export function CoralCobraConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<CoralCobraConfig>(() =>
    structuredClone(DEFAULT_CORAL_COBRA_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getCoralCobraEngineConfigFn()
      .then((res) => {
        setCfg(normalizeCoralCobraConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_CORAL_COBRA_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveCoralCobraEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeCoralCobraConfig(normalized));
      toast.success("Coral Cobra engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<CoralCobraSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Coral Cobra Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as CoralCobraSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {CORAL_COBRA_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: CORAL_COBRA_BOMB_MULTS.map((rowMult, rowIdx) => {
