...
import {
  getCarabaoCashEngineConfigFn,
  saveCarabaoCashEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { CarabaoCashIcon } from "@/components/maxhigh/carabao-cash/CarabaoCashIcon";
...
export function CarabaoCashConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CarabaoCashConfig>(() =>
    structuredClone(DEFAULT_CARABAO_CASH_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getCarabaoCashEngineConfigFn()
      .then((c) => setCfg(normalizeCarabaoCashConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_CARABAO_CASH_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCarabaoCashEngineConfigFn({
        data: { config: normalizeCarabaoCashConfig(cfg) },
      });
      setCfg(normalizeCarabaoCashConfig(next));
      toast.success("Carabao Cash engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<CarabaoCashConfig>) {
    setCfg((c) => normalizeCarabaoCashConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <CarabaoCashIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: CarabaoCashConfig;
  onPatch: (p: Partial<CarabaoCashConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
