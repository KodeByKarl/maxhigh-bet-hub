...
import {
  ...
  RUBY_RAVEN_BOMB_MULTS,
  normalizeRubyRavenConfig,
  weightPercents,
  type RubyRavenConfig,
  type RubyRavenSymbolConfig,
} from "@/lib/ruby-raven-config";
import {
  getRubyRavenEngineConfigFn,
  saveRubyRavenEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/ruby-raven/PantherIcon";
import type { RubyRavenSymKind } from "@/lib/ruby-raven-config";
...
export function RubyRavenConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<RubyRavenConfig>(() =>
    structuredClone(DEFAULT_RUBY_RAVEN_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getRubyRavenEngineConfigFn()
      .then((res) => {
        setCfg(normalizeRubyRavenConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_RUBY_RAVEN_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveRubyRavenEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeRubyRavenConfig(normalized));
      toast.success("Ruby Raven engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<RubyRavenSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Ruby Raven Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as RubyRavenSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {RUBY_RAVEN_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: RUBY_RAVEN_BOMB_MULTS.map((rowMult, rowIdx) => {
