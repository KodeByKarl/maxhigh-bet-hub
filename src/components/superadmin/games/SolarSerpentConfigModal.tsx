...
import {
  ...
  SOLAR_SERPENT_BOMB_MULTS,
  normalizeSolarSerpentConfig,
  weightPercents,
  type SolarSerpentConfig,
  type SolarSerpentSymbolConfig,
} from "@/lib/solar-serpent-config";
import {
  getSolarSerpentEngineConfigFn,
  saveSolarSerpentEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/solar-serpent/PantherIcon";
import type { SolarSerpentSymKind } from "@/lib/solar-serpent-config";
...
export function SolarSerpentConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<SolarSerpentConfig>(() =>
    structuredClone(DEFAULT_SOLAR_SERPENT_CONFIG),
  );
  ...
  async function handleSaveAll() {
    ...
    try {
      await onPatchLobby({
        ...
        rtp: Number(rtp) || game.rtp,
        volatility: (volatility.trim() || game.volatility) as SuperGameRow["volatility"],
        minBet: Number(minBet) || game.minBet,
        maxBet: Number(maxBet) || game.maxBet,
      });

      const normalized = await saveSolarSerpentEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeSolarSerpentConfig(normalized));
      toast.success("Solar Serpent engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<SolarSerpentSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Solar Serpent Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as SolarSerpentSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {SOLAR_SERPENT_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: SOLAR_SERPENT_BOMB_MULTS.map((rowMult, rowIdx) => {
