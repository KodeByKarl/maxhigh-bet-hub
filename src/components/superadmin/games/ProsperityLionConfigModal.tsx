/**
 * Superadmin Prosperity Lion engine editor — lobby + math config.
...
import {
  getProsperityLionEngineConfigFn,
  saveProsperityLionEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/prosperity-lion/animationConfig";
...
export function ProsperityLionConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ProsperityLionConfig>(() =>
    structuredClone(DEFAULT_PROSPERITY_LION_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getProsperityLionEngineConfigFn()
      .then((c) => setCfg(normalizeProsperityLionConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_PROSPERITY_LION_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<ProsperityLionConfig>) {
    setCfg((c) => {
      const next = normalizeProsperityLionConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveProsperityLionEngineConfigFn({
        data: { config: normalizeProsperityLionConfig(cfg) },
      });
      setCfg(normalizeProsperityLionConfig(next));
      ...
      toast.success("Prosperity Lion engine saved");
    } catch (e) {
