...
import {
  getJeepneyJackpotEngineConfigFn,
  saveJeepneyJackpotEngineConfigFn,
...
export function JeepneyJackpotConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<JeepneyJackpotConfig>(() =>
    structuredClone(DEFAULT_JEEPNEY_JACKPOT_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getJeepneyJackpotEngineConfigFn()
      .then((c) => setCfg(normalizeJeepneyJackpotConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_JEEPNEY_JACKPOT_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<JeepneyJackpotConfig>) {
    setCfg((c) => normalizeJeepneyJackpotConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveJeepneyJackpotEngineConfigFn({
        data: { config: normalizeJeepneyJackpotConfig(cfg) },
      });
      setCfg(normalizeJeepneyJackpotConfig(next));
      toast.success("Jeepney Jackpot engine saved");
    } catch (e) {
