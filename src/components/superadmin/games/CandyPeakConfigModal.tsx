...
import {
  ...
  type CandyPeakSymbolConfig,
} from "@/lib/candy-peak-config";
import {
  getCandyPeakEngineConfigFn,
  saveCandyPeakEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { CandyIcon } from "@/components/maxhigh/candy-peak/CandyIcon";
import type { SymKind } from "@/components/maxhigh/candy-peak/types";
...
export function CandyPeakConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CandyPeakConfig>(() =>
    structuredClone(DEFAULT_CANDY_PEAK_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getCandyPeakEngineConfigFn()
      .then((c) => setCfg(normalizeCandyPeakConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_CANDY_PEAK_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCandyPeakEngineConfigFn({
        data: { config: normalizeCandyPeakConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<CandyPeakConfig>) {
    setCfg((c) => normalizeCandyPeakConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<CandyPeakSymbolConfig>) {
    setCfg((c) =>
      normalizeCandyPeakConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Candy Peak appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: CandyPeakConfig;
  onPatch: (p: Partial<CandyPeakConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_CANDY_PEAK_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_CANDY_PEAK_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_CANDY_PEAK_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_CANDY_PEAK_CONFIG.seedClusterMax,
              minCluster: DEFAULT_CANDY_PEAK_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<CandyPeakSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <CandyIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <CandyIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
