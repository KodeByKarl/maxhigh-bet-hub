...
import {
  getMoneyComingEngineConfigFn,
  saveMoneyComingEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/money-coming/animationConfig";
...
export function MoneyComingConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<MoneyComingConfig>(() =>
    structuredClone(DEFAULT_MONEY_COMING_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getMoneyComingEngineConfigFn()
      .then((c) => setCfg(normalizeMoneyComingConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_MONEY_COMING_CONFIG));
      })
  ...
  function patch(p: Partial<MoneyComingConfig>) {
    setCfg((c) => {
      const next = normalizeMoneyComingConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: McSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeMoneyComingConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveMoneyComingEngineConfigFn({
        data: { config: normalizeMoneyComingConfig(cfg) },
      });
      setCfg(normalizeMoneyComingConfig(next));
      toast.success("Money Coming engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/money-coming/</code>
              </p>
