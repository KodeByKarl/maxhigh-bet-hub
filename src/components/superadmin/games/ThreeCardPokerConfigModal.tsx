/**
 * Superadmin Three Card Poker engine editor — lobby + payouts + qualify + limits.
 * Wired from Super Admin → Games → Three Card Poker (full config).
 */
...
import {
  ...
  saveThreeCardPokerEngineConfigFn,
...
export function ThreeCardPokerConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ThreeCardPokerConfig>(() =>
    structuredClone(DEFAULT_THREE_CARD_POKER_CONFIG),
  );
  ...
  const [betStepsText, setBetStepsText] = useState(
    DEFAULT_THREE_CARD_POKER_CONFIG.betSteps.join(", "),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getThreeCardPokerEngineConfigFn()
      .then((c) => {
        const next = normalizeThreeCardPokerConfig(c);
        setCfg(next);
        setBetStepsText(next.betSteps.join(", "));
      })
      .catch(() => {
        setCfg(structuredClone(DEFAULT_THREE_CARD_POKER_CONFIG));
        setBetStepsText(DEFAULT_THREE_CARD_POKER_CONFIG.betSteps.join(", "));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<ThreeCardPokerConfig>) {
    setCfg((prev) => normalizeThreeCardPokerConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const withSteps = normalizeThreeCardPokerConfig({
        ...cfg,
        betSteps: parseBetSteps(betStepsText),
      });
      const next = await saveThreeCardPokerEngineConfigFn({ data: { config: withSteps } });
      const normalized = normalizeThreeCardPokerConfig(next);
      setCfg(normalized);
      ...
      toast.success("Three Card Poker engine saved — live for all players");
    } catch (e) {
