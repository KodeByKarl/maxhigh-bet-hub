/**
 * Superadmin Fortune Rabbit engine editor — lobby + math config.
...
import {
  getFortuneRabbitEngineConfigFn,
  saveFortuneRabbitEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/fortune-rabbit/animationConfig";
...
function SymThumb({ kind, name }: { kind: FrSymKind; name: string }) {
  return (
    <img
...
export function FortuneRabbitConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<FortuneRabbitConfig>(() =>
    structuredClone(DEFAULT_FORTUNE_RABBIT_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFortuneRabbitEngineConfigFn()
      .then((c) => setCfg(normalizeFortuneRabbitConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_FORTUNE_RABBIT_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<FortuneRabbitConfig>) {
    setCfg((c) => {
      const next = normalizeFortuneRabbitConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveFortuneRabbitEngineConfigFn({
        data: { config: normalizeFortuneRabbitConfig(cfg) },
      });
      setCfg(normalizeFortuneRabbitConfig(next));
      ...
      toast.success("Fortune Rabbit engine saved");
    } catch (e) {
