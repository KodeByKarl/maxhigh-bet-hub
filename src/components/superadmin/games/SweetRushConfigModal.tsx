...
import {
  ...
  type SweetRushSymbolConfig,
} from "@/lib/sweet-rush-config";
import {
  getSweetRushEngineConfigFn,
  saveSweetRushEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SweetRushIcon } from "@/components/maxhigh/sweet-rush/SweetRushIcon";
import type { SymKind } from "@/components/maxhigh/sweet-rush/types";
...
export function SweetRushConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SweetRushConfig>(() =>
    structuredClone(DEFAULT_SWEET_RUSH_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getSweetRushEngineConfigFn()
      .then((c) => setCfg(normalizeSweetRushConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_SWEET_RUSH_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSweetRushEngineConfigFn({
        data: { config: normalizeSweetRushConfig(cfg) },
      });
      setCfg(next);
      toast.success("Sweet Rush engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<SweetRushConfig>) {
    setCfg((c) => normalizeSweetRushConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<SweetRushSymbolConfig>) {
    setCfg((c) =>
      normalizeSweetRushConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Sweet Rush appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: SweetRushConfig;
  onPatch: (p: Partial<SweetRushConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_SWEET_RUSH_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_SWEET_RUSH_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_SWEET_RUSH_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_SWEET_RUSH_CONFIG.seedClusterMax,
              minCluster: DEFAULT_SWEET_RUSH_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<SweetRushSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <SweetRushIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <SweetRushIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
