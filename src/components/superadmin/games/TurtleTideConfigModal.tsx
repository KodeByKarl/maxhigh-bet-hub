/**
 * Superadmin Turtle Tide engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getTurtleTideAdminConfigFn,
  saveTurtleTideAdminConfigFn,
...
export function TurtleTideConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<TurtleTideConfig>(() =>
    structuredClone(DEFAULT_TURTLE_TIDE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getTurtleTideAdminConfigFn()
      .then((c) => setCfg(normalizeTurtleTideConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_TURTLE_TIDE_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<TurtleTideConfig>) {
    setCfg((prev) => normalizeTurtleTideConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveTurtleTideAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizeTurtleTideConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Turtle Tide engine saved — live for all players");
    } catch (e) {
