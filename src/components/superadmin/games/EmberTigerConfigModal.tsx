...
import {
  ...
  EMBER_TIGER_BOMB_MULTS,
  normalizeEmberTigerConfig,
  weightPercents,
  type EmberTigerConfig,
  type EmberTigerSymbolConfig,
} from "@/lib/ember-tiger-config";
import {
  getEmberTigerEngineConfigFn,
  saveEmberTigerEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/ember-tiger/PantherIcon";
import type { EmberTigerSymKind } from "@/lib/ember-tiger-config";
...
export function EmberTigerConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<EmberTigerConfig>(() =>
    structuredClone(DEFAULT_EMBER_TIGER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getEmberTigerEngineConfigFn()
      .then((res) => {
        setCfg(normalizeEmberTigerConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_EMBER_TIGER_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveEmberTigerEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeEmberTigerConfig(normalized));
      toast.success("Ember Tiger engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<EmberTigerSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Ember Tiger Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as EmberTigerSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {EMBER_TIGER_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: EMBER_TIGER_BOMB_MULTS.map((rowMult, rowIdx) => {
