/**
 * Superadmin Fortune Tiger engine editor — lobby + math config.
...
import {
  getFortuneTigerEngineConfigFn,
  saveFortuneTigerEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/fortune-tiger/animationConfig";
...
function SymThumb({ kind, name }: { kind: FtSymKind; name: string }) {
  return (
    <img
...
export function FortuneTigerConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<FortuneTigerConfig>(() =>
    structuredClone(DEFAULT_FORTUNE_TIGER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFortuneTigerEngineConfigFn()
      .then((c) => setCfg(normalizeFortuneTigerConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_FORTUNE_TIGER_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<FortuneTigerConfig>) {
    setCfg((c) => {
      const next = normalizeFortuneTigerConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveFortuneTigerEngineConfigFn({
        data: { config: normalizeFortuneTigerConfig(cfg) },
      });
      setCfg(normalizeFortuneTigerConfig(next));
      ...
      toast.success("Fortune Tiger engine saved");
    } catch (e) {
