...
import {
  ...
  type SugarSurgeSymbolConfig,
} from "@/lib/sugar-surge-config";
import {
  getSugarSurgeEngineConfigFn,
  saveSugarSurgeEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SugarIcon } from "@/components/maxhigh/sugar-surge/SugarIcon";
import type { SymKind } from "@/components/maxhigh/sugar-surge/types";
...
export function SugarSurgeConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SugarSurgeConfig>(() =>
    structuredClone(DEFAULT_SUGAR_SURGE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getSugarSurgeEngineConfigFn()
      .then((c) => setCfg(normalizeSugarSurgeConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_SUGAR_SURGE_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSugarSurgeEngineConfigFn({
        data: { config: normalizeSugarSurgeConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<SugarSurgeConfig>) {
    setCfg((c) => normalizeSugarSurgeConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<SugarSurgeSymbolConfig>) {
    setCfg((c) =>
      normalizeSugarSurgeConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Sugar Surge appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: SugarSurgeConfig;
  onPatch: (p: Partial<SugarSurgeConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_SUGAR_SURGE_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_SUGAR_SURGE_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_SUGAR_SURGE_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_SUGAR_SURGE_CONFIG.seedClusterMax,
              minCluster: DEFAULT_SUGAR_SURGE_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<SugarSurgeSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <SugarIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <SugarIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
