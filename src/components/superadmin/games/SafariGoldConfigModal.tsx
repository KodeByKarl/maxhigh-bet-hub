...
import {
  getSafariGoldEngineConfigFn,
  saveSafariGoldEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SafariGoldIcon } from "@/components/maxhigh/safari-gold/SafariGoldIcon";
...
export function SafariGoldConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SafariGoldConfig>(() =>
    structuredClone(DEFAULT_SAFARI_GOLD_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getSafariGoldEngineConfigFn()
      .then((c) => setCfg(normalizeSafariGoldConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_SAFARI_GOLD_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSafariGoldEngineConfigFn({
        data: { config: normalizeSafariGoldConfig(cfg) },
      });
      setCfg(normalizeSafariGoldConfig(next));
      toast.success("Safari Gold engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<SafariGoldConfig>) {
    setCfg((c) => normalizeSafariGoldConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <SafariGoldIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: SafariGoldConfig;
  onPatch: (p: Partial<SafariGoldConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
