...
import {
  ...
  type BeachBonanzaSymbolConfig,
} from "@/lib/beach-bonanza-config";
import {
  getBeachBonanzaEngineConfigFn,
  saveBeachBonanzaEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { BeachBonanzaIcon } from "@/components/maxhigh/beach-bonanza/BeachBonanzaIcon";
import type { SymKind } from "@/components/maxhigh/beach-bonanza/types";
...
export function BeachBonanzaConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<BeachBonanzaConfig>(() =>
    structuredClone(DEFAULT_BEACH_BONANZA_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getBeachBonanzaEngineConfigFn()
      .then((c) => setCfg(normalizeBeachBonanzaConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_BEACH_BONANZA_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveBeachBonanzaEngineConfigFn({
        data: { config: normalizeBeachBonanzaConfig(cfg) },
      });
      setCfg(next);
      toast.success("Beach Bonanza engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<BeachBonanzaConfig>) {
    setCfg((c) => normalizeBeachBonanzaConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<BeachBonanzaSymbolConfig>) {
    setCfg((c) =>
      normalizeBeachBonanzaConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Beach Bonanza appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: BeachBonanzaConfig;
  onPatch: (p: Partial<BeachBonanzaConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <ResetLink
          onReset={() =>
            onPatch({
              deadSpinChancePercent: DEFAULT_BEACH_BONANZA_CONFIG.deadSpinChancePercent,
              seedMelonBiasPercent: DEFAULT_BEACH_BONANZA_CONFIG.seedMelonBiasPercent,
              seedClusterMin: DEFAULT_BEACH_BONANZA_CONFIG.seedClusterMin,
              seedClusterMax: DEFAULT_BEACH_BONANZA_CONFIG.seedClusterMax,
              minCluster: DEFAULT_BEACH_BONANZA_CONFIG.minCluster,
            })
          }
        />
      </div>
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<BeachBonanzaSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
        ...
        {pays.map((s) => (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <BeachBonanzaIcon kind={s.kind as SymKind} className="size-9 shrink-0" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-3">
        <BeachBonanzaIcon kind="lollipop" className="size-11 shrink-0" />
        <div>
