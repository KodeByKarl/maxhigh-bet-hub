...
import {
  ...
  type GodlyGatesSymbolConfig,
} from "@/lib/godly-gates-config";
import {
  getGodlyGatesEngineConfigFn,
  saveGodlyGatesEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SymbolIcon } from "@/components/maxhigh/godly-gates/SymbolIcon";
import type { SymKind } from "@/components/maxhigh/godly-gates/types";
...
export function GodlyGatesConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<GodlyGatesConfig>(() =>
    structuredClone(DEFAULT_GODLY_GATES_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getGodlyGatesEngineConfigFn()
      .then((c) => setCfg(normalizeGodlyGatesConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_GODLY_GATES_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveGodlyGatesEngineConfigFn({
        data: { config: normalizeGodlyGatesConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<GodlyGatesConfig>) {
    setCfg((c) => normalizeGodlyGatesConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<GodlyGatesSymbolConfig>) {
    setCfg((c) =>
      normalizeGodlyGatesConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Godly Gates appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: GodlyGatesConfig;
  onPatch: (p: Partial<GodlyGatesConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<GodlyGatesSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
