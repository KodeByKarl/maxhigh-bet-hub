...
import {
  ...
  FROST_FOX_BOMB_MULTS,
  normalizeFrostFoxConfig,
  weightPercents,
  type FrostFoxConfig,
  type FrostFoxSymbolConfig,
} from "@/lib/frost-fox-config";
import {
  getFrostFoxEngineConfigFn,
  saveFrostFoxEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/frost-fox/PantherIcon";
import type { FrostFoxSymKind } from "@/lib/frost-fox-config";
...
export function FrostFoxConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<FrostFoxConfig>(() =>
    structuredClone(DEFAULT_FROST_FOX_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFrostFoxEngineConfigFn()
      .then((res) => {
        setCfg(normalizeFrostFoxConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_FROST_FOX_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveFrostFoxEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeFrostFoxConfig(normalized));
      toast.success("Frost Fox engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<FrostFoxSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Frost Fox Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as FrostFoxSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {FROST_FOX_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: FROST_FOX_BOMB_MULTS.map((rowMult, rowIdx) => {
