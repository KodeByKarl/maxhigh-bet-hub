/**
 * Superadmin Balut Bonus engine editor — lobby + math config.
...
import {
  getBalutBonusEngineConfigFn,
  saveBalutBonusEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/balut-bonus/animationConfig";
...
export function BalutBonusConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<BalutBonusConfig>(() =>
    structuredClone(DEFAULT_BALUT_BONUS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getBalutBonusEngineConfigFn()
      .then((c) => setCfg(normalizeBalutBonusConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_BALUT_BONUS_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<BalutBonusConfig>) {
    setCfg((c) => {
      const next = normalizeBalutBonusConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveBalutBonusEngineConfigFn({
        data: { config: normalizeBalutBonusConfig(cfg) },
      });
      setCfg(normalizeBalutBonusConfig(next));
      ...
      toast.success("Balut Bonus engine saved");
    } catch (e) {
