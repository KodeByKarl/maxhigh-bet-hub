/**
 * Superadmin Deep Bass engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getDeepBassAdminConfigFn,
  saveDeepBassAdminConfigFn,
...
export function DeepBassConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<DeepBassConfig>(() =>
    structuredClone(DEFAULT_DEEP_BASS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getDeepBassAdminConfigFn()
      .then((c) => setCfg(normalizeDeepBassConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_DEEP_BASS_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<DeepBassConfig>) {
    setCfg((prev) => normalizeDeepBassConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveDeepBassAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizeDeepBassConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Deep Bass engine saved — live for all players");
    } catch (e) {
