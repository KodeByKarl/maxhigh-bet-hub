/**
 * Superadmin Lucky Nine Plus engine editor — lobby + payouts + draw rules + limits.
 * Wired from Super Admin → Games → Lucky Nine Plus (full config).
 */
...
import {
  getLuckyNinePlusEngineConfigFn,
  saveLuckyNinePlusEngineConfigFn,
...
const NAV: { id: Section; label: string; icon: typeof Store }[] = [
  ...
  { id: "payouts", label: "Payouts / Draw", icon: Spade },
...
export function LuckyNinePlusConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<LuckyNinePlusConfig>(() =>
    structuredClone(DEFAULT_LUCKY_NINE_PLUS_CONFIG),
  );
  ...
  const [lobbyRtp, setLobbyRtp] = useState(
    Number.parseFloat(String(game.rtp ?? "").replace("%", "")) || RTP_REFERENCE.dealer,
  );
  ...
  const [betStepsText, setBetStepsText] = useState(
    DEFAULT_LUCKY_NINE_PLUS_CONFIG.betSteps.join(", "),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getLuckyNinePlusEngineConfigFn()
      .then((c) => {
        const next = normalizeLuckyNinePlusConfig(c);
        setCfg(next);
        setBetStepsText(next.betSteps.join(", "));
      })
      .catch(() => {
        setCfg(structuredClone(DEFAULT_LUCKY_NINE_PLUS_CONFIG));
        setBetStepsText(DEFAULT_LUCKY_NINE_PLUS_CONFIG.betSteps.join(", "));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<LuckyNinePlusConfig>) {
    setCfg((prev) => normalizeLuckyNinePlusConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const withSteps = normalizeLuckyNinePlusConfig({
        ...cfg,
        betSteps: parseBetSteps(betStepsText),
      });
      const next = await saveLuckyNinePlusEngineConfigFn({ data: { config: withSteps } });
      const normalized = normalizeLuckyNinePlusConfig(next);
      setCfg(normalized);
      setBetStepsText(normalized.betSteps.join(", "));
      const minMain = Math.min(normalized.minPlayerBet, normalized.minDealerBet);
      const maxMain = Math.max(normalized.maxPlayerBet, normalized.maxDealerBet);
      await onPatchLobby({
      ...
      toast.success("Lucky Nine Plus engine saved — live for all players");
    } catch (e) {
  ...
  async function saveLobby() {
    ...
    try {
      ...
      patch({
        ...
        minDealerBet: lobbyMinBet,
        maxPlayerBet: lobbyMaxBet,
        maxDealerBet: lobbyMaxBet,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[92vh] w-[min(100%-1rem,52rem)] flex-col gap-0 overflow-hidden border-lime-500/20 bg-panel p-0">
          ...
          <div className="absolute bottom-3 left-4 right-12 flex items-end justify-between gap-3">
            <DialogHeader className="space-y-0 text-left">
              <DialogTitle className="flex items-center gap-2 text-xl font-black uppercase text-foreground">
                <Spade size={18} className="text-lime-400" />
                {game.name}
              </DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground">
                Full engine · Closer to 9 · No commission · Natural 9 — {game.gameId}
              </DialogDescription>
            </DialogHeader>
          </div>
          <button
          ...
          {loading ? (
            ...
            <>
              ...
              <button
                ...
                className="inline-flex h-9 items-center gap-1.5 rounded-md border border-lime-500/40 bg-lime-500/10 px-3 text-sm font-medium text-lime-200 hover:bg-lime-500/20 disabled:opacity-50"
              >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field
                label="Dealer payout"
                hint="1.0 = 1:1 — Lucky Nine Plus has NO dealer commission (unlike Baccarat)"
              >
                <Num
                  value={cfg.dealerPayout}
                  onChange={(n) => patch({ dealerPayout: n })}
                  step={0.01}
                  min={0.5}
                  max={2}
                />
              </Field>
              <Field label="Tie payout" hint="5–8 typical; 8 = 8:1">
                <Num
                  value={cfg.tiePayout}
              ...
              <Field
                ...
                hint="Draw third card when total ≤ this (default 5 → stand on 6+). Symmetric for both sides."
              >
                <Num
                  value={cfg.drawThreshold}
                  onChange={(n) => patch({ drawThreshold: n })}
                  step={1}
                  min={0}
                  max={8}
                />
              ...
              <Field
                label="Natural total"
                hint="Two-card total that counts as Natural (default 9). Natural beats any 3-card equal total."
              >
                <Num
                  value={cfg.naturalTotal}
                  onChange={(n) => patch({ naturalTotal: n })}
                  step={1}
                  min={7}
                  max={9}
                />
              ...
              <Field
                label="Dealer draw mode"
                hint="independent = both use same threshold (default). after-player reserved for future dependent variants — same threshold math today (NOT Baccarat tableau)."
              >
                <select
                  className="flex h-9 w-full rounded-md border border-white/10 bg-white/[0.06] px-3 text-sm"
                  value={cfg.dealerDrawMode}
                  onChange={(e) =>
                    patch({
                      dealerDrawMode: e.target.value as LuckyNinePlusConfig["dealerDrawMode"],
                    })
                  }
                >
                  <option value="independent">Independent (symmetric)</option>
                  <option value="after-player">After player (sequential)</option>
                </select>
              </Field>
            </div>
          ) : section === "limits" ? (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Dealer min / max">
                <div className="flex gap-2">
                  <Num value={cfg.minDealerBet} onChange={(n) => patch({ minDealerBet: n })} step={0.01} />
                  <Num value={cfg.maxDealerBet} onChange={(n) => patch({ maxDealerBet: n })} step={1} />
                </div>
              </Field>
            ...
            <div className="space-y-3">
              <Field
                ...
                hint={`Reference ≈ Dealer/Player ${RTP_REFERENCE.dealer}% · Tie ${RTP_REFERENCE.tie}% — not enforced live`}
              >
                <Num
                  value={cfg.rtpTarget}
              ...
              <p className="rounded-md border border-white/10 bg-white/[0.03] p-3 text-[11px] text-muted-foreground">
                Lucky Nine Plus RTP is fixed by the closer-to-9 math and draw threshold. Dealer pays even
                money with no commission (unlike Baccarat&apos;s 5% Banker fee). Natural is{" "}
                {cfg.naturalTotal} only — not Baccarat&apos;s 8/9. Persisted in{" "}
                <code className="text-lime-300/90">game_controls.engineConfig</code>.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-white/10 px-4 py-3">
          ...
          <button
            ...
            className="inline-flex h-9 items-center gap-1.5 rounded-md bg-lime-600 px-3 text-sm font-medium text-white hover:bg-lime-500 disabled:opacity-50"
          >
            <Save className="size-3.5" />
            {saving ? "Saving…" : section === "lobby" ? "Save engine + steps" : "Save engine"}
          </button>
        </div>
      </DialogContent>
    </Dialog>
