...
import {
  getBuffaloReignEngineConfigFn,
  saveBuffaloReignEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { StampedeIcon } from "@/components/maxhigh/wild-frontier-stampede/StampedeIcon";
...
export function BuffaloReignConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<BuffaloReignConfig>(() =>
    structuredClone(DEFAULT_BUFFALO_REIGN_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getBuffaloReignEngineConfigFn()
      .then((c) => setCfg(normalizeBuffaloReignConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_BUFFALO_REIGN_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveBuffaloReignEngineConfigFn({
        data: { config: normalizeBuffaloReignConfig(cfg) },
      });
      setCfg(normalizeBuffaloReignConfig(next));
      toast.success("Buffalo Reign engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<BuffaloReignConfig>) {
    setCfg((c) => normalizeBuffaloReignConfig({ ...c, ...partial }));
  }
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <StampedeIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: BuffaloReignConfig;
  onPatch: (p: Partial<BuffaloReignConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
