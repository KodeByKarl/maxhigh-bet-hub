...
import {
  getFrontierGoldEngineConfigFn,
  saveFrontierGoldEngineConfigFn,
...
export function FrontierGoldConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<FrontierGoldConfig>(() =>
    structuredClone(DEFAULT_FRONTIER_GOLD_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFrontierGoldEngineConfigFn()
      .then((c) => setCfg(normalizeFrontierGoldConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_FRONTIER_GOLD_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<FrontierGoldConfig>) {
    setCfg((c) => normalizeFrontierGoldConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveFrontierGoldEngineConfigFn({
        data: { config: normalizeFrontierGoldConfig(cfg) },
      });
      setCfg(normalizeFrontierGoldConfig(next));
      toast.success("Frontier Gold engine saved");
    } catch (e) {
