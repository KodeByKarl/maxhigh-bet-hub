...
import {
  ...
  JADE_JAGUAR_BOMB_MULTS,
  normalizeJadeJaguarConfig,
  weightPercents,
  type JadeJaguarConfig,
  type JadeJaguarSymbolConfig,
} from "@/lib/jade-jaguar-config";
import {
  getJadeJaguarEngineConfigFn,
  saveJadeJaguarEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/jade-jaguar/PantherIcon";
import type { JadeJaguarSymKind } from "@/lib/jade-jaguar-config";
...
export function JadeJaguarConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<JadeJaguarConfig>(() =>
    structuredClone(DEFAULT_JADE_JAGUAR_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getJadeJaguarEngineConfigFn()
      .then((res) => {
        setCfg(normalizeJadeJaguarConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_JADE_JAGUAR_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveJadeJaguarEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeJadeJaguarConfig(normalized));
      toast.success("Jade Jaguar engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<JadeJaguarSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Jade Jaguar Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as JadeJaguarSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {JADE_JAGUAR_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: JADE_JAGUAR_BOMB_MULTS.map((rowMult, rowIdx) => {
