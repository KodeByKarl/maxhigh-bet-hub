/**
 * Superadmin Whale War engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getWhaleWarAdminConfigFn,
  saveWhaleWarAdminConfigFn,
...
export function WhaleWarConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<WhaleWarConfig>(() =>
    structuredClone(DEFAULT_WHALE_WAR_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getWhaleWarAdminConfigFn()
      .then((c) => setCfg(normalizeWhaleWarConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_WHALE_WAR_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<WhaleWarConfig>) {
    setCfg((prev) => normalizeWhaleWarConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveWhaleWarAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizeWhaleWarConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Whale War engine saved — live for all players");
    } catch (e) {
