...
import {
  ...
  saveReelRiotEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/reel-riot/animationConfig";
...
function weightPercents(cfg: ReelRiotConfig, reelIndex = 0): Record<string, number> {
...
export function FruitRiotConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<ReelRiotConfig>(() =>
    structuredClone(DEFAULT_REEL_RIOT_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getReelRiotEngineConfigFn()
      .then((c) => setCfg(normalizeReelRiotConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_REEL_RIOT_CONFIG));
      })
  ...
  function patch(p: Partial<ReelRiotConfig>) {
    setCfg((c) => normalizeReelRiotConfig({ ...c, ...p }));
  }

  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveReelRiotEngineConfigFn({
        data: { config: normalizeReelRiotConfig(cfg) },
      });
      setCfg(normalizeReelRiotConfig(next));
      toast.success("Fruit Riot engine saved");
    } catch (e) {
