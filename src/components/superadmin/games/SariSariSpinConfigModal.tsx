/**
 * Superadmin Sari-Sari Spin engine editor — lobby + math config.
...
import {
  getSariSariSpinEngineConfigFn,
  saveSariSariSpinEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/sari-sari-spin/animationConfig";
...
export function SariSariSpinConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SariSariSpinConfig>(() =>
    structuredClone(DEFAULT_SARI_SARI_SPIN_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getSariSariSpinEngineConfigFn()
      .then((c) => setCfg(normalizeSariSariSpinConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_SARI_SARI_SPIN_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<SariSariSpinConfig>) {
    setCfg((c) => {
      const next = normalizeSariSariSpinConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSariSariSpinEngineConfigFn({
        data: { config: normalizeSariSariSpinConfig(cfg) },
      });
      setCfg(normalizeSariSariSpinConfig(next));
      ...
      toast.success("Sari-Sari Spin engine saved");
    } catch (e) {
