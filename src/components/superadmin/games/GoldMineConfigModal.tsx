...
import {
  getGoldMineEngineConfigFn,
  saveGoldMineEngineConfigFn,
...
export function GoldMineConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [cfg, setCfg] = useState<GoldMineConfig>(() =>
    structuredClone(DEFAULT_GOLD_MINE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getGoldMineEngineConfigFn()
      .then((c) => setCfg(normalizeGoldMineConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_GOLD_MINE_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open]);

  function patch(p: Partial<GoldMineConfig>) {
    setCfg((c) => normalizeGoldMineConfig({ ...c, ...p }));
  }

  async function save() {
    setSaving(true);
    try {
      const next = await saveGoldMineEngineConfigFn({
        data: { config: normalizeGoldMineConfig(cfg) },
      });
      setCfg(normalizeGoldMineConfig(next));
      toast.success("Gold Mine Dig engine saved");
    } catch (e) {
