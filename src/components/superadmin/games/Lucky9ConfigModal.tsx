/**
 * Superadmin Lucky 9 engine editor — lobby + payouts + draw rules + limits.
 * Wired from Super Admin → Games → Lucky 9 (full config).
 */
...
import {
  getLucky9EngineConfigFn,
  saveLucky9EngineConfigFn,
...
export function Lucky9ConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<Lucky9Config>(() =>
    structuredClone(DEFAULT_LUCKY9_CONFIG),
  );
  ...
  const [betStepsText, setBetStepsText] = useState(
    DEFAULT_LUCKY9_CONFIG.betSteps.join(", "),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getLucky9EngineConfigFn()
      .then((c) => {
        const next = normalizeLucky9Config(c);
        setCfg(next);
        setBetStepsText(next.betSteps.join(", "));
      })
      .catch(() => {
        setCfg(structuredClone(DEFAULT_LUCKY9_CONFIG));
        setBetStepsText(DEFAULT_LUCKY9_CONFIG.betSteps.join(", "));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<Lucky9Config>) {
    setCfg((prev) => normalizeLucky9Config({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const withSteps = normalizeLucky9Config({
        ...cfg,
        betSteps: parseBetSteps(betStepsText),
      });
      const next = await saveLucky9EngineConfigFn({ data: { config: withSteps } });
      const normalized = normalizeLucky9Config(next);
      setCfg(normalized);
      ...
      toast.success("Lucky 9 engine saved — live for all players");
    } catch (e) {
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field
                label="Dealer payout"
                hint="1.0 = 1:1 — Lucky 9 has NO dealer commission (unlike Baccarat)"
              >
                <Num
              ...
              >
                <select
                  ...
                  onChange={(e) =>
                    patch({
                      dealerDrawMode: e.target.value as Lucky9Config["dealerDrawMode"],
                    })
                  }
                >
            ...
            <div className="space-y-3">
              ...
              <p className="rounded-md border border-white/10 bg-white/[0.03] p-3 text-[11px] text-muted-foreground">
                Lucky 9 RTP is fixed by the closer-to-9 math and draw threshold. Dealer pays even
