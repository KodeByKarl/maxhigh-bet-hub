...
import {
  ...
  weightPercents,
  type StarlightSymbolConfig,
  type StarlightAceConfig,
} from "@/lib/starlight-ace-config";
import {
  getStarlightAceEngineConfigFn,
  saveStarlightAceEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { StarlightIcon } from "@/components/maxhigh/starlight-ace/StarlightIcon";
...
export function StarlightAceConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<StarlightAceConfig>(() =>
    structuredClone(DEFAULT_STARLIGHT_ACE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getStarlightAceEngineConfigFn()
      .then((c) => setCfg(normalizeStarlightAceConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_STARLIGHT_ACE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveStarlightAceEngineConfigFn({
        data: { config: normalizeStarlightAceConfig(cfg) },
      });
      setCfg(normalizeStarlightAceConfig(next));
      toast.success("Starlight Ace engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<StarlightAceConfig>) {
    setCfg((c) => normalizeStarlightAceConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<StarlightSymbolConfig>) {
    setCfg((c) =>
      normalizeStarlightAceConfig({
        ...c,
...
function SymbolsSection({
  ...
  cfg: StarlightAceConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<StarlightSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <StarlightIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: StarlightAceConfig;
  onPatch: (p: Partial<StarlightAceConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
