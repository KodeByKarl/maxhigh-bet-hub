/**
 * Superadmin Fortune Ox engine editor — lobby + math config.
...
import {
  getFortuneOxEngineConfigFn,
  saveFortuneOxEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/fortune-ox/animationConfig";
...
export function FortuneOxConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<FortuneOxConfig>(() =>
    structuredClone(DEFAULT_FORTUNE_OX_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getFortuneOxEngineConfigFn()
      .then((c) => setCfg(normalizeFortuneOxConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_FORTUNE_OX_CONFIG));
      })
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(p: Partial<FortuneOxConfig>) {
    setCfg((c) => {
      const next = normalizeFortuneOxConfig({ ...c, ...p });
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveFortuneOxEngineConfigFn({
        data: { config: normalizeFortuneOxConfig(cfg) },
      });
      setCfg(normalizeFortuneOxConfig(next));
      ...
      toast.success("Fortune Ox engine saved");
    } catch (e) {
