/**
 * Superadmin Dragon Fisher engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getDragonFisherAdminConfigFn,
  saveDragonFisherAdminConfigFn,
...
export function DragonFisherConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<DragonFisherConfig>(() =>
    structuredClone(DEFAULT_DRAGON_FISHER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getDragonFisherAdminConfigFn()
      .then((c) => setCfg(normalizeDragonFisherConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_DRAGON_FISHER_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<DragonFisherConfig>) {
    setCfg((prev) => normalizeDragonFisherConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveDragonFisherAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizeDragonFisherConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Dragon Fisher engine saved — live for all players");
    } catch (e) {
