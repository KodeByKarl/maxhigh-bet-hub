...
import {
  getKnockoutKingEngineConfigFn,
  saveKnockoutKingEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/knockout-king/animationConfig";
...
export function KnockoutKingConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<KnockoutKingConfig>(() =>
    structuredClone(DEFAULT_KNOCKOUT_KING_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getKnockoutKingEngineConfigFn()
      .then((c) => setCfg(normalizeKnockoutKingConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_KNOCKOUT_KING_CONFIG));
      })
  ...
  function patch(p: Partial<KnockoutKingConfig>) {
    setCfg((c) => {
      const next = normalizeKnockoutKingConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: BkSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeKnockoutKingConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveKnockoutKingEngineConfigFn({
        data: { config: normalizeKnockoutKingConfig(cfg) },
      });
      setCfg(normalizeKnockoutKingConfig(next));
      toast.success("Knockout King engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/knockout-king/</code>
              </p>
