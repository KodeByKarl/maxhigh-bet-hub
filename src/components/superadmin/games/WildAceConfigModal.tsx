...
import {
  ...
  weightPercents,
  type SaRtpProfileId,
  type WildAceSymbolConfig,
  type WildAceConfig,
} from "@/lib/wild-ace-config";
import {
  getWildAceEngineConfigFn,
  saveWildAceEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { WildAceIcon } from "@/components/maxhigh/wild-ace/WildAceIcon";
...
export function WildAceConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<WildAceConfig>(() =>
    structuredClone(DEFAULT_WILD_ACE_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getWildAceEngineConfigFn()
      .then((c) => setCfg(normalizeWildAceConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config � using defaults");
        setCfg(structuredClone(DEFAULT_WILD_ACE_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveWildAceEngineConfigFn({
        data: { config: normalizeWildAceConfig(cfg) },
      });
      setCfg(normalizeWildAceConfig(next));
      toast.success("Mega Ace engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<WildAceConfig>) {
    setCfg((c) => normalizeWildAceConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<WildAceSymbolConfig>) {
    setCfg((c) =>
      normalizeWildAceConfig({
        ...c,
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<WildAceSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <WildAceIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: WildAceConfig;
  onPatch: (p: Partial<WildAceConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-5">
            ...
            return (
              ...
              >
                <div className="size-10 overflow-hidden rounded-md bg-black/40">
                  <WildAceIcon kind={kind} className="size-10" />
                </div>
      ...
      <label className="block max-w-xs space-y-1 text-xs font-semibold text-muted-foreground">
        ...
        <select
          ...
          onChange={(e) =>
            onPatch({
              volatility: e.target.value as WildAceConfig["volatility"],
            })
          }
