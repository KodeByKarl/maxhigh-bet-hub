...
import {
  ...
  type GoldenPantherSymbolConfig,
} from "@/lib/golden-panther-config";
import {
  getGoldenPantherEngineConfigFn,
  saveGoldenPantherEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/golden-panther/PantherIcon";
import type { GoldenPantherSymKind } from "@/lib/golden-panther-config";
...
export function GoldenPantherConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<GoldenPantherConfig>(() =>
    structuredClone(DEFAULT_GOLDEN_PANTHER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getGoldenPantherEngineConfigFn()
      .then((res) => {
        setCfg(normalizeGoldenPantherConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_GOLDEN_PANTHER_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveGoldenPantherEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeGoldenPantherConfig(normalized));
      toast.success("Golden Panther engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<GoldenPantherSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Golden Panther Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as GoldenPantherSymKind} className="size-8 object-contain" />
                            <div>
