...
import {
  ...
  type OlympusWrathSymbolConfig,
} from "@/lib/olympus-wrath-config";
import {
  getOlympusWrathEngineConfigFn,
  saveOlympusWrathEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { SymbolIcon } from "@/components/maxhigh/olympus-wrath/SymbolIcon";
import type { SymKind } from "@/components/maxhigh/olympus-wrath/types";
...
export function OlympusWrathConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<OlympusWrathConfig>(() =>
    structuredClone(DEFAULT_OLYMPUS_WRATH_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getOlympusWrathEngineConfigFn()
      .then((c) => setCfg(normalizeOlympusWrathConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_OLYMPUS_WRATH_CONFIG)))
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveOlympusWrathEngineConfigFn({
        data: { config: normalizeOlympusWrathConfig(cfg) },
      });
      setCfg(next);
  ...
  function patch(partial: Partial<OlympusWrathConfig>) {
    setCfg((c) => normalizeOlympusWrathConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<OlympusWrathSymbolConfig>) {
    setCfg((c) =>
      normalizeOlympusWrathConfig({
        ...c,
...
function LobbySection({
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Lobby" subtitle="How Olympus Wrath appears on the casino site." />
      <Toggle
...
function DeadSpinSection({
  ...
  cfg: OlympusWrathConfig;
  onPatch: (p: Partial<OlympusWrathConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
...
function SymbolsSection({
  ...
  onPatchSymbol: (id: string, p: Partial<OlympusWrathSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
