...
import {
  ...
  MIDNIGHT_OWL_BOMB_MULTS,
  normalizeMidnightOwlConfig,
  weightPercents,
  type MidnightOwlConfig,
  type MidnightOwlSymbolConfig,
} from "@/lib/midnight-owl-config";
import {
  getMidnightOwlEngineConfigFn,
  saveMidnightOwlEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { PantherIcon } from "@/components/maxhigh/midnight-owl/PantherIcon";
import type { MidnightOwlSymKind } from "@/lib/midnight-owl-config";
...
export function MidnightOwlConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cfg, setCfg] = useState<MidnightOwlConfig>(() =>
    structuredClone(DEFAULT_MIDNIGHT_OWL_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getMidnightOwlEngineConfigFn()
      .then((res) => {
        setCfg(normalizeMidnightOwlConfig(res));
      })
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_MIDNIGHT_OWL_CONFIG));
      })
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      const normalized = await saveMidnightOwlEngineConfigFn({
        data: { config: cfg },
      });
      setCfg(normalizeMidnightOwlConfig(normalized));
      toast.success("Midnight Owl engine math & lobby settings saved");
      onOpenChange(false);
    } catch (e) {
  ...
  function updateSymbol(id: string, patch: Partial<MidnightOwlSymbolConfig>) {
    setCfg((prev) => ({
      ...prev,
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <DialogTitle className="text-xl font-black uppercase text-amber-300 tracking-wider">
                Midnight Owl Engine Configuration
              </DialogTitle>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-[140px]">
                            <PantherIcon kind={sym.kind as MidnightOwlSymKind} className="size-8 object-contain" />
                            <div>
                ...
                <div className="space-y-6 max-w-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {MIDNIGHT_OWL_BOMB_MULTS.map((mult, idx) => {
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setCfg((p) => ({
                                  ...p,
                                  bombTable: MIDNIGHT_OWL_BOMB_MULTS.map((rowMult, rowIdx) => {
