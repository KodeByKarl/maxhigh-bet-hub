/**
 * Superadmin Shark Hunter engine editor — lobby + fish + weapons + boss + RTP.
 */
...
import {
  getSharkHunterAdminConfigFn,
  saveSharkHunterAdminConfigFn,
...
export function SharkHunterConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<SharkHunterConfig>(() =>
    structuredClone(DEFAULT_SHARK_HUNTER_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void getSharkHunterAdminConfigFn()
      .then((c) => setCfg(normalizeSharkHunterConfig(c)))
      .catch(() => setCfg(structuredClone(DEFAULT_SHARK_HUNTER_CONFIG)))
      .finally(() => setLoading(false));
  }, [open, game]);

  function patch(partial: Partial<SharkHunterConfig>) {
    setCfg((prev) => normalizeSharkHunterConfig({ ...prev, ...partial }));
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveSharkHunterAdminConfigFn({ data: { config: cfg } });
      const normalized = normalizeSharkHunterConfig(next);
      setCfg(normalized);
      await onPatchLobby({
      ...
      toast.success("Shark Hunter engine saved — live for all players");
    } catch (e) {
