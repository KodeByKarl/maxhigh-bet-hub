...
import {
  getCashManiaEngineConfigFn,
  saveCashManiaEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { ICON_SRC } from "@/components/maxhigh/cash-mania/animationConfig";
...
export function CashManiaConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<CashManiaConfig>(() =>
    structuredClone(DEFAULT_CASH_MANIA_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getCashManiaEngineConfigFn()
      .then((c) => setCfg(normalizeCashManiaConfig(c)))
      .catch(() => {
        toast.error("Failed loading config — defaults");
        setCfg(structuredClone(DEFAULT_CASH_MANIA_CONFIG));
      })
  ...
  function patch(p: Partial<CashManiaConfig>) {
    setCfg((c) => {
      const next = normalizeCashManiaConfig({ ...c, ...p });
  ...
  function setActiveProfileWeight(kind: McSymKind, reelIndex: number, value: number) {
    setCfg((c) => {
      ...
      return normalizeCashManiaConfig({ ...c, rtpProfiles: profiles });
    });
  }
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveCashManiaEngineConfigFn({
        data: { config: normalizeCashManiaConfig(cfg) },
      });
      setCfg(normalizeCashManiaConfig(next));
      toast.success("Cash Mania engine saved");
    } catch (e) {
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-orange-300">/images/symbols/cash-mania/</code>
              </p>
