...
import {
  ...
  weightPercents,
  type NeonMakatiSymbolConfig,
  type NeonMakatiConfig,
} from "@/lib/neon-makati-config";
import {
  getNeonMakatiEngineConfigFn,
  saveNeonMakatiEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { NeonMakatiIcon } from "@/components/maxhigh/neon-makati/NeonMakatiIcon";
...
export function NeonMakatiConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<NeonMakatiConfig>(() =>
    structuredClone(DEFAULT_NEON_MAKATI_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    setSection("lobby");
    setLoading(true);
    void getNeonMakatiEngineConfigFn()
      .then((c) => setCfg(normalizeNeonMakatiConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_NEON_MAKATI_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveNeonMakatiEngineConfigFn({
        data: { config: normalizeNeonMakatiConfig(cfg) },
      });
      setCfg(normalizeNeonMakatiConfig(next));
      toast.success("Neon Makati engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<NeonMakatiConfig>) {
    setCfg((c) => normalizeNeonMakatiConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<NeonMakatiSymbolConfig>) {
    setCfg((c) =>
      normalizeNeonMakatiConfig({
        ...c,
...
function SymbolsSection({
  ...
  cfg: NeonMakatiConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<NeonMakatiSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <NeonMakatiIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: NeonMakatiConfig;
  onPatch: (p: Partial<NeonMakatiConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
