...
import {
  ...
  weightPercents,
  type StarlightWaysSymbolConfig,
  type StarlightWaysConfig,
} from "@/lib/starlight-ways-config";
import {
  getStarlightWaysEngineConfigFn,
  saveStarlightWaysEngineConfigFn,
} from "@/functions/superadmin";
import type { SuperGameRow } from "@/lib/superadmin-types";
import { toast } from "sonner";
import { StarlightWaysIcon } from "@/components/maxhigh/starlight-ways/StarlightWaysIcon";
...
export function StarlightWaysConfigModal({ game, open, onOpenChange, onPatchLobby }: Props) {
  const [section, setSection] = useState<Section>("lobby");
  const [cfg, setCfg] = useState<StarlightWaysConfig>(() =>
    structuredClone(DEFAULT_STARLIGHT_WAYS_CONFIG),
  );
  ...
  useEffect(() => {
    ...
    void getStarlightWaysEngineConfigFn()
      .then((c) => setCfg(normalizeStarlightWaysConfig(c)))
      .catch(() => {
        toast.error("Failed loading engine config — using defaults");
        setCfg(structuredClone(DEFAULT_STARLIGHT_WAYS_CONFIG));
      })
  ...
  async function saveEngine() {
    setSaving(true);
    try {
      const next = await saveStarlightWaysEngineConfigFn({
        data: { config: normalizeStarlightWaysConfig(cfg) },
      });
      setCfg(normalizeStarlightWaysConfig(next));
      toast.success("Starlight Ways engine config saved");
    } catch (e) {
  ...
  function patch(partial: Partial<StarlightWaysConfig>) {
    setCfg((c) => normalizeStarlightWaysConfig({ ...c, ...partial }));
  }

  function patchSymbol(id: string, partial: Partial<StarlightWaysSymbolConfig>) {
    setCfg((c) =>
      normalizeStarlightWaysConfig({
        ...c,
...
function SymbolsSection({
  ...
  cfg: StarlightWaysConfig;
  spawnPct: Record<string, number>;
  allPct: Record<string, number>;
  onPatchSymbol: (id: string, p: Partial<StarlightWaysSymbolConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
  return (
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              <div className="grid size-12 place-items-center overflow-hidden rounded-lg bg-black/40">
                <StarlightWaysIcon kind={s.kind} className="size-10" />
              </div>
...
function CascadesSection({
  ...
  cfg: StarlightWaysConfig;
  onPatch: (p: Partial<StarlightWaysConfig>) => void;
  onSave: () => void;
  saving: boolean;
}) {
